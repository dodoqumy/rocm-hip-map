...
Phase 1  ████████████████████████████████ 100% ✅
Phase 2  ████████████████████████████████ 100% ✅
Phase 3  ████████████████░░░░░░░░░░░░░░░░  50% 🟡 (脚本就绪，缺定时 CI)
Phase 4  ████████████████████░░░░░░░░░░░░  67% 🟡 (核心完成，翻译管道待对接)
Phase 5  ████████████████████████████████ 100% ✅
Phase 6  ██████████████████░░░░░░░░░░░░░░  60% 🟡 (deploy OK，缺 sync/validate CI)
Phase 7  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0% 🔲
────────────────────────────────────────────
总体     ██████████████████████░░░░░░░░░░  63%
```
...
| 3.5 | 干跑测试 | ✅ | 22 篇文章抓取正常 |
| 3.6 | 定时同步 CI (`sync.yml`) | 🔲 | 待创建 GitHub Actions 工作流 |
| 3.7 | 首次正式同步运行 | 🔲 | 内容仓库填充 |
...
| 4.8 | 翻译管道实际对接运行 | 🔴 阻塞 | 依赖 OP-1：用户提供 opencode-go API Key |
| 4.9 | `docs/bilingual/` 内容填充 | 🔴 阻塞 | 依赖 4.8 --- 翻译管道就绪后自动填充 |
...
| 6.5 | `sync.yml` 定时同步 | 🔲 | 每周自动 fetch + sync + translate |
| 6.6 | `validate.yml` 数据校验 | 🔲 | 链接有效性 + frontmatter 规范校验 |
| 6.7 | Release Watch → Issue 通知 | 🔲 | 新版本自动创建 GitHub Issue |
...
1. **🔴 OP-1** — 用户提供 opencode-go API Key（[手顺 → user-actions.md](docs/ops/user-actions.md#op-1获取-opencode-go-api-key)），解除翻译管道阻塞
2. **🔴 Phase 6 收尾** — 创建 `sync.yml` + `validate.yml` 工作流（`sync.yml` 需要 API Key 存入 Secrets 后方可运行）
3. **🔴 Phase 3.7** — 首次正式抓取运行，填充 `content/raw/` 和 `data/articles.json`
4. **🟡 Phase 4.8** — Key 就绪后跑首次批量翻译
5. **🟡 Phase 5.5-5.7** — 为特色组件创建对应的 MDX 文档页面
6. **🟢 Phase 7.1** — 批量导入 ROCm 7.2.2 官方文档 50 篇
