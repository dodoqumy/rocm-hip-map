...
**最后更新：** 2026-04-29 01:30 CST  
**当前分支：** `phase10-gpu-prices`  
**最新提交：** `9d5a81b` — fix(ci): smoke test 改检查静态 HTML  
...
Phase 11 ████████████████████████████████ 100% ✅
Phase 8  ████████████████████████████████ 100% ✅
| Phase 9  | ████████████████████████████████ 100% ✅ (欧洲✅ + 亚洲✅，剩余3项)
| Phase 10 ██████████████████████░░░░░░░░░░  71% 🟡 (核心管道就绪，待注册 API Key)
────────────────────────────────────────────
总体     ███████████████████░░░░░░░░░░░░░  76%
```
...
| 7.5 | 中文学术论文 + AMD 中国资料 | 🔲 | |
| 7.6 | Algolia DocSearch 双语搜索 | ✅ | @easyops-cn/docusaurus-search-local 中英分词已上线 + Algolia 申请指南 |
...
## Phase 11：图片缓存管道 🖼️（新增）

> 详细计划：[docs/plans/phase11-image-cache.md](docs/plans/phase11-image-cache.md)

| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 11.1 | `scripts/cache-images.py` 图片缓存脚本 | ✅ | 24/25 下载成功，1.7MB，WebP 压缩 |
| 11.2 | `fetch-official.py` 抓取时同步缓存图片 | 🔲 | 接口已预留 |
| 11.3 | `generate-docs.py` 远程 URL 解析 | ✅ | 相对路径→绝对远程 URL，保留主源 |
| 11.4 | 客户端 fallback 模块 | ✅ | `src/clientModules/imageFallback.ts` |
| 11.5 | CI 图片校验（broken + cached exist） | ✅ | validate.yml 新增 2 steps |
| 11.6 | ECS/CDN 部署适配 | 🔲 | 无需改代码，文档说明即可 |

### 发现：68 篇文章 ~25 张远程图片，全部热链

```
✅ 不处理：data:image/svg+xml base64 icon（自包含，不会断）
🔧 需缓存：_images/*.png, ../_images/*.png（约 25 张，~5 MB）
```
...
| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 8.1 | `scripts/fetch-papers.py` arXiv 搜索脚本 | ✅ | 87 篇论文，6 组查询，data/papers.json |
| 8.2 | `scripts/extract-pdf.py` PDF 提取脚本 | ✅ | 86/87 成功，1 篇 arXiv 404 |
| 8.3 | `translate.py` 论文专用 prompt 适配 | ✅ | --mode paper + --papers-dir |
| 8.4 | `generate-docs.py` paper 类型适配 | ✅ | PaperArticleHeader 组件 + arxiv_id |
| 8.5 | 侧边栏 📜 分类 + arXiv 子分类 | 🔲 | |
| 8.6 | CI 集成（每周同步） | ✅ | `.github/workflows/papers.yml` — 每周六自动同步 |
| 8.7 | 部署后验证 ≥5 篇论文 | 🔲 | |

---

## Phase 9：多语言一手情报检索与翻译（新增）

> 详细计划：[docs/plans/phase9-multilingual.md](docs/plans/phase9-multilingual.md)

### 覆盖 10 个国家/语言

🇫🇮 芬兰 · 🇫🇷 法国 · 🇩🇪 德国 · 🇮🇹 意大利 · 🇯🇵 日本 · 🇷🇺 俄罗斯 · 🇪🇸 西班牙 · 🇳🇱 荷兰 · 🇰🇷 韩国 · 🇸🇪 瑞典

| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 9.1 | 欧洲核心（芬兰/法国/德国）抓取脚本 | ✅ | LUMI CSC + GENCI + Jülich (7 篇) |
| 9.2 | 亚洲核心（日本/韩国）抓取脚本 | ✅ | 🇯🇵 東工大TSUBAME/RIKEN/東大/阪大 + 🇰🇷 KISTI/KAIST/SNU (11 篇) |
| 9.3 | 扩展（俄罗斯/意大利/西班牙等）抓取脚本 | 🔲 | eLibrary + CINECA + BSC |
| 9.4 | `translate.py` 多语言翻译适配 | 🔲 | xx→en→zh 管道 + 语言检测 |
| 9.5 | 多语言页面生成 + 国旗标签 | 🔲 | |
| 9.6 | 侧边栏 🌐 多语言情报分类 | 🔲 | |
| 9.7 | CI 每周多语言同步 | 🔲 | |
...
## Phase 10：AMD GPU 二手市场价格追踪（新增）

> 详细计划：[docs/plans/phase10-prices.md](docs/plans/phase10-prices.md)

### 覆盖 9 个国家 / 15 个平台 / 14 款 GPU

🇺🇸 eBay, Amazon · 🇨🇳 闲鱼, 淘宝 · 🇯🇵 Yahoo拍卖, Mercari · 🇩🇪 eBay.de, Kleinanzeigen
🇫🇷 Leboncoin · 🇬🇧 eBay.co.uk, CEX · 🇷🇺 Avito · 🇰🇷 중고나라, 번개장터 · 🇮🇹 Subito

| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 10.1 | eBay Browse API 价格抓取（US/DE/UK） | ✅ | `scripts/fetch-prices-ebay.py` — OAuth + 14 GPU + 3 站点 |
| 10.2 | 亚洲平台抓取（闲鱼/Mercari/Yahoo拍卖） | 🔲 | 网页抓取，反爬中-高 |
| 10.3 | 欧洲平台抓取（Leboncoin/Kleinanzeigen） | 🔲 | 网页抓取 |
| 10.4 | 价格标准化（货币统一 + 异常值过滤） | ✅ | `scripts/normalize-prices.py` — ECB 汇率 + IQR + 条件加权 |
| 10.5 | 历史存储 + 52 周趋势 | ✅ | JSON 快照 + price-history.json 聚合 |
| 10.6 | PriceTracker/PriceChart 前端组件 | ✅ | 对比表 + SVG 趋势图 |
| 10.7 | CI 每周价格同步 | ✅ | `sync-prices.yml` — 每周一 03:00 UTC |
...
| PriceTracker | `website/src/components/PriceTracker.tsx` | ✅ |
| PriceChart | `website/src/components/PriceChart.tsx` | ✅ |
...
| fetch-prices-ebay | `scripts/fetch-prices-ebay.py` | ✅ | ✅ eBay API 端点 |
| normalize-prices | `scripts/normalize-prices.py` | ✅ | ✅ ECB 汇率 + IQR |
...
| `data/prices/` | ✅ | 价格数据目录（price-history.json + 周快照） |
...
1. **🔴 Phase 4.8** — 翻译管道首次运行（翻译测试 workflow 已就绪）
2. **🟡 Phase 8.5** — 侧边栏添加 📜 论文分类
3. **🟡 Phase 9.3** — 多语言扩展（俄罗斯/意大利/西班牙等）
4. **🟡 Phase 10.2-3** — 亚洲/欧洲平台抓取器
5. **🟢 Phase 7.1** — 文章正文嵌入修复
6. **🟢 Phase 10.1** — 注册 eBay API Key 并首次运行价格抓取
...
| 提交 | 信息 |
|------|------|
| `9d5a81b` | fix(ci): smoke test 改检查静态 HTML |
| `59c6557` | ci: add manual single-file translation test workflow |
| `85d3513` | feat: Phase 7.6 — 双语搜索上线 + MDX构建修复 |
| `f229b09` | feat: Phase 9.2 — 亚洲多语言情报（日韩11篇） |
| `022b367` | feat: Phase 9.1 — multilingual generate-docs + sidebar + index |
| `23f854a` | feat: Phase 9.1 — 多语言情报抓取（欧洲核心）+ Phase 8.6 CI |
| `d16398f` | feat: Phase 8 complete + Phase 11 image cache |
| `4d4f428` | plan: Phase 10 — AMD GPU 二手市场价格追踪系统 |
| `dfefed6` | plan: Phase 9 — 多语言一手情报检索与翻译模块 |
| `21735d0` | plan: Phase 8 — ROCm/HIP 论文搜集翻译模块设计 |
