...
| 4.4 | 移动端自适应 | ✅ | 左右→上下 |
| 4.5 | `scripts/translate.py` | ✅ | DeepL/GPT 批量翻译脚本 |
| 4.6 | `glossary/rocm-terms.yaml` | ✅ | CUDA↔HIP 术语对照 |
| 4.7 | GlossaryTooltip 组件 | ✅ | 悬浮术语解释 |
| 4.8 | 翻译管道实际对接运行 | 🔲 | API key 配置 + 批量翻译 |
| 4.9 | `docs/bilingual/` 内容填充 | 🔲 | 双语对照内容写入 |
...
1. **🔴 Phase 6 收尾** — 创建 `sync.yml` + `validate.yml` 工作流，让脚本 CI 化
2. **🔴 Phase 3.7** — 首次正式抓取运行，填充 `content/raw/` 和 `data/articles.json`
3. **🟡 Phase 4.8** — 配置翻译 API，跑首次批量翻译
4. **🟡 Phase 5.5-5.7** — 为特色组件创建对应的 MDX 文档页面
5. **🟢 Phase 7.1** — 批量导入 ROCm 7.2.2 官方文档 50 篇
