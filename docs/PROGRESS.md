...
| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 10.1 | eBay Browse API 价格抓取（US/DE/UK） | 🔲 | 官方 API，主力源 |
| 10.2 | 亚洲平台抓取（闲鱼/Mercari/Yahoo拍卖） | 🔲 | 网页抓取，反爬中-高 |
| 10.3 | 欧洲平台抓取（Leboncoin/Kleinanzeigen） | 🔲 | 网页抓取 |
| 10.4 | 价格标准化（货币统一 + 异常值过滤） | 🔲 | IQR + 中位数 |
| 10.5 | 历史存储 + 52 周趋势 | 🔲 | JSON 快照 |
| 10.6 | PriceTracker/PriceChart 前端组件 | 🔲 | 对比表 + 趋势图 |
| 10.7 | CI 每周价格同步 | 🔲 | 每周一 03:00 UTC |
...
1. **🔴 Phase 8.5** — 侧边栏添加 📜 论文分类
2. **🟡 Phase 8.6** — CI 集成论文每周同步
3. **🟡 Phase 9.1** — 多语言情报检索（欧洲开发者社区）
4. **🟡 Phase 4.8** — sync.yml 翻译验证
