...
1. **🟡 Phase 4.8** — sync.yml 首次自动翻译完成验证（等待 CI 运行结果）
2. **🟡 Phase 4.9** — `docs/bilingual/` 内容填充（翻译产出后自动写入）
3. **🟢 Phase 7.1** — 批量导入 ROCm 7.2.2 官方文档 50 篇
4. **🟢 Phase 7.3** — 扩充 CUDA→HIP API 映射库 ≥200 条
