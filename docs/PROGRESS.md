...
**最后更新：** 2026-05-01 11:50 CST
**当前分支：** `phase2.0-database`
**最新提交：** `ca571f6` — feat(2.1.4): 第三方爬虫 — Phoronix + Reddit + arXiv + HackerNews
...
||- 2.0.0 数据库化         ✅ *(56 articles / 72 tags / 301 CUDA→HIP / 87 papers / 8 sources)*
||- 2.1.1 爬虫基础         ✅ *(BaseCrawler + HttpClient + Extractor + Dedup)*
||- 2.1.2 官方源爬虫       ✅ *(RocmDocs sitemap 发现 1500+ URL, Blog RSS, PyTorch sitemap)*
||- 2.1.3 GitHub Issues    ✅ *(ROCm/ROCm 235 + rocm-libraries 877 + triton 83, REST/GraphQL 双模式)*
||- 2.1.4 第三方爬虫       ✅ *(Phoronix + Reddit + arXiv + HackerNews)*
||- 2.2 调度器             🔲 *(Prefect Cloud flows)*
||- 2.3 智能分类           🔲 *(规则 300 条 + LLM 兜底)*
||- 2.4 质量评分           🔲 *(quality_score + is_indexed 筛选)*
...
| `crawlers/base.py` | BaseCrawler 模板基类 | ~260 |
| `crawlers/http_client.py` | httpx 客户端 + ETag 缓存 | ~105 |
| `crawlers/extractor.py` | 正文提取（trafilatura→selectolax→stdlib） | ~175 |
| `crawlers/dedup.py` | URL 规范化 + sha256 去重 | ~115 |
| `crawlers/sources/rocm_docs.py` | RocmDocs sitemap 爬虫 | ~172 |
| `crawlers/sources/rocm_blog.py` | ROCm Blog 爬虫 | ~120 |
| `crawlers/sources/pytorch_docs.py` | PyTorch ROCm 文档爬虫 | ~105 |
| `crawlers/sources/github.py` | GitHub Issues/PR 爬虫（REST+GraphQL） | ~290 |
| `crawlers/sources/third_party.py` | Phoronix/Reddit/arXiv/HN 爬虫 | ~260 |
...
1. **🔴 Phase 2.2** — 调度器（Prefect Cloud flows — 爬虫定时任务编排）
2. **🔴 Phase 2.3** — 智能分类（规则 300 条 + LLM 兜底）
3. **🟡 Phase 8.7** — 论文部署验证
4. **🟢 Phase 2.4** — 质量评分（quality_score + is_indexed 筛选）
