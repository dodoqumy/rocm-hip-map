...
本教程演示如何通过利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm（ROCm（Radeon 开放计算平台））GPU 上微调 Llama-3.1 8B 大型语言模型 (LLM)。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”，R1-Zero 通过使用 Group Relative Policy Optimization（GRPO）自主学会在不借助人类反馈的情况下分配更多思考时间。Unsloth 团队改进了整个 GRPO 流程，使其比 Hugging Face 和 Flash Attention 2 (FA2) 少用 80% 的显存。这使您能够仅用 7GB 显存，使用 Qwen2.5 (1.5B) 重现 R1-Zero 的成就。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） install guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认你的设置：此命令列出你的 AMD GPU 及相关详细信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`。

command instead.**Docker**: 确保Docker已正确安装和配置。请根据您的操作系统遵循Docker安装指南。**注意**: 确保Docker权限配置正确。要配置权限以允许非root用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
[Hugging Face](https://huggingface.co)用于下载模型。确保Hugging Face API token拥有必要的权限和访问批准。

[Meta Llama 检查点](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct).

### 数据准备[#](#data-preparation)

本教程使用来自 Hugging Face 的示例数据集，该数据集在设置步骤中准备好。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
您需要一个Hugging Face API令牌来访问Llama-3.1。请在[Hugging Face Tokens](https://huggingface.co/settings/tokens)生成您的令牌，并为[Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct)申请访问权限。令牌通常以“hf_”开头。
...
try:
    ...
    print(f"Token 验证成功！当前登录用户：{user_info['name']}")
...
model,
r = lora_rank, # 选择任意大于0的数字！建议值为8, 16, 32, 64, 128
target_modules = [
...
本教程直接利用 [willccbb](https://gist.github.com/willccbb/4676755236bb08cab5f4e54a0475d6fb) 进行数据准备及所有奖励函数。您也可以自由创建自己的方法。
...
gradient_accumulation_steps = 1, # 可增至4使训练更平滑
num_generations = 6, # 若内存不足可减小
...
可能需要等待150到200步才能看到任何动作。前100步可能不会获得任何奖励。请耐心等待！
...
现在，使用刚刚用GRPO训练好的LoRA尝试一下，但请先保存LoRA。
...
推理模型要好得多。它并不总是正确的，因为你只训练了大约一个小时。如果延长序列长度并训练更长时间，它的表现会更好。

### 保存为 float16 用于 VLLM[#](#saving-to-float16-for-vllm)
...
适配器作为回退选项。使用 `push_to_hub_merged`

上传到您的 Hugging Face 账户。访问 [Hugging Face 令牌设置](https://huggingface.co/settings/tokens) 获取您的个人令牌。
...
现在您已完成！如果您对 Unsloth 有任何疑问、需要帮助或想持续获取更新，他们有一个 [Discord](https://discord.gg/unsloth) 频道和一个 [GitHub](https://github.com/unslothai/unsloth)。您也可以查阅他们的[文档](https://docs.unsloth.ai/)获取更多信息。