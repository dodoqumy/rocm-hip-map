...
本教程演示了如何利用[Unsloth](https://unsloth.ai)在AMD ROCm（ROCm（Radeon 开放计算平台）） GPU上微调Llama-3.1 8B大语言模型（LLM）。DeepSeek的R1研究揭示了一个"啊哈时刻"，即R1-Zero通过使用组相对策略优化（GRPO）在没有人类反馈的情况下自主学会了分配更多思考时间。Unsloth团队改进了整个GRPO流程，使其相比Hugging Face和Flash Attention 2（FA2）节省了80%的VRAM。这使得您可以使用Qwen2.5（1.5B）仅用7GB VRAM重现R1-Zero的成就。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.3**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的设置：该命令将列出您的AMD GPU及其相关详细信息。

**注意**: 对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，使用`rocm-smi`

请改用命令。**Docker**：确保 Docker 已正确安装和配置。请遵循适用于您操作系统的 Docker 安装指南。**注意**：确保 Docker 权限已正确配置。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用了来自Hugging Face的示例数据集，该数据集在设置步骤中准备就绪。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI 开发者中心 GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
确保在你的系统上运行上述命令之前，该端口尚未被占用。如果已被占用，你可以通过替换`--port=8888`来指定一个不同的端口。
...
如果验证过程失败，请确保为笔记本选择了正确的 Jupyter kernel。要更改 kernel，请按照以下步骤操作：
...
**重要**：未能选择正确的kernel可能导致运行notebook时出现意外问题。

### 5. 提供您的 Hugging Face token[#](#provide-your-hugging-face-token)

您需要Hugging Face API令牌才能访问Llama-3.1。请在[Hugging Face Tokens](https://huggingface.co/settings/tokens)生成您的令牌，并申请[Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct)的访问权限。令牌通常以“hf_”开头。
...
lora_rank = 32 # 更大的秩意味着更智能，但速度更慢
model, tokenizer = FastLanguageModel.from_pretrained(
    model_name = "meta-llama/meta-Llama-3.1-8B-Instruct",
    max_seq_length = max_seq_length,
    load_in_4bit = False, # 设置为True以启用4bit LoRA
    fast_inference = True, # 启用vLLM快速推理
    max_lora_rank = lora_rank,
    gpu_memory_utilization = 0.6, # 若内存不足，请减小此值
)
model = FastLanguageModel.get_peft_model(
    model,
    r = lora_rank, # 选择任意大于0的数！建议值8、16、32、64、128
    target_modules = [
...
本教程直接利用 [willccbb](https://gist.github.com/willccbb/4676755236bb08cab5f4e54a0475d6fb) 进行数据准备以及所有奖励函数。你也可以自由创建自己的方法。
...
现在测试你刚刚训练的模型。首先测试没有经过任何GRPO训练的模型：
...
现在，用你刚刚用GRPO训练好的LoRA来试试，但要先保存LoRA。
...
推理模型要好得多。它并不总是正确的，因为你只训练了大约一个小时。如果你延长序列长度并训练更长时间，结果会更好。
...
要上传到你的Hugging Face账户。请访问 [Hugging Face token settings](https://huggingface.co/settings/tokens) 获取你的个人 tokens。
...
现在你已经完成了！如果你对 Unsloth 有任何疑问、需要帮助或想了解最新动态，他们有一个 [Discord](https://discord.gg/unsloth) 频道和一个 [GitHub](https://github.com/unslothai/unsloth) 仓库。你也可以查阅他们的 [文档](https://docs.unsloth.ai/) 以获取更多信息。