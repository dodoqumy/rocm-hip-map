...
# 使用Unsloth训练您自己的R1推理模型

**作者**：[Unsloth](https://unsloth.ai) 并由 [AMD](https://www.amd.com) 修改以在 AMD GPU 上运行。

**知识水平**: 中级

本教程演示如何利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm GPU 上微调 Llama-3.1 8B 大型语言模型 (LLM)。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”，其中 R1-Zero 通过使用组相对策略优化 (Group Relative Policy Optimization, GRPO) 在没有人类反馈的情况下自主学会了分配更多思考时间。Unsloth 团队对整个 GRPO 流程进行了增强，使其相比于 Hugging Face 和 Flash Attention 2 (FA2) 减少了 80% 的显存使用。这使得您可以使用 Qwen2.5 (1.5B) 在仅 7GB 显存的情况下复现 R1-Zero 的成就。
...
**AMD Instinct™（AMD 数据中心 GPU 系列）**：本教程已在 AMD Instinct MI300X GPU 上完成测试。请确保您使用的 AMD Instinct GPU 或兼容硬件支持 ROCm（Radeon 开放计算平台），且系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（Radeon 开放计算平台） 6.3**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认配置：  
该命令会列出 AMD GPU 及其相关信息。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在启动后将笔记本文件上传到 Jupyter Notebook 环境中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口在您的系统上尚未被占用。如果已被占用，您可以通过将 `--port=8888` 替换为其他端口号来指定不同的端口。
...
# 本笔记本已在 unsloth==2025.3.19 unsloth_zoo==2025.3.17 bitsandbytes==0.43.3.dev0 上验证
```
...
**重要**：如果未能选择正确的内核，运行此笔记本可能会导致意外问题。

### 5. 提供您的 Hugging Face 令牌[#](#provide-your-hugging-face-token)

您需要一个 Hugging Face API 令牌才能访问 Llama-3.1。在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的令牌，并为 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) 申请访问权限。令牌通常以 “hf_” 开头。
...
import torch
max_seq_length = 1024 # 可以为更长的推理轨迹增加
...
model,
r = lora_rank, # 选择任意大于0的数！推荐 8, 16, 32, 64, 128
target_modules = [
...
num_generations = 6, # 若内存不足可减小该值
max_prompt_length = max_prompt_length,
max_completion_length = max_seq_length - max_prompt_length,
# num_train_epochs = 1, # 完整训练时可设置为1
max_steps = 250,
save_steps = 250,
max_grad_norm = 0.1,
report_to = "none", # 可使用Weights & Biases进行报告
output_dir = "outputs",
)
```
...
现在你可以运行训练器。向上滚动以查看奖励表格。目标是看到 `reward`。
...
现在试试你刚刚训练的模型。首先尝试没有进行任何GRPO训练的模型：
...
现在用您刚刚用GRPO训练的LoRA尝试一下，但请先保存该LoRA。
...
推理模型要好得多。它并非总是正确的，因为你只训练了大约一个小时。如果延长序列长度并训练更长时间，它会变得更好。

### 保存为 float16 以用于 VLLM[#](#saving-to-float16-for-vllm)
...
现在你完成了！如果你对Unsloth有任何疑问、需要帮助或想保持更新，他们有 [Discord](https://discord.gg/unsloth) 频道和 [GitHub](https://github.com/unslothai/unsloth)。你也可以查阅他们的 [文档](https://docs.unsloth.ai/) 获取更多信息。