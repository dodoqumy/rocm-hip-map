...
本教程演示了如何在 AMD ROCm（ROCm（Radeon 开放计算平台）） GPU 上利用 [Unsloth](https://unsloth.ai) 微调 Llama-3.1 8B 大型语言模型（LLM）。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”：R1-Zero 通过使用 Group Relative Policy Optimization (GRPO) 自主学会了在没有人类反馈的情况下分配更多思考时间。Unsloth 团队改进了整个 GRPO 流程，使其相比 Hugging Face 和 Flash Attention 2 (FA2) 减少了 80% 的显存用量。这使您能够仅用 7GB 显存，借助 Qwen2.5 (1.5B) 重现 R1-Zero 的成果。
...
**AMD Instinct™ GPUs**：本教程已在 AMD Instinct MI300X GPU 上完成测试。请确保您使用的是支持 ROCm 的 AMD Instinct GPU 或兼容硬件，并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
请改用命令。**Docker**：确保已正确安装和配置Docker。请根据您的操作系统按照Docker安装指南进行操作。**注意**：确保正确配置Docker权限。要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
[Hugging Face](https://huggingface.co)用于下载模型。确保Hugging Face API令牌具有必要的权限和访问批准。
...
容器内的目录。确保在运行 Docker 命令之前已将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传到此目录。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
如果验证过程失败，请确保为您的notebook选择了正确的Jupyter kernel。要更改kernel，请执行以下步骤：
...
**重要**: 如果未能选择正确的kernel，运行notebook时可能导致意外问题。

### 5. 提供你的 Hugging Face token[#](#provide-your-hugging-face-token)

你需要一个 Hugging Face API 令牌才能访问 Llama-3.1。在 [Hugging Face 令牌页面](https://huggingface.co/settings/tokens) 生成你的令牌，并为 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) 请求访问权限。令牌通常以“hf_”开头。

运行以下交互式代码块，在你的 Jupyter notebook 中设置 token：
...
import torch
max_seq_length = 1024  # 可以增加以处理更长的推理轨迹
lora_rank = 32  # 更大的rank意味着更智能，但速度更慢
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    gpu_memory_utilization = 0.6,  # 若内存不足则降低此值
)
model = FastLanguageModel.get_peft_model(
    model,
    r = lora_rank,  # 选择任意大于0的值！建议8, 16, 32, 64, 128
    target_modules = [
...
# num_train_epochs = 1, # 设置为1进行完整的训练运行
max_steps = 250,
save_steps = 250,
max_grad_norm = 0.1,
...
您可能需要等待150到200步才能看到任何动作。最初100步内可能不会获得任何奖励，请耐心等待！
...
推理模型要好得多。它并非总是正确，毕竟你只训练了大约一小时。如果延长序列长度并训练更长时间，效果会更好。

### 将模型保存为float16格式用于VLLM[#](#saving-to-float16-for-vllm)
...
作为回退方案的适配器。使用 `push_to_hub_merged`

为了上传到您的 Hugging Face 账户。请访问 [Hugging Face token settings](https://huggingface.co/settings/tokens) 获取您的个人令牌。
...
现在你已经完成了！如果你对Unsloth有任何疑问、需要帮助或想要持续跟进，他们有一个[Discord](https://discord.gg/unsloth)频道和一个[GitHub](https://github.com/unslothai/unsloth)。你也可以查阅他们的[文档](https://docs.unsloth.ai/)了解更多信息。