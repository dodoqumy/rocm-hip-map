...
本教程演示了如何通过利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm（Radeon 开放计算平台） GPU 上微调 Llama-3.1 8B 大语言模型（LLM）。DeepSeek 的 R1 研究揭示了一个“灵光乍现”时刻：R1-Zero 通过使用群体相对策略优化（GRPO）自主学会了在不依赖人类反馈的情况下分配更多思考时间。Unsloth 团队对整个 GRPO 流程进行了增强，使其比 Hugging Face 和 Flash Attention 2 (FA2) 减少 80% 的显存使用。这让你仅需 7GB 显存，使用 Qwen2.5 (1.5B) 即可复现 R1-Zero 的成就。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.3**: 按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认你的设置：该命令会列出你的AMD GPU及其相关详细信息。

**注意**: 对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`。

command instead。**Docker**：确保已正确安装和配置Docker。请根据您的操作系统遵循Docker安装指南。**注意**：确保Docker权限已正确配置。要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
容器中的目录。请确保在运行Docker命令之前将笔记本文件复制到此目录，或在Jupyter Notebook环境启动后上传到其中。保存终端输出中提供的令牌或URL，以便从Web浏览器访问笔记本。您可以从[AI Developer Hub GitHub仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev)下载此笔记本。
...
确保在运行上述命令之前，你的系统上没有占用此端口。如果已被占用，你可以通过替换 `--port=8888` 来指定不同的端口。
...
# 本笔记本已在 unsloth==2025.3.19 unsloth_zoo==2025.3.17 bitsandbytes==0.43.3.dev0 下验证通过
```
...
您需要一个 Hugging Face API 令牌才能访问 Llama-3.1。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的令牌，并为 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) 请求访问权限。令牌通常以“hf_”开头。
...
lora_rank = 32 # 更大的秩意味着更聪明，但更慢
model, tokenizer = FastLanguageModel.from_pretrained(
    model_name = "meta-llama/meta-Llama-3.1-8B-Instruct",
    max_seq_length = max_seq_length,
    load_in_4bit = False, # 如需LoRA 4bit请设为True
    fast_inference = True, # 启用vLLM快速推理
    max_lora_rank = lora_rank,
    gpu_memory_utilization = 0.6, # 如果内存不足，请降低此值
)
...
gradient_accumulation_steps = 1, # 可增至4以获得更平滑的训练
num_generations = 6, # 如遇内存不足可减小
...
您可能需要等待150到200步才能看到任何动作。前100步可能没有任何奖励，请耐心等待！
...
现在试试你刚训练的模型。首先尝试没有经过任何GRPO训练的模型：
...
现在，使用你刚刚用GRPO训练的LoRA尝试一下，但先保存LoRA。
...
上传到您的 Hugging Face 账户。请访问 [Hugging Face 令牌设置](https://huggingface.co/settings/tokens) 获取您的个人令牌。
...
现在你完成了！如果你对 Unsloth 有任何疑问、需要帮助或想持续获取更新，他们有 [Discord](https://discord.gg/unsloth) 频道和 [GitHub](https://github.com/unslothai/unsloth)。你也可以查阅他们的[文档](https://docs.unsloth.ai/)以获取更多信息。