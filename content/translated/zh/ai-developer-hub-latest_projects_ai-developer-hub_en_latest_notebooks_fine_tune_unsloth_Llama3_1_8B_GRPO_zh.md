...
本教程演示如何通过利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm（ROCm（Radeon 开放计算平台）） GPU 上微调 Llama-3.1 8B 大型语言模型 (LLM)。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”（aha moment），其中 R1-Zero 通过使用群体相对策略优化（Group Relative Policy Optimization, GRPO）自主学会了在没有人类反馈的情况下分配更多思考时间。Unsloth 团队增强了整个 GRPO 流程，使其使用的显存（VRAM）比 Hugging Face 和 Flash Attention 2 (FA2) 减少 80%。这让您能够仅使用 7GB 显存，通过 Qwen2.5 (1.5B) 重现 R1-Zero 的成就。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU**：本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上完成测试。请确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 或兼容硬件，并且您的系统符合[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
改用命令。**Docker**：确保Docker已正确安装和配置。请按照适用于您操作系统的Docker安装指南进行操作。**注意**：确保正确配置Docker权限。要配置权限以允许非root用户访问，请运行以下命令：
...
[Hugging Face](https://huggingface.co)用于下载模型。确保Hugging Face API令牌具有必要的权限和批准，以访问
...
启动Docker容器并映射必要的目录。将 `/path/to/notebooks` 替换为你的实际路径。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口在你的系统上尚未被占用。如果已被占用，你可以通过替换 `--port=8888` 来指定另一个端口。
...
安装本教程所需的库。在Docker容器中运行的Jupyter notebook内执行以下命令：
...
**重要**：未能选择正确的内核可能会导致运行笔记本时出现意外问题。

### 5. 提供您的 Hugging Face token[#](#provide-your-hugging-face-token)

您需要一个 Hugging Face API token 才能访问 Llama-3.1。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的 token，并请求访问 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct)。Token 通常以“hf_”开头。

在您的Jupyter笔记本中运行以下交互块来设置令牌：
...
print(f"令牌验证成功！登录身份：{user_info['name']}")
...
gpu_memory_utilization = 0.6, # 若内存不足可减小
)
model = FastLanguageModel.get_peft_model(
model,
r = lora_rank, # 选择任意大于0的数字！推荐8、16、32、64、128
target_modules = [
...
本教程直接利用了 [willccbb](https://gist.github.com/willccbb/4676755236bb08cab5f4e54a0475d6fb) 进行数据准备和所有奖励函数的设计。您可以自由创建自己的方法。
...
现在试试你刚训练好的模型。首先尝试没有任何GRPO训练的模型：
...
现在用你刚用GRPO训练好的LoRA尝试一下，但请先保存这个LoRA。
...
推理模型要好得多。它并不总是正确的，因为你只训练了大约一小时。如果你延长序列长度并训练更长时间，它会更好。
...
上传到您的 Hugging Face 账户。访问 [Hugging Face token settings](https://huggingface.co/settings/tokens) 获取您的个人令牌。
...
现在你已经完成了！如果你对 Unsloth 有任何疑问、需要帮助，或想保持更新，他们设有一个 [Discord](https://discord.gg/unsloth) 频道和一个 [GitHub](https://github.com/unslothai/unsloth) 仓库。你也可以查阅他们的 [文档](https://docs.unsloth.ai/) 以获取更多信息。