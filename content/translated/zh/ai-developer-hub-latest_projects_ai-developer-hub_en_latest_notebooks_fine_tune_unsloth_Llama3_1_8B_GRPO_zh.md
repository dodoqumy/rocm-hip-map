...
**作者**: [Unsloth](https://unsloth.ai) ，经 [AMD](https://www.amd.com) 修改以在 AMD GPU 上运行。

**知识水平**：中级

本教程演示如何利用[Unsloth](https://unsloth.ai)在AMD ROCm（ROCm（Radeon开放计算平台）） GPU上微调Llama-3.1 8B大语言模型（LLM）。DeepSeek的R1研究揭示了一个“顿悟时刻”（aha moment）：R1-Zero通过使用组相对策略优化（Group Relative Policy Optimization, GRPO），在没有人类反馈的情况下自主学会了分配更多思考时间。Unsloth团队优化了整个GRPO流程，使其显存使用量比Hugging Face和Flash Attention 2（FA2）减少80%。这使得您只需使用Qwen2.5（1.5B）模型在仅7GB VRAM上即可复现R1-Zero的成果。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.3**: 按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的设置：该命令会列出您的AMD GPU及其相关详细信息。
...
[Hugging Face](https://huggingface.co)用于下载模型。请确保Hugging Face API令牌具有必要的权限和批准来访问
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传到此目录。保存终端输出中提供的 token 或 URL，以便从您的 web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
# 本笔记本已验证于 unsloth==2025.3.19 unsloth_zoo==2025.3.17 bitsandbytes==0.43.3.dev0
```
...
**重要**：未能选择正确内核可能导致运行笔记本时出现意外问题。

### 5. 提供您的 Hugging Face token[#](#provide-your-hugging-face-token)

您需要一个 Hugging Face API token 来访问 Llama-3.1。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的 token，并申请 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) 的访问权限。Token 通常以“hf_”开头。
...
load_in_4bit = False, # 如需LoRA 4bit则设为True
...
你可能需要等待 150 到 200 steps 才能看到任何动作。最开始 100 steps 内可能不会获得任何 reward。请耐心等待！
...
现在尝试您刚刚训练好的模型。首先，尝试未经过任何GRPO训练的模型：
...
现在尝试使用你刚刚用GRPO训练好的LoRA，但先保存LoRA。
...
现在您已完成！如果您对 Unsloth 有任何疑问、需要帮助或希望获取最新动态，可以加入他们的 [Discord](https://discord.gg/unsloth) 频道或访问 [GitHub](https://github.com/unslothai/unsloth)。您还可以查阅他们的 [文档](https://docs.unsloth.ai/) 了解更多信息。