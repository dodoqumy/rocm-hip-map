...
本教程演示了如何利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm（ROCm（Radeon 开放计算平台）） GPU 上微调 Llama-3.1 8B 大语言模型（LLM）。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”，即 R1-Zero 通过使用组相对策略优化（GRPO），无需人工反馈便自主学会了分配更多思考时间。Unsloth 团队对整个 GRPO 流程进行了优化，使其 VRAM 使用量比 Hugging Face 和 Flash Attention 2（FA2）减少 80%。这让你仅需 7GB VRAM 即可使用 Qwen2.5（1.5B）复现 R1-Zero 的成就。
...
**ROCm（Radeon 开放计算平台） 6.3**: 请参考 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，通过以下命令确认配置：该命令将列出你的 AMD GPU 及其详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用`rocm-smi`

command instead.**Docker**：确保 Docker 已正确安装和配置。请根据你的操作系统遵循 Docker 安装指南。**注意**：确保 Docker 权限配置正确。要配置允许非 root 用户访问的权限，请运行以下命令：  
...
[Hugging Face](https://huggingface.co)用于下载模型。确保 Hugging Face API 令牌拥有必要的权限和批准来访问
...
启动Docker容器并映射必要的目录。将`/path/to/notebooks`替换为

使用宿主机上存储这些 notebook 的目录的完整路径。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从网页浏览器访问该 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
# 本笔记本已在以下版本验证：unsloth==2025.3.19 unsloth_zoo==2025.3.17 bitsandbytes==0.43.3.dev0
```
...
如果验证过程失败，请确保为笔记本选择了正确的Jupyter kernel。要更改kernel，请按照以下步骤操作：
...
您需要一个 Hugging Face API 令牌才能访问 Llama-3.1。在 [Hugging Face 令牌页面](https://huggingface.co/settings/tokens) 生成您的令牌，并为 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct) 申请访问权限。令牌通常以 "hf_" 开头。

在您的Jupyter notebook中运行以下交互块来设置token：
...
import torch
max_seq_length = 1024 # 可根据更长的推理轨迹增加
lora_rank = 32 # 更大的rank值 = 更智能但更慢
...
gpu_memory_utilization = 0.6, # 当内存不足时降低此值
)
...
现在尝试使用你刚刚用GRPO训练的LoRA，但请先保存LoRA。
...
推理模型要好得多。它并不总是正确的，因为你只训练了它一个小时左右。如果你延长序列长度并训练更长时间，它会更好。

### 保存为float16用于VLLM[#](#saving-to-float16-for-vllm)
...
上传到您的Hugging Face账户。请访问[Hugging Face令牌设置](https://huggingface.co/settings/tokens)获取您的个人令牌。
...
现在你已经完成了！如果你对Unsloth有任何疑问、需要帮助，或者希望获取最新动态，可以加入他们的[Discord](https://discord.gg/unsloth)频道和[GitHub](https://github.com/unslothai/unsloth)社区。你也可以查阅他们的[文档](https://docs.unsloth.ai/)了解更多信息。