...
本教程演示如何利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm（Radeon 开放计算平台） GPU 上微调 Llama-3.1 8B 大语言模型（LLM）。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”（aha moment）：R1-Zero 通过使用分组相对策略优化（GRPO），在无需人类反馈的情况下自主学会了分配更多思考时间。Unsloth 团队对整个 GRPO 流程进行了增强，使其比 Hugging Face 和 Flash Attention 2 (FA2) 减少 80% 的显存占用。这使得你只需使用 Qwen2.5 (1.5B) 在 7GB 显存上即可复现 R1-Zero 的成果。
...
使用命令替代。**Docker**: 确保已正确安装和配置 Docker。按照适用于您的操作系统的 Docker 安装指南操作。**注意**: 确保正确配置了 Docker 权限。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
容器中的目录。请确保在运行 Docker 命令之前将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后将其上传进去。保存终端输出中提供的令牌或 URL，以便从网页浏览器访问笔记本。你可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
确保在运行上述命令之前，该端口尚未被系统占用。如果已被占用，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
# 本笔记本在 unsloth==2025.3.19 unsloth_zoo==2025.3.17 bitsandbytes==0.43.3.dev0 环境下验证通过
```
...
在您的Jupyter notebook中运行以下交互块来设置 token：
...
gradient_accumulation_steps = 1, # 增加至4以使训练更平滑
...
现在尝试你刚刚训练好的模型。首先测试未经过任何GRPO训练的模型：
...
现在尝试使用你刚刚用GRPO训练的LoRA，但先保存这个LoRA。
...
上传到您的Hugging Face账户。访问 [Hugging Face token settings](https://huggingface.co/settings/tokens) 以获取您的个人令牌。
...
现在你已经完成了！如果你对Unsloth有任何问题、需要帮助或想保持更新，他们有[Discord](https://discord.gg/unsloth)频道和[GitHub](https://github.com/unslothai/unsloth)。你也可以查阅他们的[文档](https://docs.unsloth.ai/)了解更多信息。