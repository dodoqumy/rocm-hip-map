...
本教程演示了如何通过利用 [Unsloth](https://unsloth.ai) 在 AMD ROCm（ROCm（Radeon 开放计算平台））GPU 上微调 Llama-3.1 8B 大型语言模型（LLM）。DeepSeek 的 R1 研究揭示了一个“顿悟时刻”，其中 R1-Zero 通过使用 Group Relative Policy Optimization (GRPO) 自主学会了在没有人类反馈的情况下分配更多思考时间。Unsloth 团队改进了整个 GRPO 过程，使其相比 Hugging Face 和 Flash Attention 2 (FA2) 减少了 80% 的显存使用。这使得你可以仅用 7GB 的 VRAM，使用 Qwen2.5 (1.5B) 重现 R1-Zero 的成就。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.3**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)来安装和验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认你的设置：该命令会列出你的AMD GPU及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用`rocm-smi`

**命令。**Docker：确保已正确安装和配置Docker。请遵循适用于您操作系统的Docker安装指南。

**注意**：确保已正确配置Docker权限。要配置允许非root用户访问的权限，请运行以下命令：
...
[Hugging Face](https://huggingface.co) 用于下载模型。请确保 Hugging Face API 令牌拥有必要的权限和批准，以访问该
...
容器中的目录。请确保在运行 Docker 命令之前将笔记本文件复制到此目录，或在容器启动后将其上传到 Jupyter Notebook 环境中。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
**重要**: 选择不正确的 kernel 可能会导致运行 notebook 时出现意外问题。

### 5. 提供你的Hugging Face令牌[#](#provide-your-hugging-face-token)

你需要一个 Hugging Face API token 才能访问 Llama-3.1。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成你的 token，并申请访问 [Llama-3.1 8B](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct)。Token 通常以 “hf_” 开头。
...
gpu_memory_utilization = 0.6, # 如果内存不足可减小
)
...
你可能需要等待150到200步才能看到任何动作。前100步很可能不会得到任何奖励。请耐心等待！

步骤 | 训练损失 | 奖励 | 奖励标准差 | 生成长度 | KL散度 |
|---|---|---|---|---|---|---|
1 |
0.000000 |
0.125000 |
...
推理模型好多了。它并非总是正确，因为你只训练了大约一个小时。如果延长序列长度并训练更长时间，效果会更好。
...
现在您已完成操作！如果您对Unsloth有任何疑问、需要帮助或希望保持更新，他们有 [Discord](https://discord.gg/unsloth) 频道和 [GitHub](https://github.com/unslothai/unsloth) 仓库。您也可以查阅他们的 [文档](https://docs.unsloth.ai/) 以获取更多信息。