...
- [使用 ROCm（Radeon 开放计算平台）进行 AI 开发](../index.html){.nav-link}
- [使用 ROCm（Radeon 开放计算平台）进行 AI 推理优化](index.html){.nav-link}
- 模型\...
...
- [AMD Quark](#amd-quark){.reference .internal .nav-link}
  - [安装 Quark](#installing-quark){.reference .internal .nav-link}
  - [使用 Quark 进行量化](#using-quark-for-quantization){.reference .internal .nav-link}
  - [使用 vLLM 评估量化模型](#evaluating-the-quantized-model-with-vllm){.reference .internal .nav-link}
- [GPTQ](#gptq){.reference .internal .nav-link}
  - [安装 AutoGPTQ](#installing-autogptq){.reference .internal .nav-link}
  - [使用 GPTQ 与 AutoGPTQ](#using-gptq-with-autogptq){.reference .internal .nav-link}
  - [使用 GPTQ 与 Hugging Face Transformers](#using-gptq-with-hugging-face-transformers){.reference .internal .nav-link}
  - [ExLlama-v2 支持](#exllama-v2-support){.reference .internal .nav-link}
- [bitsandbytes](#bitsandbytes){.reference .internal .nav-link}
  - [安装 bitsandbytes](#installing-bitsandbytes){.reference .internal .nav-link}
  - [使用 bitsandbytes 原语](#using-bitsandbytes-primitives){.reference .internal .nav-link}
  - [使用 bitsandbytes 与 Hugging Face Transformers](#using-bitsandbytes-with-hugging-face-transformers){.reference .internal .nav-link}

# 模型量化技术[\#](#model-quantization-techniques "Link to this heading"){.headerlink}
...
[ ![](data:image/svg+xml;base64,){.sd-octicon .sd-octicon-clock} ]{.sd-pr-2 .article-info-read-time-svg} 11 分钟阅读时间

适用于 Linux



量化会减小模型大小，相较于原生全精度版本，使大模型更容易适配到内存有限的 GPU 上。本节说明如何在 AMD Instinct 硬件上使用 AMD Quark、GPTQ 和 bitsandbytes 进行 LLM 量化。

## AMD Quark[\#](#amd-quark "链接到此标题"){.headerlink}

[AMD Quark](https://quark.docs.amd.com/latest/){.reference .external} 提供领先的高效可扩展量化解决方案，专为 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））GPU 打造。支持对激活值、权重和 KV 缓存进行 [`FP8`{.docutils .literal .notranslate}]{.pre} 和 [`INT8`{.docutils .literal .notranslate}]{.pre} 量化，包括 [`FP8`{.docutils .literal .notranslate}]{.pre} 注意力机制。对于超大规模模型，采用两级 [`INT4-FP8`{.docutils .literal .notranslate}]{.pre} 方案——以 [`INT4`{.docutils .literal .notranslate}]{.pre} 存储权重、以 [`FP8`{.docutils .literal .notranslate}]{.pre} 进行计算——实现近 4× 压缩率且不影响精度。Quark 可在多 GPU 上高效扩展，高效处理 Llama-3.1-405B 等超大模型。Llama、Mixtral 和 Grok-1 等量化 [`FP8`{.docutils .literal .notranslate}]{.pre} 模型可在 [Hugging Face 的 AMD 组织](https://huggingface.co/collections/amd/quark-quantized-ocp-fp8-models-66db7936d18fcbaf95d4405c){.reference .external} 中获取，并可通过 [vLLM](https://github.com/vllm-project/vllm/tree/main/vllm){.reference .external} 直接部署。

### 安装 Quark[\#](#installing-quark "Link to this heading"){.headerlink}



Quark 的最新版本可以使用 pip 进行安装
...
有关详细的安装说明，请参阅 [Quark 文档](https://quark.docs.amd.com/latest/install.html){.reference .external}。



### 使用 Quark 进行量化[\#](#using-quark-for-quantization "Link to this heading"){.headerlink}

1. 首先，使用 Hugging Face [`transformers`{.docutils .literal .notranslate}]{.pre} 库加载预训练模型及其对应的 tokenizer。

```python
from transformers import AutoTokenizer, AutoModelForCausalLM
```

This is a very short input. The user wants me to translate this technical text to Simplified Chinese. Let me analyze what needs to be translated:

1. "MODEL_ID = "meta-llama/Llama-2-70b-chat-hf"" - This is code/variable assignment, so I should preserve it as is since it's technical/inline code.
2. "MAX_SEQ_LEN = 512" - This is also code/variable assignment, should be preserved.

Wait, looking at the rules again:
- Rule 1: "Preserve ALL markdown formatting, code blocks, inline code, and links unchanged."
- Rule 3: "Keep API names, function names, file paths, commands unchanged."

These lines appear to be code/technical content, so according to the rules they should be preserved unchanged.

However, let me reconsider - this could be part of a larger document that needs translation. But looking at just this snippet:
- It's showing variable assignments in what appears to be Python code
- These are technical identifiers that should remain unchanged

Given the rules about preserving code and technical terms, I should output these lines unchanged. The code syntax itself (variable assignments, equals signs) is universal, and the specific values like model IDs and sequence lengths are technical identifiers that should not be translated.

Actually, re-reading the input more carefully - this appears to be a partial code snippet. Following the rule to preserve code blocks unchanged, I should return this as-is.

But wait - let me check if there's any actual prose here that needs translation. The input shows:
```
MODEL_ID = "meta-llama/Llama-2-70b-chat-hf"
MAX_SEQ_LEN = 512
```

This is entirely code. According to Rule 1, I preserve code blocks unchanged. According to Rule 3, I keep technical terms unchanged. There's nothing to translate here - it's pure code/technical identifiers.

I should output the exact same content.
...
3. 定义量化配置。请参阅以下代码片段中的注释，了解各个配置选项的说明。
