---
title: "AMD ROCm documentation"
source_url: "https://rocm.docs.amd.com/en/latest/index.html"
source_type: "official"
source_org: "amd"
original_lang: "en"
credibility: 5
lifecycle: "latest"
synced_date: 2026-04-28
---

:::: header-article-items__start
::: header-article-item
[]{.fa-solid .fa-angle-right}

::::: header-article-items__end
:::: header-article-item
::: article-header-buttons

# AMD ROCm documentation

# AMD ROCm documentation[\#](#amd-rocm-documentation "Link to this heading"){.headerlink}

[ ![](data:image/svg+xml;base64,){.sd-octicon .sd-octicon-calendar} ]{.sd-pr-2 .article-info-date-svg} 2026-04-17

[ ![](data:image/svg+xml;base64,){.sd-octicon .sd-octicon-clock} ]{.sd-pr-2 .article-info-read-time-svg} 3 min read time

Applies to Linux and Windows

ROCm is an open-source software platform optimized to extract HPC and AI workload performance from AMD Instinct GPUs and AMD Radeon GPUs while maintaining compatibility with industry software frameworks. For more information, see [[What is ROCm?]{.std .std-doc}](what-is-rocm.html){.reference .internal}

ROCm supports multiple programming languages and programming interfaces such as [[HIP]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/HIP/en/latest/index.html "(in HIP Documentation v7.2.53211)"){.reference .external}, OpenCL, and OpenMP, as explained in the [[Programming guide]{.std .std-doc}](how-to/programming_guide.html){.reference .internal}.

If you're using AMD Radeon™ GPUs or Ryzen™ APUs for graphics workloads, see the [[ROCm on Radeon and Ryzen]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/radeon-ryzen/en/latest/index.html "(in Use ROCm™ on Radeon™ and Ryzen™ Documentation)"){.reference .external} documentation.

Note

The [AMD ROCm Programming Guide](https://rocm-handbook.amd.com/projects/amd-rocm-programming-guide/en/latest/){.reference .external} presents key ROCm concepts in a structured, book-style format, a helpful starting point for those new to GPU programming.

ROCm documentation is organized into the following categories:

Install

- [[ROCm on Linux]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html "(in ROCm installation on Linux v7.2.2)"){.reference .external}

- [[HIP SDK on Windows]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/install-on-windows/en/latest/reference/system-requirements.html "(in HIP SDK for Windows v7.1.1)"){.reference .external}

- [[ROCm on Radeon and Ryzen]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/radeon-ryzen/en/latest/index.html "(in Use ROCm™ on Radeon™ and Ryzen™ Documentation)"){.reference .external}

- [[Deep learning frameworks]{.doc}](how-to/deep-learning-rocm.html){.reference .internal}

- [[Build from source]{.doc}](how-to/build-rocm.html){.reference .internal}

How to

- [[Use ROCm for AI]{.std .std-doc}](how-to/rocm-for-ai/index.html){.reference .internal}

- [AI tutorials](https://rocm.docs.amd.com/projects/ai-developer-hub/en/latest/){.reference .external}

- [[Use ROCm for HPC]{.std .std-doc}](how-to/rocm-for-hpc/index.html){.reference .internal}

- [[System optimization]{.std .std-doc}](how-to/system-optimization/index.html){.reference .internal}

- [[AMD Instinct MI300X performance validation and tuning]{.std .std-doc}](how-to/tuning-guides/mi300x/index.html){.reference .internal}

- [[System debugging]{.std .std-doc}](how-to/system-debugging.html){.reference .internal}

- [[Use advanced compiler features]{.std .std-doc}](conceptual/compiler-topics.html){.reference .internal}

- [[Set the number of CUs]{.doc .std .std-doc}](how-to/setting-cus.html){.reference .internal}

- [[Troubleshoot BAR access limitation]{.std .std-doc}](how-to/Bar-Memory.html){.reference .internal}

- [ROCm examples](https://github.com/amd/rocm-examples){.reference .external}

Conceptual

- [[GPU architecture overview]{.std .std-doc}](conceptual/gpu-arch.html){.reference .internal}

- [[File structure (Linux FHS)]{.std .std-doc}](conceptual/file-reorg.html){.reference .internal}

- [[GPU isolation techniques]{.std .std-doc}](conceptual/gpu-isolation.html){.reference .internal}

- [[Using CMake]{.std .std-doc}](conceptual/cmake-packages.html){.reference .internal}

- [[Inception v3 with PyTorch]{.std .std-doc}](conceptual/ai-pytorch-inception.html){.reference .internal}

Reference

- [[ROCm libraries]{.std .std-doc}](reference/api-libraries.html){.reference .internal}

- [[ROCm tools, compilers, and runtime API]{.std .std-doc}](reference/rocm-tools.html){.reference .internal}

- [[GPU hardware specifications]{.std .std-doc}](reference/gpu-arch-specs.html){.reference .internal}

- [[Hardware atomics operation support]{.std .std-doc}](reference/gpu-atomics-operation.html){.reference .internal}

- [[Environment variables]{.std .std-doc}](reference/env-variables.html){.reference .internal}

- [[Data types and precision support]{.std .std-doc}](reference/precision-support.html){.reference .internal}

- [[Graph safe support]{.std .std-doc}](reference/graph-safe-support.html){.reference .internal}

- [[ROCm glossary]{.std .std-doc}](reference/glossary.html){.reference .internal}

:::: prev-next-area
[](what-is-rocm.html "next page"){.right-next}

::: prev-next-info
next

What is ROCm?