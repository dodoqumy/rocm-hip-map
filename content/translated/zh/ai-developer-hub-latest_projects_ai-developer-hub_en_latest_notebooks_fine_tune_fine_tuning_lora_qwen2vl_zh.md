...
本笔记本演示了如何使用 Hugging Face 生态系统，特别是 [参数高效微调 (PEFT)](https://huggingface.co/docs/peft/index) 和 [Transformer 强化学习 (TRL)](https://huggingface.co/docs/trl/index) 库，对 [视觉语言模型 (VLM)](https://huggingface.co/blog/vlms) 进行微调。
...
你将对 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上进行微调。该数据集包含各种图表类型的图像以及对应的问题-答案对，这对于增强模型的视觉问答能力非常理想。
...
**AMD Instinct（AMD 数据中心 GPU 系列）™ GPU**：本教程已在 AMD Instinct MI300X GPU 上测试。请确保您使用的是支持 ROCm（Radeon 开放计算平台）的 AMD Instinct GPU 或兼容硬件，并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon开放计算平台））6.2**：按照[ROCm安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm。安装完成后，使用以下命令确认您的配置：该命令将列出您的AMD GPU及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用`rocm-smi`

command instead. **Docker**: 确保Docker已正确安装和配置。请遵循适用于您操作系统的Docker安装指南。**注意**: 确保Docker权限已正确配置。要配置权限以允许非root用户访问，请运行以下命令：
...
容器中的目录。确保在运行Docker命令之前将笔记本文件复制到此目录，或者在容器启动后将其上传到Jupyter Notebook环境中。保存终端输出中提供的令牌或URL，以便通过Web浏览器访问笔记本。你可以从[AI Developer Hub GitHub仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs)下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未被系统使用。如果已被占用，请通过更改 `--port=8888` 来指定另一个端口。
...
登录 Hugging Face 以上传您的微调模型。您需要使用您的 Hugging Face 账户进行验证，才能直接从该笔记本中保存和分享您的模型。
...
接下来，为VLM生成一条系统消息。该系统充当分析图表图像的专家，并基于这些图表提供简洁的答案。
...
你的任务是分析所提供的图表图像，并对查询给出简洁的回答，通常是一个单词、数字或短语。
图表包括多种类型（例如，折线图、条形图），并包含颜色、标签和文本。
专注于基于视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外解释。"""
```

```

将数据集格式化为聊天机器人结构以便进行交互。每个交互包括一条系统消息，随后是图像和用户的查询，最后是查询的答案。
...
出于教育目的，你只需加载数据集中每个分区的10%。而在实际应用场景中，通常需要加载所有样本。
...
在加载数据集后，加载模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够理解视觉数据和文本的视觉语言模型（VLM）。
...
创建一个方法，该方法接收模型、处理器和样本作为输入，以生成模型的答案。这可以让你简化推理过程，并轻松评估VLM的性能。
...
虽然模型成功检索到了正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在是时候进行微调过程了。

### 移除模型并清理GPU[#](#remove-the-model-and-clean-the-gpu)

在继续下一部分的模型训练之前，请清除当前变量并清理GPU以释放资源。
...
**注意**：也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。欲了解更多关于量化的内容，请参阅 [Hugging Face 的这篇博文](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇博文](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练设置配置 LoRA。LoRA 通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，是在不牺牲质量的情况下优化模型性能的绝佳选择。
...
使用监督微调（SFT）优化模型在任务上的表现。为此，可通过 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类来定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的响应。该方法可确保模型针对你的特定用例进行定制，从而在理解和响应视觉查询方面实现更佳性能。
...
你可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将笔记本连接到 W&B，以便在训练期间捕获关键信息。
...
模型需要一个collator函数，以便在训练过程中正确地检索和批处理数据。该函数会格式化模型的数据集输入，确保它们被正确结构化。请在下方定义collator函数。
...
def collate_fn(examples):
    ...
    # 标签使用 input_ids，并在损失计算中掩码填充令牌
    labels = batch["input_ids"].clone()  # 克隆输入ID作为标签
    labels[labels == processor.tokenizer.pad_token_id] = -100  # 在标签中掩码填充令牌
    ...
    for image_token_id in image_tokens:
        labels[labels == image_token_id] = -100  # 在标签中掩码图像令牌 ID
...
在成功微调您的视觉语言模型（VLM）之后，现在是时候评估其性能了。本节使用ChartQA数据集中的示例来测试模型，以考察它回答基于图表图像的问题的能力。这提供了一种探索结果的好方法。
...
将训练好的适配器附加到预训练模型上。该适配器包含训练期间所做的微调调整，使得基础模型能够利用新知识，而无需改变其核心参数。集成适配器增强了模型的能力，同时保持了其原始结构。
...
这个样本来自训练集，因此模型在训练过程中已经见过它。这可能被视为某种作弊行为。为了更全面地了解模型的性能，请使用一个未见过的样本进行评估。
...
模型已成功学会按照数据集中指定的方式响应查询。你已经实现了你的目标！

## 比较微调模型与使用提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探索了微调VLM是使其适应您特定需求的一种有价值的选择。另一种可考虑的方法是直接使用prompting或实现一个RAG系统，这在另一篇[recipe](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，你可以尝试使用提示方法，看看是否无需微调的开销就能达到类似的效果。
...
这个案例再次使用了之前的样本，但这次包含了如下的系统消息。这种增强有助于为模型提供上下文输入，可能提高其响应准确性。
...
如所示，模型使用预训练模型以及额外的系统消息生成正确答案，无需任何训练。根据具体用例，这种方法可成为微调的可行替代方案。