...
本笔记本演示了如何使用Hugging Face生态系统微调[视觉语言模型(VLM)](https://huggingface.co/blog/vlms)，特别是使用[参数高效微调(PEFT)](https://huggingface.co/docs/peft/index)和[Transformer强化学习(TRL)](https://huggingface.co/docs/trl/index)库。
...
你将在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上微调 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型。该数据集包含各种图表类型的图像以及对应的问答对，非常适合用于增强模型的视觉问答能力。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**: 本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上测试完成。请确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 或兼容硬件，并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认配置：该命令将列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**: 确保 Docker 已正确安装和配置。请遵循您操作系统的 Docker 安装指南。**Note**: 确保 Docker 权限已正确配置。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用了来自Hugging Face的示例数据集，该数据集在设置步骤中准备。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未被系统使用。如果已被占用，请通过替换 `--port=8888` 来指定其他端口。
...
登录Hugging Face以上传你微调后的模型。你需要用你的Hugging Face账户进行身份验证，以便直接从本notebook保存和共享你的模型。
...
在本节中，你将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含图表图像及其对应的问题和答案，非常适合用于训练视觉问答任务。

接下来，为VLM生成一条系统消息。这将创建一个系统，使其成为分析图表图像的专家，并根据这些图表提供简洁的答案。
...
**未能选择正确的 kernel 可能会导致运行 notebook 时出现意外问题。**

```
system_message = """你是一个专门解读图表图像视觉数据的视觉语言模型。
你的任务是分析所提供的图表图像，并以简洁的答案回应查询，通常是一个单词、数字或短句。
...
将数据集格式化为聊天机器人结构以进行交互。每个交互由一条系统消息、图像和用户查询，以及最终的查询答案组成。
...
出于教育目的，你只会加载数据集中每个子集的10%。然而，在实际使用场景中，你通常会加载全部样本。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及第四个你将要丢弃的特征。
...
为了评估模型性能，使用数据集中的一个样本。首先，检查该样本的内部结构。
...
创建一个方法，接受模型、处理器和样本作为输入，以生成模型的答案。这可以让你简化推理过程并轻松评估VLM的性能。
...
尽管模型能够成功检索到正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在正是进行微调过程的时候。
...
**注意**：或者，可使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇博客](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，是优化模型性能且不牺牲质量的绝佳选择。
...
使用监督微调（SFT）来优化模型在特定任务上的性能。为此，通过 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型根据输入学习生成更准确的回答。这种方法确保模型针对您的具体用例进行定制，从而在理解和响应视觉查询方面获得更佳表现。
...
模型需要一个collator函数，以便在训练过程中正确检索和批处理数据。该函数将数据集的输入格式化为模型所需的格式，确保其结构正确。请在下方定义collator函数。

更多详情，请参阅 TRL 示例 [脚本](https://github.com/huggingface/trl/blob/main/examples/scripts/sft_vlm.py#L87)。
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的包装器，并继承其属性和方法。该类通过提供一个 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时，正确初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model)，从而简化微调过程。通过使用 `SFTTrainer`
...
既然您已成功微调了您的Vision Language Model (VLM)，现在是时候评估其性能了。本节使用ChartQA数据集中的示例来测试模型，以查看其基于图表图像回答问题的能力。这提供了一种探索结果的好方法。
...
将训练好的适配器附加到预训练模型上。该适配器包含您在训练期间所做的微调调整，使基础模型能够在不改变其核心参数的情况下利用新知识。集成适配器增强了模型的能力，同时保持了其原始结构。
...
这个样本来自训练集，因此模型在训练过程中已经见过它。这可以被视为一种作弊行为。为了更全面地了解模型的性能，请使用未见过的样本进行评估。
...
您已经了解了微调VLM如何成为适配特定需求的宝贵选项。另一个值得考虑的方法是直接使用提示（Prompting）或实现RAG系统，这在另一篇[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调视觉语言模型（VLM）需要大量数据和计算资源，这会产生成本。相比之下，你可以尝试使用提示工程（prompting）来观察是否能在无需微调开销的情况下达到类似效果。
...
这个案例再次使用了之前的样本，但这次包含了如下的系统消息。这一增强有助于为模型提供上下文输入，可能提高其响应准确性。
...
如所示，模型使用预训练模型以及额外的系统消息生成正确答案，无需任何训练。根据具体使用场景，这种方法可以作为微调的可行替代方案。