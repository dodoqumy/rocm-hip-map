...
本笔记本演示了如何使用 Hugging Face 生态系统，特别是通过 [Parameter-Efficient Fine-Tuning (PEFT)](https://huggingface.co/docs/peft/index) 和 [Transformer Reinforcement Learning (TRL)](https://huggingface.co/docs/trl/index) 库，对 [Vision Language Model (VLM)](https://huggingface.co/blog/vlms) 进行微调。
...
您将使用[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型进行微调。该数据集包含各种图表类型的图像以及对应的问答对，非常适合增强模型的视觉问答能力。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：该命令会列出您的 AMD GPU 及其相关详细信息。

**注意**: 对于 ROCm（Radeon 开放计算平台）6.4 及更早版本，请使用 `rocm-smi`。

改用命令。**Docker**：确保Docker已正确安装和配置。请按照您操作系统的Docker安装指南进行操作。**注意**：确保Docker权限正确配置。要配置允许非root访问的权限，请运行以下命令：
...
容器内的目录。确保在运行 Docker 命令前已将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传到其中。保存终端输出中提供的令牌或 URL，以便通过网页浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未在你的系统上被使用。如果已被占用，请通过替换 `--port=8888` 来指定一个不同的端口。
...
import torch
print("检测到一个 ROCm（ROCm（Radeon 开放计算平台））-GPU？", torch.cuda.is_available())
...
```
# 安装微调所需的必要库，包括参数高效微调（peft）和transformers库
...
登录Hugging Face以上传你微调后的模型。你需要通过Hugging Face账户进行身份验证，才能直接从本笔记本保存和共享你的模型。
...
你是一个分析图表图像的专家，能基于这些图表提供简洁准确的答案。

⚠️ 重要：请确保选择了正确的内核

如果验证过程失败，请确保为笔记本选择了正确的Jupyter内核。要更改内核，请执行以下步骤：
...
你的任务是分析所提供的图表图像，并根据查询给出简洁的答案，通常为一个单词、数字或短语。
图表类型多样（如折线图、柱状图），包含颜色、标签和文字。
重点是基于视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外解释。"""
```

```

将数据集格式化为用于交互的聊天机器人结构。每次交互由一条系统消息、随后的图像和用户查询、最后是对查询的回答组成。
...
出于教学目的，您将仅加载数据集中每个划分的10%。然而，在实际应用场景中，您通常会加载全部样本。
...
接下来，查看数据集的结构。它包含一张图片、一条查询、一个标签（即答案），以及一个将被丢弃的第四个特征。
...
在加载数据集之后，加载模型，并使用数据集中的一个样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够同时理解视觉数据和文本的视觉语言模型 (VLM)。
...
评估模型性能时，使用数据集中的一个样本。首先，检查该样本的内部结构。
...
使用不含系统消息的样本来评估VLM的原始理解能力。请使用以下输入：
...
请提供具体的图表和查询内容，以便我根据视觉信息作答。当前未收到样本图表或对应的查询描述。
...
创建一个方法，该方法以模型（model）、处理器（processor）和样本（sample）作为输入，生成模型的答案。这可以让你简化推理过程并轻松评估VLM的性能。
...
尽管模型能够成功检索到正确的视觉信息，但在准确回答问题上仍有困难。这表明微调可能是提升其性能的关键。现在，是时候进行微调流程了。

### 移除模型并清理 GPU[#](#remove-the-model-and-clean-the-gpu)

在进入下一节的模型训练之前，请先清除当前变量并清理GPU以释放资源。
...
time.sleep(2)
# Garbage collection and clearing CUDA（CUDA（统一计算设备架构）） memory
gc.collect()
time.sleep(2)
...
**注意**：另外，也可通过 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。欲了解量化更多信息，可参阅 [来自 Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置配置 LoRA。LoRA 通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，是优化模型性能而不牺牲质量的绝佳选择。
...
模型需要一个collator函数来在训练过程中正确地检索和批处理数据。该函数将数据集输入格式化为模型可用的形式，确保其结构正确。请在下方定义collator函数。

更多详情，请参阅 TRL 示例 [scripts](https://github.com/huggingface/trl/blob/main/examples/scripts/sft_vlm.py#L87)。

```
# 创建一个数据整理函数，用于编码文本和图像对
...
既然您已成功微调了视觉语言模型（VLM），现在是时候评估其性能了。本部分将使用ChartQA数据集中的示例对模型进行测试，以观察其基于图表图像回答问题的能力。这为探索结果提供了一种良好的途径。
...
将训练好的adapter附加到预训练模型上。该adapter包含您在训练期间进行的微调调整，使基础模型能够利用新知识而不改变其核心参数。集成adapter在保持模型原有结构的同时增强了其能力。
...
此样本来自训练集，因此模型在训练过程中已见过该样本。这在一定程度上可视为一种作弊行为。为了更全面地了解模型的性能，请使用未见过的样本进行评估。
...
你已经了解到对VLM进行微调可以是使其适应特定需求的一种有价值的选择。另一种值得考虑的方法是直接使用提示（prompting）或实现RAG系统，这在另一篇[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中进行了介绍。

微调一个VLM需要大量的数据和计算资源，这可能会产生相应成本。相比之下，你可以尝试通过提示实验，看是否能在没有微调开销的情况下获得类似效果。
...
此案例再次使用了之前的示例，但这次包含了系统消息如下。这一增强有助于为模型提供输入语境，可能提高其响应准确性。
...
如所示，模型使用预训练模型及附加的系统消息生成了正确答案，无需任何训练。根据具体用例，此方法可作为fine-tuning的一种可行替代方案。