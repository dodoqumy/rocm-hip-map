...
本笔记本演示了如何使用Hugging Face生态系统，特别是利用[Parameter-Efficient Fine-Tuning (PEFT)](https://huggingface.co/docs/peft/index)和[Transformer Reinforcement Learning (TRL)](https://huggingface.co/docs/trl/index)库来微调一个[Vision Language Model (VLM)](https://huggingface.co/blog/vlms)。
...
你将使用 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集对 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型进行微调。该数据集包含各种图表类型的图像及其对应的问答对，非常适合增强模型的视觉问答能力。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**: 本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上测试。请确保您使用的是具有 ROCm（ROCm（Radeon 开放计算平台））支持的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 或兼容硬件，并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照 [ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认设置：  
此命令列出你的 AMD GPU 及其相关信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`

命令。**Docker**：确保 Docker 已正确安装和配置。请按照适用于您的操作系统的 Docker 安装指南操作。**注意**：确保正确配置了 Docker 权限。要配置允许非 root 用户访问的权限，请运行以下命令：
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
在运行上述命令之前，请确保该端口尚未在您的系统上使用。如果已在使用中，请通过替换 `--port=8888` 来指定不同的端口。
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，才能从此 notebook 中直接保存和分享您的模型。
...
在本节中，您将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含与相关问题及答案配对的图表图像，非常适合用于视觉问答任务的训练。

You are an expert in analyzing chart images. Your task is to examine the provided chart and answer user questions concisely and accurately based solely on the information shown in the chart.

**⚠️ 重要：确保选择了正确的kernel**

如果验证过程失败，请确保为您的笔记本选择了正确的Jupyter kernel。要更改kernel，请遵循以下步骤：
...
```
system_message = """你是一个专门解读图表图像中视觉数据的视觉语言模型（Vision Language Model）。
你的任务是分析提供的图表图像，并对查询给出简洁的回答，通常是一个单词、数字或短句。
...
将数据集格式化为交互式聊天机器人结构。每次交互包括一条系统消息，随后是图像和用户的查询，最后是查询的答案。
...
为了教学目的，你将只加载数据集中每个分片的10%。然而，在实际使用场景中，你通常会加载全部样本。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及第四个特征（你将要丢弃）。
...
创建一个方法，该方法以模型、处理器和样本为输入，生成模型的答案。这可以让你精简推理过程，并轻松评估VLM的性能。
...
虽然模型能够成功检索到正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在，是时候进行微调流程了。
...
**注意**：或者，可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博文](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练设置配置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，使其成为在不牺牲质量的情况下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在任务上的性能。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index) 中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的回复。这种方法确保了模型针对你的特定用例进行定制，从而在理解和响应视觉查询方面获得更好的性能。
...
你可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将你的 notebook 连接到 W&B，以在训练期间捕获关键信息。
...
模型需要一个 collator 函数，以便在训练过程中正确地检索和批处理数据。该函数对数据集的输入进行格式化，确保其结构正确。请在下面定义该 collator 函数。
...
现在你已经成功微调了视觉语言模型（VLM），是时候评估其性能了。本节使用ChartQA数据集中的示例测试模型，以观察它基于图表图像回答问题的效果。这为探索结果提供了一种很好的方式。
...
将训练好的适配器附加到预训练模型上。该适配器包含您在训练期间所做的微调调整，使基础模型能够在不更改其核心参数的情况下利用新知识。集成适配器增强了模型的能力，同时保持了其原始结构。
...
你已经了解了对VLM进行微调是使其适应特定需求的宝贵方法。另一种值得考虑的方式是直接使用提示（prompting）或实现RAG系统，这在另一篇[教程](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量数据和计算资源，这可能会产生成本。相比之下，您可以尝试使用提示（prompting）来观察是否能达到类似的效果，而无需承担微调的开销。
...
本案例再次使用之前的样本，但这次包含了如下系统消息。这一增强有助于为模型提供输入上下文，可能提高其响应准确性。
...
如所示，该模型使用预训练模型以及附加的系统消息生成了正确的答案，无需任何训练。根据具体的用例，这种方法可以作为微调的一个可行替代方案。