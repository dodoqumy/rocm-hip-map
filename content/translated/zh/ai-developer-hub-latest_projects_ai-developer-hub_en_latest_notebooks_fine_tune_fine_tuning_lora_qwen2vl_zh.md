...
本笔记本演示了如何使用Hugging Face生态系统微调[视觉语言模型(VLM)](https://huggingface.co/blog/vlms)，具体借助[参数高效微调(PEFT)](https://huggingface.co/docs/peft/index)和[Transformer强化学习(TRL)](https://huggingface.co/docs/trl/index)库。
...
你将使用 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集对 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型进行微调。该数据集包含多种图表类型的图像及其对应的问题-答案对，非常适合增强模型的视觉问答能力。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**: 本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上测试。请确保您使用的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 或兼容硬件具有 ROCm（ROCm（Radeon 开放计算平台）） 支持，且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**: 按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认设置：  
该命令将列出您的AMD GPU及其详细信息。

**注意**: 对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用`rocm-smi`

command instead.**Docker**：确保 Docker 已正确安装和配置。请按照适用于您操作系统的 Docker 安装指南进行操作。**注意**：确保 Docker 权限配置正确。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用了来自 Hugging Face 的示例数据集，该数据集在设置步骤中准备就绪。
...
容器中的目录。请确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs)下载此 notebook。
...
确保在运行上述命令之前，该端口尚未被您的系统使用。如果已被使用，请通过替换 `--port=8888` 来指定一个不同的端口。
...
import torch
# 检测到的是ROCm（ROCm（Radeon 开放计算平台））GPU吗？
print("检测到的是ROCm（ROCm（Radeon 开放计算平台））GPU吗？ ", torch.cuda.is_available())
# 检测到多少个ROCm（ROCm（Radeon 开放计算平台））GPU？
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，才能直接从此 notebook 中保存和共享您的模型。
...
在本节中，你将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含与相关问题及答案配对的图表图像，非常适合用于视觉问答任务的训练。

接下来，为VLM生成一条系统消息。这将创建一个作为分析图表图像专家的系统，并根据这些图表为问题提供简洁的答案。
...
将数据集格式化为用于交互的聊天机器人结构。每次交互包含一条系统消息，随后是图像和用户的查询，最后是对查询的回答。

关于此模型的更多使用技巧，请参阅[Model Card](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
出于教育目的，您仅加载数据集中每个划分的10%。然而，在实际使用场景中，通常会加载所有样本。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及你将丢弃的第四个特征。
...
在加载数据集之后，加载模型并使用数据集中的一个样本来评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够理解视觉数据和文本的视觉语言模型（VLM）。
...
创建一个方法，以模型、处理器和样本作为输入来生成模型的答案。这样可以简化推理过程，并轻松评估VLM的性能。
...
尽管模型成功检索到正确的视觉信息，但在准确回答问题方面仍存在困难。这表明fine-tuning可能是提升其性能的关键。现在是时候进行fine-tuning过程了。
...
def clear_memory():
    ...
    print(f"GPU 预留内存: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")
clear_memory()
```

```

## 使用 TRL 对模型进行微调[#](#fine-tune-the-model-using-trl)
...
**注**：也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇博客](https://www.maartengrootendorst.com/blog/quantization/)。
...
使用监督微调（SFT）来优化模型在任务上的表现。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index) 中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的回答。这种方法确保模型针对你的特定用例进行调整，从而在理解和响应视觉查询方面获得更好的性能。
...
你可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录你的训练进度。将你的笔记本连接到 W&B，以在训练过程中捕获关键信息。

```
import wandb
wandb.init(
project="qwen2-7b-instruct-trl-sft-ChartQA", # 请更改此项
name="qwen2-7b-instruct-trl-sft-ChartQA", # 请更改此项
...
模型需要一个整理函数（collator function）来在训练过程中正确地检索和批处理数据。该函数将数据集输入格式化为模型所需的格式，确保它们结构正确。请在下面定义整理函数。

有关更多详情，请查看 TRL 示例 [scripts](https://github.com/huggingface/trl/blob/main/examples/scripts/sft_vlm.py#L87)。
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的包装器，并继承其属性和方法。该类通过正确初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model)（当提供了 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时）来简化微调过程。通过使用 `SFTTrainer`
...
现在您已成功微调了视觉语言模型（VLM），接下来需要评估其性能。本节将使用ChartQA数据集中的示例来测试模型，考察其基于图表图像回答问题的能力。这提供了一个很好的方式来探索结果。
...
将训练好的适配器附加到预训练模型上。该适配器包含训练期间进行的微调调整，使基础模型能够利用新知识，同时不改变其核心参数。集成适配器在保持模型原有结构的同时，增强了其能力。
...
该样本来自训练集，因此模型在训练过程中已遇到过该样本。这可以被视为一种作弊行为。为了更全面地了解模型的性能，请使用未见过的样本进行评估。
...
模型已成功学会按照数据集中的指定来回应查询。您已达成目标！

## 比较微调模型与带有提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探索了微调VLM如何成为将其适配到特定需求的有价值选项。另一个可以考虑的方法是直接使用prompting或实现RAG系统，这在另一个[recipe](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量的数据和计算资源，这会产生成本。相比之下，你可以尝试使用提示（prompting）来验证是否能在无需微调开销的情况下获得类似效果。
...
此案例再次使用之前的样本，但这次包含了如下系统消息。此增强有助于为模型提供输入上下文，可能提升其响应准确性。
...
如所示，该模型利用预训练模型及附加的系统消息生成了正确回答，无需任何训练。根据具体用例，这种方法可作为微调的有效替代方案。