...
本文档演示了如何使用 Hugging Face 生态系统，特别是通过[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer 强化学习（TRL）](https://huggingface.co/docs/trl/index)库，对[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)进行微调。
...
你将在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上微调 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型。该数据集包含多种图表类型的图像，并配有问答对，非常适合提升模型的视觉问答能力。
...
**ROCm（Radeon 开放计算平台）6.2**：请按照[ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm。安装完成后，使用以下命令确认您的设置：该命令将列出您的 AMD GPU 及其相关信息。

**注意**: 对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用`rocm-smi`

command instead.**Docker**: 确保正确安装并配置了Docker。请遵循适用于您操作系统的Docker安装指南。**注意**: 确保正确配置Docker权限。要配置允许非root用户访问的权限，请运行以下命令:usermod -aG docker $USER newgrp docker
...
这个教程使用了一个来自Hugging Face的示例数据集，该数据集在设置步骤中准备好。
...
确保您的系统满足[系统需求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
容器中的目录。确保在运行 Docker 命令之前已将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未被占用。如果已被占用，请通过替换 `--port=8888` 来指定一个不同的端口。
...
登录Hugging Face以上传你微调后的模型。你需要通过Hugging Face账户进行身份验证，才能直接从该notebook保存和分享你的模型。
...
你是一个专业的图表图像分析专家。你的任务是仔细分析用户提供的图表图像，准确理解其数据、趋势和关键信息。针对用户基于该图表提出的问题，给出简洁、准确、直接的回答。避免不必要的解释或额外信息，只回答与图表内容直接相关的问题。
...
你的任务是分析提供的图表图像，并用简洁的答案回复查询，通常是一个单词、数字或短句。
图表包括多种类型（例如，折线图、柱状图），并包含颜色、标签和文本。
专注于根据视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外的解释。"""
```

```

将数据集格式化为用于交互的聊天机器人结构。每个交互包含一个系统消息，然后是图像和用户查询，最后是对查询的回答。

针对此模型的更多使用技巧，请参阅[模型卡片](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
出于教学目的，你仅加载数据集中每个分片的10%。然而，在实际使用场景中，你通常会加载整个样本集。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及一个你将要丢弃的第四特征。
...
在加载数据集之后，加载模型并使用数据集中的一个样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够理解视觉数据和文本的视觉语言模型（Vision Language Model，VLM）。
...
使用没有系统消息的样本来评估VLM的原始理解。以下是需使用的输入：
...
创建一个方法，该方法将模型（model）、处理器（processor）和样本（sample）作为输入，以生成模型的答案。这可以让你简化推理过程并轻松评估VLM的性能。
...
尽管模型能成功检索到正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在该进入微调阶段了。
...
**注意**：或者，也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练设置配置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提升训练效率，因此成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督式微调（SFT）来优化模型在任务上的性能。为此，使用[TRL库](https://huggingface.co/docs/trl/index)中的[SFTConfig](https://huggingface.co/docs/trl/sft_trainer)类定义训练参数。SFT提供带标签的数据，帮助模型根据其输入学习生成更准确的响应。该方法确保模型针对特定用例进行调整，从而在理解和响应视觉查询方面获得更好的性能。
...
greater_is_better=False, # 是否指标值越大越好
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将您的笔记本连接到 W&B，以在训练过程中捕获关键信息。
...
模型需要一个 collator 函数来在训练过程中正确检索和批处理数据。该函数格式化模型的数据集输入，确保其结构正确。请在下方定义 collator 函数。

更多详情，请参见TRL示例[脚本](https://github.com/huggingface/trl/blob/main/examples/scripts/sft_vlm.py#L87)。

```
# 创建一个数据整理器来编码文本和图像对
def collate_fn(examples):
    # 获取文本和图像，并应用聊天模板
    texts = [processor.apply_chat_template(example, tokenize=False) for example in examples]  # 准备文本以进行处理
    ...
    for image_token_id in image_tokens:
        labels[labels == image_token_id] = -100  # 在标签中掩码图像标记ID
...
，您可以高效管理训练工作流程，并确保视觉语言模型获得顺畅的微调体验。
...
现在您已成功微调了视觉语言模型（VLM），是时候评估其性能了。本节将使用 ChartQA 数据集中的示例来测试模型，以观察它基于图表图像回答问题的表现。这是一种探索结果的有效方法。
...
该样本来自训练集，因此模型在训练期间已见过该样本。这可以被视为一种作弊形式。为了更全面地了解模型的表现，请使用未见过的样本进行评估。
...
模型已经成功学会按照数据集中指定的方式回答查询。你达成了目标！

## 比较微调模型与使用提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探讨了微调VLM如何成为使其适应特定需求的宝贵选项。另一个值得考虑的方法是直接使用提示或实现RAG系统，这在另一[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有详细介绍。

微调视觉语言模型（VLM）需要大量数据和计算资源，这会带来成本。相比之下，你可以尝试通过提示工程（prompting）来观察其是否能在无需微调开销的情况下达到类似效果。
...
此例再次使用之前的示例，但这次包含了系统消息如下。这一增强有助于为模型提供上下文输入，可能提高其响应准确性。
...
如所示，模型使用预训练模型以及额外的系统消息即可生成正确答案，无需任何训练。根据具体使用场景，这种方法可以作为微调的一种可行替代方案。