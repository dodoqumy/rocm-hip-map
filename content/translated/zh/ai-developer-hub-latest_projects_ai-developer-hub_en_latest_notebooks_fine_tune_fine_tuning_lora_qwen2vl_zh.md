...
您将对 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上进行微调。该数据集包含各种图表类型的图像以及对应的问题-答案对，这对于增强模型的视觉问答能力非常理想。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**：本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 上完成测试。请确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））GPU 或兼容硬件，且系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（Radeon 开放计算平台） 6.2**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认您的配置：该命令将列出您的 AMD GPU 及其相关详细信息。
...
在运行上述命令之前，请确保该端口尚未被您的系统占用。如果已被占用，请通过替换 `--port=8888` 来指定其他端口。

使用另一个端口号，例如 `--port=8890`

你是一名专注于GPU/ROCm/HIP文档的技术翻译人员。将以下英文文本翻译成简体中文（zh-CN）。规则：1. 保留所有markdown格式、代码块、行内代码和链接不变。2. 保留ROCm、HIP、GPU、CUDA、AMD、PyTorch、TensorFlow等术语的英文原样。3. 保留API名称、函数名、文件路径、命令不变。4. 仅输出翻译内容——不包含解释、注释或前言。5. 使用GPU开发人员预期的技术中文。
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，以便从此笔记本直接保存和分享您的模型。
...
在本节中，您将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含与相关问题和答案配对的图表图像，非常适合用于视觉问答任务的训练。

你是一个图表图像分析专家。根据提供的图表图像，给出简洁准确的答案。
...
你的任务是分析提供的图表图像，并用简洁的答案回应查询，通常是一个词、数字或短语。
...
由于是教学目的，你只需加载数据集中每个分片的10%。但在实际应用场景中，通常会加载所有样本。
...
加载数据集后，加载模型并使用数据集中的样本评估其性能。本教程使用[Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够同时理解视觉数据和文本的视觉语言模型（VLM）。
...
为评估模型性能，使用数据集中的一个样本。首先，检查该样本的内部结构。
...
创建一个方法，该方法以模型、处理器和样本作为输入来生成模型的答案。这允许你简化推理过程并轻松评估VLM的性能。
...
尽管模型成功检索到正确的视觉信息，但在准确回答问题方面仍显吃力。这表明微调可能是提升其性能的关键所在。现在，正是着手进行微调流程的时候了。
...
**注意**：或者，也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的内容，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的博客](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提升训练效率，使其成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在该任务上的性能。为此，利用 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型根据输入学习生成更准确的响应。这种方法确保模型针对你的具体用例进行定制，从而在理解并回答视觉查询时获得更佳性能。
...
greater_is_better=False, # 更高的指标值是否更好
...
模型需要一个collator函数，以便在训练过程中正确地检索和批量处理数据。该函数会格式化数据集的输入，确保其结构正确。请在下方定义collator函数。
...
现在您已成功微调了您的视觉语言模型（VLM），是时候评估其性能了。本节使用ChartQA数据集中的示例来测试模型，观察它基于图表图像回答问题的效果如何。这是探索结果的良好方式。
...
将训练好的适配器附加到预训练模型上。该适配器包含训练过程中所做的微调调整，使基础模型能够利用新知识而无需改变其核心参数。集成适配器可以增强模型的能力，同时保持其原始结构。
...
该样本来自训练集，因此模型在训练期间已见过该样本。这可能导致评估结果存在一定程度的作弊。为了更全面地了解模型性能，请使用未见过的样本进行评估。
...
该模型已成功学习如何按照数据集中的指定方式响应查询。您已达成目标！

## 比较微调模型与带有提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探索了如何微调VLM，这是使其适应您的特定需求的一个有价值的选择。另一种可以考虑的方法是直接使用提示（prompting）或实现RAG系统，这在另一个[配方](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，你可以尝试通过提示来实验，看是否能在没有微调开销的情况下获得类似的结果。
...
该案例再次使用之前的样本，但这次包含了如下的系统消息。这一增强有助于为模型提供输入上下文，可能提高其响应准确性。
...
如演示所示，该模型使用预训练模型以及附加的系统消息，无需任何训练即可生成正确答案。根据具体用例，此方法可作为微调的一种可行替代方案。