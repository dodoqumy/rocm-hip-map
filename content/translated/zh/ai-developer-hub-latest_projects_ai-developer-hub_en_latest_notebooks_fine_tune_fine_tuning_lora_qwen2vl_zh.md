...
本笔记本演示了如何使用Hugging Face生态系统，具体利用[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库来微调一个[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)。
...
您将使用 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集微调 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型。该数据集包含各种图表类型的图像及配对的问题-答案对，非常适合提升模型的视觉问答能力。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU**：本教程在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上进行了测试。请确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 或兼容硬件，且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
改用命令。**Docker**：确保 Docker 已正确安装和配置。请根据您的操作系统，遵循 Docker 安装指南。**注意**：确保 Docker 权限已正确配置。要配置权限以允许非 root 用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用了来自Hugging Face的示例数据集，该数据集在设置步骤中已准备好。
...
容器中的目录。确保在运行Docker命令之前将notebook文件复制到此目录，或者在Jupyter Notebook环境启动后上传进去。保存终端输出中提供的令牌或URL，以便从Web浏览器访问notebook。您可以从 [AI Developer Hub GitHub仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此notebook。
...
在运行上述命令之前，请确保该端口在您的系统上尚未被占用。如果已被占用，请通过替换 `--port=8888` 来指定其他端口。
...
登录Hugging Face以上传您微调后的模型。您需要使用您的Hugging Face账户进行身份验证，以便直接从本notebook保存和分享您的模型。
...
在本节中，你将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含图表图像及其对应的问题与答案，使其非常适合训练视觉问答任务。

接下来，为VLM生成一条系统消息。这将创建一个系统，该系统作为分析图表图像的专家，并根据这些图表提供简洁的问题答案。
...
```
system_message = """你是专门解释图表图像中的视觉数据的视觉语言模型。
你的任务是分析提供的图表图像，并用简洁的答案回答查询，通常为一个单词、数字或短语。
图表包含多种类型（例如线图、条形图），并包含颜色、标签和文本。
专注于根据视觉信息提供准确、简洁的答案。除非绝对必要，避免额外的解释。"""
```

```

将数据集格式化为聊天机器人结构以进行交互。每个交互包括一条系统消息，然后是图像和用户的查询，最后是查询的答案。
...
出于教育目的，您将仅加载数据集中每个拆分（split）的10%。然而，在实际应用场景中，您通常会加载全部样本。
...
接下来，查看数据集的结构。它包含一个图像、一个查询、一个标签（即答案），以及你将丢弃的第四个特征。
...
在加载数据集后，加载模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct) 模型，这是一个能够理解视觉数据和文本的视觉语言模型（VLM）。
...
为了评估模型性能，请使用数据集中的一个样本。首先，检查该样本的内部结构。
...
创建一个方法，该方法以模型（model）、处理器（processor）和样本（sample）作为输入，用于生成模型的答案。这样可以帮助您简化推理过程，并轻松评估VLM的性能。
...
尽管模型成功检索到了正确的视觉信息，但在准确回答问题上仍存在困难。这表明微调可能是提升其性能的关键。现在该进行微调过程了。
...
**注意**：或者，可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 来加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇博客文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置设置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，这使其成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调(SFT)来优化模型在该任务上的性能。为此，使用来自[TRL库](https://huggingface.co/docs/trl/index)的[SFTConfig](https://huggingface.co/docs/trl/sft_trainer)类定义训练参数。SFT提供带标签的数据，帮助模型学习基于输入生成更准确的响应。这种方法确保模型针对你的特定用例进行定制，从而在理解和响应视觉查询方面获得更好的性能。
...
training_args = SFTConfig(
    ...
    warmup_ratio=0.03,   # 预热阶段占总步数的比例
    # Hub和报告
    push_to_hub=False,   # 是否将模型推送到Hugging Face Hub，默认禁用
    report_to=None,   # 用于跟踪指标的报告工具
    # 梯度检查点设置
    gradient_checkpointing_kwargs={"use_reentrant": False},   # 梯度检查点的附加选项
    # 数据集配置
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将笔记本连接到 W&B，以便在训练过程中捕获关键信息。

```
import wandb
wandb.init(
project="qwen2-7b-instruct-trl-sft-ChartQA", # 更改此项目名
name="qwen2-7b-instruct-trl-sft-ChartQA",    # 更改此运行名
...
模型需要一个整理函数（collator function），以便在训练过程中正确地检索和批处理数据。该函数对模型的数据集输入进行格式化，确保其结构正确。请在下方定义整理函数。

对于更多细节，请参见TRL示例[scripts](https://github.com/huggingface/trl/blob/main/examples/scripts/sft_vlm.py#L87)。
...
def collate_fn(examples):
    ...
    # 标签为 input_ids，并在损失计算中掩码填充标记
    labels = batch["input_ids"].clone()  # 克隆 input_ids 作为标签
    labels[labels == processor.tokenizer.pad_token_id] = -100  # 在标签中掩码填充标记
    # 在损失计算中忽略图像标记索引（模型特定）
...
现在你已经成功微调了视觉语言模型（VLM），是时候评估其性能了。本节使用 ChartQA 数据集中的示例来测试模型，观察模型基于图表图像回答问题的效果。这为探索结果提供了一种很好的方法。
...
将训练好的适配器附加到预训练模型上。该适配器包含您在训练期间进行的微调调整，使得基础模型能够利用新知识而无需更改其核心参数。集成适配器可在保持原始结构的同时增强模型的能力。
...
这个样本来自训练集，因此模型在训练过程中已经见过它。这可以被视为一种作弊行为。为了更全面地了解模型的性能，应使用未见过的样本进行评估。
...
该模型已成功学会按照数据集中的规定回应查询。你已经实现了你的目标！

## 比较微调模型与使用提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探索了如何将微调VLM作为适应特定需求的有价值选项。另一种值得考虑的方法是直接使用提示（prompting）或实现RAG系统，这在另一篇[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量的数据和计算资源，这会产生成本。相比之下，你可以尝试通过提示（prompting）来观察是否能在没有微调开销的情况下达到类似的结果。
...
此案例再次使用了之前的样本，但这次包含了如下的系统消息。这一增强有助于为模型提供输入上下文，可能提高其响应准确性。
...
如所示，模型使用预训练模型及附加的系统消息生成了正确答案，无需任何训练。此方法可作为微调的一种可行替代方案，具体取决于特定用例。