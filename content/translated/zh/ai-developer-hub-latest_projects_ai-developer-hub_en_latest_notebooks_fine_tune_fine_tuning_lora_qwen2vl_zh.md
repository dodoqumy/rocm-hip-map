...
本笔记本演示了如何使用 Hugging Face 生态系统，特别是结合 [参数高效微调（PEFT）](https://huggingface.co/docs/peft/index) 和 [Transformer 强化学习（TRL）](https://huggingface.co/docs/trl/index) 库，对 [视觉语言模型（VLM）](https://huggingface.co/blog/vlms) 进行微调。

**注意**：本notebook源自[fine_tuning_vlm_trl](https://huggingface.co/learn/cookbook/en/fine_tuning_vlm_trl)。

## 模型与数据集概览[#](#model-and-dataset-overview)

您将在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上对 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型进行微调。该数据集包含各种图表类型的图像及其对应的问答对，非常适合增强模型的视觉问答能力。
...
**AMD Instinct™ GPU**：本教程已在 AMD Instinct MI300X GPU 上测试。请确保你使用的是支持 ROCm 的 AMD Instinct GPU 或兼容硬件，并且系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的配置：该命令将列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`。

命令替代。**Docker**：确保 Docker 已正确安装和配置。请按照适用于您操作系统的 Docker 安装指南进行操作。**注意**：确保 Docker 权限已正确配置。要配置权限以允许非 root 用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
容器中的目录。确保在运行 Docker 命令前将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传到其中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此笔记本。
...
登录 Hugging Face 上传你微调后的模型。你需要用你的 Hugging Face 账户进行身份验证，以便直接从这个 notebook 保存和分享你的模型。
...
接下来，为 VLM 生成一条系统消息。这将创建一个系统，该系统作为分析图表图像并根据这些图表提供简洁答案的专家。
...
**选择错误的kernel可能导致运行notebook时出现意外问题。**

```
system_message = """你是一个专门从图表图像中解读视觉数据的视觉语言模型。
你的任务是分析提供的图表图像，并以简洁的答案（通常是一个单词、数字或短语）回应查询。
...
将数据集格式化为用于交互的聊天机器人结构。每次交互包含一条系统消息，随后是图像和用户的查询，最后是查询的答案。

针对此模型更多使用技巧，请参阅[Model Card](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
出于教学目的，你只需要加载数据集中每个分片的10%。然而，在实际应用场景中，通常你会加载整个样本集。
...
在加载数据集之后，加载模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct) 这一视觉语言模型（VLM），该模型能够理解视觉数据和文本。
...
接下来，加载模型和 tokenizer 以准备进行 inference。
...
使用不带系统消息的样本评估VLM的原始理解。以下是需使用的输入：
...
创建一个方法，该方法以模型、处理器和样本为输入，生成模型的答案。这样即可简化推理过程，并轻松评估VLM的性能。
...
虽然模型能够成功检索到正确的视觉信息，但在准确回答问题上仍有困难。这表明微调可能是提升其性能的关键。现在该进入微调流程了。

### 移除模型并清理 GPU[#](#remove-the-model-and-clean-the-gpu)

在开始下一节的模型训练之前，请清除当前变量并清理GPU以释放资源。

```
import gc
import time
def clear_memory():
# 如果变量存在于当前全局作用域，则删除它们
...
**注意**：或者，可以通过使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [this blog post from Hugging Face](https://huggingface.co/blog/merve/quantization) 或 [one from Maarten Grootendorst](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置设置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提升训练效率，是在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在任务上的性能。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index) 中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供标注数据，帮助模型学习根据输入生成更准确的响应。这种方法确保模型针对你的特定用例进行定制，从而在理解和响应视觉查询方面获得更好的性能。
...
per_device_train_batch_size=4, # 每个设备训练批大小
per_device_eval_batch_size=4, # 每个设备评估批大小
...
你可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将你的笔记本连接到 W&B 以在训练期间捕获关键信息。
...
模型需要一个collator函数来在训练过程中正确地检索和批处理数据。该函数为模型格式化数据集输入，确保它们结构正确。请在下面定义collator函数。
...
def collate_fn(examples):
    ...
    labels[labels == processor.tokenizer.pad_token_id] = -100 # 屏蔽标签中的填充标记
    # 在损失计算中忽略图像标记索引（模型特定）
    if isinstance(processor, Qwen2VLProcessor): # 检查处理器是否为 Qwen2VLProcessor
        image_tokens = [151652, 151653, 151655] # Qwen2VLProcessor 的特定图像标记ID
    else:
        image_tokens = [processor.tokenizer.convert_tokens_to_ids(processor.image_token)] # 将图像标记转换为ID
    # 屏蔽标签中的图像标记ID
    for image_token_id in image_tokens:
        labels[labels == image_token_id] = -100 # 屏蔽标签中的图像标记ID
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是围绕 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的包装器，并继承其属性和方法。当提供了 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时，该类通过正确地初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model) 来简化微调过程。通过使用 `SFTTrainer`
...
现在你已经成功微调了你的视觉语言模型（VLM），是时候评估其性能了。本节使用ChartQA数据集中的示例来测试模型，看看它基于图表图像回答问题的能力如何。这是探索结果的好方法。
...
将训练好的适配器附加到预训练模型上。该适配器包含您在训练期间所做的微调调整，使基础模型能够在不改变核心参数的情况下利用新知识。集成适配器可在保持原有结构的同时增强模型的能力。
...
该样本来自训练集，因此模型在训练过程中已见过它。这可视作某种形式的作弊。为更全面地了解模型性能，请使用未见过的样本进行评估。
...
该模型已成功学会按照数据集中指定的方式响应查询。你已实现目标！

## 比较微调模型与通过提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经探索了微调VLM如何成为使其适应特定需求的有价值选项。另一个需要考虑的方法是直接使用提示或实现RAG系统，这在另一篇[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有介绍。

微调视觉语言模型（VLM）需要大量的数据和计算资源，这可能会产生高昂的成本。相比之下，您可以尝试通过提示工程（prompting）来探索是否能在无需微调开销的情况下达到类似的效果。
...
此案例再次使用了之前的示例，但这次包含了如下系统消息。这种增强有助于为模型提供输入的上下文，可能提高其响应准确性。
...
如图所示，该模型仅通过预训练模型及附加的系统消息便生成了正确答案，无需任何训练。根据具体应用场景，这种方法可作为微调的有效替代方案。