...
这个笔记本演示了如何使用Hugging Face生态系统，特别是通过[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[变换器强化学习（TRL）](https://huggingface.co/docs/trl/index)库，对[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)进行微调。
...
你将在[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集上对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型进行微调。该数据集包含多种图表类型的图像及其对应的问答对，非常适合提升模型的视觉问答能力。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**：本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 上测试。请确保您使用的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））GPU 或兼容硬件支持 ROCm（ROCm（Radeon 开放计算平台）），并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认您的设置：此命令会列出您的 AMD GPU 及相关详细信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`

使用命令代替。**Docker**：确保 Docker 已正确安装和配置。请根据您的操作系统参考 Docker 安装指南。**注意**：确保正确配置 Docker 权限。要配置权限以允许非 root 用户访问，请运行以下命令：
...
用于下载模型的 [Hugging Face](https://huggingface.co)。
...
在运行上述命令之前，请确保该端口尚未在您的系统上被占用。如果已被占用，请通过替换 `--port=8888` 来指定一个不同的端口。
...
```
# 安装进行微调所需的库，包括参数高效微调(peft)和transformer
...
登录Hugging Face以上传您的微调模型。您需要使用Hugging Face账户进行身份验证，以便直接从本notebook保存和共享您的模型。
...
try:
    ...
    print(f"Token 验证成功！登录为：{user_info['name']}")
...
You are an expert in analyzing chart images. Given a chart, answer questions concisely and accurately based on the visual data. Provide only the direct answer without unnecessary explanation, unless the question asks for reasoning.

**⚠️ 重要：确保选择了正确的kernel**

如果验证过程失败，请确保为笔记本选择了正确的 Jupyter 内核。要更改内核，请按照以下步骤操作：
...
**未能选择正确的kernel会导致运行notebook时出现意外问题。**

```
system_message = """你是一个视觉语言模型，专门从图表图像中解读视觉数据。
你的任务是分析提供的图表图像，并以简洁的答案（通常是一个单词、数字或短语）回应查询。
这些图表包含各种类型（例如折线图、柱状图），并包含颜色、标签和文本。
专注于基于视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外解释。"""
```

```

将数据集格式化为聊天机器人结构以便交互。每个交互由系统消息、图像和用户查询，以及最后对查询的答案组成。
...
由于教学目的，您只需加载数据集中每个分片的10%。然而，在实际应用场景中，通常需要加载全部样本。
...
接下来，查看数据集的结构。它包含一个图像、一个查询、一个标签（即答案），以及一个你将要丢弃的第四个特征。
...
使用没有系统消息的样本评估VLM的原始理解。以下是使用的输入：
...
虽然模型成功检索到正确的视觉信息，但它在准确回答问题方面仍有困难。这表明微调可能是提升其性能的关键。现在正是进行微调的时候了。

### 移除模型并清理GPU[#](#remove-the-model-and-clean-the-gpu)

在进入下一节训练模型之前，请先清除当前变量并清理GPU以释放资源。
...
**注意**：另一种选择是使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置设置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提升训练效率，是在不损失质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在任务上的性能。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的响应。这种方法确保模型针对您的特定用例进行定制，从而在理解和响应视觉查询方面获得更好的性能。
...
per_device_train_batch_size=4, # 训练的批次大小
per_device_eval_batch_size=4, # 评估的批次大小
...
模型需要一个collator函数来在训练过程中正确地检索和批处理数据。该函数为模型格式化数据集输入，确保它们结构正确。在下方定义collator函数。
...
labels = batch["input_ids"].clone() # 克隆 input IDs 作为标签
...
成功微调您的视觉语言模型（VLM）后，接下来需要评估其性能。本节将使用 ChartQA 数据集中的示例测试模型，检验它在基于图表图像回答问题方面的能力。这为探索结果提供了一种有效方式。
...
将训练好的适配器附加到预训练模型上。该适配器包含了您在训练期间进行的微调调整，使得基础模型在不改变其核心参数的情况下能够利用新知识。集成适配器增强了模型的能力，同时保持了其原始结构。
...
该模型已成功学会按照数据集中指定的方式响应查询。你已达成目标！

## 比较微调模型与带提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探讨了微调VLM如何成为将其适配到特定需求的有价值选项。另一个值得考虑的方法是直接使用提示或实现RAG系统，这在另一篇[recipe](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中介绍。

微调一个VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，您可以尝试使用prompting来观察是否能在没有微调开销的情况下取得类似的结果。
...
此案例再次使用了之前的示例，但这次包含了如下系统消息。这一增强有助于将输入置于上下文中供模型理解，可能提高其响应准确性。
...
如所示，该模型使用预训练模型以及额外的系统消息生成正确的答案，无需任何训练。根据具体使用情况，这种方法可以作为微调的一个可行替代方案。