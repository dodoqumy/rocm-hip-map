...
本笔记本演示了如何利用Hugging Face生态系统对[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)进行微调，具体使用[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库。
...
您将在[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集上对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型进行微调。该数据集包含各种图表类型的图像以及对应的问题-答案对，非常适合增强模型的视觉问答能力。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**: 按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，通过以下命令确认您的配置：该命令会列出您的AMD GPU及其详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`

改用命令。**Docker**：确保Docker已正确安装和配置。请根据您的操作系统参考Docker安装指南。**注意**：确保Docker权限正确配置。要配置允许非root用户访问，请运行以下命令：
...
本教程使用来自 Hugging Face 的一个示例数据集，该数据集在设置步骤中准备。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问 notebook。你可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
在运行上述命令之前，请确保该端口尚未被系统占用。如果已被占用，请通过替换`--port=8888`来指定其他端口。
...
登录 Hugging Face 以上传你微调后的模型。你需要通过 Hugging Face 账户进行身份验证，才能直接从本 notebook 保存并分享你的模型。
...
**无法选择正确的内核可能导致运行笔记本时出现意外问题。**

```
system_message = """你是一个专门解读图表图像视觉数据的视觉语言模型。
你的任务是分析所提供的图表图像，并以简洁的答案回应用户的查询，通常为一个单词、数字或短句。
图表类型多样（例如折线图、柱状图），包含颜色、标签和文本。
专注于基于视觉信息提供准确且简洁的答案，除非绝对必要，否则避免额外解释。"""```

```

将数据集格式化为聊天机器人结构以便交互。每个交互包含一条系统消息，后跟图像和用户的查询，最后是对查询的回答。
...
出于教学目的，您只会加载数据集中每个划分的10%。然而，在实际使用案例中，您通常会加载全部样本集。
...
接下来，查看数据集的结构。它包括一个图像、一个查询、一个标签（即答案），以及你将丢弃的第四个特征。
...
在加载数据集之后，加载模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够同时理解视觉数据和文本的视觉语言模型（VLM）。
...
创建一个方法，该方法将模型、处理器和样本作为输入，以生成模型的答案。这可以使您简化推理过程并轻松评估VLM的性能。
...
尽管模型能够成功检索到正确的视觉信息，但它在准确回答问题时仍存在困难。这表明微调可能是提升其性能的关键。现在，是时候进行微调过程了。

### 移除模型并清理 GPU[#](#remove-the-model-and-clean-the-gpu)

在进入下一节训练模型之前，请清除当前变量并清理 GPU 以释放资源。
...
**注意**：或者，也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [this blog post from Hugging Face](https://huggingface.co/blog/merve/quantization) 或 [one from Maarten Grootendorst](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置 LoRA。LoRA 通过应用低秩近似来降低内存使用，从而进一步减少内存需求并提高训练效率，是在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在任务上的性能。为此，请使用 [TRL 库](https://huggingface.co/docs/trl/index) 中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的回答。这种方法确保模型针对你的具体用例进行定制，从而在理解和响应视觉查询方面取得更好的性能。
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将您的笔记本连接到 W&B，以便在训练期间捕获关键信息。
...
模型需要一个collator函数来在训练过程中正确地检索和批处理数据。该函数将数据集输入格式化以供模型使用，确保它们结构正确。请在下方定义collator函数。
...
, 您可以高效地管理训练工作流，并确保视觉语言模型获得流畅的微调体验。
...
现在您已经成功微调了视觉语言模型（VLM），接下来需要评估其性能。本节将使用ChartQA数据集中的示例对模型进行测试，观察它基于图表图像回答问题的表现如何。这为探索结果提供了良好的途径。
...
将训练好的适配器附加到预训练模型上。该适配器包含了您在训练期间所做的微调调整，使基础模型能够利用新知识而不改变其核心参数。集成适配器可在保持模型原有结构的同时增强其能力。
...
该样本来自训练集，因此模型在训练过程中已见过它。这可视作一种作弊行为。为更全面地了解模型性能，请使用未见过的样本进行评估。
...
模型已成功学会了按照数据集中的指定方式回应查询。你已经达成了目标！

## 比较微调模型与基于提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经探索了如何微调VLM（视觉语言模型）是使其适应特定需求的一个有价值的选择。另一种考虑的方法是直接使用提示（prompting）或实现RAG系统，这在另一份[配方](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有介绍。

微调VLM需要大量数据和计算资源，这可能产生成本。相比之下，你可以尝试通过提示（prompting）来观察是否能在无需微调开销的情况下获得类似结果。
...
此案例再次使用了之前的样本，但这次包含了如下的系统消息。这一增强有助于模型理解输入的上下文，从而可能提高其响应的准确性。
...
如所示，模型使用预训练模型和额外的系统提示（system message）生成了正确答案，无需任何训练。根据具体用例，这种方法可作为微调（fine-tuning）的可行替代方案。