...
本笔记本演示了如何使用Hugging Face生态系统，特别是通过[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库来微调[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)。
...
你将在[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集上对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型进行微调。该数据集包含各种图表类型的图像以及对应的问题-答案对，非常适合提升模型的视觉问答能力。
...
**AMD Instinct™（Instinct™（AMD 数据中心 GPU 系列））GPU**：本教程在 AMD Instinct™（Instinct™（AMD 数据中心 GPU 系列））MI300X GPU 上进行了测试。请确保您在使用支持 ROCm™（ROCm™（Radeon 开放计算平台））的 AMD Instinct™（Instinct™（AMD 数据中心 GPU 系列））GPU 或兼容硬件，且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认配置：此命令将列出您的AMD GPU及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**：确保 Docker 已正确安装和配置。请遵循适用于您操作系统的 Docker 安装指南。**注意**：确保 Docker 权限已正确配置。要配置权限以允许非 root 用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用来自Hugging Face的一个示例数据集，该数据集在设置步骤中准备。
...
在运行上述命令之前，请确保该端口尚未被您的系统使用。如果已被占用，请通过替换 `--port=8888` 来指定另一个端口。
...
登录 Hugging Face 以上传你微调后的模型。你需要使用 Hugging Face 账户进行身份验证，才能直接从本笔记本保存和分享你的模型。
...
try:
    ...
    print(f"Token 验证成功！当前登录用户为: {user_info['name']}")
...
接下来，为VLM生成一条系统消息。这将创建一个系统，使其成为分析图表图像的专家，并能基于这些图表提供简洁的答案。
...
出于教育目的，你只会加载数据集中每个 split 的 10%。然而，在实际使用场景中，你通常会加载全部样本。
...
接下来，查看数据集的结构。它包含一个图像、一个查询、一个标签（即答案），以及第四个特征，该特征将被丢弃。
...
加载数据集后，加载模型并使用数据集中的样本评估其性能。本教程使用[Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个能够理解视觉数据和文本的视觉语言模型(Vision Language Model, VLM)。
...
使用不带系统消息的样本来评估VLM的原始理解。以下是要使用的输入：
...
创建一个接受模型、处理器和样本作为输入的方法，以生成模型的答案。这可以让您简化推理过程并轻松评估VLM的性能。
...
虽然模型成功检索了正确的视觉信息，但在准确回答问题上仍存在困难。这表明微调可能是提升其性能的关键。现在该进入微调流程了。

### 移除模型并清理GPU[#](#remove-the-model-and-clean-the-gpu)

在进入下一部分训练模型之前，清除当前变量并清理GPU以释放资源。
...
**注意**：量化模型也可以通过[bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index)加载。要了解量化的更多信息，请参阅[Hugging Face的这篇博客](https://huggingface.co/blog/merve/quantization)或[Maarten Grootendorst的这篇文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练设置配置LoRA。LoRA通过应用低秩近似来降低内存使用，从而进一步降低内存需求并提高训练效率，使其成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在任务上的表现。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型根据输入生成更准确的响应。这种方法确保模型针对特定使用场景进行定制，从而在理解和响应视觉查询方面获得更优的性能。
...
# Hub和报告
push_to_hub=False, # 是否推送模型到Hugging Face Hub，默认禁用。
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将您的笔记本连接到 W&B，以捕获训练过程中的关键信息。
...
模型需要一个 collator 函数来在训练过程中正确地检索和批处理数据。该函数格式化数据集输入，确保它们结构正确。请在下面定义 collator 函数。

有关更多详细信息，请参阅 TRL 示例 [scripts](https://github.com/huggingface/trl/blob/main/examples/scripts/sft_vlm.py#L87)。

```
# 创建一个数据整理器，用于编码文本和图像对
def collate_fn(examples):
    # 获取文本和图像，并应用聊天模板
    texts = [processor.apply_chat_template(example, tokenize=False) for example in examples]  # 准备文本进行处理
    ...
    for image_token_id in image_tokens:
        labels[labels == image_token_id] = -100  # 屏蔽标签中的图像标记 ID
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的封装，并继承了其属性和方法。该类通过正确初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model)（当提供了 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时），简化了微调过程。通过使用 `SFTTrainer`
...
现在您已成功微调了视觉语言模型（VLM），接下来是评估其性能的时候了。本节将使用 ChartQA 数据集中的示例测试模型，观察它基于图表图像回答问题的效果。这为探索结果提供了良好的方式。
...
将训练好的适配器附加到预训练模型上。该适配器包含了您在训练期间进行的微调调整，使得基础模型能够利用新知识，同时无需修改其核心参数。集成适配器可以在保持模型原始结构的同时增强其能力。
...
该样本来自训练集，因此模型在训练过程中已经见过它。这可以被视为一种作弊形式。为了更全面地了解模型的性能，请使用未见过的样本进行评估。
...
模型已成功学会按照数据集中的指定方式响应查询。你已达成目标！

## 比较微调模型与基础模型加提示[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经探索了微调VLM如何成为适应特定需求的宝贵选项。另一种值得考虑的方法是直接使用提示（prompting）或实现RAG系统，这在另一个[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有详细介绍。

微调VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，你可以通过尝试提示（prompting）来观察是否能在没有微调开销的情况下达到类似的效果。
...
这个案例再次使用了之前的示例，但这次包含了如下的系统消息。这一增强有助于为模型提供输入的上下文信息，可能提升其响应的准确性。
...
如演示所示，该模型使用预训练模型以及附加的系统消息生成了正确的答案，无需任何训练。根据具体用例，这种方法可以作为微调的一个可行替代方案。