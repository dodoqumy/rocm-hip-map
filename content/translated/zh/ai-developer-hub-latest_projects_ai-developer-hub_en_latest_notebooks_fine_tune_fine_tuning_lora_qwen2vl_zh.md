...
本笔记本展示了如何使用Hugging Face生态系统微调一个[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)，具体利用[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库。
...
您将在[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集上对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型进行微调。该数据集包含各种图表类型的图像及其对应的问题-答案对，非常适合增强模型的视觉问答能力。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认设置：该命令会列出您的AMD GPU及相关详细信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`。

而是使用命令。**Docker**：确保 Docker 已正确安装和配置。请遵循适用于您操作系统的 Docker 安装指南。**注意**：确保正确配置 Docker 权限。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用一个来自 Hugging Face 的示例数据集，该数据集在设置步骤中准备完毕。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传到该目录。保存终端输出中提供的 token 或 URL，以便从您的 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
在运行上述命令之前，请确保该端口在你的系统上尚未被占用。如果已被占用，请通过替换 `--port=8888` 来指定其他端口。
...
import torch
print("检测到ROCm（Radeon开放计算平台）GPU了吗？", torch.cuda.is_available())
...
```
# 安装微调所需的库，包括参数高效微调库(peft)和transformers
...
登录 Hugging Face 以上传你的微调模型。你需要使用 Hugging Face 账号进行认证，以直接从本 notebook 中保存和共享你的模型。
...
如果验证过程失败，请确保为您的笔记本选择了正确的 Jupyter 内核。要更改内核，请遵循以下步骤：
...
你的任务是分析提供的图表图像，并以简洁的答案回复查询，通常是一个单词、数字或短句。
图表包含多种类型（例如，折线图、条形图），并包含颜色、标签和文本。
专注于基于视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外解释。"""
```

```

将数据集格式化为聊天机器人交互结构。每次交互包括一条系统消息，随后是图像和用户查询，最后是对查询的回答。

关于此模型的更多使用技巧，请参见[Model Card](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
为了教学目的，你仅会加载数据集中每个划分的10%。然而，在实际应用场景中，你通常会加载整个样本集。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及第四个你不打算使用的特征。
...
尽管模型成功检索到了正确的视觉信息，但在准确回答问题时仍存在困难。这表明微调可能是提升其性能的关键。现在正是着手进行微调流程的时候了。

### 移除模型并清理GPU[#](#remove-the-model-and-clean-the-gpu)

在下一节继续训练模型之前，清除当前变量并清理GPU以释放资源。
...
使用监督微调（SFT）来优化模型在任务上的表现。为此，使用[TRL库](https://huggingface.co/docs/trl/index)中的[SFTConfig](https://huggingface.co/docs/trl/sft_trainer)类定义训练参数。SFT提供标注数据，帮助模型学习根据输入生成更准确的响应。这种方法确保模型针对你的特定用例进行定制，从而在理解和响应视觉查询方面取得更好的性能。
...
模型需要一个 collator 函数，以便在训练过程中正确检索和批处理数据。该函数将数据集的输入格式化为模型所需的格式，确保其结构正确。请在下方定义 collator 函数。
...
，你可以高效地管理训练工作流，并确保视觉语言模型获得流畅的微调体验。
...
现在你已经成功微调了你的视觉语言模型（VLM），是时候评估其性能了。本节使用ChartQA数据集中的示例来测试模型，看看它基于图表图像回答问题的效果如何。这是一个探索结果的好方法。
...
将训练好的适配器附加到预训练模型上。该适配器包含您在训练期间进行的微调调整，使基础模型能够利用新知识，同时不改变其核心参数。集成适配器可在保持模型原始结构的同时增强其能力。
...
模型已成功学会根据数据集中的指定内容响应查询。你已经达成了目标！

## 比较微调模型与带提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经了解了如何通过微调VLM来适应你的特定需求。另一种值得考虑的方法是直接使用提示或实施RAG系统，这将在另一篇[教程](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中介绍。

微调VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，你可以尝试使用提示来观察是否能在没有微调开销的情况下获得类似的结果。
...
此案例再次使用了之前的样本，但这次包含了系统消息（system message）如下。此增强有助于为模型提供输入上下文，可能提升其响应准确性。
...
如所示，该模型使用预训练模型及额外的系统消息生成正确答案，无需任何训练。根据具体用例情况，此方法可作为微调的有效替代方案。