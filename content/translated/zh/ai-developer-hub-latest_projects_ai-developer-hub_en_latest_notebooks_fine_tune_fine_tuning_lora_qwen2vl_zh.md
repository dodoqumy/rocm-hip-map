...
这个笔记本演示了如何使用Hugging Face生态系统微调[视觉语言模型 (VLM)](https://huggingface.co/blog/vlms)，具体使用[参数高效微调 (PEFT)](https://huggingface.co/docs/peft/index)和[Transformer强化学习 (TRL)](https://huggingface.co/docs/trl/index)库。
...
您将在[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集上微调[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型。该数据集包含各种图表类型的图像及对应的问答对，非常适合增强模型的视觉问答能力。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：请按照[ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm。安装完成后，使用以下命令确认配置：

该命令会列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**: 确保Docker已正确安装和配置。请按照适用于您操作系统的Docker安装指南进行操作。**Note**: 确保正确配置Docker权限。要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用来自 Hugging Face 的示例数据集，该数据集在设置步骤中准备完毕。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或在 Jupyter Notebook 环境启动后将其上传到该环境。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
确保端口在上面的命令运行之前未在你的系统上被占用。如果是，请通过替换 `--port=8888` 来指定不同的端口。
...
print("检测到了多少个 ROCm（ROCm（Radeon 开放计算平台））-GPU？", torch.cuda.device_count())
```
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，以便直接从本笔记本保存和共享您的模型。
...
print(f"Token 验证成功！登录用户: {user_info['name']}")
...
You are an expert in analyzing chart images. You provide concise answers to questions based on the charts presented.

⚠️ 重要：确保选择了正确的内核

如果验证过程失败，请确保为你的 notebook 选择了正确的 Jupyter kernel。要更改 kernel，请按照以下步骤操作：
...
将数据集格式化为聊天机器人交互结构。每次交互包括一条系统消息，随后是图像和用户查询，最后是该查询的答案。

如需了解该模型的更多使用技巧，请参阅 [Model Card](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
出于教学目的，您将仅加载数据集中每个分片的10%。然而，在实际使用场景中，通常需要加载全部样本。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及第四个特征（需要丢弃）。
...
使用不含系统消息的样本评估VLM的原始理解能力。以下是所使用的输入：
...
创建一个方法，该方法以模型、处理器和样本作为输入，生成模型的答案。这样可以简化推理过程并轻松评估VLM的性能。
...
虽然模型成功检索到了正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键所在。现在该进入微调流程了。
...
**注意**：另外，量化模型也可以通过 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载。要了解有关量化的更多信息，请参见 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的博客](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，配置训练设置中的 LoRA。LoRA 通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，是优化模型性能而不牺牲质量的绝佳选择。
...
使用监督式微调（SFT）来优化模型在任务上的表现。具体地，通过[TRL库](https://huggingface.co/docs/trl/index)中的[SFTConfig](https://huggingface.co/docs/trl/sft_trainer)类定义训练参数。SFT利用标注数据帮助模型学习根据输入生成更精准的响应。这种方法确保模型针对特定用例进行定制，从而在理解与回应视觉查询方面取得更优表现。
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录您的训练进度。将笔记本连接到 W&B 以捕获训练期间的关键信息。
...
模型需要一个collator函数，以便在训练过程中正确地检索和批处理数据。该函数对数据集的输入进行格式化，确保它们结构正确。请在下方定义collator函数。
...
, 您可以高效地管理训练工作流程，并确保视觉语言模型的微调体验顺畅。
...
现在您已成功微调了视觉语言模型（VLM），是时候评估其性能了。本节将使用ChartQA数据集中的示例对模型进行测试，以考察模型基于图表图像回答问题的能力。这是一个探索结果的绝佳方式。
...
将训练好的适配器附加到预训练模型上。该适配器包含了您在训练期间进行的微调调整，使基础模型能够利用新知识而不改变其核心参数。集成适配器可在保持原始结构的同时增强模型的能力。
...
此样本来自训练集，因此模型在训练过程中已经见过它。这可以被视为一种作弊行为。为了更全面地了解模型的性能，请使用未见过的样本进行评估。
...
您已经探索了微调VLM如何成为使其适应特定需求的宝贵选项。另一种值得考虑的方法是直接使用提示（prompting）或实现RAG系统，这在另一个[教程](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有详细介绍。

微调VLM需要大量数据和计算资源，这会产生成本。相比之下，您可以尝试通过提示工程来查看是否能在无需微调开销的情况下获得类似结果。
...
此案例再次使用之前的示例，但这次包含了如下所示的系统消息。这一增强有助于为模型提供输入上下文，从而可能提升其回应的准确性。
...
如图所示，模型利用预训练模型以及额外的系统消息生成了正确答案，无需任何训练。根据具体的使用场景，这种方法可以作为微调的一个可行替代方案。