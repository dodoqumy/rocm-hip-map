...
本笔记本演示了如何使用Hugging Face生态系统微调一个[Vision Language Model (VLM)](https://huggingface.co/blog/vlms)，具体使用了[Parameter-Efficient Fine-Tuning (PEFT)](https://huggingface.co/docs/peft/index)和[Transformer Reinforcement Learning (TRL)](https://huggingface.co/docs/trl/index)库。
...
您将对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型在[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集上进行微调。该数据集包含各种图表类型的图像及其对应的问题-答案对，这对于增强模型的视觉问答能力非常理想。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**：本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上测试。请确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 或兼容硬件，并且您的系统满足[official requirements](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm。安装后，使用以下命令确认设置：此命令会列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

使用命令替代。**Docker**：确保Docker已正确安装和配置。请按照适用于您操作系统的Docker安装指南进行操作。**注意**：确保Docker权限已正确配置。若要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
[权重与偏差（Weights & Biases）](https://wandb.ai/)。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此笔记本。
...
请确保该端口在你的系统上尚未被占用，然后再运行上述命令。如果已被占用，请通过替换 `--port=8888` 来指定不同的端口。
...
登录 Hugging Face 以上传您微调后的模型。您需要使用您的 Hugging Face 账户进行身份验证，以便直接从本笔记本保存和分享您的模型。
...
在本节中，您将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含图表图像及其对应的问题与答案，非常适合用于训练视觉问答任务。

接下来，为VLM生成一条系统消息。这将创建一个系统，作为分析图表图像的专家，并根据这些图表提供简洁的答案。

**⚠️ 重要：确保选择了正确的内核**

如果验证过程失败，请确保为你的笔记本选择了正确的 Jupyter 内核。要更改内核，请按以下步骤操作：
...
```
system_message = """你是一个专注于从图表图像中解读视觉数据的视觉语言模型。
你的任务是分析所提供的图表图像，并对查询给出简洁的答案，通常是一个单词、数字或短句。
...
要了解此模型的具体使用技巧，请参阅[Model Card](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
出于教学目的，你只需加载数据集中每个分割部分的 10%。然而，在实际应用场景中，你通常会加载所有样本。
...
接下来，查看数据集的结构。该数据集包括一个图像、一个查询、一个标签（即答案），以及一个你将丢弃的第四个特征。
...
加载数据集后，加载模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一款能够同时理解视觉数据和文本的视觉语言模型（VLM）。
...
创建一个方法，该方法接受模型、处理器和样本作为输入，以生成模型的答案。这可以简化推理过程，并轻松评估VLM的性能。
...
尽管模型能成功检索到正确的视觉信息，但在准确回答问题方面仍有困难。这表明微调可能是提升其性能的关键。现在是时候进行微调过程了。

### 删除模型并清理 GPU[#](#remove-the-model-and-clean-the-gpu)
...
**注意**：另外，也可以使用[bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index)加载量化模型。要了解更多关于量化的信息，请参阅[来自 Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization)或[来自 Maarten Grootendorst 的文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置设置LoRA。LoRA通过应用低秩近似来降低内存使用，从而进一步降低内存需求并提高训练效率，使其成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在特定任务上的性能。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 利用标注数据，帮助模型学习根据输入生成更准确的回答。这种方法确保模型针对你的具体用例进行调整，从而在理解和响应视觉查询方面获得更好的性能。
...
warmup_ratio=0.03, # 预热步骤占总步骤的比例
# Hub 和报告
push_to_hub=False, # 是否将模型推送到 Hugging Face Hub，默认关闭。
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录您的训练进度。将您的笔记本连接到 W&B，以便在训练过程中捕获关键信息。
...
模型需要一个collator函数来在训练过程中正确检索和批量处理数据。该函数将数据集的输入格式化为模型所需的格式，确保它们结构正确。请在下方定义collator函数。
...
# 标签是input_ids，在损失计算中我们将填充token屏蔽掉
labels = batch["input_ids"].clone() # 克隆input_ids作为标签
labels[labels == processor.tokenizer.pad_token_id] = -100 # 屏蔽标签中的填充token
# 在损失计算中忽略图像token索引（模型特定）
if isinstance(processor, Qwen2VLProcessor): # 检查处理器是否为Qwen2VLProcessor
image_tokens = [151652, 151653, 151655] # Qwen2VLProcessor特定的图像token ID
else:
image_tokens = [processor.tokenizer.convert_tokens_to_ids(processor.image_token)] # 将图像token转换为ID
# 屏蔽标签中的图像token ID
for image_token_id in image_tokens:
labels[labels == image_token_id] = -100 # 屏蔽标签中的图像token ID
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的封装，并继承其属性和方法。该类通过在使用 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时正确初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model)，从而简化了微调过程。通过使用 `SFTTrainer`

您可以高效地管理训练工作流，并确保视觉语言模型获得顺畅的微调体验。
...
既然您已成功微调了您的视觉语言模型（VLM），是时候评估其性能了。本节将使用ChartQA数据集中的示例来测试模型，以查看它基于图表图像回答问题的情况。这提供了一个探索结果的好方法。
...
将训练好的适配器附加到预训练模型上。此适配器包含您在训练期间所做的微调调整，使基础模型能够利用新知识，同时不改变其核心参数。集成适配器在不改变原有结构的前提下，增强了模型的能力。
...
这个样本是从训练集中抽取的，因此模型在训练过程中已经遇到过它。这可以被视为一种作弊行为。为了更全面地了解模型的性能，请使用一个未见过的样本进行评估。
...
模型已成功学会按照数据集中指定的方式响应查询。你已经达成了目标！

## 比较微调模型与基础模型结合提示[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经探索了微调VLM如何成为使其适应你的特定需求的有价值的选择。另一个值得考虑的方法是直接使用prompting或实现RAG系统，这在另一份[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调 VLM 需要大量数据和计算资源，这会产生成本。相比之下，你可以尝试通过提示（prompting）来观察是否能在没有微调开销的情况下获得类似结果。
...
本案例再次使用了之前的示例，但这次包含了如下的系统消息。这一增强功能有助于为模型提供输入上下文，从而可能提升其响应的准确性。
...
如演示所示，模型使用预训练模型以及附加的系统消息生成了正确答案，无需任何训练。根据具体用例，这种方法可以作为微调的一个可行替代方案。