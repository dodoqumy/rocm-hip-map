...
本笔记本演示了如何使用Hugging Face生态系统，特别是通过[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库来微调一个[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**: 按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认配置：

该命令会列出您的AMD GPU及其相关信息。

**注意**：对于 ROCm（Radeon 开放计算平台）6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**: 确保 Docker 已正确安装和配置。请根据您的操作系统遵循 Docker 安装指南。**注意**: 确保 Docker 权限配置正确。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用 Hugging Face 中的一个示例数据集，该数据集在设置步骤中已准备好。
...
容器中的目录。确保在执行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 存储库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未在你的系统上使用。如果已被占用，请通过替换 `--port=8888` 来指定另一个端口。
...
登录 Hugging Face 以上传微调后的模型。您需要使用 Hugging Face 账号进行身份验证，以便从此 notebook 直接保存和分享您的模型。
...
接下来，为VLM生成一条系统消息。这将创建一个系统，该系统如同专家般分析图表图像，并基于这些图表提供简洁的问题答案。
...
**未能选择正确的内核可能会在运行笔记本时导致意外问题。**

```
system_message = """你是一个专门从图表图像中解读视觉数据的视觉语言模型。
你的任务是分析所提供的图表图像，并对查询给出简洁的答案，通常是一个单词、数字或简短短语。
图表包括多种类型（如折线图、柱状图），并包含颜色、标签和文本。
专注于基于视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外解释。"""
```

```

将数据集格式化为交互式聊天机器人结构。每个交互包含一条系统消息，然后是图像和用户的查询，最后是对查询的回答。
...
出于教学目的，你只加载数据集中每个划分的10%。然而，在实际使用场景中，通常会加载完整的样本集。
...
使用没有系统消息的样本来评估 VLM 的原始理解。这是要使用的输入：

train_dataset[0][1:2]

```

目前我无法查看图表或视觉信息。请提供图表中的关键数据或文字描述，以便我协助回答相关问题。
...
创建一个方法，以模型、处理器和样本作为输入来生成模型的答案。这可以让你简化推理过程，并轻松评估VLM的性能。
...
虽然模型成功检索到了正确的视觉信息，但在准确回答问题方面存在困难。这表明微调可能是提升其性能的关键。现在应当着手进行微调流程。
...
**注意**：另外，也可通过使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博文](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的博文](https://www.maartengrootendorst.com/blog/quantization/)。
...
使用监督微调（SFT）来优化模型在任务上的表现。为此，使用 [TRL库](https://huggingface.co/docs/trl/index) 中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT提供带标签的数据，帮助模型学习根据输入生成更准确的回复。这种方法确保模型针对你的特定用例进行调整，从而在理解和响应视觉查询方面获得更好的性能。
...
您可以使用[Weights & Biases (W&B)](https://wandb.ai/)记录训练进度。将您的笔记本连接到W&B以在训练期间捕获关键信息。
...
模型需要一个 collator 函数，在训练过程中正确检索并批处理数据。该函数用于格式化模型的数据集输入，确保其结构正确。请在下方定义 collator 函数。
...
既然您已成功微调了视觉语言模型（VLM），现在该评估其性能了。本节使用ChartQA数据集中的示例对模型进行测试，以观察其基于图表图像回答问题的表现。这是探索结果的一个好方法。
...
将训练好的适配器附加到预训练模型上。该适配器包含了训练期间所做的微调调整，使基础模型能够在不改变核心参数的情况下利用新知识。集成适配器可在保持模型原始结构的同时增强其能力。
...
该样本来自训练集，因此模型在训练过程中已遇到过它。这可视为一种作弊行为。为了更全面地了解模型性能，请使用未见过的样本进行评估。
...
## 比较微调模型与基础模型配合提示[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探讨了微调VLM如何成为使其适应您特定需求的一个有价值的选择。另一种值得考虑的方法是直接使用提示或实施RAG系统，这在另一份[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，你可以尝试使用提示来查看是否能在没有微调开销的情况下获得类似的结果。
...
本例再次使用了之前的样本，但这次包含了如下的系统消息。这一增强有助于为模型输入提供上下文，可能提高其响应的准确性。
...
正如所示，该模型使用预训练模型以及额外的系统消息生成正确回答，无需任何训练。根据具体使用场景，这种方法可以作为微调的一种可行替代方案。