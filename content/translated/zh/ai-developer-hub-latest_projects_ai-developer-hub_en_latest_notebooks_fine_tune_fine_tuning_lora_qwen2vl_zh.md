...
您将在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上微调 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型。该数据集包含各种图表类型的图像及其对应的问题-答案对，非常适合提升模型的视觉问答能力。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：此命令将列出您的AMD GPU及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**：确保 Docker 已正确安装和配置。请根据您的操作系统参考 Docker 安装指南。**注意**：确保 Docker 权限已正确配置。若要配置权限以允许非 root 用户访问，请运行以下命令：`usermod -aG docker $USER` `newgrp docker`
...
[权值与偏差（W&B）](https://wandb.ai/)。

### 数据准备[#](#data-preparation)

本教程使用Hugging Face中的一个示例数据集，该数据集已在设置步骤中准备完毕。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问该 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
在运行上述命令之前，请确保该端口未在你的系统上使用。如果已被占用，请通过替换 `--port=8888` 指定一个不同的端口。
...
print("检测到多少块ROCm（ROCm（Radeon 开放计算平台））-GPU？ ", torch.cuda.device_count())
```
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，以便直接从本 notebook 中保存并共享您的模型。
...
在本节中，你将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含图表图像及其相关问题和答案，非常适合用于训练视觉问答任务。

接下来，为VLM生成一条系统消息。这会创建一个系统，该系统充当分析图表图像并根据这些图表提供简洁答案的专家。

**⚠️ 重要：确保选择了正确的内核**

如果验证过程失败，请确保为你的 notebook 选择了正确的 Jupyter 内核。要更换内核，请按照以下步骤操作：
...
你的任务是分析所提供的图表图像，并以简洁的答案回应查询，通常是一个单词、数字或短语。
图表包括各种类型（如折线图、柱状图）并包含颜色、标签和文本。
专注于基于视觉信息提供准确、简洁的答案。除非绝对必要，否则避免额外解释。"""
```

```

将数据集格式化为用于交互的聊天机器人结构。每次交互包括一条系统消息，随后是图像和用户的查询，最后是查询的答案。
...
为教育目的，您将仅加载数据集中每个分块的10%。然而，在实际应用场景中，通常需要加载整个样本集。
...
接下来，查看数据集的结构。它包含一张图像、一个查询、一个标签（即答案），以及第四个将被丢弃的特征。
...
加载数据集后，载入模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct) 这一视觉语言模型（VLM），该模型能够理解视觉数据和文本。
...
使用不含系统消息的样本来评估 VLM 的原始理解能力。以下是使用的输入：
...
创建一个方法，该方法将模型、处理器和样本作为输入以生成模型的答案。这使您能够简化推理过程并轻松评估VLM的性能。
...
在继续下一节训练模型之前，请清除当前变量并清理GPU以释放资源。
...
**注意**：另外，也可以通过使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 来加载量化模型。要了解更多关于量化的信息，请参阅 [这篇来自 Hugging Face 的博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的博客](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置设置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，是在不牺牲质量的情况下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在特定任务上的表现。为此，需使用[TRL库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT利用标注数据，帮助模型学习根据输入生成更准确的响应。该方法确保模型针对您的具体用例进行定制，从而在理解和回答视觉查询方面表现更佳。
...
模型需要一个collator函数来在训练过程中正确检索和批处理数据。该函数格式化数据集的输入，确保它们结构正确。在下方定义collator函数。
...
def collate_fn(examples):
    ...
    if isinstance(processor, Qwen2VLProcessor):  # 检查处理器是否为Qwen2VLProcessor
        image_tokens = [151652, 151653, 151655]  # Qwen2VLProcessor特有的图像标记ID
    else:
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的包装器，并继承了其属性与方法。该类通过当提供 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时正确地初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model)，简化了微调过程。使用 `SFTTrainer`

，您可以高效管理训练工作流，确保为视觉语言模型提供顺畅的微调体验。
...
既然您已经成功微调了您的视觉语言模型（VLM），现在是时候评估其性能了。本节使用 ChartQA 数据集中的示例来测试模型，看它如何基于图表图像回答问题。这提供了一个好方法来探索结果。
...
将训练后的适配器附加到预训练模型上。该适配器包含你在训练期间所做的微调调整，使基础模型能够利用新知识而无需更改其核心参数。集成适配器增强了模型的能力，同时保持其原有结构不变。
...
此样本来自训练集，因此在训练过程中模型已接触过它。这可视作一种作弊行为。为更全面了解模型性能，请使用未见过的样本进行评估。
...
模型已成功学会按照数据集中指定的方式回应用户查询。你已经实现了目标！

## 比较微调模型与带提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探索了微调VLM可以成为使其适应您特定需求的一种有价值的选择。另一种需要考虑的方法是直接使用提示或实施RAG系统，这在另一篇[教程](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量的数据和计算资源，这可能会产生成本。相比之下，您可以尝试使用提示（prompting）来观察是否能在无需微调开销的情况下获得类似的结果。
...
此案例再次使用了之前的样本，但这次包含了如下的系统消息。这一增强有助于为模型提供上下文输入，可能提高其响应准确性。
...
如所示，该模型使用预训练模型以及附加的系统消息生成了正确答案，无需任何训练。这种方法可以作为微调的一种可行替代方案，具体取决于特定用例。