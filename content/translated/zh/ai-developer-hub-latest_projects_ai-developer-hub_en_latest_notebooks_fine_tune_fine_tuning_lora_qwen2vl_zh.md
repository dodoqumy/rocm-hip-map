...
# 使用 Hugging Face 生态进行微调 (TRL)

**作者**：[Sergio Paniego](https://github.com/sergiopaniego)，并由 [AMD](https://www.amd.com) 修改以在 AMD GPU 上运行。

**知识水平**：中级

本笔记本演示了如何使用Hugging Face生态系统微调[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)，具体而言，通过[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库。

**Note**: 本笔记本源自 [fine_tuning_vlm_trl](https://huggingface.co/learn/cookbook/en/fine_tuning_vlm_trl)。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs**: 本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 上测试。请确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））GPU 或兼容硬件，并且系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（Radeon 开放计算平台）6.2**：请按照[ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm。安装完成后，使用以下命令确认配置：该命令将列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用`rocm-smi`。

使用命令替代。**Docker**: 确保 Docker 已正确安装和配置。请遵循适用于您操作系统的 Docker 安装指南。**注意**: 确保 Docker 权限已正确配置。若要配置允许非 root 用户访问的权限，请运行以下命令：
...
本教程使用来自Hugging Face的示例数据集，该数据集在设置步骤中已准备就绪。
...
--shm-size 8G \
--hostname=ROCm（ROCm（ROCm（Radeon 开放计算平台）））-FT \
-v $(pwd):/workspace \
...
在运行上述命令之前，请确保该端口尚未被您的系统使用。如果已被占用，请通过替换 `--port=8888` 来指定一个不同的端口。

使用另一个端口号，例如，`--port=8890`

我们被要求将一段英文文本翻译成简体中文（zh-CN）。文本以‘We are given a text to translate...’开头。实际上，那是输入内容。等等，指令中说：‘You are a technical translator... Translate the following English text to Simplified Chinese (zh-CN).’然后又说了‘Output ONLY the translation’。但用户消息是：‘We are given a text to translate...’
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，以便直接从此笔记本保存和分享您的模型。
...
你是一位专门分析图表图像的专家，能够根据这些图表提供简洁的答案。
...
**未能选择正确的内核可能导致在运行笔记本时出现意外问题。**

```
system_message = """你是一个专门从图表图像中解读视觉数据的视觉语言模型。
你的任务是分析所提供的图表图像，并以简洁的答案（通常为单个词、数字或短短语）回应用户的查询。
这些图表包含多种类型（例如折线图、柱状图），并包含颜色、标签和文本。
...
关于此模型的更多使用技巧，请参见[模型卡片](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
为了教学目的，您只需加载数据集中每个划分的10%。然而，在实际使用场景中，您通常会加载整个样本集。
...
接下来，查看数据集的结构。它包含一个图像、一个查询、一个标签（即答案），以及你将要丢弃的第四个特征。
...
为了评估模型的性能，从数据集中使用一个样本。首先，检查这个样本的内部结构。
...
使用去掉系统消息的样本来评估VLM的原始理解能力。以下是要使用的输入：
...
尽管模型成功检索到了正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在是时候着手进行微调了。
...
**注意**：另外，也可以通过使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 来加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst的这篇文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置 LoRA。LoRA 通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提升训练效率，使其成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在该任务上的性能。为此，通过 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供标注数据，帮助模型学习根据输入生成更准确的响应。该方法确保模型针对您的特定用例进行定制，从而在理解和回答视觉查询方面获得更优的表现。
...
warmup_ratio=0.03, # 预热步数占训练总步数的比例
# Hub和报告
...
你可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 来记录训练进度。将你的 notebook 连接到 W&B 以捕获训练过程中的关键信息。
...
模型需要一个整理函数来在训练过程中正确地检索和批处理数据。该函数格式化数据集的输入以供模型使用，确保它们结构正确。在下方定义整理函数。
...
# 获取文本和图像，并应用聊天模板
texts = [processor.apply_chat_template(example, tokenize=False) for example in examples] # 准备文本以待处理
...
# 标签是 input_ids，在损失计算中屏蔽填充 token
labels = batch["input_ids"].clone() # 克隆输入 ID 作为标签
labels[labels == processor.tokenizer.pad_token_id] = -100 # 将标签中的填充 token 屏蔽
# 在损失计算中忽略图像 token 索引（模型特定）
...
labels[labels == image_token_id] = -100 # 屏蔽标签中的图像 token ID
...
现在，定义 [SFTTrainer](https://huggingface.co/docs/trl/sft_trainer)，它是对 [Transformers Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) 类的一个包装，并继承其属性和方法。当提供一个 [PeftConfig](https://huggingface.co/docs/peft/v0.6.0/en/package_reference/config#peft.PeftConfig) 对象时，该类通过正确初始化 [PeftModel](https://huggingface.co/docs/peft/v0.6.0/package_reference/peft_model) 来简化微调过程。通过使用 `SFTTrainer`

，你可以高效管理训练工作流，并确保视觉语言模型（Vision Language Model）获得流畅的微调体验。
...
现在您已经成功微调了视觉语言模型（VLM），是时候评估其性能了。本节使用 ChartQA 数据集中的示例测试模型，检查它基于图表图像回答问题的能力。这为探索结果提供了良好的途径。
...
将训练好的适配器附加到预训练模型上。该适配器包含训练期间所做的微调调整，使基础模型能够利用新知识，同时不改变其核心参数。集成适配器可在保持模型原始结构的同时增强其能力。
...
该样本来自训练集，因此模型在训练过程中已经遇到过它。这可视作某种形式的作弊行为。为更全面了解模型性能，请使用未见过的样本进行评估。
...
模型已成功学会按照数据集中的指定方式响应查询。你实现了目标！

## 比较微调模型与基础模型（带提示）[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经探索了如何将微调VLM作为使其适应特定需求的宝贵选项。另一种值得考虑的方法是直接使用提示或实现RAG系统，这在另一份[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量数据和计算资源，这会产生成本。相比之下，你可以尝试使用提示（prompting），看看是否能在没有微调开销的情况下达到类似的效果。
...
此案例再次使用之前的示例，但这次包含了如下的系统消息。这一增强有助于为模型的输入提供上下文，可能提高其响应准确性。
...
如所示，模型使用预训练模型以及额外的系统消息来生成正确答案，无需任何训练。根据具体使用场景，这种方法可以成为微调的可行替代方案。