...
**作者**: [Sergio Paniego](https://github.com/sergiopaniego)，经 [AMD](https://www.amd.com) 修改以在 AMD GPUs 上运行。

**知识水平**：中级

本笔记本演示了如何使用 Hugging Face 生态系统，特别是通过[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer 强化学习（TRL）](https://huggingface.co/docs/trl/index)库，对[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)进行微调。
...
你将微调 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上。该数据集包含各种图表类型的图像以及对应的问答对，非常适合提升模型的视觉问答能力。
...
**AMD Instinct（AMD 数据中心 GPU 系列）™ GPUs**：本教程已在 AMD Instinct MI300X GPU 上完成测试。请确保你使用的是支持 ROCm（Radeon 开放计算平台）的 AMD Instinct GPU 或兼容硬件，且系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台））6.2**：通过遵循[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)来安装和验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：该命令将列出您的AMD GPU及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`。

请使用以下命令代替。**Docker**：确保Docker已正确安装和配置。请根据你的操作系统参考Docker安装指南。**注意**：确保Docker权限配置正确。要配置允许非root用户访问的权限，请运行以下命令：  
...
确保您的系统符合[系统要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
使用不同的端口号，例如 `--port=8890`

您是一名技术翻译，专门负责GPU/ROCm/HIP文档。将以下英文文本翻译成简体中文（zh-CN）。规则：  
1. 保留所有markdown格式、代码块、内联代码和链接不变。  
2. 保留技术术语如ROCm, HIP, GPU, CUDA, AMD, PyTorch, TensorFlow的英文原样。  
3. 保留API名称、函数名、文件路径、命令不变。  
4. 仅输出翻译——不解释、不注释、不前言。  
5. 使用GPU开发者所期望的中文技术术语。
...
登录Hugging Face以上传您微调的模型。您需要使用Hugging Face账户进行身份验证，以便从此笔记本直接保存和共享您的模型。
...
在本节中，你将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含图表图像及其对应的问题和答案，非常适合用于视觉问答任务的训练。

你是一个专门分析图表图像的专家。根据提供的图表，简洁准确地回答问题。只输出答案，不加解释或额外信息。
...
```
system_message = """你是一个专长于解读图表图像中视觉数据的视觉语言模型。
你的任务是分析提供的图表图像，并以简洁的答案（通常是单个词、数字或短语）回应用户的查询。
图表包含多种类型（例如折线图、柱状图），并包含颜色、标签和文字。
专注于基于视觉信息提供准确、简洁的答案。除非绝对必要，避免额外解释。"""
```

```

将数据集格式化为聊天机器人交互结构。每次交互包含一条系统消息，接着是图像和用户的查询，最后是对查询的回答。

更多针对此模型的使用技巧，请参见 [Model Card](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
为了教学目的，您仅需加载数据集中每个分组的10%。然而，在实际应用场景中，通常会加载全部样本。
...
接下来，分析数据集的结构。其中包含一张图片、一个查询、一个标签（即答案），以及你要丢弃的第四个特征。
...
加载数据集后，加载模型并使用数据集中的一个样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一个 Vision Language Model (VLM)，能够理解视觉数据和文本。
...
创建一个方法，该方法以模型、处理器和样本作为输入，生成模型的回答。这可以让您简化推理过程，并轻松评估VLM的性能。
...
尽管该模型成功检索到正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在需要进入微调流程。
...
gc.collect()
time.sleep(2)
print(f"GPU 分配内存: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
...
**注**: 也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 来加载量化模型。想了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇博客](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练设置配置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，使其成为在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在该任务上的性能。为此，需要使用 [TRL 库](https://huggingface.co/docs/trl/index)中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类来定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的响应。这种方法确保模型针对你的特定用例进行定制，从而在理解和响应视觉查询方面获得更好的性能。
...
你可以使用[Weights & Biases (W&B)](https://wandb.ai/)记录训练进度。将你的笔记本连接到W&B以捕获训练过程中的关键信息。
...
模型需要一个整理函数，以便在训练过程中正确检索和批处理数据。该函数会对数据集输入进行格式化，确保其结构正确。请在下方定义该整理函数。
...
既然您已成功微调了您的Vision Language Model（VLM），现在是时候评估其性能了。本节使用ChartQA数据集中的示例来测试模型，以了解它如何基于图表图像回答问题。这提供了一个探索结果的绝佳方式。
...
将训练后的适配器附加到预训练模型上。该适配器包含了你在训练过程中进行的微调调整，使基础模型能够在不改变核心参数的前提下利用新知识。集成适配器在保持模型原始结构的同时，增强了其能力。
...
该样本来自训练集，因此模型在训练过程中已经见过它。这可以被视为一种作弊行为。为了更全面地了解模型的性能，请使用一个未见过的样本来进行评估。
...
模型已成功学会根据数据集中的指定要求来响应查询。你已达成目标！

## 比较微调模型与基础模型的提示方法[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经探索了如何微调VLM，这可以是一个有价值的选项来使其适应你的特定需求。另一个值得考虑的方法是直接使用提示或实现RAG系统，这将在另一篇[指南](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中介绍。

微调VLM需要大量的数据和计算资源，这会产生相应的成本。相比之下，你可以尝试使用提示（prompting）方法，看是否能在无需承担微调开销的情况下达到类似的效果。
...
此案例再次使用了之前的示例，但这次包含了如下的系统消息。这一增强有助于模型对输入进行上下文化，从而可能提高其回答的准确性。
...
如所示，该模型使用预训练模型以及额外的系统消息生成了正确答案，无需任何训练。这种方法可根据具体用例作为微调的有效替代方案。