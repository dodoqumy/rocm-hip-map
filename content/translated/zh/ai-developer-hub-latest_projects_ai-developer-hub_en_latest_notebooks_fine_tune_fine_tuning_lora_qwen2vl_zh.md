...
本笔记本演示了如何利用Hugging Face生态系统微调[视觉语言模型（VLM）](https://huggingface.co/blog/vlms)，具体使用[参数高效微调（PEFT）](https://huggingface.co/docs/peft/index)和[Transformer强化学习（TRL）](https://huggingface.co/docs/trl/index)库。
...
您将对 [Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/) 模型在 [ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集上进行微调。该数据集包含各种图表类型的图像以及对应的问答对，非常适合增强模型的视觉问答能力。
...
**ROCm（Radeon 开放计算平台）6.2**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认您的设置：
该命令将列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`

改用命令。**Docker**: 确保Docker已正确安装和配置。请遵循适用于您操作系统的Docker安装指南。**注意**: 确保Docker权限已正确配置。要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
本教程使用了Hugging Face的一个样本数据集，该数据集在设置步骤中已准备好。
...
--shm-size 8G \
--hostname=ROCm（Radeon 开放计算平台）-FT \
-v $(pwd):/workspace \
...
容器内的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从您的 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
在运行上述命令之前，确认该端口在你的系统上尚未被占用。如果已被占用，请通过替换 `--port=8888` 来指定其他端口。
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，以便直接从该 notebook 保存和共享您的模型。
...
在本节中，你将加载 [HuggingFaceM4/ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA) 数据集。该数据集包含图表图像及其相关的问题与答案，非常适合用于视觉问答任务的训练。

接下来，为VLM生成一条系统消息。这将创建一个系统，该系统充当分析图表图像的专家，并基于这些图表提供简洁的答案。

⚠️ 重要：确保选择了正确的内核

如果验证过程失败，请确保为您的笔记本选择了正确的 Jupyter 内核。要更换内核，请按照以下步骤操作：
...
图表包含多种类型（如折线图、柱状图），并带有颜色、标签和文字。
专注于根据视觉信息提供准确、简洁的回答。除非绝对必要，否则避免额外的解释。"""
```

```

将数据集格式化为用于交互的聊天机器人结构。每个交互包括一条系统消息，后跟图像和用户的查询，最后是对查询的回答。
...
出于教育目的，您将只加载数据集中每个分片的10%。然而，在实际使用场景中，您通常会加载整个样本集。
...
接下来，查看数据集的结构。它包含一张图片、一个查询、一个标签（即答案），以及你将丢弃的第四个特征。
...
创建一个方法，该方法以模型、处理器和样本作为输入，生成模型的答案。这可以使您简化推理过程，并轻松评估VLM的性能。
...
尽管模型成功检索到了正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调或许是提升其性能的关键。现在该进入微调流程了。
...
**注意**：此外，也可以使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇文章](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练设置配置LoRA。LoRA通过应用低秩近似来减少内存使用，从而进一步降低内存需求并提高训练效率，这使得它成为在不牺牲质量的情况下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在特定任务上的性能。为此，需要利用[TRL库](https://huggingface.co/docs/trl/index)中的[SFTConfig](https://huggingface.co/docs/trl/sft_trainer)类来定义训练参数。SFT使用带标签的数据，帮助模型根据输入学习生成更准确的响应。这种方法确保模型针对你的特定使用场景进行定制，从而在理解和响应视觉查询方面取得更好的性能。
...
import wandb
wandb.init(
project="qwen2-7b-instruct-trl-sft-ChartQA", # 更改此项
name="qwen2-7b-instruct-trl-sft-ChartQA", # 更改此项
...
模型需要一个 collator 函数来在训练过程中正确检索并批量处理数据。该函数格式化模型的数据集输入，确保其结构正确。请在下面定义 collator 函数。
...
def collate_fn(examples):
    ...
    # 标签使用 input_ids，并在损失计算中屏蔽填充 token
...
您能够高效管理训练工作流，并确保视觉语言模型获得顺畅的微调体验。
...
现在您已成功微调了视觉语言模型（VLM），接下来需要评估其性能。本节使用ChartQA数据集中的示例测试模型，考察它依据图表图像回答问题的能力。这为探索结果提供了有效途径。
...
将训练好的适配器附加到预训练模型上。该适配器包含您在训练过程中进行的微调调整，使基础模型能够利用新知识而不改变其核心参数。集成适配器可在保持模型原始结构的同时增强其能力。
...
此样本来自训练集，因此模型在训练期间已见过它。这可以被视为一种作弊。为了更全面地了解模型的性能，请使用一个未见过的样本对其进行评估。
...
## 对比微调模型与使用提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

你已经探索了微调VLM如何成为使其适应特定需求的有价值选项。另一种值得考虑的方法是直接使用提示工程或实现RAG系统，这在另一份[教程](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中有所介绍。

微调VLM需要大量数据和计算资源，这会产生成本。相比之下，您可以尝试通过提示（prompting）来观察是否能在没有微调开销的情况下获得类似结果。
...
这个案例再次使用了之前的示例，但这次包含了如下的系统消息。这一增强有助于为模型提供输入上下文，可能提高其响应准确性。
...
如图所示，模型在无需任何训练的情况下，仅使用预训练模型和附加的系统消息即可生成正确答案。根据具体用例，此方法可作为微调的有效替代方案。