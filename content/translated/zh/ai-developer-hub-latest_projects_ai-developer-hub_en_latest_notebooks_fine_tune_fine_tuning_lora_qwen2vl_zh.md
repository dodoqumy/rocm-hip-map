...
本笔记本演示如何使用 Hugging Face 生态系统微调视觉语言模型（VLM），具体使用参数高效微调（PEFT）和 Transformer 强化学习（TRL）库。
...
您将使用[ChartQA](https://huggingface.co/datasets/HuggingFaceM4/ChartQA)数据集对[Qwen2-VL-7B](https://qwenlm.github.io/blog/qwen2-vl/)模型进行微调。该数据集包含各类图表图像及其对应的问题-答案对，非常适合增强模型的视觉问答能力。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装和验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认配置：该命令列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**: 确保已正确安装和配置Docker。请按照您操作系统的Docker安装指南进行操作。**注意**：确保正确配置了Docker权限。要为非root用户配置访问权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
确保你的系统满足[系统需求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传到其中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev-docs) 下载此 notebook。
...
在运行上述命令之前，请确保该端口尚未在你的系统上被占用。如果已被占用，请通过替换 `--port=8888` 来指定其他端口。
...
```
# 安装微调所需的必要库，包括参数高效微调（peft）和transformsers
...
登录 Hugging Face 以上传您微调后的模型。您需要使用 Hugging Face 账户进行身份验证，才能直接从本笔记本保存和共享您的模型。
...
print(f"令牌验证成功！已登录用户：{user_info['name']}")
...
你是一名图表图像分析专家。你的任务是根据提供的图表图像，准确解读其中的数据、趋势和关键信息，并用简洁的中文直接回答用户提出的问题。避免冗长解释，只给出基于图表的答案。
...
你的任务是分析提供的图表图像，并用简洁的答案（通常是一个单词、数字或短语）回应查询。
...
将数据集格式化为用于交互的聊天机器人结构。每个交互包含一条系统消息，随后是图像和用户的查询，最后是对查询的回答。

针对该模型的更多使用技巧，请参见[模型卡片](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct#more-usage-tips)。
...
为了教学目的，你将仅加载数据集中每个分区的10%。然而，在实际应用场景中，通常会加载全部样本。
...
接下来，查看数据集的结构。它包含一个图像、一个查询、一个标签（即答案），以及第四个你将要丢弃的特征。
...
加载数据集后，加载模型并使用数据集中的样本评估其性能。本教程使用 [Qwen/Qwen2-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct)，这是一种能够理解视觉数据和文本的视觉语言模型 (VLM)。
...
使用不包含系统消息的样本来评估VLM的原始理解能力。以下是需使用的输入：

train_dataset[0][1:2]

```

现在查看样本对应的图表。你能根据视觉信息回答这个查询吗？
...
创建一个方法，该方法以模型、处理器和样本作为输入，生成模型的答案。这允许你简化推理过程并轻松评估VLM的性能。
...
尽管模型能够成功检索到正确的视觉信息，但在准确回答问题方面仍存在困难。这表明微调可能是提升其性能的关键。现在可以开始进行微调流程了。
...
**注意**：另外，可以通过使用 [bitsandbytes](https://huggingface.co/docs/bitsandbytes/main/en/index) 来加载量化模型。要了解更多关于量化的信息，请参阅 [Hugging Face 的这篇博客文章](https://huggingface.co/blog/merve/quantization) 或 [Maarten Grootendorst 的这篇](https://www.maartengrootendorst.com/blog/quantization/)。
...
接下来，为训练配置LoRA。LoRA通过应用低秩近似来降低内存使用，从而进一步减少内存需求并提升训练效率，是在不牺牲质量的前提下优化模型性能的绝佳选择。
...
使用监督微调（SFT）来优化模型在该任务上的性能。为此，使用 [TRL 库](https://huggingface.co/docs/trl/index) 中的 [SFTConfig](https://huggingface.co/docs/trl/sft_trainer) 类定义训练参数。SFT 提供带标签的数据，帮助模型学习根据输入生成更准确的响应。这种方法确保模型针对您的特定用例进行定制，从而在理解和响应视觉查询方面获得更好的性能。
...
您可以使用 [Weights & Biases (W&B)](https://wandb.ai/) 记录训练进度。将您的笔记本连接到 W&B，从而在训练过程中捕获关键信息。
...
模型需要一个 collator function 以在训练过程中正确地检索和批处理数据。该函数将数据集输入格式化为模型所需的结构，确保其结构正确。请在下方定义 collator function。
...
# labels 是 input_ids，并在损失计算中屏蔽填充标记
labels = batch["input_ids"].clone() # 克隆 input_ids 作为 labels
labels[labels == processor.tokenizer.pad_token_id] = -100 # 在 labels 中屏蔽填充标记
...
现在您已经成功微调了视觉语言模型（VLM），是时候评估其性能了。本节使用 ChartQA 数据集中的示例来测试模型，观察它在根据图表图像回答问题时的表现。这为探索结果提供了一种有效途径。
...
将训练好的适配器附加到预训练模型上。此适配器包含您在训练期间进行的微调调整，使基础模型能够利用新知识，同时不改变其核心参数。集成适配器增强了模型的能力，同时保持其原始结构。
...
该样本来自训练集，因此模型在训练期间已见过它。这可以被视为一种作弊形式。为了更全面地了解模型性能，请使用一个未见过的样本来进行评估。
...
模型已成功学会按照数据集中的指定响应查询。你已实现目标！

## 对比微调模型与基于提示的基础模型[#](#compare-a-fine-tuned-model-versus-a-base-model-with-prompting)

您已经了解了微调VLM如何成为将其适配到特定需求的有价值选项。另一种可以考虑的方法是直接使用提示（prompting）或实现RAG系统，这将在另一篇[配方](https://huggingface.co/learn/cookbook/multimodal_rag_using_document_retrieval_and_vlms)中介绍。

微调VLM需要大量的数据和计算资源，这会产生成本。相比之下，你可以尝试通过提示工程来观察是否能在无需微调开销的情况下获得类似的效果。
...
此案例再次使用之前的样本，但这次包含系统消息如下。此增强有助于将输入置于模型的上下文中，可能提高其响应准确性。
...
如图所示，该模型使用预训练模型以及额外的系统消息生成了正确答案，无需任何训练。根据具体用例，这种方法可以作为微调的一个可行替代方案。