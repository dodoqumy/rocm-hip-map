...
- [ROCm（ROCm（Radeon 开放计算平台）） 组件](#rocm-components){.reference .internal .nav-link}
  - [库](#libraries){.reference .internal .nav-link}
    - [机器学习与计算机视觉](#machine-learning-computer-vision){.reference .internal .nav-link}
    - [通信](#communication){.reference .internal .nav-link}
    - [数学](#math){.reference .internal .nav-link}
    - [基元](#primitives){.reference .internal .nav-link}
  - [工具](#tools){.reference .internal .nav-link}
    - [系统管理](#system-management){.reference .internal .nav-link}
    - [性能](#performance){.reference .internal .nav-link}
    - [开发](#development){.reference .internal .nav-link}
  - [编译器](#compilers){.reference .internal .nav-link}
  - [运行时 API](#runtime-api){.reference .internal .nav-link}

# 什么是ROCm（ROCm（Radeon 开放计算平台））？[\#](#what-is-rocm "Link to this heading"){.headerlink}
...
ROCm（ROCm（Radeon 开放计算平台））是一个软件栈，主要由开源软件组成，为编写 AMD 图形处理器 (GPU) 程序提供工具，从底层内核到高层终端用户应用。
...
ROCm（ROCm（Radeon 开放计算平台））由以下组件组成。有关每个组件的许可证信息，请参阅 [[ROCm（ROCm（Radeon 开放计算平台）） licensing]{.doc}](about/license.html){.reference .internal}。

### 库[\#](#libraries "Link to this heading"){.headerlink}

#### 机器学习与计算机视觉[\#](#machine-learning-computer-vision "链接到此标题"){.headerlink}
...
[rocCV](https://rocm.docs.amd.com/projects/rocCV/en/latest/index.html){.reference .external} 是一个高效的 GPU 加速图像预处理和后处理库。rocCV 处于早期访问状态，不建议将其用于生产工作负载。
...
[ROCprof Compute Viewer](https://rocm.docs.amd.com/projects/rocprof-compute-viewer/en/amd-mainline/){.reference .external} 是一个用于可视化和分析 GPU 线程跟踪数据的工具，该数据通过 [[rocprofv3]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/rocprofiler-sdk/en/latest/index.html "(in Rocprofiler SDK v1.1.0)"){.reference .external} 收集。请注意，[ROCprof Compute Viewer](https://rocm.docs.amd.com/projects/rocprof-compute-viewer/en/amd-mainline/){.reference .external} 处于早期访问状态。不建议用于生产工作负载。

#### 开发[\#](#development "链接至此标题"){.headerlink}
