...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大型语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批量处理请求并充分利用GPU资源来优化文本生成工作负载，使开发者能够应对代码生成和大规模对话AI等复杂任务。

vLLM可以利用[Quark](https://quark.docs.amd.com/latest/index.html)——一个灵活且强大的量化工具包，来生成高性能的量化模型，以在AMD GPU上运行。Quark专门支持对大型语言模型进行量化，包括权重、激活和KV缓存量化，并采用前沿的量化算法，如AWQ、GPTQ、Rotation和SmoothQuant。

本教程将指导您设置 Quark 并将 LLM 模型量化至 FP8，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct™ GPU 上运行 FP8 模型。了解如何配置 Quark 参数、实现不同的模型精度，并通过不同量化算法比较性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认您的设置：此命令将列出您的 AMD GPU 及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

使用命令代替。**Docker**：确保 Docker 已正确安装和配置。请按照您操作系统的 Docker 安装指南进行操作。**注意**：确保正确配置 Docker 权限。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
[Hugging Face](https://huggingface.co)用于下载模型。确保Hugging Face API令牌具有必要的权限和批准以访问该
...
启动Docker容器并映射必要的目录。从你的主机上，运行以下命令：
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到该目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU wheel 版本，这样 Quark 就可以在没有 GPU 的笔记本电脑上运行。虽然速度会慢一些，但用于尝试 Quark 已经足够了。从 PyPI 安装 Quark，它会自动拉取所需的依赖项。
...
Quark 使用 PyTorch dataloader 来加载校准数据。有关如何高效使用校准数据集的更多详情，请参见[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
在权重、激活和 KV 缓存上执行逐张量量化，量化算法为 AutoSmoothQuant。更多详情请参见 [Quark 配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 定义全局量化配置，输入张量和权重使用 FP8_PER_TENSOR_SPEC。
global_quant_config = QuantizationConfig(input_tensors=FP8_PER_TENSOR_SPEC,
weight=FP8_PER_TENSOR_SPEC)
# 为 KV 缓存层定义量化配置，输出张量使用 FP8_PER_TENSOR_SPEC。
...
# 根据提示词生成文本。输出是RequestOutput对象的列表，包含提示词、生成文本及其他信息。
...
import gc
gc.collect()
# 第三步：如果使用PyTorch后端，清除CUDA缓存（可选但有用）
import torch
...
除了上述 Python API 示例之外，Quark 还提供了一个量化脚本，用于更方便地量化大语言模型。它支持使用多种不同的量化方案和优化算法对模型进行量化。它可以在进行过程中导出量化模型并运行评估任务。
...
**Figure 2:** AMD Quark PyTorch 量化最佳实践
...
AMD Quark 支持多种专为 LLMs 设计的量化算法。您可以尝试以下算法以提高精度。

**注意：** 本节中的模型精度不限于 FP8。

### AWQ（激活感知权重量化）[#](#awq-activation-aware-weight-quantization)

AWQ 通过平滑网格搜索确定最优缩放因子，广泛应用于低位权重量化（例如，分组大小为128的W4量化）。该算法可通过以下命令应用。
...
AutoSmoothQuant 通过自动为每一层选择最优值来增强 SmoothQuant，这一过程由跨块的均方误差（MSE）损失指导。
...
QuaRot 使用名为 Hadamard 变换的旋转技术消除激活异常值。AMD Quark 支持 QuaRot 算法，使用方法如下。
...
QuaRot在其算法中采用了在线Hadamard变换，这需要内核支持以实现硬件部署。受QuaRot和QServer启发，AMD Quark引入了Rotation方法，该方法无需修改内核即可提升精度。
...
尝试各种量化方案有助于提高精度。但请记住，选择合适的方案取决于您的具体需求和硬件约束。

### 关键量化方案 [#](#key-quantization-schemes)

权重量化与权重-激活量化：激活量化可能导致显著的精度下降，而采用极低位宽的权重量化可能会产生更好的结果。

量化粒度

权重量化：可选方案包括逐张量、逐通道或逐组量化。

激活量化：选项包括 per-tensor（逐张量）或 per-token（逐 token）量化。

动态量化与静态量化：对于激活量化，动态量化通常比静态量化获得更好的精度。

对称 vs 非对称：根据模型对有符号值或无符号值的敏感程度，尝试使用对称量化或非对称量化进行实验。
...
. 选择最能平衡模型精度与效率的数据类型。  
KV缓存量化：跳过KV缓存量化通常能带来更好的性能。将该方法应用于整个KV缓存或其特定部分，可能会获得更高的精度。

如果应用上述方法后精度问题仍然存在，可考虑尝试AMD Quark调试工具来识别异常层并将其排除在量化范围之外。