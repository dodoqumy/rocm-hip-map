...
量化可以有效减少内存和带宽使用，加速计算，并在最小化精度损失的同时提高吞吐量。

[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量与低延迟。它通过高效地批量处理请求并充分利用GPU资源来优化文本生成工作负载，使开发者能够处理如代码生成和大规模对话式AI等复杂任务。

vLLM 可利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）来生成高性能的量化模型，从而在 AMD GPU 上运行。Quark 专为大型语言模型的量化提供支持，包括权重、激活和 KV 缓存量化，以及 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将指导您设置Quark并将LLM模型量化至FP8，然后使用[ROCm（ROCm（Radeon 开放计算平台））](https://rocm.docs.amd.com/en/latest/index.html)软件栈在AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU上运行FP8模型。学习如何配置Quark参数，实现不同的模型精度，以及比较不同量化算法的性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的设置：该命令会列出您的 AMD GPU 及相关详细信息。

**注意**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`。

command instead.**Docker**：确保 Docker 已正确安装和配置。请按照适用于您操作系统的 Docker 安装指南进行操作。**注意**：确保 Docker 权限已正确配置。要配置权限以允许非 root 用户访问，请执行以下命令：usermod -aG docker $USER newgrp docker
...
启动Docker容器并映射必要的目录。从您的主机上运行此命令：
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在其启动后上传到 Jupyter Notebook 环境中。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
在运行上述命令之前，请确保端口8888在您的系统上未被使用。如果已被占用，您可以通过替换`--port=8888`来指定一个不同的端口。
...
接下来，安装CMake和Quark。选择PyTorch的CPU wheel，以便Quark可以在没有GPU的笔记本电脑上运行。这样速度较慢，但对于试用Quark来说没问题。从PyPI安装Quark，它会自动拉取所需的依赖项。

在运行于Docker容器内的Jupyter notebook中运行以下命令：
...
您需要一个 Hugging Face API 令牌才能访问 Llama-3.1-8B。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的令牌，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 申请访问权限。令牌通常以“hf_”开头。

在您的Jupyter笔记本中运行以下交互式代码块以设置令牌：
...
安装 Quark 后，请按照以下步骤学习如何使用。Quark 量化过程包括以下步骤：
...
Quark 使用 transformers 来获取 model 和 tokenizer。
...
Quark使用PyTorch的数据加载器（dataloader）来加载校准数据。有关如何高效使用校准数据集的更多详情，请参阅[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存执行逐张量量化，量化算法为AutoSmoothQuant。有关更多详细信息，请参阅[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
logger.error(f"保存 tokenizer 时发生错误: {e}。你可以尝试手动保存 tokenizer")
...
import gc
gc.collect()
# 步骤 3：如果使用 PyTorch 后端，清理 CUDA（可选但建议）
import torch
...
除了上述Python API示例之外，Quark还提供了一个量化脚本，以便更方便地对大型语言模型进行量化。它支持使用多种不同的量化方案和优化算法来量化模型。它可以导出量化后的模型，并在此过程中运行评估任务。

**Note:** 你可以使用 `--output_dir` 来更改该脚本的输出目录。
...
这部分概述了使用 AMD Quark PyTorch 进行 PTQ 的最佳实践。它提供了调整量化策略以解决精度下降问题的指导。以下示例使用了 `meta-llama/Llama-3.1-8B-Instruct`。
...
AMD Quark 支持多种专门为 LLMs 设计的量化算法。您可以尝试以下算法以提升精度。

**注意：** 在本节中，模型精度不限于 FP8。

### AWQ (激活感知的权重量化)[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑的网格搜索确定最佳缩放因子，广泛应用于低位仅权重量化（例如，组大小为128的W4量化）。该算法可通过以下命令应用。
...
AutoSmoothQuant 通过在每个块上使用均方误差（MSE）损失作为指导，自动为每一层选择最优值，从而增强了 SmoothQuant。
...
QuaRot使用一种名为Hadamard变换的旋转技术来消除激活异常值。AMD Quark支持QuaRot算法，可以按如下方式使用。
...
QuaRot在其算法中采用了在线Hadamard变换，这需要内核支持以实现硬件部署。受QuaRot和QServer启发，AMD Quark引入了Rotation方法，该方法在不修改内核的情况下提升了精度。
...
尝试各种量化方案有助于提高精度。但请记住，选择合适的方案取决于您的具体要求及硬件约束。

### 关键量化方案[#](#key-quantization-schemes)

权重量化 vs. 权重-激活量化：激活量化可能导致显著的精度下降，而使用极低比特宽度的权重量化则可以获得更好的结果。
...
对称与非对称：根据模型对有符号或无符号值的敏感程度，尝试采用对称或非对称量化进行实验。
...
. 为您的模型选择最能平衡准确性和效率的数据类型。
KV缓存量化：跳过KV缓存量化通常能获得更好的性能。对整个KV缓存或其特定部分应用此方法可能会带来更好的准确性。

如果在应用上述方法后精度问题仍然存在，考虑尝试使用 AMD Quark 调试工具来识别异常层并将它们从量化中排除。