...
量化可以有效降低内存和带宽使用，加速计算，并在最小精度损失的前提下提高吞吐量。

[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批量处理请求并充分利用 GPU 资源来优化文本生成工作负载，使开发者能够处理代码生成和大规模对话式 AI 等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）来生成高性能的量化模型，以在 AMD GPU 上运行。Quark 特别支持对大语言模型进行量化，包括权重、激活和 KV 缓存量化，并支持 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将指导您设置Quark并将LLM模型量化为FP8，然后在AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU上使用[ROCm（ROCm（Radeon 开放计算平台））](https://rocm.docs.amd.com/en/latest/index.html)软件栈运行FP8模型。学习如何配置Quark参数，实现不同的模型精度，并比较不同量化算法的性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)进行安装和验证。安装完成后，使用以下命令确认配置：此命令将列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

取而代之使用命令。**Docker**：确保已正确安装和配置Docker。请遵循适用于您操作系统的Docker安装指南。**注意**：确保正确配置了Docker权限。要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
用于下载模型的 [Hugging Face](https://huggingface.co)。确保 Hugging Face API token 具有必要的权限和批准以访问。
...
启动 Docker 容器并映射必要的目录。从你的宿主机上，运行此命令：
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或在 Jupyter Notebook 环境启动后上传至该环境。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
确保在运行上述命令之前，该端口在您的系统上尚未被占用。如果已被占用，您可以通过替换 `--port=8888` 来指定其他端口。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU wheel，这样 Quark 可以在没有 GPU 的笔记本电脑上运行。这样速度较慢，但用于试用 Quark 是可以的。从 PyPI 安装 Quark，它会拉取所需的依赖项。
...
您需要一个Hugging Face API令牌才能访问Llama-3.1-8B。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的令牌，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 申请访问权限。令牌通常以“hf_”开头。
...
print(f"令牌验证成功！以如下身份登录: {user_info['name']}")
...
Quark 使用 PyTorch dataloader 来加载校准数据。关于如何高效使用校准数据集的更多细节，请参阅 [Adding Calibration Datasets](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存进行逐张量量化，量化算法为AutoSmoothQuant。更多详细信息，请参阅 [Quark 配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 从提示生成文本。输出是一个包含提示、生成文本等信息的 RequestOutput 对象列表。
...
import gc
gc.collect()
# 步骤 3: 如果使用 PyTorch 后端，清除 CUDA（CUDA（统一计算设备架构））（可选但有帮助）
import torch
...
除了上述Python API示例外，Quark还提供了一种量化脚本，可更方便地对大型语言模型进行量化。该脚本支持使用多种不同的量化方案和优化算法对模型进行量化，并能导出量化模型并实时运行评估任务。
...
确认当前工作目录为 `./amd_quark-0.8.1/examples/torch/language_modeling/llm_ptq/`
...
AMD Quark 支持多种专为 LLM 设计的量化算法。您可以尝试使用以下算法来提升精度。

**注意:** 本节中模型精度不限于FP8。

### AWQ (激活感知权重量化)[#](#awq-activation-aware-weight-quantization)

AWQ 通过平滑网格搜索确定最优缩放因子，并广泛用于低比特权重量化（例如，分组大小为128的W4量化）。该算法可使用以下命令应用。
...
AutoSmoothQuant 通过自动选择每层的最优值来增强 SmoothQuant，该过程以块间的均方误差（MSE）损失为指导。
...
QuaRot通过一种名为Hadamard变换的旋转技术消除激活值异常。AMD Quark支持QuaRot算法，使用方法如下。
...
QuaRot在其算法中采用了在线Hadamard变换，这需要内核支持才能进行硬件部署。受QuaRot和QServer的启发，AMD Quark引入了Rotation方法，该方法无需修改内核即可提升精度。
...
尝试不同的量化方案有助于提高精度。但需注意，选择合适的方案取决于你的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

权重量化与权重-激活量化：激活量化可能导致显著的精度下降，而采用极低比特宽度的权重量化则可以获得更好的结果。
...
. 为你的模型选择最佳平衡精度和效率的数据类型。KV cache量化：跳过KV cache量化通常会带来更好的性能。将这种方法应用于整个KV cache或其特定部分可能会带来更高的精度。

在应用以上方法后，如果精度问题仍然存在，请考虑尝试使用AMD Quark调试工具来识别异常层，并将其排除在量化之外。