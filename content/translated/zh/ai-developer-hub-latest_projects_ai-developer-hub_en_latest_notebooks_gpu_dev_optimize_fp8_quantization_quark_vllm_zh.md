...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批量处理请求并充分利用 GPU 资源来优化文本生成工作负载，使开发人员能够处理诸如代码生成和大规模对话式 AI 等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活而强大的量化工具包）来生成高效的量化模型，以在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV 缓存的量化，以及 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将引导您设置 Quark，将 LLM 模型量化为 FP8，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上运行 FP8 模型。了解如何配置 Quark 参数、实现不同的模型精度，以及比较不同量化算法的性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：此命令列出您的 AMD GPU 及其相关详细信息。

**注意**: 对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`

**command instead.**  
**Docker**：确保 Docker 已正确安装并配置。请根据您的操作系统参考 Docker 安装指南。  
...
容器中的目录。确保在运行 Docker 命令前将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传至其中。保存终端输出中提供的令牌或 URL，以便从浏览器访问笔记本。你可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm/gpuaidev) 下载此笔记本。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU wheel，这样 Quark 就可以在没有 GPU 的笔记本上运行。这种方式速度较慢，但用于尝试 Quark 是没问题的。从 PyPI 安装 Quark，它会自动拉取所需的依赖。
...
你需要一个 Hugging Face API token 才能访问 Llama-3.1-8B。前往 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成你的 token，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 申请访问权限。Token 通常以 “hf_” 开头。
...
Quark 使用 transformers 获取模型和分词器。
...
在权重、激活和KV缓存上执行每张量量化，量化算法为AutoSmoothQuant。更多详情请参阅[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 定义KV缓存层的量化配置，输出张量应用FP8_PER_TENSOR_SPEC。
...
# 从提示生成文本。输出是 RequestOutput 对象列表，包含提示、生成的文本以及其他信息。
...
import gc
gc.collect()
# 第3步：如果使用 PyTorch 后端，清除 CUDA 缓存（可选但有用）
import torch
...
除了上述 Python API 示例外，Quark 还提供了一个量化脚本，用于更方便地量化大型语言模型。该脚本支持使用多种不同的量化方案和优化算法来量化模型，并可在量化过程中导出量化后的模型并运行评估任务。
...
本节概述了在使用 AMD Quark PyTorch 进行 PTQ 时的最佳实践，提供了调整量化策略以解决精度下降问题的指导。以下示例使用了 `meta-llama/Llama-3.1-8B-Instruct` 模型。

来自 `quark/examples/torch/language_modeling/llm_ptq` 的模型和代码文件

以演示该方法。

**图2：** AMD Quark PyTorch量化的最佳实践
...
AMD Quark 支持专为 LLMs 设计的多种量化算法。您可以尝试以下算法以提升精度。

**注意：** 在本节中，模型精度不限于 FP8。

### AWQ（激活感知权重量化）[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑网格搜索确定最优缩放因子，广泛应用于低位权重量化（例如，分组大小为128的W4量化）。可使用以下命令应用该算法。
...
AutoSmoothQuant 通过自动为每一层选择最优值来增强 SmoothQuant，这一过程由跨块的均方误差（MSE）损失引导。
...
QuaRot 使用一种名为 Hadamard 变换的旋转技术消除激活值异常。AMD Quark 支持 QuaRot 算法，其使用方式如下。
...
QuaRot在其算法中采用了在线Hadamard变换，这需要内核支持以实现硬件部署。受QuaRot和QServer启发，AMD Quark引入了旋转方法，该方法无需修改内核即可提升精度。
...
尝试不同的量化方案可以帮助提高准确性。但请注意，选择合适的方案取决于您的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

权重量化 vs. 权重-激活量化：激活量化可能导致显著的精度下降，而极低比特宽度的权重量化可以产生更好的结果。
...
对称与非对称：根据模型对有符号或无符号值的敏感程度，尝试实验对称量化或非对称量化。
...
选择最能平衡模型准确性和效率的数据类型。KV cache 量化：跳过 KV cache 量化通常能带来更好的性能。将这种方法应用于整个 KV cache 或其特定部分可能会提高准确性。

如果应用上述方法后精度问题仍然存在，请考虑尝试AMD Quark调试工具来识别异常层并将其排除在量化之外。