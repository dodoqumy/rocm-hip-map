...
# 使用 AMD Quark 进行 vLLM 的 FP8 量化

**作者**：Charles Yang

**知识水平**: 中级

量化可以有效降低内存和带宽占用，加速计算，并在精度损失极小的情况下提升吞吐量。

[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批处理请求并充分利用 GPU 资源来优化文本生成工作负载，使开发者能够管理诸如代码生成和大规模对话式 AI 等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活而强大的量化工具包）来生成高性能的量化模型，以在 AMD GPU 上运行。Quark 针对大语言模型的量化提供了专门支持，包括权重、激活和 KV 缓存的量化，以及诸如 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将指导您设置 Quark 并将 LLM 模型量化为 FP8，然后使用 [ROCm（ROCm（Radeon 开放计算平台））](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上运行 FP8 模型。了解如何配置 Quark 参数，实现不同的模型精度，并比较不同量化算法的性能。
...
**Ubuntu 22.04**: 确保你的系统运行 Ubuntu 22.04 版本。

### Software[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认设置：此命令将列出您的 AMD GPU 及相关详细信息。

**注意**：对于 ROCm（Radeon 开放计算平台） 6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**: 确保 Docker 已正确安装和配置。请根据您的操作系统遵循 Docker 安装指南。**注意**: 确保 Docker 权限已正确配置。要配置允许非 root 用户访问的权限，请运行以下命令：
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在其启动后上传到 Jupyter Notebook 环境中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未在您的系统上使用。如果已被占用，您可以通过替换 `--port=8888` 来指定不同的端口。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU wheel，这样 Quark 可以在没有 GPU 的笔记本电脑上运行。虽然速度较慢，但用于试用 Quark 是可行的。从 PyPI 安装 Quark，它会自动拉取所需的依赖项。
...
你需要一个 Hugging Face API 令牌才能访问 Llama-3.1-8B。在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成你的令牌，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 请求访问权限。令牌通常以 “hf_” 开头。
...
安装Quark后，请按照以下步骤学习如何使用它。Quark量化流程包括以下步骤：
...
Quark使用PyTorch数据加载器加载校准数据。关于如何高效使用校准数据集的更多细节，请参见[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存的逐张量量化，量化算法为AutoSmoothQuant。更多详情，请参阅[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
import gc
gc.collect()
# 步骤 3：如果使用 PyTorch 后端，清除 CUDA（CUDA（统一计算设备架构））（可选但有用）
import torch
...
除了上述Python API示例外，Quark还提供了一个量化脚本，以便更方便地对大型语言模型进行量化。它支持使用各种不同的量化方案和优化算法来量化模型，并能导出量化模型，同时边运行边执行评估任务。
...
本节概述了使用 AMD Quark PyTorch 进行 PTQ 的最佳实践，提供了微调量化策略以解决精度下降问题的指导。下面的示例使用了 `meta-llama/Llama-3.1-8B-Instruct`。
...
**注：** 在本节中，模型精度并不仅限于 FP8。

### AWQ（激活感知权重量化）[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑遍历网格搜索确定最优缩放因子，并广泛用于低比特权重量化（例如，组大小为128的W4量化）。该算法可通过以下命令应用。
...
AutoSmoothQuant 通过自动为每一层选择最优值来增强 SmoothQuant，这些选择由跨块的均方误差（MSE）损失指导。
...
QuaRot使用名为Hadamard变换的旋转技术消除激活异常值。AMD Quark支持QuaRot算法，可按如下方式使用。
...
QuaRot 在其算法中采用在线哈达玛变换，这需要硬件部署时的内核支持。受 QuaRot 和 QServer 启发，AMD Quark 引入了 Rotation 方法，该方法在无需修改内核的情况下提升了精度。
...
权重量化 vs. 权值-激活量化：激活量化可能导致显著的精度下降，而采用极低比特宽度的纯权重量化则能取得更好的效果。
...
动态量化与静态量化：对于激活量化而言，动态量化通常比静态量化能获得更好的精度。
...
. 选择最能平衡模型准确性和效率的数据类型。KV缓存量化：跳过KV缓存量化通常可获得更好的性能。将这种方法应用于整个KV缓存或其特定部分可能会带来更高的准确性。

如果在应用上述方法后精度问题仍然存在，可以考虑尝试使用AMD Quark调试工具来识别异常层并将其从量化中排除。