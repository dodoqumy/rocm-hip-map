...
# 在 vLLM 中使用 AMD Quark 进行 FP8 量化[#](#fp8-quantization-with-amd-quark-for-vllm)

**作者**: Charles Yang

**知识水平**: 中级

量化可以有效减少内存和带宽使用，加速计算，并提高吞吐量，同时将精度损失降至最低。

[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大型语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效批处理请求并充分利用 GPU 资源来优化文本生成工作负载，使开发者能够管理诸如代码生成和大规模对话式 AI 等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）生成高性能的量化模型，以便在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV 缓存的量化，并集成了 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将指导您设置 Quark，将 LLM 模型量化至 FP8，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct™ GPU 上运行 FP8 模型。了解如何配置 Quark 参数、实现不同的模型精度，以及比较不同量化算法的性能。

## 支持的模型[#](#supported-models)

**Figure 1:** AMD Quark工具中支持的模型。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照[ROCm安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认你的设置：此命令会列出你的AMD GPU及相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

改用命令。**Docker**：确保Docker已正确安装和配置。请根据您的操作系统的Docker安装指南进行操作。**注意**：确保Docker权限配置正确。要配置权限以允许非root用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
## 使用 Docker 和 ROCm 的环境搭建（ROCm（Radeon 开放计算平台））[#](#environment-setup-with-docker-and-rocm)
...
启动Docker容器并映射必要的目录。在你的宿主机上，运行以下命令：
...
directory in the container. 确保在运行 Docker 命令前将笔记本文件复制到此目录，或者在容器启动后将其上传到 Jupyter Notebook 环境中。保存终端输出中提供的 token 或 URL，以便从浏览器访问笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，该端口在您的系统上尚未被使用。如果已被使用，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU 版本，这样 Quark 就可以在没有 GPU 的笔记本上运行。虽然速度较慢，但用于试用 Quark 已经足够。从 PyPI 安装 Quark，这会自动拉取所需的依赖项。
...
您需要一个Hugging Face API token才能访问Llama-3.1-8B。在[Hugging Face Tokens](https://huggingface.co/settings/tokens)生成您的token，并为[Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct)请求访问权限。Token通常以“hf_”开头。
...
安装 Quark 后，请按照以下步骤学习如何使用它。Quark 的量化过程包括以下步骤：
...
Quark使用PyTorch dataloader来加载校准数据。关于如何高效使用校准数据集的更多详情，请参阅[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存进行逐张量量化，量化算法为AutoSmoothQuant。更多细节请参见[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 定义kv-cache层的量化配置，输出张量应用 FP8_PER_TENSOR_SPEC。
...
### 6. 在vLLM中的评估[#](#evaluation-in-vllm)

现在你可以直接通过 LLM 入口点加载并运行 Quark 量化模型：
...
import gc
gc.collect()
# 第三步：如果使用PyTorch后端，清除CUDA（CUDA（统一计算设备架构））缓存（可选但推荐）
import torch
...
除了上面的Python API示例之外，Quark还提供了一个量化脚本，可以更方便地对大型语言模型进行量化。它支持使用多种不同的量化方案和优化算法来量化模型。它可以在进行过程中导出量化模型并运行评估任务。
...
AMD Quark 支持多种专门为 LLM 设计的量化算法。您可以尝试以下算法以提高准确性。

**Note:** 本节中的模型精度不限于FP8。

### AWQ（激活感知权重量化）[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑网格搜索确定最优缩放因子，并广泛应用于低比特权重量化（例如，组大小为128的W4量化）。可以使用以下命令应用该算法。
...
AutoSmoothQuant 通过跨块的均方误差 (MSE) 损失引导，自动选择每个层的最优值，从而增强了 SmoothQuant。
...
QuaRot通过名为Hadamard变换的旋转技术消除激活值异常。AMD Quark支持QuaRot算法，可按如下方式使用。
...
QuaRot在其算法中采用了在线Hadamard变换，这需要内核支持以实现硬件部署。受QuaRot和QServer的启发，AMD Quark引入了Rotation方法，在不修改内核的情况下提高了精度。
...
尝试不同的量化方案有助于提高精度。但请注意，选择合适的方案取决于您的具体要求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

仅权重量化 vs. 权值-激活量化：激活量化可能导致显著的精度下降，而采用极低比特宽度的仅权重量化则可获得更好的结果。
...
Data types (Dtypes): AMD Quark 支持多种数据类型，包括
...
. 选择最适合模型精度与效率平衡的数据类型。KV缓存量化：跳过KV缓存量化通常可获得更好性能。将此方法应用于整个KV缓存或其特定部分，可能带来更高精度。

如果在应用上述方法后精度问题仍然存在，请考虑尝试使用AMD Quark调试工具来识别异常层并将其排除在量化之外。