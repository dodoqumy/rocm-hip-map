...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批处理请求并充分利用GPU资源来优化文本生成工作负载，使开发者能够管理诸如代码生成和大规模对话式AI等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html) —— 一个灵活且强大的量化工具包，来生成高性能的量化模型，并在 AMD GPU 上运行。Quark 为大型语言模型的量化提供了专门支持，包括权重、激活和 KV 缓存的量化，以及 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：请按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 进行安装与验证。安装完成后，通过以下命令确认环境配置：该命令将列出您的 AMD GPU 及其相关详细信息。

**注意**: 对于 ROCm 6.4 及更早版本，使用 `rocm-smi`。

改为使用命令。**Docker**：确保Docker已正确安装和配置。请根据您的操作系统遵循Docker安装指南。**注意**：确保Docker权限配置正确。要配置权限以允许非root用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
启动Docker容器并映射必要的目录。在您的宿主机上，运行此命令：
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以从您的 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未被您的系统占用。如果已被占用，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
接下来，安装CMake和Quark。选择PyTorch的CPU wheel版本，这样Quark可以在没有GPU的笔记本上运行。这样做速度较慢，但足够用来试用Quark。从PyPI安装Quark，它会自动拉取所需的依赖项。
...
访问 Llama-3.1-8B 需要 Hugging Face API 令牌。请前往 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的令牌，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 申请访问权限。令牌通常以“hf_”开头。
...
安装 Quark 后，按照以下步骤学习如何使用它。Quark 的量化流程包括以下步骤：
...
Quark 使用 PyTorch 的 dataloader 加载校准数据。关于如何高效使用校准数据集的更多细节，请参阅[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
在权重、激活和KV缓存上执行逐张量量化，量化算法为AutoSmoothQuant。有关更多详细信息，请参阅[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 根据提示词生成文本。输出是一组RequestOutput对象，
# 包含提示词、生成文本及其他信息。
...
# 步骤3：如果使用PyTorch后端，清除CUDA（CUDA（统一计算设备架构））缓存（可选但推荐）
import torch
...
除了上述 Python API 示例之外，Quark 还提供了一种量化脚本，可以更方便地对大型语言模型进行量化。该脚本支持使用多种不同的量化方案和优化算法来量化模型，并且能够在导出量化模型的同时运行评估任务。
...
AWQ通过平滑网格搜索确定最优缩放因子，广泛用于低位权重量化（例如，分组大小为128的W4量化）。该算法可通过以下命令应用。
...
QuaRot通过一种名为Hadamard变换的旋转技术消除激活值异常点。AMD Quark支持QuaRot算法，可按如下方式使用。
...
QuaRot 在其算法中采用了在线 Hadamard 变换，这需要内核支持才能在硬件上部署。受 QuaRot 和 QServer 的启发，AMD Quark 引入了 Rotation 方法，在无需修改内核的前提下提升了精度。
...
尝试不同的量化方案可以帮助提高精度。但请注意，选择合适的方案取决于您的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

仅权重量化与权重-激活量化：激活量化可能导致显著的精度下降，而使用极低位宽的仅权重量化可以产生更好的结果。
...
. 选择最能平衡模型准确性和效率的数据类型。KV cache量化：跳过KV cache量化通常能带来更好的性能。将这种方法应用于整个KV cache或其中的特定部分，可能会提高准确性。

如果在应用上述方法后精度问题仍然存在，可以考虑尝试AMD Quark调试工具来识别异常层并将其排除在量化之外。