...
# 为vLLM使用AMD Quark的FP8量化[#](#fp8-quantization-with-amd-quark-for-vllm)
...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效批处理请求并充分利用 GPU 资源来优化文本生成任务，使开发者能够应对代码生成和大规模对话式 AI 等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）来生成高性能的量化模型，以在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，涵盖权重、激活和 KV 缓存量化，并采用先进的量化算法，如 AWQ、GPTQ、Rotation 和 SmoothQuant。

本教程指导您设置Quark并将LLM模型量化为FP8，然后在AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU上使用[ROCm（ROCm（Radeon 开放计算平台））](https://rocm.docs.amd.com/en/latest/index.html)软件栈运行FP8模型。了解如何配置Quark参数、实现不同的模型精度，并使用不同量化算法比较性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） install guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认您的配置：此命令会列出您的 AMD GPU 及其相关详细信息。
...
[Hugging Face](https://huggingface.co) 用于下载模型。确保 Hugging Face API token 拥有必要的权限和批准，以访问

[Meta Llama 检查点](https://huggingface.co/meta-llama/Llama-3.1-8B)

## 使用Docker和ROCm设置环境（ROCm（Radeon开放计算平台））[#](#environment-setup-with-docker-and-rocm)
...
启动Docker容器并映射必要的目录。在您的宿主机上运行以下命令：
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在启动后将其上传到 Jupyter Notebook 环境中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 代码仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未被您的系统占用。如果已被占用，您可以通过替换 `--port=8888` 来指定其他端口。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU 版本的 wheel，以便 Quark 可以在没有 GPU 的笔记本电脑上运行。这样会更慢，但对于试用 Quark 来说是可以的。从 PyPI 安装 Quark，这会自动拉取所需的依赖项。
...
您需要一个 Hugging Face API token 才能访问 Llama-3.1-8B。请前往 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的 token，并申请 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 的访问权限。Token 通常以 "hf_" 开头。

在您的Jupyter notebook中运行以下交互式代码块来设置token：
...
安装Quark后，请按照以下步骤学习如何使用。Quark的量化过程包括以下步骤：
...
Quark 使用 PyTorch 数据加载器加载校准数据。关于如何高效使用校准数据集的更多详情，请参阅[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存的逐张量量化，量化算法为AutoSmoothQuant。更多详情请参阅[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 定义全局量化配置，对输入张量和权重应用 FP8_PER_TENSOR_SPEC。
global_quant_config = QuantizationConfig(input_tensors=FP8_PER_TENSOR_SPEC,
weight=FP8_PER_TENSOR_SPEC)
# 定义 kv-cache 层的量化配置，对输出张量应用 FP8_PER_TENSOR_SPEC。
...
现在你可以直接通过LLM entrypoint加载并运行Quark量化模型：
...
# 第3步：如果使用PyTorch后端，清除CUDA缓存（可选但有助于释放显存）
import torch
...
除了上述Python API示例之外，Quark还提供了一个量化脚本，用于更便捷地对大型语言模型进行量化。该脚本支持使用多种不同的量化方案与优化算法来量化模型。它能够导出量化后的模型，并同时运行评估任务。
...
AMD Quark 支持多种专门为大型语言模型（LLM）设计的量化算法。您可以尝试以下算法来提升精度。
...
AutoSmoothQuant 通过自动选择每层的最优值来增强 SmoothQuant，这些值由跨块的均方误差（MSE）损失指导。
...
QuaRot通过一种名为Hadamard变换的旋转技术来消除激活值中的异常点。AMD Quark支持QuaRot算法，具体使用方法如下。
...
QuaRot 在其算法中采用在线 Hadamard 变换，这需要内核支持才能进行硬件部署。受 QuaRot 和 QServer 的启发，AMD Quark 引入了 Rotation 方法，该方法在无需修改内核的情况下提升了精度。
...
仅权重量化 vs. 权值激活量化：激活量化可能导致显著的精度下降，而采用极低比特宽度的仅权重量化则可以产生更好的结果。
...
动态量化与静态量化：对于激活量化而言，动态量化通常比静态量化能获得更好的准确率。
...
- 选择最适合模型准确性和效率平衡的数据类型。  
KV缓存量化：跳过KV缓存量化通常能带来更好的性能。将这种方法应用于整个KV缓存或其特定部分，可能会获得更高的准确性。

如果应用上述方法后精度问题仍然存在，可以考虑尝试 AMD Quark 调试工具来识别异常层并将其从量化中排除。