...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大型语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效批处理请求并充分利用 GPU 资源来优化文本生成工作负载，使开发者能够处理代码生成和大规模对话式 AI 等复杂任务。

vLLM 可借助 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）生成高性能的量化模型，从而在 AMD GPU 上运行。Quark 为大型语言模型的量化提供专项支持，包括权重、激活和 KV 缓存量化，以及 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认配置：此命令将列出您的 AMD GPU 及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`。

改用命令。**Docker**：确保Docker已正确安装和配置。请参阅适用于您操作系统的Docker安装指南。**注意**：确保Docker权限已正确配置。要配置允许非root用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
容器中的目录。确保在运行 Docker 命令之前已将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。你可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev)下载此笔记本。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU 版本 wheel，以便 Quark 可以在没有 GPU 的笔记本电脑上运行。虽然速度较慢，但用于试用 Quark 已经足够。从 PyPI 安装 Quark，这会自动拉取所需的依赖项。
...
在 Jupyter 笔记本中运行以下交互块来设置 token：
...
安装Quark后，请按照以下步骤学习如何使用它。Quark量化过程包含以下步骤：
...
Quark使用PyTorch dataloader加载校准数据。有关如何高效使用校准数据集的更多细节，请参阅[Adding Calibration Datasets](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存进行per-tensor量化，量化算法为AutoSmoothQuant。详见[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
logger.error(f"保存tokenizer时发生错误：{e}。你可以尝试手动保存tokenizer")
...
# 从提示词生成文本。输出是RequestOutput对象的列表，
# 包含提示词、生成的文本以及其他信息。
...
除了上述 Python API 示例外，Quark 还提供了一个量化脚本，可以更方便地对大型语言模型进行量化。它支持使用多种不同的量化方案和优化算法对模型进行量化，并能在进行量化的同时导出量化模型并运行评估任务。
...
AMD Quark 支持专为LLM设计的多种量化算法。您可以尝试以下算法来提高精度。

**注意：** 本节中的模型精度不限于FP8。

### AWQ (激活感知权重量化)[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑遍历网格搜索确定最优缩放因子，广泛应用于低位权重量化（例如，分组大小为128的W4量化）。该算法可使用以下命令应用。
...
AutoSmoothQuant通过自动为每一层选择最优值来增强SmoothQuant，选择过程由跨块的均方误差（MSE）损失引导。
...
QuaRot在其算法中采用在线Hadamard变换，这需要内核支持才能进行硬件部署。受QuaRot与QServer启发，AMD Quark引入了Rotation方法，可在无需修改内核的前提下提升精度。
...
尝试不同的量化方案有助于提高准确性。但请记住，选择适当的方案取决于您的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

权重量化与权重激活量化：激活量化可能导致显著精度下降，而采用极低比特的权重量化可以取得更好的效果。
...
. 选择最能平衡模型准确性和效率的数据类型。  
KV缓存量化：跳过KV缓存量化通常能带来更好的性能。将此方法应用于整个KV缓存或其特定部分，可能会提高准确性。

如果在应用上述方法后精度问题仍然存在，可考虑使用AMD Quark调试工具识别异常层并将其排除在量化之外。