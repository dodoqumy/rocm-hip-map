...
量化可以有效减少内存和带宽占用，加速计算，并提高吞吐量，同时仅带来极小的精度损失。

[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大型语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批量处理请求并充分利用GPU资源来优化文本生成任务，赋能开发者管理代码生成、大规模对话式AI等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)，一个灵活且强大的量化工具包，来生成高性能的量化模型，以在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV 缓存量化，并支持诸如 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将指导您搭建 Quark 并将 LLM 模型量化为 FP8，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct™ GPU 上运行 FP8 模型。学习如何配置 Quark 参数，实现不同的模型精度，并比较不同量化算法的性能。

## Supported models[#](#supported-models)

**图 1:** AMD Quark工具中支持的模型。
...
**Ubuntu 22.04**：确保您的系统运行在 Ubuntu 22.04 版本。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：该命令会列出您的 AMD GPU 及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，使用`rocm-smi`

command instead.**Docker**: 确保 Docker 已正确安装和配置。请遵循适用于您操作系统的 Docker 安装指南。**注意**: 确保 Docker 权限已正确配置。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
## 使用Docker和ROCm（ROCm（Radeon开放计算平台））进行环境设置[#](#environment-setup-with-docker-and-rocm)
...
目录在容器内。确保在运行Docker命令之前将笔记本文件复制到此目录，或在启动后上传到Jupyter Notebook环境中。保存终端输出中提供的token或URL，以便从web浏览器访问笔记本。您可以从[AI Developer Hub GitHub仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev)下载此笔记本。
...
接下来，安装CMake和Quark。选择PyTorch的CPU wheel版本，这样Quark可以在没有GPU的笔记本电脑上运行。这样速度会慢一些，但用于试用Quark是可以的。从PyPI安装Quark，它会自动拉取所需的依赖项。
...
您需要一个 Hugging Face API token 才能访问 Llama-3.1-8B。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的 token，并申请 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 的访问权限。Token 通常以 "hf_" 开头。
...
对权重、激活值和KV缓存进行per-tensor量化，量化算法为AutoSmoothQuant。详情请参阅[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
### 量化模型[#](#quantize-the-model)

应用量化。量化完成后，在导出之前先冻结量化模型。
...
logger.info(f"量化模型（hf 格式）已成功导出到 {export_path}。")
```
...
# 从提示生成文本。输出是一个 RequestOutput 对象的列表，
# 包含提示、生成的文本和其他信息。
...
import gc
gc.collect()
# 第三步：如果使用 PyTorch 后端，清理 CUDA（CUDA（统一计算设备架构））（可选但有益）
import torch
...
除了上述 Python API 示例外，Quark 还提供量化脚本，以便更方便地量化大型语言模型。该脚本支持使用多种不同的量化方案和优化算法对模型进行量化，并可导出量化后的模型，同时支持在过程中运行评估任务。

**注意：** 你可以通过 `--output_dir` 参数更改脚本的输出目录。
...
AMD Quark支持多种专为LLM设计的量化算法。您可以尝试以下算法以提高准确度。

**注意：** 本节中的模型精度不仅限于FP8。

### AWQ (激活感知权重量化)[#](#awq-activation-aware-weight-quantization)

AWQ 通过平滑网格搜索确定最优缩放因子，广泛应用于低位权重量化（例如，组大小为128的W4量化）。该算法可通过以下命令应用。
...
AutoSmoothQuant通过自动选择每层的最优值来增强SmoothQuant，其依据是跨块的均方误差（MSE）损失。
...
QuaRot 在其算法中使用了在线 Hadamard 变换，这需要硬件部署时的内核支持。受 QuaRot 和 QServer 启发，AMD Quark 引入了 Rotation 方法，该方法在不修改内核的情况下提升了精度。
...
尝试不同的量化方案有助于提高准确性。但请注意，选择合适的方案取决于您的具体要求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

仅权重量化与权重-激活量化：激活量化可能导致显著的准确率下降，而使用极低位宽的仅权重量化可以产生更好的结果。
...
动态量化与静态量化：对于激活量化，动态量化通常比静态量化具有更好的精度。

对称量化与非对称量化：根据模型对有符号值或无符号值的敏感程度，尝试使用对称量化或非对称量化进行实验。
...
. 选择最能平衡模型准确性和效率的数据类型。  
KV cache量化：跳过KV cache量化通常能带来更好的性能。将此方法应用于整个KV cache或其特定部分，可能会提高准确性。

如果在应用上述方法后精确度问题仍然存在，请考虑尝试使用AMD Quark调试工具来识别异常层并将其从量化中排除。