...
# 用于vLLM的AMD Quark FP8量化[#](#fp8-quantization-with-amd-quark-for-vllm)
...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，专为大语言模型（LLM）推理提供高吞吐量和低延迟而设计。它通过高效批处理请求并充分利用GPU资源来优化文本生成工作负载，赋能开发者管理代码生成和大规模对话式AI等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）来生成高效的量化模型，以便在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV 缓存量化，并采用了 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将指导您设置 Quark，将 LLM 模型量化为 FP8，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct™ GPU 上运行 FP8 模型。学习如何配置 Quark 参数、实现不同的模型精度，以及比较不同量化算法的性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认配置：  
此命令会列出你的 AMD GPU 及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`

command instead.**Docker**: 确保Docker已正确安装和配置。请按照适用于您操作系统的Docker安装指南进行操作。**注意**: 确保Docker权限已正确配置。要配置权限以允许非root用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
使用 [Hugging Face](https://huggingface.co) 下载模型。确保 Hugging Face API token 拥有必要的权限和批准以访问该
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传该文件。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口未在您的系统上被占用。如果已被占用，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
接下来，安装 CMake 和 Quark。选择 PyTorch 的 CPU 版本，这样 Quark 可以在没有 GPU 的笔记本电脑上运行。虽然速度较慢，但足以体验 Quark 的功能。通过 PyPI 安装 Quark，它将自动拉取所需的依赖项。
...
您需要获取 Hugging Face API token 才能访问 Llama-3.1-8B。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 页面生成您的 token，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 请求访问权限。Token 通常以 “hf_” 开头。

在您的Jupyter notebook中运行以下交互块来设置令牌：
...
Quark 使用 transformers 获取模型和 tokenizer。
...
Quark 使用 PyTorch dataloader 加载校准数据。有关如何有效使用校准数据集的更多详细信息，请参阅 [Adding Calibration Datasets](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
import gc
gc.collect()
# 步骤 3：如果使用 PyTorch 后端，清空 CUDA（CUDA（统一计算设备架构））（可选但有益）
import torch
...
除了上述Python API示例外，Quark还提供了一个量化脚本，可以更方便地对大型语言模型进行量化。它支持使用多种不同的量化方案和优化算法对模型进行量化。它可以导出量化模型并同时运行评估任务。
...
本节概述了使用 AMD Quark PyTorch 进行 PTQ 的最佳实践，指导如何微调量化策略以解决精度下降问题。以下示例使用了 `meta-llama/Llama-3.1-8B-Instruct`。
...
确认你当前的工工作目录是 `./amd_quark-0.8.1/examples/torch/language_modeling/llm_ptq/`
...
AWQ 通过平滑遍历网格搜索确定最优缩放因子，并广泛应用于低位权重量化（例如，分组大小为 128 的 W4 量化）。可通过以下命令应用该算法。
...
AutoSmoothQuant通过自动选择每层的最优值来增强SmoothQuant，并以跨块的均方误差（MSE）损失作为指导。
...
QuaRot 使用名为 Hadamard 变换的旋转技术消除激活异常值。AMD Quark 支持 QuaRot 算法，可按如下方式使用。
...
QuaRot 在其算法中使用了在线 Hadamard 变换，这需要内核支持才能在硬件上部署。受 QuaRot 和 QServer 启发，AMD Quark 引入了 Rotation 方法，该方法无需修改内核即可提升精度。
...
仅权重量化与权重-激活量化：激活量化可能导致显著的精度下降，而采用极低比特宽度的仅权重量化可以产生更好的结果。
...
动态量化与静态量化：对于激活量化，动态量化通常比静态量化具有更高的准确性。

对称量化与非对称量化：根据模型对带符号或无符号值的敏感程度，尝试使用对称或非对称量化进行实验。
...
. 选择能够最佳平衡模型准确性和效率的数据类型。KV缓存量化：跳过KV缓存量化通常能带来更好的性能。将这种方法应用于整个KV缓存或其特定部分，可能会获得更高的准确性。

如果在应用上述方法后准确性问题仍然存在，考虑尝试使用AMD Quark调试工具来识别异常层并将其排除在量化之外。