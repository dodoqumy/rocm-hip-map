...
量化可以有效地减少内存和带宽使用，加速计算，并在最小精度损失的情况下提高吞吐量。

[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批量处理请求并充分利用GPU资源来优化文本生成工作负载，使开发者能够处理代码生成和大规模对话式AI等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）来生成高性能的量化模型，以在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV 缓存量化，以及先进的量化算法，如 AWQ、GPTQ、Rotation 和 SmoothQuant。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2或6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的设置：此命令列出您的AMD GPU及相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

**命令。**  
**Docker**：确保 Docker 已正确安装和配置。请根据您操作系统的 Docker 安装指南进行操作。  
**注意**：请确保 Docker 权限已正确配置。要配置允许非 root 用户访问的权限，请运行以下命令：  
...
## 使用Docker和ROCm的环境设置（ROCm（Radeon 开放计算平台））[#](#environment-setup-with-docker-and-rocm)
...
启动Docker容器并映射必要的目录。从宿主机上运行以下命令：
...
将目录置于容器中。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或在 Jupyter Notebook 环境启动后上传该文件。保存终端输出中提供的 token 或 URL，以通过 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
你需要一个 Hugging Face API token 来访问 Llama-3.1-8B。在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成你的 token，并为 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) 请求访问权限。Token 通常以 “hf_” 开头。
...
Quark使用PyTorch dataloader加载校准数据。有关如何高效使用校准数据集的更多细节，请参阅[添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
logger.info(f"HuggingFace格式的量化模型已导出至 {export_path} 成功。")
```
...
现在您可以直接通过LLM entrypoint加载并运行Quark量化模型了：
...
# 步骤 3：如果使用 PyTorch 后端，则清除 CUDA（统一计算设备架构）（可选但有助于释放显存）
import torch
...
除了上述 Python API 示例外，Quark 还提供了一个量化脚本，以便更方便地对大语言模型进行量化。它支持使用多种不同的量化方案和优化算法对模型进行量化，并能导出量化后的模型，同时运行评估任务。
...
本节概述了使用 AMD Quark PyTorch 进行 PTQ 的最佳实践，提供有关微调量化策略以解决精度下降问题的指导。以下示例使用了 `meta-llama/Llama-3.1-8B-Instruct`。
...
AMD Quark 支持多种专为 LLM 设计的量化算法。您可以尝试以下算法来提升准确性。

**注意：** 本节中模型精度不限于FP8。

### AWQ（激活感知权重量化）[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑网格搜索确定最优缩放因子，广泛应用于低位权重量化（例如，分组大小为128的W4量化）。该算法可使用以下命令应用。
...
AutoSmoothQuant通过自动选择每层的最优值来增强SmoothQuant，其指导原则是跨块的均方误差（MSE）损失。
...
QuaRot 使用一种名为哈达玛变换的旋转技术消除激活值中的离群点。AMD Quark 支持 QuaRot 算法，可按如下方式使用。
...
QuaRot在其算法中使用了在线Hadamard变换，这需要内核支持才能进行硬件部署。受QuaRot和QServer的启发，AMD Quark引入了Rotation方法，该方法无需修改内核即可提升精度。
...
尝试各种量化方案有助于提高精度。但请注意，选择合适的方案取决于您的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

仅权重量化 vs. 权激活量化：激活量化可能导致显著的精度下降，而使用极低比特宽度的仅权重量化则能产生更好的结果。

量化粒度

权重量化：选项包括per-tensor、per-channel或per-group量化。

激活量化：选项包括逐张量（per-tensor）或逐词元（per-token）量化。

动态量化与静态量化：对于激活量化而言，动态量化通常比静态量化产生更好的精度。

对称量化与非对称量化：尝试根据模型对有符号值或无符号值的敏感程度，实验对称量化或非对称量化。
...
. 选择最适合模型精度和效率平衡的数据类型。 KV 缓存量化：跳过 KV 缓存量化通常能带来更好的性能。将这种方法应用于整个 KV 缓存或其中的特定部分，可能会获得更高的精度。
