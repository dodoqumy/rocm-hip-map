...
# 针对vLLM的AMD Quark FP8量化[#](#fp8-quantization-with-amd-quark-for-vllm)
...
vLLM 可以借助 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包），生成高性能的量化模型以在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV cache 量化，并支持 AWQ、GPTQ、Rotation 和 SmoothQuant 等前沿量化算法。

本教程将引导您完成 Quark 的设置并对 LLM 模型进行 FP8 量化，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct™ GPU 上运行 FP8 模型。学习如何配置 Quark 参数、实现不同模型精度，以及比较不同量化算法的性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：该命令会列出您的 AMD GPU 及相关详细信息。

**注意**: 对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`。

command instead.**Docker**：确保 Docker 已正确安装和配置。请遵循针对您操作系统的 Docker 安装指南。**注意**：确保 Docker 权限配置正确。要配置允许非 root 访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
## 使用Docker和ROCm（ROCm（Radeon 开放计算平台））进行环境设置[#](#environment-setup-with-docker-and-rocm)
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传文件。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，该端口尚未在你的系统上被使用。如果已被占用，你可以通过替换 `--port=8888` 来指定不同的端口。
...
接下来，安装CMake和Quark。选择PyTorch的CPU wheel，这样Quark可以在没有GPU的笔记本电脑上运行。虽然速度较慢，但用于尝试Quark是可以的。从PyPI安装Quark，它会自动拉取所需的依赖项。
...
您需要一个 Hugging Face API token 才能访问 Llama-3.1-8B。请在 [Hugging Face Tokens](https://huggingface.co/settings/tokens) 生成您的 token，并申请访问 [Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct)。Token 通常以 “hf_” 开头。
...
Quark 使用 PyTorch 的 dataloader 加载校准数据。有关如何高效使用校准数据集的更多详细信息，请参阅 [添加校准数据集](https://quark.docs.amd.com/latest/pytorch/calibration_datasets.html)。
...
对权重、激活和KV缓存进行每张量量化，而量化算法是AutoSmoothQuant。详见[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
# 为 KV 缓存层定义量化配置，输出张量应用 FP8_PER_TENSOR_SPEC。
...
import gc
gc.collect()
# 第3步：如果使用 PyTorch 后端，则清除 CUDA（可选但有助于释放内存）
import torch
...
除了上述 Python API 示例外，Quark 还提供了一套量化脚本，以便更方便地对大型语言模型进行量化。该脚本支持使用多种不同的量化方案和优化算法对模型进行量化，并可在过程中导出量化模型并执行评估任务。
...
AMD Quark 支持多种专门为大型语言模型（LLM）设计的量化算法。你可以尝试以下算法来提升精度。

**注意：** 本节中的模型精度不限于FP8。

### AWQ (激活感知权重量化)[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑网格搜索确定最优缩放因子，广泛应用于低位仅权重量化（例如，组大小为128的W4量化）。该算法可通过以下命令应用。
...
AutoSmoothQuant 通过自动选择每层的最优值来增强 SmoothQuant，其优化依据是跨块的均方误差（MSE）损失。
...
QuaRot 使用一种名为 Hadamard 变换的旋转技术来消除激活值异常点。AMD Quark 支持 QuaRot 算法，其使用方法如下。
...
QuaRot 在其算法中使用了在线 Hadamard 变换，这需要内核支持才能在硬件上部署。受 QuaRot 和 QServer 启发，AMD Quark 引入了 Rotation 方法，该方法在不修改内核的情况下提升了精度。
...
尝试各种量化方案有助于提高准确性。但请注意，选择适当的方案取决于您的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

仅权重量化 vs. 权-激活量化：激活量化可能导致显著的精度下降，而采用极低比特宽度的仅权重量化却能获得更好的结果。
...
. 选择能够在准确性和效率之间取得最佳平衡的数据类型来适配您的模型。  
KV cache 量化：跳过 KV cache 量化通常能带来更好的性能。将这种方法应用于整个 KV cache 或其特定部分，可能会提高准确性。

如果应用上述方法后精度问题仍然存在，可以考虑尝试AMD Quark调试工具来识别异常层，并将它们从量化中排除。