...
[vLLM](https://docs.vllm.ai/en/latest/) 是一个开源库，旨在为大型语言模型（LLM）推理提供高吞吐量和低延迟。它通过高效地批处理请求并充分利用GPU资源来优化文本生成工作负载，使开发者能够管理诸如代码生成和大规模对话AI等复杂任务。

vLLM 可以利用 [Quark](https://quark.docs.amd.com/latest/index.html)（一个灵活且强大的量化工具包）来生成高性能的量化模型，以在 AMD GPU 上运行。Quark 专门支持对大型语言模型进行量化，包括权重、激活和 KV 缓存量化，并支持先进的量化算法，如 AWQ、GPTQ、Rotation 和 SmoothQuant。

本教程将指导您设置 Quark 并将 LLM 模型量化为 FP8，然后使用 [ROCm](https://rocm.docs.amd.com/en/latest/index.html) 软件栈在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上运行 FP8 模型。了解如何配置 Quark 参数，实现不同的模型精度，并比较不同量化算法的性能。
...
**ROCm（ROCm（Radeon 开放计算平台）） 6.2 或 6.3**：按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 进行安装和验证。安装完成后，使用以下命令确认设置：此命令会列出您的 AMD GPU 及其相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

改用以下命令。**Docker**：确保 Docker 已正确安装和配置。请遵照您操作系统的 Docker 安装指南进行操作。**注意**：确保 Docker 的权限配置正确。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
目录在容器中。确保在运行Docker命令之前将笔记本文件复制到此目录，或者在Jupyter Notebook环境启动后上传该文件。保存终端输出中提供的令牌或URL，以便从Web浏览器访问笔记本。您可以从 [AI Developer Hub GitHub存储库](https://github.com/ROCm/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，指定的端口在您的系统上应尚未使用。如果已在使用中，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
接下来，安装CMake和Quark。选择PyTorch的CPU wheel，这样Quark就可以在没有GPU的笔记本电脑上运行。虽然速度较慢，但用于试用Quark是可以的。从PyPI安装Quark，它会自动拉取所需的依赖项。
...
您需要Hugging Face API令牌才能访问Llama-3.1-8B。在[Hugging Face Tokens](https://huggingface.co/settings/tokens)生成您的令牌，并申请[Llama-3.1-8B-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct)的访问权限。令牌通常以“hf_”开头。
...
对权重、激活和KV缓存进行逐张量量化，而量化算法是AutoSmoothQuant。更多详情请参见[Quark配置指南](https://quark.docs.amd.com/latest/pytorch/user_guide_config_description.html)。
...
除了上述Python API示例之外，Quark还提供了一个量化脚本，以便更便捷地对大型语言模型进行量化。该脚本支持使用多种不同的量化方案和优化算法对模型进行量化，可导出量化模型，并在过程中执行评估任务。
...
AMD Quark 支持多种专为 LLMs 设计的量化算法。您可以尝试以下算法来提高准确性。

**注意：** 本部分中的模型精度不限于 FP8。

### AWQ (激活感知权重量化)[#](#awq-activation-aware-weight-quantization)

AWQ通过平滑网格搜索确定最优缩放因子，并广泛应用于低位权重量化（例如组大小为128的W4量化）。该算法可通过以下命令应用。
...
AutoSmoothQuant 通过自动选择每个层的最优值来增强 SmoothQuant，并以跨块的均方误差（MSE）损失为指导。
...
尝试不同的量化方案有助于提升精度。但请注意，选择合适的方案取决于您的具体需求和硬件限制。

### 关键量化方案[#](#key-quantization-schemes)

仅权重量化 vs. 权值-激活量化：激活量化可能导致显著的精度下降，而使用极低比特宽度的仅权重量化可以获得更好的结果。
...
动态量化与静态量化：对于激活量化而言，动态量化通常比静态量化获得更好的精度。

对称与不对称：根据模型对有符号值或无符号值的敏感程度，尝试使用对称或非对称量化进行实验。
...
选择能够最好地平衡模型准确性和效率的数据类型。KV缓存量化：跳过KV缓存量化通常会带来更好的性能。将这种方法应用于整个KV缓存或其特定部分，可能会获得更好的准确性。

如果应用上述方法后精度问题仍然存在，请考虑尝试使用AMD Quark调试工具来识别异常层并将其从量化中排除。