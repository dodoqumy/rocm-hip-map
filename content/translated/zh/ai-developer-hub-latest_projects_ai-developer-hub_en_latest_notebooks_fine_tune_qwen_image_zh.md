...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个拥有 860 亿参数的庞大模型集合）的能力，并解释了如何利用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地进行微调。教程展示了 AMD Instinct™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以支持涉及推理、编辑和训练的复杂工作流。

**注意**：本教程由ModelScope与[Tongyi Lab](https://www.linkedin.com/company/alibaba-tongyi-lab/)（阿里巴巴集团）共同开发。
...
**操作系统**： Linux（推荐 Ubuntu 22.04）。查看[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)了解支持的操作系统。**硬件**： AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**： ROCm（ROCm（Radeon 开放计算平台）） 6.0 或更高版本，Docker，以及 Python 3.10 或更高版本

**注**: 按照 [ROCm（ROCm（Radeon 开放计算平台）） install guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 专为生成式 AI 工作负载提供峰值性能而设计。在开始之前，请确认你的 GPU 已被正确检测并处于就绪状态。
...
**注意：** 安装后，手动更新系统路径，以确保笔记本（notebook）无需重启内核即可立即导入库（library）。
...
# AMD ROCm（Radeon 开放计算平台）6.4 wheels 的索引（优先）
...
生成你的第一张图像，使用简单的提示词：*“一个美丽亚洲女性的肖像”*
...
Qwen-Image系列包含针对不同任务的专门模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和修复（in-painting）任务设计的模型。
...
你可以通过将刚刚生成的肖像进行outpainting，将其扩展为带有森林背景的远景图像。

```prompt = "逼真摄影：一位身着长裙的美丽女子，背景是森林。"
negative_prompt = "让角色的手指残缺变形，把头部放大，造成头部与身体比例不自然，使人物变成矮个子大头娃娃。生成刺眼、强烈的阳光，用过度饱和的色彩渲染整个场景。将腿部扭曲成X型或O型畸形。"
...
这张照片中的面孔看起来不一致。加载专门的LoRA模型[DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型可以根据面部参考生成一致的图像。
...
Qwen-Image 文本编码器足够鲁棒，能够理解它未被显式训练过的语言的提示词。为了尝试这一点，使用韩语提示词生成一个角色。首先，使用英文生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男子，身着深灰色修身西装，眼神平静而微笑，透露出自信与沉着。他坐在桌旁，手中捧着一束红玫瑰。"
...
现在你有了两张图像：森林中的女人和拿着花的男人。使用支持多图像编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两张独立的图像合并成一个连贯的场景，其中角色们正在互动。
...
您当前已同时将三个大规模模型加载到内存中。请计算总参数数量，以了解此工作负载的规模。
...
在标准 GPU 上处理这个问题是不可能的。然而，AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 拥有 192 GB 的显存，因此它可以将所有这些模型驻留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义LoRA适配器，让模型学习一个特定概念——本例中是一只特定的狗。
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功地在单个 GPU 上使用具有 86B 集体参数的模型进行了推理、以高度一致性编辑了图像，并训练了自定义适配器。