...
**注意**：请按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装和验证 ROCm（ROCm（Radeon 开放计算平台））。
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。开始前，请确认您的 GPU 已被正确检测并可供使用。
...
**注意**：配置环境，使用ModelScope作为下载权重文件的域。
...
### 外扩绘制与一致性[#](#outpainting-with-consistency)

你可以通过获取刚刚生成的肖像，将其扩展为具有森林背景的远景图像来执行外绘任务。

```
prompt = "一位穿着长裙的美丽女性的写实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以创造不自然的头身比例，将人物变成矮个子大头娃娃。产生刺眼、炫目的阳光，并以过度饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
negative_prompt = "使角色的手指残缺扭曲，放大头部以形成不自然的头身比例，将人物变成矮小的大头娃娃。产生刺眼、耀眼的阳光，并用过饱和的颜色渲染整个场景。将腿部扭曲成X型或O型畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
## 步骤 5：多语言与多图像编辑[#](#step-5-multilingual-and-multi-image-editing)

Qwen-Image 文本编码器足够鲁棒，能够理解它没有明确训练过的语言中的提示。为了尝试这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图片。
...
尽管Qwen-Image并未接受韩文文本训练，但其文本编码器的基础能力仍可提供多语言理解。

### 使用 Qwen-Image-Edit-2509 合并对象[#](#merging-subjects-with-qwen-image-edit-2509)

你现在拥有两张图片：森林中的女人和捧着花的男人。使用支持多图编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两张独立的图像合并成一个连贯的场景，其中角色正在互动。
...
你当前同时将三个大规模模型加载到内存中。计算总参数量以理解此工作负载的规模。
...
在标准GPU上处理这将是不可行的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有192 GB的显存，因此可以将所有这些模型常驻内存，以便在推理、编辑和训练任务之间无缝切换。
...
终于，可以开始从推理转向训练了。训练一个自定义的LoRA适配器，让模型学习特定概念，在本例中是一只特定的狗。
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功地在单块 GPU 上执行了总参数量达 86B 的模型推理、以高度一致性编辑图像，并训练了自定义适配器。