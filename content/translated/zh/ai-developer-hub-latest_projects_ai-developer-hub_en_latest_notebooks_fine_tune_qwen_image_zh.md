...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个拥有 860 亿参数的大规模模型集合）的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地进行微调。它展示了 AMD Instinct™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以支持涉及推理、编辑和训练的复杂工作流。
...
**操作系统**：Linux（推荐 Ubuntu 22.04）。支持的操作系统请参见[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。  
**硬件**：AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU  
**软件**：ROCm（Radeon 开放计算平台）6.0 或更高版本、Docker 以及 Python 3.10 或更高版本

**注**：请按照 [ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。在开始之前，请确认您的 GPU 已被正确检测并可供使用。
...
# 优先从 AMD ROCm（Radeon 开放计算平台）6.4 wheels 索引下载
...
## 步骤二：基础模型推理[#](#step-2-basic-model-inference)
...
为了提高图像质量，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 可以显著提升生成图像的视觉保真度和艺术细节。
...
Qwen-Image系列包括针对不同任务的专用模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和修复任务设计的模型。
...
你可以通过取用刚生成的肖像，将其扩展为带有森林背景的长景图像，从而执行外绘任务。

```
prompt = "一位穿着长裙的美丽女性的写实摄影。背景是森林。"
negative_prompt = "让角色的手指残缺畸形，放大头部以制造不自然的头身比，将人物变成矮小的大头娃娃。产生刺眼、眩目的阳光，并使用过度饱和的色彩渲染整个场景。将腿扭曲成X形或O形畸形。"
...
negative_prompt = "让角色的手指残缺扭曲，放大头部造成不自然的头身比，将人物变成矮小大头娃娃。产生刺眼灼热的阳光，以过饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够鲁棒，能够理解它未经过明确训练的语言中的提示。要尝试这一点，使用韩语提示生成一个角色。首先，用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身着深灰色修身西装，眼神平静含笑，透露出自信与沉着。他坐在桌前，手中捧着一束红色鲜花。"
...
```python
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身着深灰色修身西装，眼神沉稳而带着微笑，散发出自信与从容的气质。他坐在桌前，手中拿着一束红色花束。"
image = qwen_image(prompt, seed=2, num_inference_steps=40)
...
虽然Qwen-Image未针对韩语文本进行训练，但其文本编码器的基础能力仍具备多语言理解功能。

### 使用Qwen-Image-Edit-2509合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

你现在有两张图片：森林中的女人和手持鲜花的男人。使用[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)（支持多图编辑），你可以将这两张独立的图片合并成一个统一的场景，让其中的角色产生互动。
...
## 步骤 6：Instinct MI300X 的强大力量[#](#step-6-the-power-of-the-instinct-mi300x)

您当前已同时将三个大规模模型加载到内存中。计算总参数量以了解此工作负载的规模。
...
在标准 GPU 上处理这个问题是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 的显存，因此可以将所有这些模型保留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学会一个特定概念——在这个示例中，是一只特定的狗。
...
本教程展示了 AMD Instinct MI300X 的端到端能力。您成功地在单个 GPU 上使用总参数量为 86B 的模型执行推理、以高一致性编辑图像并训练了自定义适配器。