...
本教程介绍了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有 860 亿参数的大型模型集合——的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地对其进行微调。它演示了 AMD Instinct™ MI300X GPU 的高内存容量如何支持同时加载多个大型模型，以完成涉及推理、编辑和训练等复杂工作流。

**注**：本教程由ModelScope和[Tongyi Lab](https://www.linkedin.com/company/alibaba-tongyi-lab/)(阿里巴巴集团)共同开发。
...
**操作系统**：Linux（建议使用 Ubuntu 22.04）。查看[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)了解支持的操作系统。  
**硬件**：AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU  
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。在开始之前，请验证您的 GPU 已被正确检测并准备好使用。
...
# Index for AMD ROCm（ROCm（Radeon 开放计算平台）） 6.4 wheels (优先使用)
...
## 步骤3：使用LoRA增强质量[#](#step-3-enhancing-quality-with-lora)

你可能会注意到基准图像缺乏精细细节。

为了提升图像质量，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 可以显著增强生成图像的视觉保真度和艺术细节。
...
### 一致性外绘制[#](#outpainting-with-consistency)

你可以通过将刚刚生成的人像扩展成一个带有森林背景的全景图像，来执行一次 outpainting 任务。

```
prompt = "一位穿着长裙的美丽女性的写实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比，将人物变成矮胖的大头娃娃。产生刺眼、耀眼的阳光，并以过度饱和的颜色渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
prompt = "真实摄影风格：一位穿着长裙的美丽女子，背景是森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比例，将人物变成矮小的大头娃娃。生成刺眼、眩目的阳光，并以过饱和的色彩渲染整个场景。将双腿扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够稳健，能够理解它未经过明确训练的语言的提示词。要尝试这一点，请使用韩语提示词生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男士，穿着深灰色修身西装，眼神平静含笑，流露出自信与沉着。他坐在桌旁，手中捧着一束红色鲜花。"
...
尽管Qwen-Image并未使用韩文文本进行训练，但其文本编码器的基础能力仍然提供了多语言理解能力。

### 使用Qwen-Image-Edit-2509合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

您现在有两张图像：森林中的女人和拿花的男人。利用支持多图像编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两张独立的图像合并成一个连贯的场景，其中角色之间产生互动。
...
您当前已同时将三个庞大的模型加载到内存中。计算总参数量以了解此工作负载的规模。
...
在标准 GPU 上处理这将是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 显存，因此可以将所有这些模型常驻内存，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学会一个特定概念，在本例中是一只有特定特征的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个小型数据集，包含五张狗的图像及其相关元数据。
...
在训练之前，验证基础模型对提示符`"a dog"`的输出。

正如预期的那样，它生成了一只普通的狗，而不是您特定的主题。
...
本教程展示了AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X的端到端能力。您成功地在单个GPU上使用总参数为86B的模型进行了推理、以高一致性编辑图像，并训练了自定义适配器。