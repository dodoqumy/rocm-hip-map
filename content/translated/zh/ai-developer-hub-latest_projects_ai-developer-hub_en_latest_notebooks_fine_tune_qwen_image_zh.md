...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个拥有 860 亿参数的大规模模型集合）的能力，并解释了如何利用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效微调该模型。它展示了 AMD Instinct™ MI300X GPU 的高内存容量如何支持同时加载多个大型模型，以完成涉及推理、编辑和训练的复杂工作流。
...
**注意**：请按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 进行安装和验证。
...
为确保与AMD ROCm（ROCm（Radeon开放计算平台））完全兼容，请直接从源代码安装[DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)。

**注意：** 安装后，请手动更新系统路径，以确保 notebook 可以立即导入库，无需重启内核。
...
# AMD ROCm（ROCm（Radeon 开放计算平台））6.4 wheels 索引（优先）
...
使用简单的prompt生成你的第一张图像: *“a portrait of a beautiful Asian woman”*
...
## 第 4 步：高级图像编辑[#](#step-4-advanced-image-editing)

本节介绍一些用于生成更复杂图像的高级技术。

### 加载编辑流水线[#](#load-the-editing-pipeline)

Qwen-Image系列包含针对不同任务的专用模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专门为图像编辑和修复（in-painting）任务设计的模型。
...
你可以通过使用刚刚生成的肖像，将其扩展为带有森林背景的全景图来执行外绘任务。

```
prompt = "一位身穿长裙的美丽女性的写实摄影。背景是森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以创造出不自然的头身比，将人物变成矮个子大头娃娃。生成刺眼、耀眼的阳光，并使整个场景颜色过饱和。将腿部扭曲成X形或O形畸形。"
...
这张照片中的人脸看起来不一致。加载专门用于根据面部参考生成一致性图像的LoRA模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)。
...
negative_prompt = "让角色的手指残缺扭曲，放大头部以创造不自然的头身比，将人物变成矮个子大头娃娃。产生刺眼、炫目的阳光，并用过度饱和的颜色渲染整个场景。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够鲁棒，能够理解它未经明确训练的语言中的提示。要尝试这个，请使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男士，身穿深灰色修身西装，目光平静而含笑，流露出自信与从容。他坐在桌前，手中捧着一束红玫瑰。"
...
尽管Qwen-Image并未针对韩语文本进行训练，但其文本编码器的基础能力仍提供了多语言理解功能。

### 使用Qwen-Image-Edit-2509合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

用户现在拥有两张图片：森林中的女人和手持鲜花的男人。使用支持多图像编辑功能的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两张独立的图像合并成一个统一的场景，其中角色彼此互动。
...
你当前已同时将三个巨型模型加载到内存中。计算总参数量以了解此工作负载的规模。
...
在标准GPU上处理这种情况是不可能的。然而，AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 拥有 192 GB 的 VRAM，因此可以将所有这些模型常驻内存，从而实现推理、编辑和训练任务之间的无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，教给模型一个特定的概念，在本例中，是一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个包含五张狗的图像和相关元数据的小型数据集。
...
首先，释放一些GPU内存以便为训练过程腾出空间。然后下载官方训练脚本并使用 `accelerate` 启动它。
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功使用具有 86B 集体参数的模型进行了推理，以高一致性编辑了图像，并在单个 GPU 上训练了自定义适配器。