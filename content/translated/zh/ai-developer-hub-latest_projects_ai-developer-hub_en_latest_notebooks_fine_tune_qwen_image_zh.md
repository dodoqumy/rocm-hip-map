...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有 860 亿参数的庞大模型集合——的强大功能，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地对其进行微调。教程展示了 AMD Instinct™（AMD 数据中心 GPU 系列）MI300X GPU 的高内存容量如何使同时加载多个大型模型成为可能，从而支持涉及推理、编辑和训练在内的复杂工作流。
...
**操作系统**: Linux（推荐 Ubuntu 22.04）。请参阅[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)了解支持的操作系统。**硬件**: AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**: ROCm（ROCm（Radeon 开放计算平台）） 6.0 或更高版本，Docker 以及 Python 3.10 或更高版本

**注意**：按照 [ROCm（Radeon 开放计算平台）安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。在开始之前，请确认您的 GPU 已被正确检测并可供使用。
...
**注意：** 安装后，请手动更新系统路径，以确保 notebook 能立即导入库而无需重启内核。
...
# AMD ROCm（ROCm（Radeon Open Compute平台））6.4 wheel包索引（优先）
...
## 第2步：基础模型推理[#](#step-2-basic-model-inference)
...
为了提升图像质量，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 可显著增强生成图像的视觉保真度和艺术细节。
...
### 加载编辑 pipeline[#](#load-the-editing-pipeline)

Qwen-Image系列包含针对不同任务的专用模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专门为图像编辑和修补任务设计的模型。
...
你可以通过将刚刚生成的肖像扩展为带有森林背景的长镜头图像来执行外绘任务。

```
prompt = "一位穿着长裙的美丽女性的逼真摄影作品。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以制造不自然的头身比例，将人物变成矮小的大头娃娃。生成刺眼、眩目的阳光，并以过度饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
这张照片中的面部看起来不一致。加载专用的LoRA模型[DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，它可以根据面部参考生成一致的图像。
...
prompt = "一张写实摄影，一位穿着长裙的美丽女子。背景是森林。"
negative_prompt = "让角色的手指残缺扭曲，放大头部造成不自然的头身比例，将人物变成矮小的大头娃娃。产生刺眼、耀眼的阳光，用过度饱和的颜色渲染整个场景。将腿部扭曲成X型或O型畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够强大，能够理解它未经专门训练的语言的提示。为了尝试这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身穿深灰色修身西装，眼神平静含笑，流露出自信与从容。他坐在桌旁，手中捧着一束红色鲜花。"
...
尽管 Qwen-Image 没有使用韩语文本进行训练，但其文本编码器的基础能力仍然提供了多语言理解能力。

### 使用 Qwen-Image-Edit-2509 合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

您现在有两张图片：森林中的女人和带花的男人。使用支持多图编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两张独立的图片合并为一个连贯的场景，其中角色正在互动。
...
## 第6步：Instinct（AMD 数据中心 GPU 系列）MI300X 的力量[#](#step-6-the-power-of-the-instinct-mi300x)

当前您已将三个大规模模型同时加载到内存中。计算总参数数量以理解此工作负载的规模。
...
在标准GPU上处理这是不可能的。但AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU拥有192 GB的VRAM，因此可以将所有这些模型保留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，我们终于要从推理转向训练了。训练一个自定义的LoRA适配器，让模型学会一个特定的概念——这里是一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载包含五张狗狗图片及其相关元数据的小型数据集。
...
首先清理一些GPU内存，为训练过程腾出空间。然后下载官方训练脚本并使用 `accelerate` 启动它。
...
本教程演示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X 的端到端能力。你成功地在单个 GPU 上使用具有 86B 总参数的模型进行了推理，以高一致性编辑图像，并训练了一个自定义适配器。