...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有 860 亿参数的庞大模型集合——的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地进行微调。教程演示了 AMD Instinct™ MI300X GPU 的高内存容量如何支持同时加载多个大型模型，从而实现涉及推理、编辑和训练的复杂工作流程。
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 专为生成式 AI 工作负载提供峰值性能而设计。开始前，请确认您的 GPU 已正确识别并可供使用。
...
**注意：** 安装后，请手动更新系统路径，以确保 notebook 可以立即导入库而无需重启内核。
...
# AMD ROCm（ROCm（Radeon 开放计算平台）） 6.4 版本的索引（优先）
...
Qwen-Image系列包含针对不同任务的专门模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和修补任务设计的模型。
...
你可以通过使用刚刚生成的肖像，将其扩展成带有森林背景的远景图像，来执行外补绘（outpainting）任务。

```
prompt = "一位身着长裙的美女的逼真摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺并扭曲，放大头部以形成不自然的头身比，将人物变成矮小的大头娃娃。生成刺眼、眩目的阳光，并以过饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
negative_prompt = "让角色的手指残缺扭曲，放大头部造成不自然的头身比例，将人物变成矮小的大头娃娃。产生刺眼刺目的阳光，用过于饱和的色彩渲染整个场景。将双腿扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够鲁棒，能够理解它没有明确训练过的语言的提示。为了尝试这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身穿深灰色修身西装，眼神平静微笑，散发着自信和沉稳。他坐在桌前，手中捧着一束红色鲜花。"
...
尽管Qwen-Image没有在韩语文本上训练过，但其文本编码器的基础能力仍然提供了多语言理解能力。

### 使用 Qwen-Image-Edit-2509 融合主体[#](#merging-subjects-with-qwen-image-edit-2509)

你现在有两张图：林中的女人和持花的男人。使用支持多图编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两张独立的图像合并成一个连贯的场景，让其中的人物产生互动。

```python
# 从预训练模型加载 QwenImageEdit2509 管道
qwen_image_edit_2509 = QwenImagePipeline.from_pretrained(
    torch_dtype=torch.bfloat16,  # 使用 bfloat16 精度
    device="cuda",               # 指定 CUDA 设备
    model_configs=[
...
## 步骤6：Instinct 的力量（Instinct（AMD 数据中心 GPU 系列）） MI300X[#](#step-6-the-power-of-the-instinct-mi300x)

你当前同时将三个庞大模型加载到了内存中。计算总参数量以了解此工作负载的规模。
...
在标准GPU上处理这是不可能的。然而，AMD Instinct（AMD 数据中心 GPU 系列） MI300X GPU 拥有 192 GB 的显存，因此可以将其所有模型驻留在内存中，从而在推理、编辑和训练任务之间实现无缝切换。
...
最后，是从推理转向训练的时候了。训练一个自定义的 LoRA 适配器，让模型学习一个特定的概念，在这个例子中，是一只特定的狗。
...
首先，清空部分GPU内存以为训练过程腾出空间。然后下载官方训练脚本并使用 `accelerate` 启动它。
...
本教程展示了 AMD Instinct（AMD 数据中心 GPU 系列）MI300X 的端到端能力。您成功地在单块 GPU 上使用总参数量达 860 亿的模型进行推理、以高一致性编辑图像，并训练了自定义适配器。