...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个拥有 860 亿参数的大规模模型集合）的能力，并解释了如何在 AMD 硬件上使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 高效地进行微调。它展示了 AMD Instinct™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以支持涉及推理、编辑和训练的复杂工作流。
...
**操作系统**: Linux（推荐 Ubuntu 22.04）。支持的操作系统请参阅[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。  
**硬件**: AMD Instinct（AMD 数据中心 GPU 系列） MI300X GPU  
**软件**: ROCm（Radeon 开放计算平台）6.0 或更高版本、Docker 和 Python 3.10 或更高版本

**注意**：请按照 [ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 旨在为生成式 AI 工作负载提供卓越性能。在开始之前，请确认您的 GPU 已被正确检测并可供使用。
...
**注意：** 安装完成后，请手动更新系统路径，以确保笔记本能立即导入库而无需重启内核。
...
# 6. 强制当前 notebook 能够看到已安装的包
sys.path.append(os.getcwd())
print(f"已将 {os.getcwd()} 添加到系统路径以实现立即导入。")
# 7. 返回根目录
os.chdir("..")
```
...
生成您的第一张图像，使用简单的提示词：*“a portrait of a beautiful Asian woman”*
...
Qwen-Image系列中包含针对不同任务的专用模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和修复任务设计的模型。
...
您可以通过获取刚刚生成的肖像并将其扩展为具有森林背景的远景图像来执行外绘任务。

```
prompt = "一位身穿长裙的美丽女人的真实摄影。背景是森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比例，将人物变成身材矮小的大头娃娃。生成刺眼耀眼的阳光，并用过饱和的颜色渲染整个场景。将腿部扭曲成X型或O型畸形。"
...
negative_prompt = "使角色的手指残缺扭曲，放大头部以造成不自然的头身比例，将人物变成短身材的大头娃娃。生成刺眼、强烈的阳光，并用过度饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够稳健，能够理解它未经过明确训练的提示语言。要尝试这一点，请使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身着深灰色修身西装，眼神平静含笑，流露出自信与从容。他坐在桌旁，手中捧着一束红玫瑰。"
...
虽然Qwen-Image未经过韩语文本训练，但其文本编码器的基础能力仍提供了多语言理解能力。

### 使用 Qwen-Image-Edit-2509 合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

您现在拥有两张图片：森林中的女子和手持鲜花的男子。使用支持多图编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两张独立的图片合并成一个连贯的场景，让其中的人物产生互动。
...
目前您已同时将三个大型模型加载到内存中。计算总参数数量以了解此工作负载的规模。
...
在标准 GPU 上处理这是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 的 VRAM，因此它可以保持所有这些模型驻留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最终，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学会一个特定的概念，在这里是一只特定的狗。
...
现在训练已完成，请重新加载模型，注入新训练的`lora_dog`
...
本教程展示了AMD Instinct（AMD 数据中心 GPU 系列）MI300X的端到端能力。您成功地在单个GPU上使用总参数量为86B的模型执行了推理、编辑了具有高度一致性的图像，并训练了自定义适配器。