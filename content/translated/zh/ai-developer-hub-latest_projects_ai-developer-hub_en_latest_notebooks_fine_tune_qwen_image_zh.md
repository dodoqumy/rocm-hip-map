...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有 860 亿参数的大规模模型集合——的强大功能，并解释了如何利用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上进行高效微调。它展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU 的高内存容量如何支持同时加载多个大型模型，从而完成涉及推理、编辑和训练的复杂工作流。

**注意**：本教程由 ModelScope 和 [通义实验室](https://www.linkedin.com/company/alibaba-tongyi-lab/)(阿里巴巴集团) 开发。

## 关键组件[#](#key-components)

**硬件**: AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**:[DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)和[ROCm（ROCm（Radeon 开放计算平台））](https://rocm.docs.amd.com/en/latest/index.html)**模型**:[Qwen-Image](https://qwen-image.org/),[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit),和自定义LoRA适配器

## 先决条件[#](#prerequisites)

开始之前，请确保您的环境满足以下要求：

**操作系统**：Linux（推荐 Ubuntu 22.04）。有关受支持的操作系统，请参阅[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。**硬件**：AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**：ROCm（ROCm（Radeon 开放计算平台）） 6.0 或更高版本、Docker 和 Python 3.10 或更高版本

**Note**: Install and verify ROCm（ROCm（Radeon 开放计算平台）） by following the [ROCm（ROCm（Radeon 开放计算平台）） install guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html).
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 专为生成式 AI 工作负载提供峰值性能而设计。开始前，请确认你的 GPU 已被正确识别并可供使用。
...
**注意：** 安装完成后，请手动更新系统路径，以确保 notebook 可以立即导入该库，无需重启内核。
...
# 4. 创建AMD专用的requirements文件
requirements_content = """
# 用于 AMD ROCm（ROCm（Radeon 开放计算平台）） 6.4 的索引（优先）
...
# 6. 强制当前notebook感知已安装的包
sys.path.append(os.getcwd())
print(f"已将 {os.getcwd()} 添加到系统路径，从而实现立即导入。")
# 7. 返回根目录
os.chdir("..")
```
...
## 第二步：基础模型推理[#](#step-2-basic-model-inference)
...
生成你的第一张图像，使用简单的 prompt：*“a portrait of a beautiful Asian woman”*
...
为了改进图像，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 可显著提升生成图像的视觉保真度和艺术细节。
...
## 第四步：高级图像编辑[#](#step-4-advanced-image-editing)

本节介绍一些用于生成更复杂图像的高级技巧。

### 加载编辑管线[#](#load-the-editing-pipeline)

Qwen-Image系列包含针对不同任务的专门模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和in-painting任务设计的模型。
...
你可以通过将刚刚生成的肖像扩展为一幅带有森林背景的远景图像来执行外扩任务。

```
prompt = "一位身穿长裙的美女的写实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以形成不自然的头身比例，将人物变成矮个子大头娃娃。生成刺眼、耀眼的阳光，并用过度饱和的色彩渲染整个场景。将双腿扭曲成X型或O型畸形。"
...
这张照片中的人脸看起来不一致。加载专门的LoRA模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型可以基于面部参考生成一致的图像。
...
negative_prompt = "使角色的手指残缺畸形，放大头部以产生不自然的头身比，将人物变成矮个子大头娃娃。生成刺眼、眩目的阳光，并用过度饱和的色彩渲染整个场景。将腿部扭曲成X型或O型畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
## 步骤5：多语言与多图像编辑[#](#step-5-multilingual-and-multi-image-editing)

Qwen-Image 文本编码器足够强大，能够理解它未经明确训练的语言中的提示。为了尝试这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身穿深灰色修身西装，眼神平静而含笑，流露出自信与沉着。他坐在桌旁，手中捧着一束红玫瑰。"
...
尽管Qwen-Image没有在韩语文本上进行训练，但其文本编码器的基础能力仍然提供了多语言理解能力。

### 合并主题与 Qwen-Image-Edit-2509[#](#merging-subjects-with-qwen-image-edit-2509)

您现在拥有两张图片：森林中的女人和拿着花的男人。使用支持多图编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两张独立的图片合并成一个连贯的场景，使其中的人物产生互动。
...
你目前同时将三个巨大的模型加载到了内存中。计算总参数量以了解此工作负载的规模。
...
在标准 GPU 上处理这一点是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 的显存，因此它可以将所有这些模型驻留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的 LoRA 适配器，让模型学会一个特定概念——在这里，是一只特定的狗。

### Prepare the dataset[#](#prepare-the-dataset)

下载一个包含五张狗的图像及相关元数据的小数据集。
...
在训练前，验证基础模型对提示词 `"a dog"` 的输出。

. 正如预期的那样，它生成的是一只普通的狗，而不是您特定的主体。
...
首先，释放一些GPU显存，为训练过程腾出空间。然后下载官方训练脚本，并使用 `accelerate` 启动它。
...
本教程演示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X 的端到端能力。您成功地在单个 GPU 上使用总参数为 86B 的模型执行推理，以高度一致性编辑图像，并训练了一个自定义适配器。