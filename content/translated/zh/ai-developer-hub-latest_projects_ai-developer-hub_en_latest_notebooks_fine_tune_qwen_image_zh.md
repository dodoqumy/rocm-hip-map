...
本教程探索了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有860亿参数的大型模型集合——的强大能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地对其进行微调。它展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU 的高内存容量如何使得能够同时加载多个大型模型，以处理涉及推理、编辑和训练的复杂工作流。

**注**：本教程由 ModelScope 和 [Tongyi Lab](https://www.linkedin.com/company/alibaba-tongyi-lab/)(阿里巴巴集团) 开发。
...
**操作系统**：Linux（推荐使用 Ubuntu 22.04）。有关支持的操作系统，请参阅[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。**硬件**：AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**：ROCm（ROCm（Radeon 开放计算平台）） 6.0 或更高版本、Docker 和 Python 3.10 或更高版本

**注意**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 专为生成式 AI 工作负载提供峰值性能而设计。开始之前，请验证您的 GPU 已被正确检测并已准备就绪。
...
为了确保与 AMD ROCm（ROCm（Radeon 开放计算平台））完全兼容，请直接从源码安装 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)。
...
# 4. 创建AMD专用requirements文件
requirements_content = """
# AMD ROCm 6.4 wheels 的索引（优先）
...
您可以通过将刚刚生成的肖像扩展为带有森林背景的长镜头图像来执行外绘任务。

```
prompt = "一位穿着长裙的美丽女性的逼真摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺畸形，放大头部以创造不自然的头身比例，将人物变成矮个子大头玩偶。生成强烈刺眼的阳光，并用过度饱和的颜色渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
这张照片中的人脸看起来不一致。加载专门的LoRA模型[DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型能够基于人脸参考生成一致的图像。
...
prompt = "一位穿着长裙的美丽女性的真实摄影照片，背景是森林。"
negative_prompt = "让角色的手指残缺扭曲，放大头部，造成不自然的头身比，将人物变成矮胖大头娃娃。产生刺眼、眩目的阳光，使整个场景颜色过度饱和。将腿部扭曲成X型或O型畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够鲁棒，能够理解其未经过专门训练的语言中的提示。要尝试这一点，请使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男士，身穿深灰色修身西装，眼神平静含笑，透露出自信和从容。他坐在桌前，手中捧着一束红色鲜花。"
...
```
qwen_image.clear_lora()
prompt = "英俊的亚洲男性，身穿深灰色修身西装，眼神沉稳而含笑，散发出自信从容的气质。他坐在桌前，手里拿着一束红色花束。"
image = qwen_image(prompt, seed=2, num_inference_steps=40)
...
尽管Qwen-Image未经韩语文本训练，但其文本编码器的基础能力仍能支持多语言理解。

### 使用 Qwen-Image-Edit-2509 合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

您现在拥有两幅图像：森林中的女人和持花的男人。使用支持多图编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两幅独立图像合并为一个场景，使其中的人物产生互动。
...
## 第6步：Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的强大功能[#](#step-6-the-power-of-the-instinct-mi300x)
...
在标准 GPU 上处理这个任务是做不到的。然而，AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 拥有 192 GB 的 VRAM，因此可以将所有这些模型常驻在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学会一个特定的概念，本例中是一只特定的狗。
...
本教程展示了 AMD Instinct（AMD 数据中心 GPU 系列） MI300X 的端到端能力。您已成功在单块 GPU 上使用总参数量为 86B 的模型进行推理、以高度一致性编辑图像，并训练自定义适配器。