...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个拥有 860 亿参数的大规模模型集合）的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地对其进行微调。它演示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以执行涉及推理、编辑和训练的复杂工作流。
...
为了确保与 AMD ROCm（ROCm（Radeon 开放计算平台））完全兼容，请从源代码直接安装 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)。
...
# AMD ROCm（Radeon 开放计算平台）6.4 轮子的索引（优先）
...
使用简单提示生成您的第一张图像：*“a portrait of a beautiful Asian woman”*
...
### 具有一致性的外绘制[#](#outpainting-with-consistency)

你可以通过获取你刚刚生成的肖像，并将其扩展为一张具有森林背景的长镜头图像来执行outpainting任务。
...
negative_prompt = "让角色的手指残缺并扭曲，放大头部以产生不自然的头身比，将人物变成矮小大头娃娃。生成刺眼、耀眼的阳光，并用过度饱和的色彩渲染整个场景。将腿部扭曲成X型或O型畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够鲁棒，能够理解未显式训练过的语言的提示词。为尝试这一点，请使用韩语提示词生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身穿深灰色修身西装，眼神平静而含笑，流露出自信与从容。他坐在桌前，手中捧着一束红色鲜花。"
...
尽管Qwen-Image没有在韩文文本上进行训练，但其文本编码器的基础能力仍然提供了多语言理解能力。

### 将主体与Qwen-Image-Edit-2509合并[#](#merging-subjects-with-qwen-image-edit-2509)

你现在有两张图片：森林中的女人和拿着花的男人。使用[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，该模型支持多图像编辑，你可以将这两张独立的图像合并成一个连贯的场景，让其中的人物进行互动。
...
您当前已同时将三个大型模型加载到内存中。计算总参数数量以了解此工作负载的规模。
...
处理此任务时，标准GPU无法胜任。但AMD Instinct（AMD数据中心GPU系列）MI300X GPU拥有192 GB的显存，足以将所有这些模型常驻内存，从而在推理、编辑和训练任务之间实现无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的 LoRA 适配器，让模型学会一个特定概念，这里是一只特定的狗。
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功地在单个 GPU 上，使用总计 86B 参数的模型执行了推理、以高一致性编辑了图像，并训练了自定义适配器。