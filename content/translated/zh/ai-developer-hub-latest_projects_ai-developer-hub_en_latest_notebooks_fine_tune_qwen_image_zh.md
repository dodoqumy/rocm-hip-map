...
本教程探讨了[Qwen-Image](https://qwen-image.org/)系列——一个庞大的860亿参数模型集合——的功能，并解释了如何使用[DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)在AMD硬件上高效地进行微调。它展示了AMD Instinct™ MI300X GPU的高内存容量如何能够同时加载多个大型模型，以处理涉及推理、编辑和训练的复杂工作流程。
...
**模型**：[Qwen-Image](https://qwen-image.org/)、[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit) 以及 Custom LoRA adapters

## 先决条件[#](#prerequisites)

在开始之前，请确保您的环境满足以下要求：

**操作系统**：Linux（推荐 Ubuntu 22.04）。请参阅[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)以了解支持的操作系统。**硬件**：AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**：ROCm（ROCm（Radeon 开放计算平台））6.0 或更高版本、Docker 和 Python 3.10 或更高版本
...
AMD Instinct MI300X GPU 旨在为生成式 AI 工作负载提供卓越性能。开始之前，请确认您的 GPU 已被正确检测并可供使用。
...
为了确保与 AMD ROCm（ROCm（Radeon 开放计算平台））的完全兼容，请直接从源代码安装 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)。

**注意：** 安装完成后，请手动更新系统路径，以确保笔记本无需重启内核即可立即导入库。
...
# AMD ROCm（Radeon 开放计算平台）6.4 的索引 wheels（优先）
...
为了改进图像，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 可以显著提升生成图像的视觉保真度和艺术细节。
...
Qwen-Image系列包含针对不同任务的专用模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和修复（in-painting）任务设计的模型。
...
您可以通过取用刚刚生成的肖像，将其扩展为具有森林背景的全景图像，从而执行外绘任务。

```
prompt = "一个穿着长裙的美丽女人的写实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比，将人物变成矮个子大头娃娃。生成刺眼、耀眼的阳光，并以过度饱和的色彩渲染整个场景。将双腿扭曲成X形或O形畸形。"
...
这个照片中的面部看起来不一致。请加载专用的LoRA模型[DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，它可以根据面部参考生成一致的图像。
...
negative_prompt = "使角色的手指残缺扭曲，放大头部，产生不自然的头身比，将人物变成短身材大头娃娃。生成刺眼、眩目的阳光，并以过度饱和的颜色渲染整个场景。将双腿扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够鲁棒，能够理解它未经明确训练的语言中的提示。要尝试这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图片。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身着深灰色修身西装，眼神平静且带着微笑，流露出自信与沉着。他坐在餐桌旁，手中捧着一束红玫瑰。"
...
죄송합니다만, 저는 이미지 내용을 이해할 수 없습니다. 현재 제공된 이미지가 없기 때문에 모델이 이미지 내용을 이해할 수 있는지 여부를 판단할 수 없습니다.
...
尽管Qwen-Image未在韩文文本上进行训练，但其文本编码器的基本能力仍提供了多语言理解功能。

### 使用 Qwen-Image-Edit-2509 合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

你现在有两张图片：森林中的女人和拿着花的男人。使用[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，它支持多图编辑，你可以将这两张独立图片合并成一个单一连贯的场景，其中角色正在互动。
...
## 第 6 步：Instinct（AMD 数据中心 GPU 系列）MI300X 的强大能力[#](#step-6-the-power-of-the-instinct-mi300x)

当前你已将三个大型模型同时加载到内存中。计算总参数数量以了解此工作负载的规模。
...
在标准GPU上处理这个是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU拥有192 GB的显存，因此它可以将所有这些模型常驻内存，从而在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学习一个特定的概念，在此案例中，是一只特定的狗。
...
本教程演示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功地在单个 GPU 上使用具有 86B 总参数的模型进行了推理，以高度一致性编辑了图像，并训练了一个自定义适配器。