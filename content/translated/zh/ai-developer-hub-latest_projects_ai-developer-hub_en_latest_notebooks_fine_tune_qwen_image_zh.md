...
本教程探索了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有860亿参数的大规模模型集合——的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地进行微调。它展示了 AMD Instinct™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以支持涉及推理、编辑和训练的复杂工作流。

**注意**：本教程由ModelScope和[Tongyi Lab](https://www.linkedin.com/company/alibaba-tongyi-lab/)（Alibaba Group）开发。
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。开始之前，请确认您的 GPU 已被正确检测并做好使用准备。
...
**注意：** 安装后，手动更新系统路径以确保笔记本能够立即导入库，无需重启内核。
...
# AMD ROCm（ROCm（Radeon 开放计算平台））6.4 wheels 的索引（优先）
...
print(f"已将 {os.getcwd()} 添加到系统路径，以允许立即导入。")
# 7. 返回根目录
os.chdir("..")
```
...
你可以通过将刚刚生成的人像扩展为带有森林背景的远景图像来执行outpainting任务。

```
prompt = "一位穿着长裙的美丽女性的写实摄影。背景是森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比，将人物变成矮个子大头娃娃。产生刺眼、耀眼的阳光，并渲染整个场景为过度饱和的颜色。将腿扭曲成X型或O型畸形。"
...
照片中的面部看起来不一致。加载专门的LoRA模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P) ，该模型可以根据面部参考生成一致的图像。
...
negative_prompt = "使角色的手指残缺扭曲，放大头部，造成不自然的头身比例，将人物变成矮胖的大头娃娃。生成刺眼、耀眼的阳光，并用过饱和的颜色渲染整个场景。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够强大，能够理解它未经明确训练的语言提示。要尝试这一点，请使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男士，身穿深灰色修身西装，眼神平静含笑，透露出自信与沉着。他坐在桌前，手捧一束红色鲜花。"
...
```
qwen_image.clear_lora()
prompt = "一个英俊的亚洲男性，穿着深灰色修身西装，眼神沉着含笑，透出自信和从容的气质。他坐在桌前，手里拿着一束红色花束。"
image = qwen_image(prompt, seed=2, num_inference_steps=40)
...
虽然Qwen-Image未在韩语文本上进行训练，但其文本编码器的基础能力仍具备多语言理解能力。

### 合并主体与Qwen-Image-Edit-2509[#](#merging-subjects-with-qwen-image-edit-2509)

您现在有两张图片：森林中的女人和拿着花的男人。使用支持多图编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两张独立的图片合并成一个连贯的场景，其中角色在互动。
...
你目前同时将三个大型模型加载到内存中。请计算总参数量，以理解此工作负载的规模。
...
**Total Parameters**: ~860亿

在标准 GPU 上处理此任务将是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 拥有 192 GB 显存，因此它可以保持所有这些模型常驻内存，从而实现推理、编辑和训练任务之间的无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学会特定概念——在这里，是一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个小型数据集，包含五张狗的图像及相关元数据。
...
首先，清理部分GPU内存以便为训练过程腾出空间。然后下载官方训练脚本并使用 `accelerate` 启动它。
...
本教程演示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。你成功使用具有 86B 总参数的模型进行了推理，以高一致性编辑图像，并训练了一个自定义适配器，全部在单个 GPU 上完成。