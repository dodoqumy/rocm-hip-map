...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有 860 亿参数的大规模模型集合——的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效进行微调。它展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU 的高内存容量如何支持同时加载多个大型模型，以执行涉及推理、编辑和训练的复杂工作流。

**Note**: 本教程由 ModelScope 和 [通义实验室](https://www.linkedin.com/company/alibaba-tongyi-lab/)（阿里巴巴集团）开发。
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。开始之前，请确认您的 GPU 已被正确检测并可供使用。
...
**注意：** 安装后，请手动更新系统路径，以确保笔记本无需重启内核即可立即导入该库。
...
logger.setLevel(logging.CRITICAL)  # 设置日志级别为 CRITICAL
import os
os.environ["TOKENIZERS_PARALLELISM"] = "false"  # 禁用分词器并行
os.environ["MODELSCOPE_DOMAIN"] = "www.modelscope.ai"  # 设置 ModelScope 域名
...
qwen_image.enable_lora_magic()  # 启用 LoRA 魔法功能
```
...
您可以通过将刚刚生成的肖像扩展为带有森林背景的全景图像，来执行外绘（outpainting）任务。

```
prompt = "一位穿着长裙的美丽女性的逼真摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以形成不自然的头身比，将人物变成身材矮小的大头娃娃。产生刺眼、耀眼的阳光，并用过饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
这张照片中的面孔看起来不一致。加载专门设计的LoRA模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型能够根据面部参考生成一致的图像。
...
negative_prompt = "使角色的手指残缺扭曲，放大头部造成不自然的头身比，将人物变成矮个子大头娃娃。产生刺眼、眩目的阳光，整个场景用过度饱和的色彩渲染。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够鲁棒，能够理解它未经明确训练的语言的提示。为了验证这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一个英俊的亚洲男性，身穿深灰色修身西装，眼神平静、微笑，流露出自信和从容。他坐在桌旁，手里拿着一束红色鲜花。"
...
尽管Qwen-Image并未使用韩语文本进行训练，但其文本编码器的基本能力仍支持多语言理解。
...
## 第 6 步：Instinct MI300X 的力量[#](#step-6-the-power-of-the-instinct-mi300x)

你目前已经同时加载了三个巨型模型到内存中。计算总参数数量以了解此工作负载的规模。
...
在标准GPU上处理这是不可能的。然而，AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU拥有192 GB的显存，因此它可以将所有这些模型驻留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，以教会模型一个特定的概念，在本例中是一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个包含五张狗图片的小型数据集及其相关元数据。
...
首先，清理一些 GPU 内存以为训练过程腾出空间。然后下载官方训练脚本并使用 `accelerate` 启动它。
...
本教程展示了 AMD Instinct（AMD 数据中心 GPU 系列）MI300X 的端到端能力。您成功地在单个 GPU 上使用具有 86B 集体参数的模型执行推理、以高度一致性编辑图像，并训练了自定义适配器。