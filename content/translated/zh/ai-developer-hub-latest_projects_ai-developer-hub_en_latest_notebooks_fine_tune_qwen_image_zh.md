...
本教程探索了[Qwen-Image](https://qwen-image.org/)系列——一个拥有860亿参数的庞大模型集合——的能力，并解释了如何使用[DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)在AMD硬件上高效地对其进行微调。它展示了AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU的高内存容量如何能够同时加载多个大型模型，以完成涉及推理、编辑和训练等复杂工作流。

**注**：本教程由ModelScope和[Tongyi Lab](https://www.linkedin.com/company/alibaba-tongyi-lab/)(Alibaba Group)共同开发。
...
**操作系统**：Linux（推荐 Ubuntu 22.04）。参见[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)了解支持的操作系统。**硬件**：AMD Instinct MI300X GPU。**软件**：ROCm 6.0 或更高版本、Docker 和 Python 3.10 或更高版本。

**注意**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 专为生成式 AI 工作负载提供峰值性能而设计。在开始之前，请确认您的 GPU 已被正确检测并准备就绪。
...
**注意：** 安装后，请手动更新系统路径，以确保笔记本可以立即导入库而无需重启内核。
...
# AMD ROCm（Radeon 开放计算平台）6.4 版本的索引（优先）
...
[Qwen-Image](https://www.modelscope.ai/models/Qwen/Qwen-Image) 是一个大规模图像生成模型。配置流水线并将模型组件（Transformer、Text Encoder 和 VAE）加载到GPU上。

**注意**：配置环境以将ModelScope用作下载权重的域名。
...
为了改善图像，请加载[Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1)以显著提升生成图像的视觉保真度和艺术细节。
...
Qwen-Image系列包含针对不同任务的专用模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专门为图像编辑和修复任务设计的模型。
...
negative_prompt = "使角色的手指残缺扭曲，放大头部以制造不自然的头身比，将人物变成矮胖的大头娃娃。生成刺眼、眩目的阳光，并用过饱和的色彩渲染整个场景。将双腿扭曲成X型或O型畸形。"
...
这张照片中的脸部看起来不一致。请加载专门的LoRA模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型能够基于面部参考生成一致的图像。
...
prompt = "写实摄影，一位穿着长裙的美丽女性，背景是森林。"
negative_prompt = "使角色的手指残缺变形，放大头部造成不自然的头身比，将人物变成短身材大头娃娃。产生刺眼、炫目的阳光，用过度饱和的色彩渲染整个场景。将双腿扭成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image text encoder 足够稳健，能够理解未经过明确训练的语言中的提示词。要尝试这一点，请使用韩语提示词生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男子，身穿深灰色修身西装，眼神平静而带微笑，流露出自信与沉着。他坐在桌子旁，手中拿着一束红色的鲜花。"
...
尽管Qwen-Image未经过韩语文本训练，但其文本编码器的基础能力仍能提供多语言理解。

### 使用 Qwen-Image-Edit-2509 合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

你现在有两张图片：森林中的女人和拿着花的男人。使用支持多图像编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两张独立的图片合并成一个连贯的场景，让角色之间产生互动。
...
## 步骤 6：Instinct 的力量（Instinct（AMD 数据中心 GPU 系列）） MI300X[#](#step-6-the-power-of-the-instinct-mi300x)

你当前已经将三个大型模型同时加载到内存中。计算总参数量以了解此工作负载的规模。
...
在标准 GPU 上处理这一任务是不可能的。然而，AMD Instinct MI300X GPU 拥有 192 GB 的显存，因此能将所有此类模型驻留在内存中，从而在推理、编辑和训练任务之间实现无缝切换。
...
## 第 7 步：训练自定义 LoRA[#](#step-7-training-a-custom-lora)

最后，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学习一个特定的概念，在这个案例中，是一只特定的狗。
...
本教程展示了 AMD Instinct（AMD 数据中心 GPU 系列）MI300X 的端到端能力。您成功在单个 GPU 上使用总参数量为 86B 的模型执行推理、以高一致性编辑图像，并训练了一个自定义适配器。