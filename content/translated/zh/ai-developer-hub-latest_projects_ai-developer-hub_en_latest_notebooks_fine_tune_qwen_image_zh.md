...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 旨在为生成式 AI 工作负载提供峰值性能。开始之前，请确认您的 GPU 已被正确检测并准备好使用。
...
为了确保与 AMD ROCm（Radeon开放计算平台）完全兼容，请直接从源代码安装 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)。

**注意：** 安装后，手动更新系统路径，以确保笔记本能立即导入库而无需重启内核。
...
# Index for AMD ROCm（ROCm（Radeon 开放计算平台）） 6.4 wheels (Priortized)
...
print(f"已添加 {os.getcwd()} 到系统路径，以便立即导入。")
# 7. 返回根目录
os.chdir("..")
```
...
## 第 2 步：基本模型推断[#](#step-2-basic-model-inference)
...
### 一致性外扩绘画[#](#outpainting-with-consistency)

你可以通过将刚才生成的肖像扩展为带有森林背景的远景图像来执行外绘任务。

```
prompt = "一位身着长裙的美丽女性的写实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺变形，放大头部，造成不自然的头身比例，将人物变成矮小的大头娃娃。生成刺眼、炫目的阳光，并让整个场景呈现过度饱和的色彩。将腿部扭曲成X形或O形畸形。"
...
这张照片中的人脸看起来不一致。请加载专门的 LoRA 模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型能够基于面部参考生成一致的图像。
...
negative_prompt = "让角色的手指残缺扭曲，放大头部造成不自然的头身比例，使人物变成矮个子大头娃娃。生成刺眼、刺目的阳光，让整个场景颜色过饱和。将腿部扭曲成X型或O型畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够鲁棒，能够理解它未经明确训练的语言中的提示。为了尝试这一点，使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身穿深灰色修身西装，眼神平静而含笑，流露出自信与从容。他坐在桌旁，手中捧着一束红色鲜花。"
...
尽管Qwen-Image没有接受过韩语文本训练，但其文本编码器的基础能力仍然提供了多语言理解。

### 使用 Qwen-Image-Edit-2509 合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

您现在有两张图像：森林中的女人和持花的男人。使用支持多图像编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，可以将这两张独立的图像合并成一个连贯的场景，让角色相互互动。
...
你当前已同时加载了三个大型模型到内存中。计算总参数量以了解此工作负载的规模。
...
在标准GPU上根本无法处理这种情况。然而，AMD Instinct™ MI300X GPU拥有192 GB的显存（VRAM），因此它可以常驻所有模型于内存中，在推理、编辑和训练任务之间无缝切换。
...
## 步骤7：训练自定义 LoRA[#](#step-7-training-a-custom-lora)

最后，从推理转向训练的时刻到了。训练一个自定义 LoRA 适配器，让模型学会一个特定概念——在本例中，是一只特定的狗。
...
首先，清理部分GPU内存以便为训练进程腾出空间。然后下载官方训练脚本并使用`accelerate`启动它。
...
## 步骤8: 使用自定义LoRA推理[#](#step-8-inference-with-the-custom-lora)
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功地在单个 GPU 上使用总参数量为 86B 的模型进行了推理、以高一致性编辑了图像，并训练了自定义适配器。