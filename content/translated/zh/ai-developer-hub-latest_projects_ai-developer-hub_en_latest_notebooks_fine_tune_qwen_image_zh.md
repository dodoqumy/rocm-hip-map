...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列——一个拥有 860 亿参数的大规模模型集合——的能力，并讲解了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效微调该模型。它展示了 AMD Instinct™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以处理涉及推理、编辑和训练的复杂工作流。
...
**注意：** 安装完成后，请手动更新系统路径，以确保笔记本能够立即导入库而无需重启内核。
...
# 优先使用 AMD ROCm（Radeon 开放计算平台）6.4 的 PyTorch 索引
...
print(f"已将 {os.getcwd()} 添加到系统路径，从而支持立即导入。")
# 7. 返回根目录
os.chdir("..")
```
...
使用简单的提示词生成您的第一张图像：*“a portrait of a beautiful Asian woman”*
...
Qwen-Image 系列包含针对不同任务的专用模型。接下来，加载 [Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一款专为图像编辑和修复（in-painting）任务设计的模型。
...
你可以通过将刚刚生成的肖像进行外绘，将其扩展为一张带有森林背景的远景图像。
...
prompt = "美丽女性穿着长裙的真实摄影，背景为森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比，将人物变成短身高头大娃娃。生成刺眼、强烈的阳光，并用过度饱和的颜色渲染整个场景。将腿部扭成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image text encoder 足够稳健，能够理解未经显式训练的语言中的提示词。为了尝试这一点，使用韩语提示词生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男性，身穿深灰色修身西装，眼神平静而含笑，透露出自信与沉着。他坐在桌前，手中捧着一束红色鲜花。"
...
虽然Qwen-Image未在韩语文本上进行训练，但其文本编码器的基础能力仍能提供多语言理解。

### 使用Qwen-Image-Edit-2509合并主体[#](#merging-subjects-with-qwen-image-edit-2509)

现在您有两张图片：森林中的女人和拿着花的男人。使用支持多图编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，您可以将这两个独立的图像合并成一个连贯的场景，其中人物之间产生互动。
...
## 步骤6：Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X 的强大功能[#](#step-6-the-power-of-the-instinct-mi300x)
...
在标准 GPU 上处理这是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 拥有 192 GB 的 VRAM，因此可以将所有这些模型保留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。您成功地在单个 GPU 上使用总参数量为86B的模型进行推理、高度一致地编辑图像，并训练了一个自定义适配器。