...
本教程探讨了[Qwen-Image](https://qwen-image.org/)系列——一个拥有860亿参数的庞大模型集合——的能力，并解释了如何在AMD硬件上使用[DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)高效地对其进行微调。它展示了AMD Instinct™ MI300X GPU的高内存容量如何使得能够同时加载多个大型模型，以执行涉及推理、编辑和训练的复杂工作流。
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 专为生成式 AI 工作负载提供峰值性能而设计。在开始之前，请确认你的 GPU 已被正确检测并可供使用。
...
**注意：** 安装后，请手动更新系统路径，以确保 notebook 能够立即导入库而无需重启内核。
...
# AMD ROCm（ROCm（Radeon 开放计算平台））6.4 版本的 wheel 包索引（优先）
...
print(f"已将 {os.getcwd()} 添加至系统路径，以便立即导入。")
# 7. 返回根目录
os.chdir("..")
```
...
为改善图像，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 可显著提升生成图像的视觉逼真度和艺术细节。
...
### 一致性外延绘制[#](#outpainting-with-consistency)

您可以通过将刚刚生成的人像扩展为带有森林背景的远景图像来执行 outpainting 任务。

```
prompt = "一位身着长裙的美丽女性的写实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺不全且扭曲，放大头部造成不自然的头身比，将人物变成矮个子大头娃娃。生成刺眼、耀眼的阳光，并用过饱和的色彩渲染整个场景。将腿部扭曲成X型或O型畸形。"
...
prompt = "一位穿着长裙的美女的真实摄影。背景是森林。"
negative_prompt = "让角色的手指残缺且扭曲，放大头部造成不自然的头身比例，将人物变成矮个子的大头娃娃。产生刺眼、炫目的阳光，用过度饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器的鲁棒性足以理解其未经显式训练的语言的提示词。要尝试这一点，请使用韩语提示词生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位英俊的亚洲男士，身穿深灰色修身西装，目光平静含笑，透露出自信与沉着。他坐在桌前，手里拿着一束红色的花。"
...
你现在有两张图片：森林中的女人和持花的男人。使用支持多图编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两张独立的图片合并成一个连贯的场景，其中两个角色在互动。
...
您目前同时在内存中加载了三个大型模型。计算总参数量以了解此工作负载的规模。
...
在标准GPU上处理这一任务是无法实现的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 的 VRAM，因此能够将所有模型保留在内存中，从而在推理、编辑和训练任务之间实现无缝切换。
...
首先，释放一些 GPU 内存，为训练过程腾出空间。然后下载官方训练脚本，并使用 `accelerate` 启动它。
...
现在训练已完成，再次加载模型，并注入新训练的 `lora_dog`
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X 的端到端能力。您成功地在单个 GPU 上使用总参数量为 86B 的模型进行了推理，以高一致性编辑了图像，并训练了一个自定义适配器。