...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个包含 860 亿参数的大规模模型集合）的能力，并解释了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地进行微调。它展示了 AMD Instinct™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以完成涉及推理、编辑和训练的复杂工作流。
...
**操作系统**: Linux（推荐 Ubuntu 22.04）。支持的完整操作系统列表请参见[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。  
**硬件**: AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU  
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X GPU 旨在为生成式 AI 工作负载提供极致性能。开始之前，请确认您的 GPU 已被正确检测并准备就绪。
...
为了确保与AMD ROCm（ROCm（Radeon 开放计算平台））完全兼容，请直接从源代码安装[DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio)。

**注意：** 安装后，请手动更新系统路径，以确保笔记本能够立即导入库而无需重启内核。
...
# AMD ROCm（Radeon开放计算平台）6.4 wheels的索引（优先）
...
使用简单的提示词生成您的第一张图像：*“一位美丽的亚洲女性的肖像”*
...
为了改善图像，加载 [Qwen-Image-LoRA-ArtAug-v1](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-LoRA-ArtAug-v1) 以显著增强生成图像的视觉保真度和艺术细节。
...
Qwen-Image系列包含针对不同任务的专门模型。接下来，加载[Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专为图像编辑和填充（in-painting）任务设计的模型。
...
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比，将人物变成矮个子大头娃娃。生成刺眼、炫目的阳光，并用过度饱和的颜色渲染整个场景。将腿部扭曲成X形或O形畸形。"
...
negative_prompt = "让角色的手指残缺扭曲，放大头部造成不自然的头身比，将人物变成短身材大头娃娃。生成刺眼、耀眼的阳光，并用过饱和的色彩渲染整个场景。将双腿扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够健壮，能够理解它没有明确训练过的语言的提示。要尝试这一点，请使用韩语提示生成一个角色。首先，使用英语生成一个图像。

```
qwen_image.clear_lora()
prompt = "一个英俊的亚洲男人，穿着深灰色修身西装，眼神平静含笑，散发着自信和沉着。他坐在桌前，手里拿着一束红色的花。"
...
尽管Qwen-Image并未使用韩语文本进行训练，但其文本编码器的基础能力仍然提供了多语言理解。

### 使用 Qwen-Image-Edit-2509 合并主题[#](#merging-subjects-with-qwen-image-edit-2509)

你手头有两张图片：森林中的女人和拿着花的男人。使用支持多图编辑的[Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两张独立的图片合并成一个连贯的场景，其中人物在互动。
...
## 步骤 6：Instinct MI300X 的强大之处[#](#step-6-the-power-of-the-instinct-mi300x)

你当前同时将三个巨型模型加载到内存中。计算总参数数量以了解此工作负载的规模。
...
在标准 GPU 上处理这将是不可行的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 的 VRAM，因此可以将所有这些模型常驻在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义LoRA适配器，让模型学习一个特定概念，在本例中，是一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个包含五张狗图片及其相关元数据的小数据集。
...
本教程展示了AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X的端到端能力。您成功地在单个GPU上使用具有860亿集体参数的模型进行了推理，以高一致性编辑了图像，并训练了自定义适配器。