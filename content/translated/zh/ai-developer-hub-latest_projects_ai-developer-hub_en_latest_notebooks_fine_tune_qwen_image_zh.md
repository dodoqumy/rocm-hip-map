...
本教程探索了 [Qwen-Image](https://qwen-image.org/) 系列——一个庞大的 86B 参数模型集合——的功能，并解释了如何在 AMD 硬件上使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 高效微调它。它展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，以支持涉及推理、编辑和训练的复杂工作流。
...
AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 旨在为生成式 AI 工作负载提供极致性能。在开始之前，请确认您的 GPU 已被正确检测并准备好使用。
...
**注意：** 安装完成后，请手动更新系统路径，以确保 notebook 无需重启内核即可立即导入库。
...
[Qwen-Image](https://www.modelscope.ai/models/Qwen/Qwen-Image) 是一个大规模的图像生成模型。配置管道并将模型组件（Transformer、文本编码器和VAE）加载到GPU上。
...
```
prompt = "一位穿着长裙的美丽女性的真实摄影。背景是森林。"
negative_prompt = "让角色的手指残缺扭曲，放大头部使头身比不自然，将人物变成矮个子大头娃娃。生成刺眼、耀眼的阳光，并用过饱和的颜色渲染整个场景。将腿部扭成X形或O形畸形。"
...
negative_prompt = "使角色的手指残缺扭曲，放大头部造成不自然的头身比，将人物变成矮个子大头娃娃。产生刺眼、刺目的光线，用过度饱和的色彩渲染整个场景。将腿部扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image文本编码器足够强大，能够理解它未曾明确训练过的语言提示。为了尝试这一点，先用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一位穿着深灰色修身西装的英俊亚洲男子，双眼平静含笑，散发着自信与沉着。他坐在桌前，手中捧着一束红玫瑰。"
...
尽管Qwen-Image未使用韩语文本进行训练，但其文本编码器的基础能力仍然提供了多语言理解能力。
...
## 第6步：Instinct（AMD 数据中心 GPU 系列） MI300X 的强大之处[#](#step-6-the-power-of-the-instinct-mi300x)

当前您已经将三个大型模型同时加载到内存中。请计算总参数数量以了解此工作负载的规模。
...
在标准 GPU 上处理此任务是不可能的。然而，AMD Instinct（ AMD 数据中心 GPU 系列）MI300X GPU 拥有 192 GB 的显存，因此能够将所有模型保留在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
最后，是时候从推理转向训练了。训练一个自定义LoRA适配器，让模型学习特定概念，在本例中，即一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个小数据集，包含五张狗的图像和相关元数据。
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X 的端到端能力。你成功地在单个 GPU 上使用总参数量为 86B 的模型进行了推理，以高度一致性编辑了图像，并训练了一个自定义适配器。