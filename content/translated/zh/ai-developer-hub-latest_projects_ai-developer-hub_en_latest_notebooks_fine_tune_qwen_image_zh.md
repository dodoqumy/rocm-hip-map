...
本教程探讨了 [Qwen-Image](https://qwen-image.org/) 系列（一个拥有 860 亿参数的庞大模型集合）的能力，并阐述了如何使用 [DiffSynth-Studio](https://github.com/modelscope/DiffSynth-Studio) 在 AMD 硬件上高效地进行微调。它展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ MI300X GPU 的高内存容量如何能够同时加载多个大型模型，从而支持涉及推理、编辑和训练等复杂工作流。
...
**操作系统**：Linux（推荐 Ubuntu 22.04）。请参阅[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)了解支持的操作系统。**硬件**：AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**软件**：ROCm（ROCm（Radeon 开放计算平台）） 6.0 或更高版本、Docker 和 Python 3.10 或更高版本
...
AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 旨在为生成式 AI 工作负载提供卓越性能。在开始之前，请确认您的 GPU 已被正确识别并可供使用。
...
**注意：** 安装后，手动更新系统路径，以确保 notebook 可以立即导入库而无需重新启动内核。
...
[Qwen-Image](https://www.modelscope.ai/models/Qwen/Qwen-Image)是一个大规模图像生成模型。配置pipeline并将模型组件（Transformer、Text Encoder和VAE）加载到GPU上。
...
Qwen-Image 系列包含针对不同任务的专业化模型。接下来，加载 [Qwen-Image-Edit](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit)，这是一个专门用于图像编辑和补画任务的模型。
...
### 一致性外推填充[#](#outpainting-with-consistency)

你可以通过取刚才生成的肖像，并将其扩展为具有森林背景的远景图像，来执行外绘任务。

```
prompt = "一位穿着长裙的美丽女性的真实摄影。背景是一片森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以产生不自然的头身比，将人物变成矮小的大头娃娃。产生刺眼、眩目的阳光，并用过度饱和的色彩渲染整个场景。将双腿扭曲成X形或O形畸形。"
...
这张照片中的人脸看起来不一致。加载专门的LoRA模型 [DiffSynth-Studio/Qwen-Image-Edit-F2P](https://www.modelscope.ai/models/DiffSynth-Studio/Qwen-Image-Edit-F2P)，该模型能够根据面部参考生成一致的图像。
...
prompt = "一位身着长裙的美丽女性的写实摄影。背景是森林。"
negative_prompt = "使角色的手指残缺扭曲，放大头部以形成不自然的头身比，将人物变为矮小大头娃娃。产生刺眼、眩目的阳光，并用过度饱和的颜色渲染整个场景。将双腿扭曲成X形或O形畸形。"
image = qwen_image_edit(prompt, negative_prompt=negative_prompt, edit_image=Image.open("image_face.jpg"), seed=1, num_inference_steps=40)
...
Qwen-Image 文本编码器足够鲁棒，能够理解它未经过明确训练的语言中的提示。为了尝试这个，使用韩语提示生成一个角色。首先，使用英语生成一张图像。

```
qwen_image.clear_lora()
prompt = "一个英俊的亚洲男士，穿着深灰色修身西装，眼神平静含笑，透露出自信和沉着。他坐在桌子旁，手中拿着一束红玫瑰。"
...
尽管Qwen-Image并未针对韩语文本进行训练，但其文本编码器的基础能力仍具备多语言理解功能。

### 合并主体与 Qwen-Image-Edit-2509[#](#merging-subjects-with-qwen-image-edit-2509)

你现在有两个图像：森林中的女人和拿着花的男人。使用支持多图像编辑的 [Qwen-Image-Edit-2509](https://www.modelscope.cn/models/Qwen/Qwen-Image-Edit-2509)，你可以将这两个独立的图像合并成一个连贯的场景，使得角色之间产生互动。
...
你当前已将三个庞大的模型同时加载到内存中。计算总参数量以了解此工作负载的规模。
...
在标准 GPU 上处理这个是不可能的。然而，AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 拥有 192 GB 的显存，因此它可以将所有这些模型常驻在内存中，以便在推理、编辑和训练任务之间无缝切换。
...
终于，是时候从推理转向训练了。训练一个自定义的LoRA适配器，让模型学习一个特定概念，在本例中是一只特定的狗。

### 准备数据集[#](#prepare-the-dataset)

下载一个小型数据集，其中包含五张狗的图片以及相关的元数据。
...
## 第 8 步：使用自定义 LoRA 进行推理[#](#step-8-inference-with-the-custom-lora)
...
本教程展示了 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））MI300X 的端到端能力。您成功在单个 GPU 上使用总参数量为 86B 的模型进行推理、以高一致性编辑图像，并训练了自定义适配器。