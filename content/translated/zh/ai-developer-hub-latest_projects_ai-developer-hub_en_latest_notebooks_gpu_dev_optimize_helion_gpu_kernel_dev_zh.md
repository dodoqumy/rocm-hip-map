...
# 用于AMD GPU的GPU内核开发与评估的Helion DSL[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**：Charles Yang

**知识水平**：中级

[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个 Python 嵌入式领域特定语言 (DSL)，用于编写机器学习内核。它会被编译为 [Triton](https://openai.com/index/triton/)——OpenAI 开发的一个高性能后端，用于对 GPU 及其他设备进行编程。与 Triton 相比，Helion 旨在提升抽象层次，使得编写正确且高效的内核变得更加容易，同时还能在自动调优过程中实现更高程度的自动化。

Helion 既可以视为带分块（tiles）的 PyTorch，也可以看作更高层次的 Triton 应用。相比 Triton，Helion 通过自动调优减少了手动编码的工作量。Helion 花费更多时间（约 10 分钟）进行自动调优，因为它评估了从单个 Helion 内核生成的数百种潜在 Triton 实现。更大的搜索空间也使内核在不同硬件之间具备更强的性能可移植性。

Helion 受 AMD GPU 支持。本教程演示如何在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上搭建 Helion 开发环境、实现 Helion kernel，以及使用 Triton 和 Torch 进行性能基准测试。

## Helion 自动调优器[#](#the-helion-autotuner)

Helion 的关键区别在于其自动化的提前编译（AOT）自动调优引擎。在 Triton 中，开发者需要手动定义优化的搜索空间。这要求显式地列举出每个待测试的配置，是一项繁琐的过程，限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言会自动构建一个覆盖实现选择的、巨大的多维度搜索空间。例如，一个单独的 `hl.tile`

call隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为单一维度。因此，一个Helion内核定义可以映射到数千个Triton配置，使自动调优器能够创建一个更大更丰富的搜索空间，以发现更优的配置。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**：本教程在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上进行了测试。请确保您使用的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 支持 ROCm（ROCm（Radeon 开放计算平台）），并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（Radeon 开放计算平台）7.0**：请按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装和验证 ROCm（Radeon 开放计算平台）。安装完成后，使用以下命令确认配置：  
此命令会列出您的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

命令。AMD 也提供预构建的 ROCm（ROCm（Radeon 开放计算平台）） Docker 镜像，包括一个

[ROCm（ROCm（Radeon 开放计算平台）） PyTorch 镜像](https://hub.docker.com/r/rocm/pytorch)，[ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 22.04 镜像](https://hub.docker.com/r/rocm/dev-ubuntu-22.04) 和 [ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 24.04 镜像](https://hub.docker.com/r/rocm/dev-ubuntu-24.04)。您可以使用这些预构建的 Docker 镜像来减少搭建 ROCm（ROCm（Radeon 开放计算平台））环境所需的工作量。**Docker**：确保 Docker 已正确安装并配置。请参考适用于您操作系统的 Docker 安装指南。**注意**：请确保正确配置 Docker 权限。要配置权限以允许非 root 用户访问，请运行以下命令：usermod -aG docker $USER newgrp docker
...
使用 [Hugging Face](https://huggingface.co) 下载模型。确保 Hugging Face API 令牌拥有必要的权限。

## 1. 使用 Docker 和 ROCm 设置环境（ROCm（Radeon 开放计算平台））[#](#environment-setup-with-docker-and-rocm)

按照以下步骤设置环境、启动Jupyter Notebooks并安装依赖项。

### 启动Docker容器[#](#launch-the-docker-container)

启动Docker容器。从您的主机运行以下命令：
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到该目录，或者在 Jupyter Notebook 环境启动后上传文件。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
在运行上述命令之前，确保该端口尚未被系统占用。如果已被占用，可以通过替换 `--port=8888` 来指定不同的端口。
...
强烈建议您在项目中使用最新版本的Helion。AMD及其他供应商会频繁更新[Helion](https://github.com/pytorch/helion)中的优化pass和算法，这有助于提升您的Helion内核性能。
...
使用以下命令安装 Helion、Triton 及其他必备组件。
...
# 查找 'Successfully installed triton-xxx' 字符串以确认 Triton 已成功安装。
```
...
本示例演示如何使用Helion实现逐元素指数（exp）函数。它利用Helion的平铺系统进行并行计算，同时支持前向传播和反向传播。该实现与PyTorch自动求导系统无缝集成，支持高性能的可自动微分运算。示例还展示了如何将实现与原生PyTorch指数函数进行对比验证，并包含完整的梯度支持。
...
## 3. 软最大化算法的细节[#](#details-of-the-softmax-algorithm)

softmax 函数常用于分类 CNN 模型乃至基于 Transformer 的 LLM 模型。它通过计算每个值的指数并除以所有指数的总和，将原始输出得分（即 logits）归一化为概率。此过程确保输出值处于 (0,1) 区间且总和为 1，从而可将其解释为概率。PyTorch 已将 softmax 函数实现为 [标准 API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
为了达到数值稳定性，在对每个输入元素求指数之前，先减去行向量的最大值。因此定义变为：

其中 \(x,y \in \mathbb{R}^V\)。这被称为Safe Softmax算法。

根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值以及所有指数对应的和，并使用另一个for循环来计算最终的softmax结果。因此，它总共使用了三个循环。Safe Softmax算法在[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)中有更完整的描述。

这个kernel在8192x8192 tensor上的性能计算如下：

的块大小

`col`

维度为256。输入张量的每行分配一个程序。这意味着网格大小为
...
## 4. 创建一个Helion两遍softmax kernel[#](#creating-a-helion-two-pass-softmax-kernel)

这个示例展示了softmax函数的多个Helion内核实现，包括围绕PyTorch softmax实现的简单封装，以及一个数值优化后的两遍版本。它还包含一个检查函数，用于将这些内核与PyTorch内置的softmax函数进行比较，以验证正确性。
...
"""
执行正确性检查，比较 Helion softmax 内核与 PyTorch 的 softmax。
参数：
m (int): 输入张量的行数。
n (int): 输入张量的列数。
"""
...
该示例演示了如何使用 Triton 实现一个融合的 softmax 内核，并针对基于 CDNA（CDNA（计算 DNA 架构））后端的 AMD ROCm（ROCm（Radeon 开放计算平台））进行架构感知。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个参考的 softmax 示例，名为 `fused-softmax`。

基于在线 softmax，它简化了最大值计算以去除一个 for 循环。它还通过增加 warp 数量，要求编译器每行使用更多线程。这通常经过调优以获得更好的性能。最后，它将内核启动方案基于 GPU 硬件属性，从而获得更高的 GPU 内核占用率和更好的性能。
...
# 另一个技巧是通过增加每行分布的warp数量（`num_warps`）来让编译器每行使用更多线程。
num_warps = 8
# 软件流水线阶段数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为最大化占用率，先预热内核以获取寄存器使用情况，并计算合适的程序数量。
...
现在对所有版本的softmax内核运行基准测试和可视化，以获得结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg='provider',    # 对应图中不同线条的参数名称
        ...
        args={'M': 4096},       # 除`x_names`和`y_name`外的函数参数
    ))
...
根据最终的性能基准测试结果，Helion 不仅简化了高性能 GPU 内核的开发，还实现了接近极致的性能，甚至超越了基于 Triton 的 GPU 内核。

理想情况下，本教程鼓励您在ROCm（ROCm（Radeon开放计算平台））和AMD GPU上编写、调优、测试并贡献Helion内核，助力塑造AI加速的未来。