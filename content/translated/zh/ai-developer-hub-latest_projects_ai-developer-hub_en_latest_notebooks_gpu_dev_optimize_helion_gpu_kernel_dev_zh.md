...
# Helion DSL 用于在 AMD GPU 上进行 GPU 内核开发与评估[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**: Charles Yang

**知识水平**：中级

[Helion](https://github.com/pytorch/helion)是Meta推出的一个嵌入Python的领域特定语言(DSL)，用于编写机器学习内核。它编译到[Triton](https://openai.com/index/triton/)——OpenAI为GPU和其他设备编程提供的性能后端。Helion旨在相比Triton提高抽象层次，使编写正确且高效的内核更加容易，同时在自动调优过程中实现更高的自动化程度。

Helion 既可以看作是对 PyTorch 进行 tile 化处理，也可以看作是更高级别的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 需要花费更多时间（约 10 分钟）进行自动调优，因为它在评估由单个 Helion 内核生成的数百种可能的 Triton 实现。这种更大的搜索空间也使得内核在不同硬件之间的性能更具可移植性。

Helion 受 AMD GPU 支持。本教程演示如何在 AMD Instinct™ GPU 上搭建 Helion 开发环境、实现 Helion kernel，并使用 Triton 和 Torch 进行性能基准测试。

## Helion自动调优器[#](#the-helion-autotuner)

Helion的关键区别在于其自动化的提前编译（AOT）自动调优引擎。而在Triton中，开发者需要手动定义优化的搜索空间。这要求明确枚举每一个待测试的配置，过程繁琐且限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动构建了一个巨大的、多维的搜索空间，涵盖实现选择。例如，单个 `hl.tile`

调用隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为单一维度。因此，一个Helion内核定义可以映射到数千种Triton配置，使自动调优器能够创建更大、更丰富的搜索空间，以发现更优的配置。
...
[Hugging Face](https://huggingface.co) 用于下载模型。确保 Hugging Face API 令牌拥有必要的权限。

## 1. 使用 Docker 和 ROCm（ROCm（Radeon 开放计算平台））进行环境搭建 [#](#environment-setup-with-docker-and-rocm)

遵循以下步骤来设置环境、启动Jupyter Notebooks并安装依赖项。

### 启动Docker容器[#](#launch-the-docker-container)

启动 Docker 容器。从您的主机运行此命令：
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或在 Jupyter Notebook 环境启动后上传到其中。保存终端输出中提供的 token 或 URL，以便从 Web 浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
在运行上述命令之前，请确保端口 8888 在您的系统上未被其他程序占用。如果已被占用，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
强烈建议您在项目中使用最新版本的 Helion。AMD 及其他供应商会频繁更新 [Helion](https://github.com/pytorch/helion) 中的优化通道和算法，这有助于提升您的 Helion 内核性能。
...
本示例演示了如何使用Helion实现逐元素指数（exp）函数。它利用Helion的分块系统同时实现了前向和反向传播，以进行并行计算。该实现与PyTorch的自动求导系统无缝集成，支持高性能的自动微分操作。示例展示了如何通过完整的梯度支持，将实现与原生PyTorch指数函数进行验证。
...
softmax函数常用于分类CNN模型，甚至基于transformer的LLM模型中。它将原始输出分数（也称为logits）转换为概率，通过归一化每个值的指数，除以所有指数的和。这个过程确保输出值在(0,1)范围内且总和为1，使其可解释为概率。PyTorch已将softmax函数实现为[一个标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
其中 \(x,y \in \mathbb{R}^V\)。这被称为安全 Softmax 算法。

根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值数据及所有指数的相应和，并额外使用一个for循环来计算最终的softmax结果。因此，总共使用了三个循环。Safe Softmax算法在[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)中有更详细的描述。
...
## 4. 创建Helion两遍softmax内核[#](#creating-a-helion-two-pass-softmax-kernel)

此示例演示了softmax函数的多个Helion内核实现，包括一个围绕PyTorch softmax实现的简单封装和一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的PyTorch softmax函数进行比较，以验证正确性。
...
def main() -> None:
"""
主函数，用于使用示例输入大小运行softmax内核的正确性检查。
"""
check(4096, 2560)
# %%
...
此示例演示了如何使用 Triton 实现融合的 softmax 内核，并针对基于 CDNA 后端的 AMD ROCm（ROCm（Radeon 开放计算平台））具有架构感知能力。

#### 实现 Triton fused-softmax[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例。

基于在线softmax，它简化了最大值数据计算，从而移除一个for循环。它还通过增加warp数量，要求编译器为每行使用更多线程。这通常是为了获得更好的性能而调整的。最后，它根据GPU硬件属性确定内核启动方案，从而实现了更高的GPU内核占用率和更好的性能。
...
# 块大小是大于n_cols的最接近的2的幂，以便每行能放入一个块中
...
# 注意，Triton中的指数运算快速但近似（即类似于CUDA中的__expf）
...
# 每次循环迭代的块大小是大于 `x` 列数的下一个2的幂
BLOCK_SIZE = triton.next_power_of_2(n_cols*2)
# 另一个技巧是让编译器通过增加每行分布的 warp 数量（`num_warps`）来为每行使用更多线程。
num_warps = 8
# 软件流水线阶段数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为最大化占用率，预热内核以获取寄存器使用量，并计算适当的程序数量。
...
现在对所有版本的softmax内核运行基准测试和可视化以获取结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg='provider',  # 参数名称，其不同取值对应图中不同曲线
        line_vals=['helion', 'triton', 'aiter', 'torch'],  # `line_arg` 可能的取值
        line_names=["Helion Softmax", "Triton Softmax", "Aiter Softmax", "Torch Softmax"],  # 曲线的标签名
        styles=[('red', 'solid'), ('cyan', 'solid'), ('black', 'solid'), ('orange', 'dashdot')],  # 曲线的样式
        ylabel="GB/s",  # y 轴标签
        plot_name="Softmax 性能基准测试",  # 图名，同时也用于保存图文件
...
根据最终的性能基准测试结果，Helion不仅简化了高性能GPU内核的开发，还提供了接近极限的性能，甚至超越了基于Triton的GPU内核。

本教程旨在鼓励您在 ROCm（Radeon 开放计算平台）和 AMD GPU 上编写、调优、测试并贡献 Helion 内核，助力塑造 AI 加速的未来。