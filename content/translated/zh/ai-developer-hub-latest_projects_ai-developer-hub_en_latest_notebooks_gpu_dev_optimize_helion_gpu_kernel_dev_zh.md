...
# Helion DSL：在AMD GPU上进行GPU内核开发与评估[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**: Charles Yang

**知识水平**：中级

[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个嵌入式 Python 领域特定语言（DSL），用于编写机器学习内核。它向下编译为 [Triton](https://openai.com/index/triton/)，一个来自 OpenAI 的高性能后端，可用于 GPU 及其他设备编程。Helion 旨在提高与 Triton 相比的抽象层级，使编写正确且高效的内核更简单，同时实现自动调优过程的更高自动化程度。

Helion 可以被视为带有 tile 操作的 PyTorch，或者是一个更高级的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 会花费更多时间（大约 10 分钟）进行自动调优，因为它需要评估从单个 Helion kernel 生成的数百个潜在 Triton 实现。更大的搜索空间也使得 kernel 在不同硬件之间具有更好的性能可移植性。

Helion 由 AMD GPU 支持。本教程演示如何在 AMD Instinct™ GPU 上搭建 Helion 开发环境、实现 Helion 内核，并配合 Triton 和 Torch 进行性能基准测试。

## Helion 自动调优器[#](#the-helion-autotuner)

Helion的关键区别在于其自动化的提前编译（AOT）自动调优引擎。在Triton中，开发者需要手动定义优化的搜索空间，这要求明确列举每种待测试的配置，过程繁琐且限制了探索范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言会自动构建一个涵盖实现选择的庞大、多维搜索空间。例如，一个单独的 `hl.tile`

调用会隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。因此，一个 Helion 内核定义可以映射到数千种 Triton 配置，从而允许自动调优器创建一个更大、更丰富的搜索空间，以发现更优的配置。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**：按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认你的设置：该命令会列出你的AMD GPU及相关详细信息。

**注意**: 对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`。

命令。AMD还提供预构建的ROCm（ROCm（Radeon 开放计算平台））Docker镜像，包括一个

[ROCm（ROCm（Radeon 开放计算平台）） PyTorch image](https://hub.docker.com/r/rocm/pytorch), [ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 22.04 image](https://hub.docker.com/r/rocm/dev-ubuntu-22.04) 和 [ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 24.04 image](https://hub.docker.com/r/rocm/dev-ubuntu-24.04)。您可以使用这些预构建的 Docker 镜像来减少搭建 ROCm 环境所需的工作量。**Docker**：确保 Docker 已正确安装和配置。请根据您的操作系统参考 Docker 安装指南。**注意**：确保 Docker 权限已正确配置。要配置权限以允许非 root 用户访问，请运行以下命令：`usermod -aG docker $USER` `newgrp docker`
...
## 1. 使用Docker和ROCm（Radeon 开放计算平台）进行环境搭建 [#](#environment-setup-with-docker-and-rocm)

按以下步骤设置环境、启动 Jupyter Notebook 并安装依赖项。

### 启动 Docker 容器[#](#launch-the-docker-container)

启动Docker容器。在宿主机上运行此命令：
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在容器启动后将其上传到 Jupyter Notebook 环境。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。你可以从 [AI 开发者中心 GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
强烈建议您在项目中使用最新版本的Helion。AMD与其他供应商会频繁更新[Helion](https://github.com/pytorch/helion)中的优化过程和算法，这有助于提升您的Helion内核性能。
...
本示例演示如何使用 Helion 实现逐元素指数（exp）函数。它利用 Helion 的分块系统进行并行计算，同时支持前向和反向传播。该实现与 PyTorch 自动求导系统无缝集成，提供高性能、可自动微分操作。示例展示了如何通过完整的梯度支持，将实现与 PyTorch 原生指数函数进行对比验证。
...
softmax函数常用于分类CNN模型乃至基于Transformer的LLM模型。它通过计算每个值的指数并除以所有指数之和，将原始输出分数（即logits）归一化为概率。这一过程确保输出值在(0,1)范围内且总和为1，从而可解读为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。

函数 \(y = Softmax(x)\) 的定义是：

其中 \(x,y \in \mathbb{R}^V\)。

### 朴素版：安全Softmax[#](#naive-version-safe-softmax)

为了达到数值稳定性，在计算每个输入元素的指数之前，减去行向量的最大值。因此定义变为：

其中 \(x,y \in \mathbb{R}^V\)。这被称为 Safe Softmax 算法。

根据 softmax 算法定义，Triton 内核实现了朴素版本（公式 2）。该内核需要两个 for 循环来获取最大值数据及所有指数对应的和，以及另一个 for 循环来计算最终的 softmax 结果。因此它总共使用了三个循环。Safe Softmax 算法在 [Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867) 中有更全面的描述。
...
这个示例展示了softmax函数的多个Helion内核实现，包括一个围绕PyTorch softmax实现的简单封装以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的PyTorch softmax函数进行比较以验证正确性。
...
通过将 Helion softmax 内核与 PyTorch softmax 函数进行对比来检查正确性。

```python
# %%
def check(m: int, n: int) -> None:
"""
运行正确性检查，比较Helion softmax内核与PyTorch的softmax结果。
参数:
m (int): 输入张量的行数。
n (int): 输入张量的列数。
"""
...
本示例演示了如何使用Triton实现一个融合softmax内核，针对基于CDNA（计算 DNA 架构）的AMD ROCm（Radeon 开放计算平台）后端进行架构感知优化。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例。

基于在线softmax，它简化了最大数据计算，从而移除一个for循环。它还要求编译器通过增加warp数量来每行使用更多线程。这通常是为了获得更好的性能而调优。最后，它将内核启动方案基于GPU硬件属性，从而获得更高的GPU内核占用率和更好的性能。
...
# 块大小是大于 n_cols 的下一个2的幂，这样每一行都可以放入一个块中
...
# 注意：Triton 中的指数运算快速但近似（例如，类似于 CUDA 中的 __expf）
...
# 每次循环迭代的块大小是大于`x`列数的最小的2的幂
BLOCK_SIZE = triton.next_power_of_2(n_cols*2)
# 另一个技巧是通过增加每行分布所使用的线程束数（`num_warps`），让编译器为每行使用更多线程。
num_warps = 8
# 软件流水线级数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为了最大化占用率，先预热内核以获取寄存器使用情况，然后计算合适的程序数量。
...
根据最终性能基准测试结果，Helion不仅简化了高性能GPU kernel的开发，而且提供了接近最佳的性能，甚至超过了基于Triton的GPU kernel。

理想情况下，本教程鼓励您在 ROCm（ROCm（Radeon 开放计算平台）） 和 AMD GPU 上编写、调优、测试并贡献 Helion kernel，助力塑造AI加速的未来。