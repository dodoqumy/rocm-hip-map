...
[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一种嵌入 Python 的领域特定语言 (DSL)，用于编写机器学习内核。它能够编译成 [Triton](https://openai.com/index/triton/)，这是一个来自 OpenAI 的高性能后端，用于编写 GPU 及其他设备的程序。与 Triton 相比，Helion 旨在提高抽象层次，使得编写正确且高效的内核更加容易，同时在自动调优过程中实现更高的自动化程度。

Helion 可被视为带有块状操作的 PyTorch，或更高级别的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 花费更多时间（约 10 分钟）进行自动调优，因为它会评估从单个 Helion 内核生成的数百种潜在 Triton 实现。这种更大的搜索空间也使内核在不同硬件之间更具性能可移植性。
...
Helion 的关键差异在于其自动化的提前（AOT）自动调优引擎。在 Triton 中，开发者需要手动定义优化的搜索空间。这要求明确列出每一个待测试的配置，过程繁琐且限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言会自动构建一个关于实现选择的巨大多维搜索空间。例如，单个 `hl.tile`

call 隐式指示自动调谐器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。因此，一个 Helion 内核定义可以映射到数千个 Triton 配置，使自动调谐器能够创建一个更大更丰富的搜索空间，以发现更优的配置。
...
**AMD Instinct（AMD 数据中心 GPU 系列） MI300X GPU**：本教程已在 AMD Instinct（AMD 数据中心 GPU 系列） MI300X GPU 上完成测试。请确保您使用的 AMD Instinct（AMD 数据中心 GPU 系列） GPU 支持 ROCm（Radeon 开放计算平台），并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 7.0**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm。安装完成后，使用以下命令确认你的配置：该命令会列出你的 AMD GPU 及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`。

command 代替。AMD 还提供了预构建的 ROCm（ROCm（Radeon 开放计算平台）） Docker 镜像，包括一个

[ROCm（ROCm（Radeon 开放计算平台）） PyTorch image](https://hub.docker.com/r/rocm/pytorch)、[ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 22.04 image](https://hub.docker.com/r/rocm/dev-ubuntu-22.04) 以及 [ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 24.04 image](https://hub.docker.com/r/rocm/dev-ubuntu-24.04)。您可以使用这些预构建的 Docker 镜像来简化搭建 ROCm（ROCm（Radeon 开放计算平台））环境的步骤。**Docker**：确保 Docker 已正确安装和配置。请根据您的操作系统参考 Docker 安装指南。**注意**：确保 Docker 权限配置正确。要配置允许非 root 用户访问的权限，请运行以下命令：
...
使用 [Hugging Face](https://huggingface.co) 下载模型。确保 Hugging Face API 令牌具有必要的权限。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。

### 在容器中启动Jupyter Notebook[#](#launch-jupyter-notebooks-in-the-container)
...
确保在运行上述命令之前，该端口尚未在您的系统上被占用。如果已被占用，您可以通过替换 `--port=8888` 来指定不同的端口。
...
强烈建议您在项目中使用最新版本的 Helion。AMD 及其他供应商会频繁更新 [Helion](https://github.com/pytorch/helion) 中的优化流程与算法，这有助于提升您的 Helion 内核性能。

#### 卸载Helion和Triton旧版本[#](#uninstall-older-versions-of-helion-and-triton)
...
本示例演示如何使用Helion实现逐元素的指数（exp）函数。它利用Helion的tiling系统实现了前向和反向传播，用于并行计算。该实现与PyTorch的autograd系统无缝集成，支持高性能、可自动求导的操作。示例展示了如何将该实现与原生PyTorch指数函数（具有完整梯度支持）进行验证对比。
...
softmax函数常用于分类CNN模型，甚至基于transformer的LLM模型。它通过将每个值的指数除以所有指数的总和来进行归一化，从而将原始输出分数（也称为logits）转换为概率。这个过程确保输出值在(0,1)范围内且总和为1，使其可解释为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值以及所有指数对应的和，还需要一个额外的for循环来计算最终的softmax结果，因此总共使用了三个循环。Safe Softmax算法在《[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)》中有更完整的描述。

该 kernel 在 8192x8192 张量上的性能计算如下：
...
程序实例（thread block）扫描张量的一行，并迭代处理当前行的数据块，以计算当前行的分母（指数和）值 \(d_j\)。这是第二个for循环。

程序实例（线程块）扫描张量的一行，并迭代处理当前行的数据块，以计算当前行的最终softmax值 \(y_i\)。这是第三个for循环。

## 4. 创建一个 Helion two-pass softmax kernel[#](#creating-a-helion-two-pass-softmax-kernel)

此示例展示了 softmax 函数的多种 Helion 内核实现，包括围绕 PyTorch softmax 实现的简单封装，以及一个数值优化的两遍版本。此外，还包含一个检查函数，用于将这些内核与内置的 PyTorch softmax 函数进行比较，以验证正确性。
...
def softmax_two_pass(x: torch.Tensor) -> torch.Tensor:
    """
    数值优化的 Helion 内核，通过两次遍历执行 softmax。
    参数：
...
本节比较了Helion、Triton、PyTorch和Aiter的性能。

### 对照组：Triton fused-softmax and Aiter softmax[#](#control-group-triton-fused-softmax-and-aiter-softmax)

这个示例演示了如何使用 Triton 实现一个融合的 softmax 内核，并对基于 CDNA（CDNA（计算 DNA 架构））的 AMD ROCm（ROCm（Radeon 开放计算平台））后端具有架构感知能力。

#### Triton 融合 softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例

基于在线softmax，它简化了最大值数据计算，去除了一个for循环。同时，通过增加每个线程束（warp）的数量，它要求编译器为每一行使用更多线程。这一参数通常经过调优以获得更佳性能。最终，它根据GPU硬件属性设计内核启动方案，从而提升GPU内核占用率并实现更优性能。
...
# 块大小是大于 n_cols 的下一个 2 的幂，这样每行可以放入单个块中
col_offsets = tl.arange(0, BLOCK_SIZE)
input_ptrs = row_start_ptr + col_offsets
# 将行加载到 SRAM，由于 BLOCK_SIZE 可能大于 n_cols，因此使用掩码
mask = col_offsets < n_cols
row = tl.load(input_ptrs, mask=mask, other=-float('inf'))
# 减去最大值以保证数值稳定性
row_minus_max = row - tl.max(row, axis=0)
# 注意，Triton 中的指数运算快速但近似（例如，可以将其视为 CUDA 中的 __expf（CUDA（统一计算设备架构）））
...
现在运行 softmax 内核所有版本的基准测试和可视化，以获取结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        x_names=['N'],                          # 用作图x轴的参数名称
        x_vals=[128 * i for i in range(55, 95)], # `x_name`的可选值
        line_arg='provider',                     # 其值对应图中不同线的参数名称
        line_vals=['helion','triton', 'aiter','torch'], # `line_arg`的可选值
        line_names=["Helion Softmax","Triton Softmax", "Aiter Softmax","Torch Softmax"], # 各条线的标签名称
        styles=[('red', 'solid'),('cyan', 'solid'), ('black', 'solid'), ('orange', 'dashdot')], # 线型样式
        ylabel="GB/s",                           # y轴标签名称
        plot_name="Softmax性能基准测试",          # 图名称，同时用作保存图的文件名
        args={'M': 4096},                        # 不在`x_names`和`y_name`中的函数参数的数值
    ))
...
祝贺你！通过运行本Helion GPU内核开发教程，你学会了如何在AMD GPU上开发和优化Helion内核。

根据最终性能基准测试结果，Helion不仅简化了高性能GPU内核开发，还能提供接近极限的性能，甚至超越了基于Triton的GPU内核。

理想情况下，本教程鼓励您在ROCm（ROCm（Radeon 开放计算平台））和AMD GPU上编写、调优、测试并为Helion kernel做出贡献，助力塑造AI加速的未来。