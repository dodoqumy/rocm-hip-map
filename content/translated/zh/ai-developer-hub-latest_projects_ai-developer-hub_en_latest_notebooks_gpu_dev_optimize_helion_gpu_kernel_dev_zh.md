...
# 用于AMD GPU上的GPU核函数开发和评估的Helion DSL[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**：Charles Yang

**知识水平**: 中级

[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一种嵌入 Python 的领域特定语言（DSL），用于编写机器学习内核。它编译到 [Triton](https://openai.com/index/triton/)（OpenAI 提供的高性能后端，用于 GPU 及其他设备的编程）之上。Helion 旨在相比 Triton 提升抽象层次，降低编写正确且高效内核的难度，同时为自动调优过程提供更强的自动化能力。

Helion 可被视为带有 tile 的 PyTorch，或更高层次的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 会花费更多时间（约 10 分钟）进行自动调优，因为它需要评估由单个 Helion 内核生成的数百种潜在 Triton 实现方案。更大的搜索空间也使得内核在不同硬件之间具有更好的性能可移植性。

Helion 受 AMD GPU 支持。本教程演示如何设置 Helion 开发环境、实现 Helion 内核，并在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上使用 Triton 和 Torch 进行性能基准测试。

## Helion 自动调谐器[#](#the-helion-autotuner)

Helion 的主要差异化优势在于其自动化的提前编译（AOT）自动调优引擎。而在 Triton 中，开发者需要手动定义优化的搜索空间。这要求明确列举所有待测试的配置，过程繁琐且限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动构建了一个包含实现选择的广阔多维搜索空间。例如，一个单独的 `hl.tile`

调用隐式地指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。一个Helion内核定义因此可以映射到数千个Triton配置，使得自动调优器能够创建一个更大、更丰富的搜索空间，从而发现更优的配置。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**: 请按照[ROCm安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)进行安装和验证。安装完成后，使用以下命令确认配置：该命令将列出您的AMD GPU及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用`rocm-smi`

请改用命令。AMD 也提供预构建的 ROCm（ROCm（Radeon 开放计算平台）） Docker 镜像，包括一个
...
**注意**：请确保正确配置 Docker 权限。如需配置非 root 用户访问权限，请运行以下命令：
...
容器中的目录。确保在运行 Docker 命令之前将 notebook 文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传到其中。保存终端输出的令牌或 URL，以便通过浏览器访问 notebook。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm/gpuaidev) 下载此 notebook。
...
你是一名专注于GPU/ROCm/HIP文档的技术翻译。将以下英文文本翻译成简体中文（zh-CN）。规则：
1. 保留所有Markdown格式、代码块、行内代码和链接不变。
2. 保留技术术语如ROCm、HIP、GPU、CUDA、AMD、PyTorch、TensorFlow的原始英文形式。
...
强烈建议您在项目中使用最新版本的Helion。AMD及其他供应商会频繁更新[Helion](https://github.com/pytorch/helion)中的优化遍和算法，这有助于提升您的Helion内核性能。
...
# 请查找字符串 'Successfully installed triton-xxx' 以确认 Triton 安装成功。
```
...
此示例展示了如何使用Helion实现逐元素指数（exp）函数。它利用Helion的分块系统进行并行计算，同时支持前向和反向传播。该实现无缝集成到PyTorch自动求导系统中，实现了高性能、可自动微分的操作。示例还演示了如何通过完整的梯度支持，将实现与原生PyTorch指数函数进行验证对比。
...
softmax函数常用于分类CNN模型乃至基于transformer的LLM模型。它通过将每个值的指数除以所有指数的总和进行归一化，将原始输出分数（也称为logits）转换为概率。这个过程确保输出值在(0,1)范围内且总和为1，从而可以解释为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
为了实现数值稳定性，在对每个输入元素取指数之前，减去行向量的最大值。因此定义变为：

其中 \(x,y \in \mathbb{R}^V\)。这称为 Safe Softmax 算法。

根据softmax算法定义，Triton kernel实现了朴素版本（公式2）。该kernel需要两个for循环来获取最大值及所有指数之和，并额外使用一个for循环计算最终的softmax结果，因此总共使用了三个循环。Safe Softmax算法在《Online normalizer calculation for softmax》（https://arxiv.org/pdf/1805.02867）中有更完整的描述。

此kernel在一个8192x8192张量上的性能计算方式如下：
...
## 4. 创建Helion两遍softmax核[#](#creating-a-helion-two-pass-softmax-kernel)

此示例展示了 softmax 函数的多个 Helion kernel 实现，包括围绕 PyTorch softmax 实现的简单包装器以及数值优化的两遍版本。它还包含一个检查函数，用于将这些 kernel 与内置的 PyTorch softmax 函数进行比较，以验证正确性。
...
"""
数值优化的Helion核函数，通过两次遍历执行softmax。
...
此示例演示了如何使用 Triton 实现一个融合 softmax 内核，并针对基于 AMD ROCm（ROCm（Radeon 开放计算平台））与 CDNA（CDNA（计算 DNA 架构））的后端进行架构感知。

#### Triton融合softmax的实现[#](#implementation-of-triton-fused-softmax)

Triton提供了一个名为`fused-softmax`的参考softmax示例。

基于在线softmax，简化了最大值数据的计算，去除了一个for循环。同时通过增加线程束数量，要求编译器为每行使用更多线程。这通常是为了获得更好的性能而调优的。最后，根据GPU硬件属性确定内核启动方案，从而获得更高的GPU内核占用率和更好的性能。
...
# 块大小是大于 n_cols 的 2 的下一次幂，因此我们可以将每行
# 放入一个块中
...
# 注意，Triton 中的指数运算快速但近似（即类似于 CUDA（统一计算设备架构）中的 __expf）
...
# 另一个技巧是让编译器通过增加每行分布所在的 warp 数量 (`num_warps`) 来使用更多线程。
num_warps = 8
# 软件流水线阶段数。
...
#### Install the ROCm（ROCm（Radeon 开放计算平台）） Aiter kernel library[#](#install-the-rocm-aiter-kernel-library)

使用以下命令安装用于内置softmax核函数的Aiter核库：
...
现在运行所有版本的softmax内核的基准测试和可视化以获得结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="GB/s", # y轴标签名
        plot_name="Softmax性能基准测试", # 图名称，也用作保存图的文件名
...
根据最终性能基准测试结果，Helion 不仅简化了高性能 GPU 内核开发，还实现了近乎最优的性能，甚至超越了基于 Triton 的 GPU 内核。

理想情况下，本教程鼓励您在ROCm（ROCm（Radeon 开放计算平台））和AMD GPU上编写、调优、测试并为Helion kernel做出贡献，帮助塑造AI加速的未来。