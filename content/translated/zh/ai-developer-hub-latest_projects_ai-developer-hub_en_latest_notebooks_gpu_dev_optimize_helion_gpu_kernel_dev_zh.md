...
[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一种基于 Python 的领域特定语言（DSL），用于编写机器学习内核。它编译为 [Triton](https://openai.com/index/triton/)，这是 OpenAI 开发的一个高性能后端，用于 GPU 及其他设备的编程。Helion 旨在相比 Triton 提高抽象层次，使编写正确且高效的内核更加容易，同时实现自动调优过程中更高程度的自动化。

Helion 既可被视为带分块（tiles）的 PyTorch，也可看作更高级别的 Triton 应用。与 Triton 相比，Helion 通过自动调优（autotuning）减少了手动编码的工作量。Helion 花费更多时间（约 10 分钟）进行自动调优，因为它会评估由单个 Helion kernel 生成的数百种潜在 Triton 实现。更大的搜索空间也使 kernel 在不同硬件之间具有更好的性能可移植性。

Helion is supported by AMD GPUs. This tutorial demonstrates how to set up the Helion development environment, implement a Helion kernel, and benchmark performance with Triton and Torch on AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPUs.

## Helion自动调优器[#](#the-helion-autotuner)

Helion的关键区别在于其自动化的、提前编译(AOT)自动调优引擎。在Triton中，开发者需要手动定义优化的搜索空间。这要求显式列举所有待测试的配置，是一个繁琐的过程，限制了探索的范围。

Helion通过使用隐式搜索空间改变了这一动态。高级语言自动构建了一个广阔的多维搜索空间，涵盖实现选择。例如，单个 `hl.tile`

调用隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间扁平化为单一维度。因此，一个Helion内核定义可以映射到数千种Triton配置，从而允许自动调优器创建一个更大且更丰富的搜索空间，以发现更优的配置。
...
**AMD Instinct（AMD 数据中心 GPU 系列） MI300X GPU**：本教程已在 AMD Instinct（AMD 数据中心 GPU 系列） MI300X GPU 上完成测试。请确保您使用的是支持 ROCm（Radeon 开放计算平台）的 AMD Instinct（AMD 数据中心 GPU 系列） GPU，并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 7.0**：按照 [ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的设置：该命令会列出您的 AMD GPU 及相关详细信息。
...
**注意**：确保 Docker 权限正确配置。要配置允许非 root 用户访问的权限，请运行以下命令：
...
按照以下步骤设置环境、启动 Jupyter Notebook 并安装依赖项。
...
容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传到其中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未在您的系统上使用。如果已被占用，您可以通过替换`--port=8888`来指定一个不同的端口。
...
本示例演示了如何使用 Helion 实现逐元素的指数（exp）函数。它利用 Helion 的平铺（tiling）系统进行并行计算，同时支持前向传播和反向传播。该实现与 PyTorch autograd 系统无缝集成，可实现高性能的自动可微操作。示例还展示了如何对照原生 PyTorch 指数函数（支持完整梯度）验证该实现。
...
softmax函数常用于分类CNN模型甚至基于Transformer的LLM模型。它通过计算每个值的指数并除以所有指数之和，将原始输出分数（也称为logits）转换为概率。这一过程确保输出值在(0,1)范围内且总和为1，使其可作为概率来解释。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
为了实现数值稳定性，在计算每个输入元素的指数之前，减去行向量的最大值。因此定义变为：

其中 \(x,y \in \mathbb{R}^V\)。这被称为 Safe Softmax 算法。

根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值数据以及所有指数对应的和，还需要一个额外的for循环来计算最终的softmax结果，因此总共使用了三个循环。Safe Softmax算法在《Online normalizer calculation for softmax》（https://arxiv.org/pdf/1805.02867）中有更全面的描述。
...
## 4. 创建 Helion two-pass softmax 内核[#](#creating-a-helion-two-pass-softmax-kernel)

此示例演示了softmax函数的多个Helion内核实现，包括对PyTorch softmax实现的简单封装以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的PyTorch softmax函数进行比较，以验证正确性。
...
通过将Helion softmax核与PyTorch softmax函数进行比较来检查正确性。

```
# %%
def check(m: int, n: int) -> None:
"""
执行正确性检查，比较Helion softmax内核与PyTorch的softmax。
参数：
    m (int): 输入张量的行数。
    n (int): 输入张量的列数。
"""
...
本例演示如何使用 Triton 实现一个融合 softmax 内核，并具有针对基于 CDNA（CDNA（计算 DNA 架构））的 AMD ROCm（ROCm（Radeon 开放计算平台））后端的架构感知。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例。

基于在线softmax，它简化了最大值数据的计算，从而移除了一个for循环。同时，它通过增加warp的数量，要求编译器在每一行中使用更多线程，这通常是为了优化性能而调整的。最后，它将内核启动方案基于GPU硬件属性进行设计，从而实现了更高的GPU内核占用率和更优的性能。
...
# 注意：Triton中的指数运算快速但近似（即，类似于CUDA中的__expf）
...
# 另一个技巧是让编译器通过增加每行分配的线程束数 (`num_warps`) 来使用更多线程每行。
num_warps = 8
# 软件流水线阶段数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为最大化占用率，先预热内核以获取寄存器使用情况，并计算合适的程序数。
...
现在运行所有版本的softmax内核的基准测试和可视化以获取结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="GB/s", # y 轴标签
        plot_name="Softmax 性能基准测试", # 图名，也用作保存图片的文件名
...
恭喜！通过运行此Helion GPU内核开发教程，您学会了如何在AMD GPU上开发和优化Helion内核。

根据最终的性能基准测试结果，Helion不仅简化了高性能GPU内核开发，而且提供了接近最大的性能，甚至优于基于Triton的GPU内核。

理想情况下，本教程鼓励您在 ROCm（ROCm（Radeon 开放计算平台））和 AMD GPU 上编写、调优、测试并为 Helion kernel 做出贡献，助力塑造 AI 加速的未来。