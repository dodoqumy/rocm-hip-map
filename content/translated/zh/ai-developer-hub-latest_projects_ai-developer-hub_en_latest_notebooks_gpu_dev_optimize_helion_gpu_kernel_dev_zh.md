...
# Helion DSL用于AMD GPU上的GPU内核开发与评估[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**：Charles Yang

**知识水平**：中级

[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个嵌入 Python 的领域特定语言（DSL），用于编写机器学习内核。它编译为 [Triton](https://openai.com/index/triton/)，后者是 OpenAI 提供的一个高性能后端，用于编程 GPU 和其他设备。Helion 旨在提升相对于 Triton 的抽象层级，使编写正确且高效的内核更加容易，同时为自动调优过程提供更大的自动化能力。

Helion 可被视作带有 Tile 操作的 PyTorch，或作为更高层次的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编程的工作量。Helion 花费更多时间（约 10 分钟）进行自动调优，因为它会评估从单个 Helion 内核生成的数百种潜在 Triton 实现。更大的搜索空间也使得内核在不同硬件之间具有更强的性能可移植性。
...
Helion的关键差异化特性在于其自动化的提前编译（AOT）自动调优引擎。在Triton中，开发者需要手动定义优化的搜索空间。这要求显式枚举所有待测试的配置，这是一个繁琐的过程，限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动针对实现选择构建一个庞大且多维的搜索空间。例如，单个 `hl.tile`

调用隐式地指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。因此，一个 Helion 内核定义可以映射到数千种 Triton 配置，从而使自动调优器能够创建更大、更丰富的搜索空间，以发现更优的配置。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**: 本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上测试。确保您使用的是支持 ROCm（ROCm（Radeon 开放计算平台））的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU，并且您的系统符合[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（ROCm（Radeon 开放计算平台）） 7.0**：按照 [ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html) 安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认设置：该命令将列出您的 AMD GPU 及相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台））6.4 及更早版本，请使用 `rocm-smi`

命令替代。AMD还提供了预构建的ROCm（ROCm（Radeon 开放计算平台））Docker镜像，包括一个
...
**Docker**：确保已正确安装并配置 Docker。请根据您的操作系统参考 Docker 安装指南。  
**注意**：确保正确配置了 Docker 权限。如需配置非 root 用户访问权限，请运行以下命令：  
...
容器中的目录。确保在运行 Docker 命令前已将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub 仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
强烈建议您在项目中使用最新版本的 Helion。AMD 及其他供应商会经常更新 [Helion](https://github.com/pytorch/helion) 中的优化阶段和算法，这有助于提升您的 Helion 内核性能。
...
此示例演示了如何使用 Helion 实现逐元素指数（exp）函数。它利用 Helion 的分块系统进行并行计算，同时支持前向传播和后向传播。该实现与 PyTorch 自动求导系统无缝集成，可实现高性能的自动可微操作。示例还展示了如何针对原生 PyTorch 指数函数（含完整梯度支持）验证该实现的正确性。
...
softmax函数常用于分类CNN模型甚至基于transformer的LLM模型。它通过将每个值的指数除以所有指数的总和进行归一化，将原始输出分数（也称为logits）转换为概率。这个过程确保输出值在(0,1)范围内且总和为1，使其可解释为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
此示例展示了softmax函数的多个Helion内核实现，包括对PyTorch softmax实现的简单包装器，以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的PyTorch softmax函数进行比较以验证正确性。
...
def softmax_two_pass(x: torch.Tensor) -> torch.Tensor:
    """
    数值优化的Helion内核，分两遍执行softmax计算。

    Args:
        x (torch.Tensor): 输入张量，形状为 [m, n]。

    Returns:
        torch.Tensor: 与输入形状相同的softmax输出张量。
    """
    m, n = x.size()
    out = torch.empty_like(x)
...
通过将 Helion softmax 核与 PyTorch softmax 函数进行比较来检查正确性。
...
本示例演示了如何使用Triton实现一个融合的softmax内核，并具备对基于CDNA（计算DNA架构）的AMD ROCm（Radeon开放计算平台）的架构感知能力。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例。

基于在线softmax，它简化了最大数据计算，移除了一个for循环。同时，通过增加线程束数量，要求编译器每行使用更多线程，这通常通过调整来获得更好性能。最后，它基于GPU硬件属性来确定核函数启动方案，从而获得更高的GPU核函数占用率和更好的性能。
...
# 注意：Triton中的指数运算速度快但近似（类似于CUDA中的__expf）
...
# 另一个技巧是通过增加每行分配的线程束数量（`num_warps`）来让编译器使用更多线程。
num_warps = 8
# 软件流水线阶段数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为最大化占用率，预热内核以获取寄存器使用情况，并计算适当的程序数量。
...
使用以下命令安装内置 softmax 内核函数的 Aiter kernel library：
...
现在运行所有版本的 softmax 内核的基准测试和可视化以获取结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="GB/s", # y 轴标签名
        plot_name="Softmax 性能基准测试", # 图表名称，也用于保存图表的文件名
...
根据最终性能基准测试结果，Helion不仅简化了高性能GPU内核开发，还提供了接近极致的性能，甚至超越了基于Triton的GPU内核。
