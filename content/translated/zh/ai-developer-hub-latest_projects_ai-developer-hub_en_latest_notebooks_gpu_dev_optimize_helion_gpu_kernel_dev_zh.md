...
# 用于AMD GPU上GPU内核开发和评估的Helion DSL[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**：Charles Yang

**知识水平**：中级

[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个 Python 嵌入式领域特定语言（DSL），用于编写机器学习内核。它可编译为 [Triton](https://openai.com/index/triton/)，后者是 OpenAI 提供的高性能后端，用于对 GPU 及其他设备进行编程。与 Triton 相比，Helion 旨在提升抽象层次，使编写正确且高效的内核更加容易，同时也能在自动调优过程中实现更高程度的自动化。

Helion 既可以看作带有 tile 的 PyTorch，也可以看作更高级别的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 花费更多时间（大约10分钟）进行自动调优，因为它会评估从单个 Helion kernel 生成的数百种潜在的 Triton 实现。更大的搜索空间也使得 kernel 在不同硬件之间具有更好的性能可移植性。

Helion 受到 AMD GPU 支持。本教程演示如何在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上设置 Helion 开发环境、实现 Helion 内核，以及使用 Triton 和 Torch 进行性能基准测试。

## Helion自动调优器[#](#the-helion-autotuner)

Helion 的核心区别在于其自动化、提前（AOT）的自动调优引擎。而在 Triton 中，开发者需手动定义优化的搜索空间，这要求显式枚举每种待测试的配置——一个繁琐的过程，从而限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动构建了一个庞大且多维的搜索空间，覆盖实现选择。例如，一个单独的 `hl.tile`

call 隐式地指示自动调谐器探索不同的块大小和循环排序，并考虑是否将迭代空间展平为一维。因此，一个 Helion 内核定义可以映射到数千种 Triton 配置，使自动调谐器能够创建一个更大、更丰富的搜索空间，以发现优越的配置。
...
**AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU**：本教程已在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） MI300X GPU 上进行了测试。请确保您使用的 AMD Instinct（Instinct（AMD 数据中心 GPU 系列）） GPU 支持 ROCm（ROCm（Radeon 开放计算平台）），并且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。

### 软件[#](#software)

**ROCm（Radeon 开放计算平台）7.0**：按照[ROCm 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm。安装完成后，使用以下命令确认您的设置：该命令将列出您的 AMD GPU 及相关详细信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

命令代替。AMD 还提供预构建的 ROCm（ROCm（Radeon 开放计算平台）） Docker 镜像，包括一个

[ROCm（ROCm（Radeon 开放计算平台）） PyTorch image](https://hub.docker.com/r/rocm/pytorch), [ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 22.04 image](https://hub.docker.com/r/rocm/dev-ubuntu-22.04), 以及 [ROCm（ROCm（Radeon 开放计算平台）） Ubuntu 24.04 image](https://hub.docker.com/r/rocm/dev-ubuntu-24.04)。您可以使用这些预构建的Docker镜像来减少设置ROCm环境所需的工作量。

**Docker**: 确保Docker已正确安装和配置。请根据您的操作系统遵循Docker安装指南。
...
在容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或在 Jupyter Notebook 环境启动后上传到其中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。你可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口尚未在您的系统上被占用。如果已被占用，您可以通过替换 `--port=8888` 来指定不同的端口。
...
强烈建议您在项目中使用最新版本的 Helion。AMD 及其他供应商会频繁更新 [Helion](https://github.com/pytorch/helion) 中的优化通道和算法，这有助于提升 Helion 内核的性能。
...
本示例演示如何使用Helion实现逐元素的指数（exp）函数。它利用Helion的平铺系统进行并行计算，同时支持前向和反向传播。该实现与PyTorch的autograd系统无缝集成，支持高性能的自动微分操作。本示例展示了如何对照具有完整梯度支持的PyTorch原生指数函数来验证该实现。
...
softmax函数常用于分类CNN模型乃至基于transformer的LLM模型。它通过将每个值的指数除以所有指数的总和来进行归一化，从而将原始输出分数（也称为logits）转换为概率。此过程确保输出值在(0,1)范围内且总和为1，使其可解释为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
按照softmax算法定义，Triton kernel实现了朴素版本（公式2）。该kernel需要两个for循环来获取最大值及所有指数之和，并需要另一个for循环来计算最终的softmax结果，因此总共使用了三个循环。Safe Softmax算法在[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)中有更完整的描述。
...
该程序实例（线程块）扫描张量的一行，并迭代处理当前行的数据块，以计算当前行的最终 softmax 值 \(y_i\)。这是第三个 for 循环。

## 4. 创建 Helion 两遍 softmax kernel[#](#creating-a-helion-two-pass-softmax-kernel)

此示例展示了 softmax 函数的多个 Helion kernel 实现，包括一个围绕 PyTorch softmax 实现的简单包装器和一个数值优化的两遍版本。它还包含一个检查函数，用于将这些 kernel 与内置的 PyTorch softmax 函数进行比较以验证正确性。
...
返回：
    torch.Tensor: 与输入相同形状的 softmax 输出张量。
"""
m, n = x.size()
out = torch.empty_like(x)
...
通过对比 Helion softmax 内核与 PyTorch softmax 函数来检查正确性。
...
此示例演示了如何使用Triton实现一个融合的softmax内核，并针对基于AMD ROCm（ROCm（Radeon 开放计算平台））和CDNA（CDNA（计算 DNA 架构））的后端具备架构感知。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例

基于在线softmax，它简化了最大值数据计算，从而移除一个for循环。它还通过增加warp数量，要求编译器在每行使用更多线程。这通常需要调整以获得更好的性能。最后，它根据GPU硬件属性确定内核启动方案，从而获得更高的GPU内核占用率和更好的性能。
...
# 注意，Triton中的指数运算快速但近似（即类似于CUDA（CUDA（统一计算设备架构））中的__expf）
...
# 另一个技巧是通过增加每个行分布的线程束数（`num_warps`）来让编译器使用更多每行的线程。
num_warps = 8
# 软件流水线阶段数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用量的限制。为了最大化占用率，预热内核以获取寄存器使用量，并计算合适的程序数量。
...
使用以下命令安装内置 softmax 内核函数所需的 Aiter 内核库：
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        x_names=['N'], # 用作 x 轴的自变量名
        x_vals=[128 * i for i in range(55, 95)], # x_name 的不同取值
        line_arg='provider', # 对应图中不同折线的自变量名
        line_vals=['helion', 'triton', 'aiter', 'torch'], # line_arg 的可能取值
        line_names=["Helion Softmax", "Triton Softmax", "Aiter Softmax", "Torch Softmax"], # 折线标签名
        styles=[('red', 'solid'), ('cyan', 'solid'), ('black', 'solid'), ('orange', 'dashdot')], # 折线样式
        ylabel="GB/s", # y 轴标签
        plot_name="Softmax 性能基准测试", # 图名，同时也作为保存图片的文件名
...
恭喜！通过运行本Helion GPU内核开发教程，您已经学会了如何在AMD GPU上开发和优化Helion内核。

根据最终的性能基准测试结果，Helion不仅简化了高性能GPU内核的开发，还提供了接近极致的性能，甚至超越了基于Triton的GPU内核。

理想情况下，本教程鼓励你在 ROCm（ROCm（Radeon 开放计算平台））和 AMD GPU 上编写、调优、测试并贡献 Helion 内核，助力塑造 AI 加速的未来。