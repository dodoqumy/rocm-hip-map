...
[Helion](https://github.com/pytorch/helion) 是Meta推出的一种内嵌于Python的领域特定语言（DSL），用于编写机器学习内核。它可编译为 [Triton](https://openai.com/index/triton/)——OpenAI开发的一款用于GPU及其他设备编程的高性能后端。与Triton相比，Helion旨在提高抽象层次，使得编写正确且高效的内核更为简便，同时能在自动调优过程中实现更高程度的自动化。

Helion既可被视为带有分块的PyTorch，也可被视为更高级的Triton应用。与Triton相比，Helion通过自动调优减少了手动编码的工作量。Helion在自动调优上花费更多时间（约10分钟），因为它会评估由单个Helion内核生成的数百种潜在Triton实现方案。这种更大的搜索空间也使得内核在不同硬件之间具有更好的性能可移植性。

Helion 受 AMD GPU 支持。本教程演示如何在 AMD Instinct™ GPU 上搭建 Helion 开发环境、实现 Helion 内核，并对比 Triton 和 Torch 的性能基准。

## Helion自动调优器[#](#the-helion-autotuner)

Helion 的核心差异化优势在于其自动化、提前（AOT）的自动调优引擎。在 Triton 中，开发者需要手动定义优化的搜索空间。这要求显式列举每一个待测试的配置，过程繁琐且限制了探索范围。

Helion通过使用隐式搜索空间改变了这种动态。高级语言自动构建了一个广阔的多维搜索空间，覆盖实现选择。例如，一个单独的`hl.tile`

调用隐式地指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。因此，一个Helion内核定义可以映射到数千种Triton配置，从而使自动调优器能够创建一个更大、更丰富的搜索空间，以发现更优的配置。
...
**Ubuntu 22.04/24.04**：确保您的系统运行的是Ubuntu 22.04或24.04。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**: 按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：此命令将列出您的 AMD GPU 及其相关详细信息。

**注意**: 对于ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，使用`rocm-smi`

命令。AMD还提供预构建的ROCm（ROCm（Radeon 开放计算平台）） Docker镜像，包括一个

[ROCm (ROCm (Radeon 开放计算平台)) PyTorch 镜像](https://hub.docker.com/r/rocm/pytorch)、[ROCm (ROCm (Radeon 开放计算平台)) Ubuntu 22.04 镜像](https://hub.docker.com/r/rocm/dev-ubuntu-22.04) 和 [ROCm (ROCm (Radeon 开放计算平台)) Ubuntu 24.04 镜像](https://hub.docker.com/r/rocm/dev-ubuntu-24.04)。您可以使用这些预构建的 Docker 镜像来减少搭建 ROCm (ROCm (Radeon 开放计算平台)) 环境的工作量。

**Docker**：确保已正确安装并配置 Docker。请按照适用于您操作系统的 Docker 安装指南进行操作。

**注意**：确保正确配置 Docker 权限。要配置权限以允许非 root 用户访问，请运行以下命令：
...
容器中的目录。确保在运行Docker命令之前将notebook文件复制到此目录，或者在Jupyter Notebook环境启动后上传到该环境。保存终端输出中提供的token或URL，以便从浏览器访问notebook。你可以从[AI Developer Hub GitHub仓库](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev)下载此notebook。
...
在运行上述命令之前，确保其未在你的系统中已被占用。如果已被占用，你可以通过替换 `--port=8888` 来指定一个不同的端口。
...
强烈建议在项目中使用最新版本的Helion。AMD及其他供应商会频繁更新[Helion](https://github.com/pytorch/helion)中的优化遍次和算法，这有助于提升Helion内核的性能。
...
本示例演示了如何使用Helion实现逐元素指数（exp）函数。它利用Helion的分块系统进行并行计算，支持前向和反向传播。该实现与PyTorch自动求导系统无缝集成，实现了高性能的自动可微分运算。该示例展示了如何将实现结果与具有完整梯度支持的原生PyTorch指数函数进行验证。
...
验证 exp kernel 实现与 PyTorch 原生 `exp` 的一致性
...
softmax函数常用于分类CNN模型乃至基于Transformer的LLM模型。它通过将每个值的指数除以所有指数的和进行归一化，将原始输出分数（也称为logits）转换为概率。这一过程确保输出值落在(0,1)范围内，且总和为1，从而可解释为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
为了实现数值稳定性，在计算指数之前，从每个输入元素中减去行向量的最大值。因此，定义变为：

其中 \(x,y \in \mathbb{R}^V\)。这被称为 Safe Softmax 算法。

根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值数据以及所有指数之和，并额外需要一个for循环来计算最终的softmax结果。因此，它总共使用了三个循环。Safe Softmax算法在[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)中有更完整的描述。

在8192x8192的张量上，这个kernel的性能计算方式如下：
...
程序实例（线程块）扫描张量的一行，并迭代处理当前行的数据块，以计算当前行的最终 softmax 值 \(y_i\)。这是第三个 for 循环。

## 4. 创建Helion双次softmax kernel[#](#creating-a-helion-two-pass-softmax-kernel)

这个示例展示了 softmax 函数的多个 Helion 内核实现，包括一个围绕 PyTorch softmax 实现的简单包装器，以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的 PyTorch softmax 函数进行比较，以验证正确性。
...
"""
数值优化的 Helion 内核，通过双重遍历执行 softmax。
参数：
...
本示例演示如何使用 Triton 实现一个融合的 softmax 内核，并针对基于 AMD ROCm（ROCm（Radeon 开放计算平台））和 CDNA（CDNA（计算 DNA 架构））的后端进行架构感知优化。

#### Triton融合softmax的实现[#](#implementation-of-triton-fused-softmax)

Triton提供了一个名为 `fused-softmax` 的参考softmax示例。

基于在线softmax，它简化了最大值数据计算，移除了一次for循环。它还要求编译器通过增加warp数量来使用每行更多线程。这通常是为了获得更好的性能而调整的。最后，它基于GPU硬件属性来设定内核启动方案，从而带来更高的GPU内核占用率和更优的性能。
...
# stride 表示指针向前推进一行需要增加的量
row_start_ptr = input_ptr + row_idx * input_row_stride
# 块大小是大于 n_cols 的下一个2的幂，这样每行可以放入一个块中
col_offsets = tl.arange(0, BLOCK_SIZE)
input_ptrs = row_start_ptr + col_offsets
# 将行加载到 SRAM 中，由于 BLOCK_SIZE 可能大于 n_cols，需要使用掩码
mask = col_offsets < n_cols
row = tl.load(input_ptrs, mask=mask, other=-float('inf'))
# 减去最大值以保证数值稳定性
row_minus_max = row - tl.max(row, axis=0)
# 注意：Triton 中的指数运算快速但近似（即类似 CUDA（CUDA（统一计算设备架构））中的 __expf）
...
# 另一个技巧是要求编译器通过增加每行分布的线程束数（`num_warps`）来使用更多线程
num_warps = 8
# 软件流水线阶段数
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为了最大化占用率，预热内核以获取寄存器使用量，并计算适当的程序数量。
...
使用以下命令安装用于内置 softmax 核函数的 Aiter 核库：
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg='provider', # 参数名，其值对应图中的不同线条
        ...
        ylabel="GB/s", # y 轴标签
        plot_name="Softmax 性能基准测试", # 图名称，也用于保存图文件
...
根据最终性能基准测试结果，Helion不仅简化了高性能GPU kernel的开发，还提供了接近最大性能，甚至优于基于Triton的GPU kernel。
