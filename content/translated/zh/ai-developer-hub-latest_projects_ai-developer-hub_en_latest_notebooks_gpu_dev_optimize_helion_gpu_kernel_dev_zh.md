...
[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个 Python 嵌入式领域特定语言（DSL），用于编写机器学习内核。它编译至 [Triton](https://openai.com/index/triton/)，后者是 OpenAI 为 GPU 及其他设备编程提供的高性能后端。Helion 旨在相比 Triton 提升抽象层级，让编写正确且高效的内核更加容易，同时在自动调优过程中实现更高程度的自动化。

Helion 可以被视为带图块的 PyTorch，或更高层次的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 在自动调优上花费更多时间（约10分钟），因为它会评估由单个 Helion 内核生成的数百种潜在 Triton 实现。这种更大的搜索空间也使内核在不同硬件之间具有更好的性能可移植性。

Helion 受 AMD GPU 支持。本教程演示如何在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上设置 Helion 开发环境、实现 Helion 内核以及使用 Triton 和 Torch 进行基准测试性能。

## Helion 自动调优器[#](#the-helion-autotuner)

Helion的关键区别在于其自动化的提前（AOT）自动调优引擎。在Triton中，开发者需要负责手动定义优化的搜索空间，这要求显式枚举每个待测试的配置——一个繁琐的过程，限制了探索范围。

Helion通过使用隐式搜索空间改变了这一动态。高级语言自动构建一个覆盖所有实现选择的巨大多维搜索空间。例如，单个`hl.tile`

调用隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。因此，一个 Helion 内核定义可以映射到数千种 Triton 配置，使自动调优器能够创建一个更大且更丰富的搜索空间，从而发现更优的配置。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**: 按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认您的设置：该命令会列出您的AMD GPU及相关详细信息。
...
**注意**：确保正确配置 Docker 权限。要配置允许非 root 用户访问的权限，请运行以下命令：  
...
请按以下步骤设置环境、启动Jupyter Notebooks并安装依赖项。
...
在容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或在其启动后上传到 Jupyter Notebook 环境中。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口在您的系统上尚未被使用。如果已被使用，您可以通过替换 `--port=8888` 来指定一个不同的端口。
...
本示例演示如何使用Helion实现逐元素的指数（exp）函数。它利用Helion的tile系统进行并行计算，同时支持前向和反向传播。该实现与PyTorch自动求导系统无缝集成，能够实现高性能、可自动微分的操作。示例还展示了如何对照原生PyTorch指数函数（支持完整梯度）验证该实现的正确性。
...
"""
验证 exp 核函数实现与 PyTorch 原生 exp 函数的一致性。
参数:
n: 测试张量的大小
"""
...
## 3. softmax 算法详解[#](#details-of-the-softmax-algorithm)

softmax函数常用于分类CNN模型，甚至基于transformer的LLM模型。它通过将每个值的指数除以所有指数的总和进行归一化，将原始输出分数（也称为logits）转换为概率。这个过程确保输出值在(0,1)范围内并且总和为1，使其可解释为概率。PyTorch已将softmax函数实现为[一个标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值以及所有指数对应的和，还需要一个额外的for循环来计算最终的softmax结果。因此总共使用了三个循环。Safe Softmax算法在[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)中有更全面的描述。
...
维度是256。输入张量的每一行分配一个程序。这意味着网格大小是
...
程序实例（线程块）扫描张量的一行，并迭代处理当前行的数据块以计算当前行的最终softmax值\(y_i\)。这是第三个for循环。

## 4. 创建 Helion 两遍 softmax kernel[#](#creating-a-helion-two-pass-softmax-kernel)

此示例展示了 softmax 函数的多个 Helion 内核实现，包括围绕 PyTorch softmax 实现的简单封装以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的 PyTorch softmax 函数进行对比，以验证其正确性。
...
"""
数值优化的 Helion 内核，通过两趟扫描实现 softmax。
...
通过将Helion softmax核函数与PyTorch softmax函数进行比较来检查正确性。
...
本节比较Helion与Triton、PyTorch和Aiter的性能。

### 对照组: Triton fused-softmax 和 Aiter softmax[#](#control-group-triton-fused-softmax-and-aiter-softmax)

此示例演示如何使用 Triton 实现一个融合的 softmax 内核，具有对基于 CDNA（CDNA（计算 DNA 架构））后端的 AMD ROCm（ROCm（Radeon 开放计算平台））的架构感知。

#### Triton fused-softmax 实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 样本

基于在线softmax，它简化了最大值数据的计算，从而移除一个for循环。同时，它通过增加线程束数量，让编译器为每行使用更多线程。这通常是为了优化性能而调优的。最后，它根据GPU硬件特性来确定内核启动方案，从而获得更高的GPU内核占用率和更优的性能。
...
# 块大小是比n_cols大的下一个2的幂，这样我们可以将每一行
# 放入单个块中
...
# 注意，Triton中的指数运算快速但近似（即，就像CUDA中的__expf（CUDA（统一计算设备架构）））
...
# 另一个技巧是要求编译器通过增加每行分配的 warp 数量（`num_warps`）来使用更多每行线程。
num_warps = 8
# 软件流水线级数
...
#### 安装 ROCm（Radeon 开放计算平台）Aiter kernel library[#](#install-the-rocm-aiter-kernel-library)

使用以下命令来安装内置softmax内核函数的Aiter内核库：
...
恭喜！通过运行这个Helion GPU内核开发教程，你学会了如何在AMD GPU上开发和优化Helion内核。

根据最终性能基准测试结果，Helion不仅简化了高性能GPU内核开发，还提供了接近最大的性能，甚至超越了基于Triton的GPU内核。

理想情况下，本教程鼓励您在 ROCm（ROCm（Radeon 开放计算平台））和 AMD GPU 上编写、调优、测试 Helion 内核并为其做出贡献，从而共同塑造 AI 加速的未来。