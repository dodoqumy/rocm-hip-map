...
# 用于 AMD GPU 上 GPU 内核开发与评估的 Helion DSL[#](#helion-dsl-for-gpu-kernel-development-and-assessment-on-amd-gpus)

**作者**: Charles Yang

**知识水平**：中级

[Helion](https://github.com/pytorch/helion) 是Meta开发的一种Python嵌入式领域特定语言（DSL），用于编写机器学习内核。它编译为[Triton](https://openai.com/index/triton/)，即OpenAI提供的用于GPU及其他设备编程的高性能后端。与Triton相比，Helion旨在提升抽象层次，使编写正确且高效的内核更加容易，同时在自动调优过程中实现更高程度的自动化。

Helion 既可以视为带分块的 PyTorch，也可以视为更高级的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 花费更多时间（约 10 分钟）进行自动调优，因为它会评估从单个 Helion 内核生成的数百个潜在的 Triton 实现。这种更大的搜索空间也使得内核在不同硬件之间具有更好的性能可移植性。

Helion 受 AMD GPU 支持。本教程演示如何在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上搭建 Helion 开发环境、实现 Helion 内核，并使用 Triton 和 Torch 对性能进行基准测试。

## Helion 自动调优器[#](#the-helion-autotuner)

Helion 的核心差异化优势在于其自动化的提前（AOT）自动调优引擎。而在 Triton 中，开发者需要手动定义优化的搜索空间，即必须明确枚举所有待测试的配置——这一繁琐过程限制了探索范围。

Helion 通过使用隐式搜索空间改变了这种动态。高级语言自动在实现选择上构建一个庞大、多维的搜索空间。例如，一个单独的 `hl.tile`

call 隐式指示自动调优器探索不同的块大小和循环排序，并考虑是否将迭代空间展平为单一维度。因此，一个 Helion 内核定义可映射到数千个 Triton 配置，使自动调优器能够创建更大、更丰富的搜索空间，以便在其中发现更优的配置。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**：按照[ROCm（ROCm（Radeon 开放计算平台）） 安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)进行安装并验证 ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认你的环境设置：该命令列出你的 AMD GPU 及相关细节。
...
**Docker**：确保 Docker 已正确安装和配置。按照适用于您操作系统的 Docker 安装指南进行操作。
...
使用 [Hugging Face](https://huggingface.co) 下载模型。确保 Hugging Face API token 具备必要的权限。

## 1. 使用 Docker 和 ROCm 的环境搭建 [#](#environment-setup-with-docker-and-rocm)

按照以下步骤设置环境、启动Jupyter Notebooks并安装相关依赖项。

### 启动 Docker 容器[#](#launch-the-docker-container)

启动Docker容器。在您的主机上运行以下命令：
...
目录在容器中。请确保在运行 Docker 命令之前，将该 notebook 文件复制到此目录，或在 Jupyter Notebook 环境启动后将其上传。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问该 notebook。你可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此 notebook。
...
在运行上述命令之前，请确保该端口在你的系统上未被使用。如果已被占用，你可以通过替换 `--port=8888` 来指定一个不同的端口。
...
强烈建议在你的项目中使用最新版本的 Helion。AMD 及其他供应商会频繁更新 [Helion](https://github.com/pytorch/helion) 中的优化通道和算法，这有助于提升你的 Helion 内核性能。
...
本示例演示如何使用Helion实现逐元素指数（exp）函数。它利用Helion的分块系统进行并行计算，同时支持前向传播和反向传播。该实现与PyTorch自动求导系统无缝集成，能够实现高性能且可自动微分的运算。示例展示了如何通过完整的梯度支持，与原生PyTorch指数函数进行实现验证。
...
将 exp 内核实现与 PyTorch 的原生 `exp` 进行验证
...
## 3. softmax算法详解[#](#details-of-the-softmax-algorithm)

Softmax 函数常用于分类CNN模型甚至是基于transformer的LLM模型中。它通过将每个值的指数除以所有指数的和进行归一化，将原始输出分数（也称为 logits）转换为概率。这个过程确保输出值在 (0,1) 范围内且总和为1，从而使它们可解释为概率。PyTorch 已将 softmax 函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值数据以及所有指数对应的和，还需要一个额外的for循环来计算最终的softmax结果，因此总共使用了三个循环。Safe Softmax算法在[Online normalizer calculation for softmax](https://arxiv.org/pdf/1805.02867)中有更详细的描述。
...
维度为256。每个输入张量的行分配一个程序。这意味着grid size为
...
本示例展示了 softmax 函数的多个 Helion kernel 实现，包括围绕 PyTorch softmax 实现的简单包装器以及一个经过数值优化的两遍版本。它还包含一个检查函数，用于将这些 kernel 与 PyTorch 内置的 softmax 函数进行对比，从而验证正确性。
...
本示例演示如何使用 Triton 实现融合的 softmax 核函数，并具备对基于 AMD ROCm（ROCm（Radeon 开放计算平台））且采用 CDNA（CDNA（计算 DNA 架构））后端的架构感知能力。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例。

基于在线softmax（online softmax），它简化了最大值计算，从而移除一个for循环。同时，它通过增加warp数量，让编译器为每一行分配更多线程。这一参数通常经过调优以获得更佳性能。最后，它根据GPU硬件属性确定kernel启动方案，从而提升GPU kernel占用率，带来更优性能。
...
# 另一个技巧是要求编译器通过增加每行分布的warp数量（`num_warps`）来为每一行使用更多线程。
num_warps = 8
# 软件流水线阶段数。
...
现在运行所有版本softmax核的基准测试和可视化，以获取结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        x_names=['N'], # 用作折线图 x 轴的参数名
        x_vals=[128 * i for i in range(55, 95)], # `x_name` 的可能取值
        line_arg='provider', # 参数名，其不同值对应图中的不同折线
        line_vals=['helion','triton', 'aiter','torch'], # `line_arg` 的可能取值
        line_names=["Helion Softmax","Triton Softmax", "Aiter Softmax","Torch Softmax"], # 各折线的标签名
        styles=[('red', 'solid'),('cyan', 'solid'), ('black', 'solid'), ('orange', 'dashdot')], # 折线样式
        ylabel="GB/s", # y 轴标签名
        plot_name="Softmax 性能基准测试", # 图表的名称，也用作保存文件的名称
...
根据最终性能基准测试结果，Helion不仅简化了高性能GPU内核的开发，还提供了接近最大的性能，甚至超越了基于Triton的GPU内核。

理想情况下，本教程鼓励您在ROCm（ROCm（Radeon 开放计算平台））和 AMD GPU 上编写、调优、测试并为 Helion 内核做出贡献，从而帮助塑造 AI 加速的未来。