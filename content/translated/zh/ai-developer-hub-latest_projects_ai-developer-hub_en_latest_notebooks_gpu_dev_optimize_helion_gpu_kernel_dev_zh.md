...
[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个嵌入在 Python 中的领域特定语言（DSL），用于编写机器学习内核。它会编译为 [Triton](https://openai.com/index/triton/)，后者是 OpenAI 开发的一个高性能后端，用于 GPU 及其他设备的编程。Helion 的目标是相较于 Triton 提升抽象层级，使得编写正确且高效的内核更加容易，同时在自动调优过程中实现更高程度的自动化。

Helion 可以被视为带有 tiles 的 PyTorch，或视为一个更高层次的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了手动编码的工作量。Helion 花费更多时间（大约 10 分钟）进行自动调优，因为它会评估从单个 Helion kernel 生成的数百种潜在的 Triton 实现。这种更大的搜索空间也使内核在不同硬件之间具有更好的性能可移植性。

Helion 由 AMD GPU 提供支持。本教程演示了如何在 AMD Instinct™ GPU 上搭建 Helion 开发环境、实现 Helion 内核，并使用 Triton 和 Torch 进行性能基准测试。

## Helion 自动调优器[#](#the-helion-autotuner)

Helion 的核心差异化优势在于其自动化的提前编译（AOT）自调优引擎。在 Triton 中，开发者需要手动定义优化的搜索空间，这要求明确列举出所有待测试的配置——一个繁琐且会限制探索范围的过程。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动构建了一个广阔的多维搜索空间，覆盖了实现选择的范围。例如，单个 `hl.tile`

调用会隐式地指示自动调优器探索不同的块大小和循环次序，并考虑是否将迭代空间展平为一维。因此，一个Helion内核定义可以映射到数千种Triton配置，从而使自动调优器能够创建更大、更丰富的搜索空间，以发现更优的配置。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**：按照[ROCm（ROCm（Radeon 开放计算平台））安装指南](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装完成后，使用以下命令确认设置：该命令会列出您的AMD GPU及其相关详细信息。

**注**：对于 ROCm 6.4 及更早版本，请使用 `rocm-smi`

command instead.AMD 还提供了预构建的 ROCm（ROCm（Radeon 开放计算平台）） Docker 镜像，包括一个
...
**注意**：确保 Docker 权限配置正确。要配置允许非 root 用户访问的权限，请运行以下命令：usermod -aG docker $USER newgrp docker
...
使用[Hugging Face](https://huggingface.co)下载模型。确保Hugging Face API token具有必要的权限。
...
强烈建议您在项目中使用最新版本的 Helion。AMD 及其他供应商会频繁更新 [Helion](https://github.com/pytorch/helion) 中的优化传递和算法，这有助于提升您的 Helion 内核性能。
...
这个示例演示了如何使用Helion实现逐元素指数（exp）函数。它利用Helion的分片系统进行并行计算，实现了前向和反向传播。该实现与PyTorch的自动求导系统无缝集成，支持高性能、自动可微的操作。该示例展示了如何验证该实现与原生PyTorch指数函数（含完整梯度支持）的一致性。
...
softmax 函数常用于分类 CNN 模型，甚至基于 transformer 的 LLM 模型。它通过将每个值的指数除以所有指数之和进行归一化，从而将原始输出分数（即 logits）转换为概率。该过程确保输出值范围在 (0,1) 内且总和为 1，使其可解释为概率。PyTorch 已将 softmax 函数实现为 [标准 API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。

函数 \(y = Softmax(x)\) 的定义是：

其中 \(x,y \in \mathbb{R}^V\).

### 简单版本：Safe Softmax[#](#naive-version-safe-softmax)
...
该kernel在8192x8192 tensor上的性能计算如下：

块大小

`col`

维度是256。每行输入张量分配一个程序。这意味着网格大小是
...
该示例展示了softmax函数的多个Helion内核实现，包括一个围绕PyTorch softmax实现的简单封装器，以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的PyTorch softmax函数进行比较，以验证正确性。
...
通过与 PyTorch softmax 函数对比 Helion softmax kernel，检查其正确性。

```python
# %%
def check(m: int, n: int) -> None:
    """
    运行正确性检查，将 Helion 的 softmax 内核与 PyTorch 的 softmax 进行比较。
    参数:
        m (int): 输入张量的行数。
        n (int): 输入张量的列数。
    """
...
### 对照实验组：Triton fused-softmax 和 Aiter softmax[#](#control-group-triton-fused-softmax-and-aiter-softmax)

本示例演示了如何使用Triton实现融合softmax内核，并针对基于CDNA（计算DNA架构）后端的AMD ROCm（Radeon开放计算平台）进行架构感知优化。

#### Triton融合softmax的实现[#](#implementation-of-triton-fused-softmax)

Triton提供了一个名为`fused-softmax`的参考softmax示例

基于在线 softmax，它简化了最大值数据计算，去除了一个 for 循环。同时，通过增加 warp 数量，它要求编译器为每行使用更多线程。这一过程通常经过调优以提升性能。最后，它基于 GPU 硬件属性设计内核启动方案，从而获得更高的 GPU 内核占用率和更佳性能。
...
# 注意，Triton中的指数运算快速但近似（即，类比CUDA（CUDA（统一计算设备架构））中的__expf）
...
# 另一个技巧是让编译器通过增加每一行分布的线程束数量（`num_warps`）来使用更多每行的线程。
num_warps = 8
# 软件流水线阶段数。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="GB/s", # y轴的标签名
        plot_name="Softmax Performance Benchamrk", # 图表的名称，也用作保存图表的文件名
...
根据最终的性能基准测试结果，Helion 不仅简化了高性能 GPU 内核开发，而且能够提供接近极致的性能，甚至超越了基于 Triton 的 GPU 内核。

理想情况下，本教程鼓励你在 ROCm（Radeon 开放计算平台）和 AMD GPU 上编写、调优、测试并贡献 Helion kernel，助力塑造 AI 加速的未来。