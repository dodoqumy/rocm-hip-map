...
[Helion](https://github.com/pytorch/helion) 是 Meta 开发的一种嵌入式 Python 领域特定语言 (DSL)，用于编写机器学习内核。它编译为 [Triton](https://openai.com/index/triton/)，即 OpenAI 提供的一个高性能后端，用于对 GPU 和其他设备进行编程。与 Triton 相比，Helion 旨在提高抽象层次，使编写正确且高效的内核更加容易，同时实现自动调优过程中更高程度的自动化。

Helion可以看作带有tiles的PyTorch，或者更高级的Triton应用。与Triton相比，Helion通过自动调优减少了手动编码的工作量。Helion花费更多时间（大约10分钟）进行自动调优，因为它要从单个Helion内核生成的数百种潜在Triton实现中进行评估。这种更大的搜索空间也使得内核在不同硬件之间具有更好的性能可移植性。

Helion 得到 AMD GPU 的支持。本教程演示如何在 AMD Instinct（Instinct（AMD 数据中心 GPU 系列））™ GPU 上设置 Helion 开发环境、实现 Helion 内核以及使用 Triton 和 Torch 进行性能基准测试。

## Helion自动调优器[#](#the-helion-autotuner)

Helion的关键区别在于其自动化的提前（AOT）自动调优引擎。在Triton中，开发者需要手动定义优化的搜索空间。这要求明确列举出每个待测试的配置，这是一个繁琐的过程，限制了探索的范围。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动构建了一个涵盖实现选择的广阔多维搜索空间。例如，单个 `hl.tile`

`call` 隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展开为单维度。因此，一个 Helion 内核定义可以映射到数千种 Triton 配置，从而使自动调优器能够创建更大、更丰富的搜索空间，以发现更优的配置。
...
**Ubuntu 22.04/24.04**：请确保您的系统运行的是 Ubuntu 22.04 或 24.04。

### 硬件[#](#hardware)

**AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU**：本教程已在 AMD Instinct（AMD 数据中心 GPU 系列）MI300X GPU 上完成测试。请确保您使用的 AMD Instinct（AMD 数据中心 GPU 系列）GPU 支持 ROCm（Radeon 开放计算平台），且您的系统满足[官方要求](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/system-requirements.html)。
...
请使用命令代替。AMD 还提供了预构建的 ROCm（ROCm（Radeon 开放计算平台）） Docker 镜像，包括一个
...
在运行上述命令之前，请确保该端口尚未在您的系统上使用。如果已在使用，您可以通过替换`--port=8888`来指定不同的端口。
...
此示例演示如何使用 Helion 实现逐元素的指数（exp）函数。它利用 Helion 的分块系统进行并行计算，同时支持前向和反向传播。该实现与 PyTorch 的自动求导系统无缝集成，从而实现高性能、自动可微分的运算。示例展示了如何将实现与原生 PyTorch 的指数函数进行对比验证，并支持完整的梯度计算。
...
验证 exp 核函数实现与 PyTorch 的原生 `exp` 函数的一致性。
...
softmax 函数常用于分类 CNN 模型乃至基于 transformer 的 LLM 模型中。它通过计算每个值的指数，并除以所有指数的总和，将原始输出分数（即 logits）归一化为概率。这一过程确保了输出值范围在 (0,1) 之间且总和为 1，从而可解释为概率。PyTorch 已将 softmax 函数实现为[标准 API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
为了达到数值稳定性，在计算指数之前，从每个输入元素中减去该行向量的最大值。因此定义变为：

其中 \(x,y \in \mathbb{R}^V\)。这就是所谓的 Safe Softmax 算法。

根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值数据以及所有指数对应的和，并额外需要一个for循环来计算最终的softmax结果，因此总共使用了三个循环。Safe Softmax算法在《Online normalizer calculation for softmax》（[https://arxiv.org/pdf/1805.02867](https://arxiv.org/pdf/1805.02867)）中有更完整的描述。

这个核函数在 8192x8192 张量上的性能计算如下：
...
该示例展示了softmax函数的多个Helion内核实现，包括一个围绕PyTorch softmax实现的简单包装器和一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的PyTorch softmax函数进行比较，以验证正确性。
...
通过比较 Helion softmax kernels 与 PyTorch softmax 函数来检查正确性。
...
本示例演示如何使用Triton实现一个融合的softmax内核，并针对基于CDNA（计算DNA架构）的AMD ROCm（ROCm（Radeon开放计算平台））后端进行架构感知。

#### Triton fused-softmax 的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的 softmax 参考示例。

基于在线softmax，它简化了最大值数据的计算，从而移除了一层for循环。同时，通过增加每个数据行的线程束数量，它要求编译器为每行使用更多线程。这一过程通常经过调优以提升性能。最后，它将内核启动方案建立在GPU硬件属性之上，从而实现了更高的GPU内核占用率和更佳的性能。
...
# 另一个技巧是通过增加每行分配的线程束数量（`num_warps`）来让编译器每行使用更多线程。
num_warps = 8
# 软件流水线阶段数。
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用率受寄存器使用限制。为了最大化占用率，请预热内核以获取寄存器使用情况，并计算合适的程序数量。
...
现在运行所有版本的softmax核的基准测试和可视化以获取结果。
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="GB/s", # y轴的标签名
        plot_name="Softmax性能基准测试", # 图表名称。也用作保存图表的文件名。
...
根据最终性能基准测试结果，Helion 不仅简化了高性能 GPU 内核的开发，而且提供了接近极致的性能，甚至超越了基于 Triton 的 GPU 内核。

理想情况下，本教程鼓励您在 ROCm（ROCm（Radeon 开放计算平台））和 AMD GPU 上编写、调优、测试和贡献 Helion 内核，从而帮助塑造人工智能加速的未来。