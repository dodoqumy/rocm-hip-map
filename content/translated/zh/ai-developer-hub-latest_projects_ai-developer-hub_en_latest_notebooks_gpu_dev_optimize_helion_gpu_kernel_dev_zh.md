...
[Helion](https://github.com/pytorch/helion) 是 Meta 推出的一个用于编写机器学习内核的 Python 嵌入式领域特定语言（DSL）。它编译为 [Triton](https://openai.com/index/triton/)，一个来自 OpenAI 的高性能后端，用于对 GPU 及其他设备进行编程。与 Triton 相比，Helion 旨在提高抽象层次，使编写正确且高效的内核更加容易，同时在自动调优过程中实现更高的自动化程度。

Helion 既可以看作带有分块（tiles）的 PyTorch，也可以视为更高层次的 Triton 应用。与 Triton 相比，Helion 通过自动调优减少了人工编码工作量。Helion 花费更多时间（约 10 分钟）进行自动调优，因为它会评估由单个 Helion 内核生成的数百种潜在 Triton 实现。更大的搜索空间也使得内核在不同硬件之间具有更好的性能可移植性。

Helion 支持 AMD GPU。本教程演示如何在 AMD Instinct™ GPU 上设置 Helion 开发环境、实现 Helion 内核，并基准测试 Triton 和 Torch 的性能。

## Helion 自动调谐器[#](#the-helion-autotuner)

Helion 的关键差异化特性在于其自动化的提前编译（AOT）自动调优引擎。在 Triton 中，开发者需要手动定义优化的搜索空间。这要求显式枚举每个待测试的配置，这一繁琐的过程限制了探索的广度。

Helion 通过使用隐式搜索空间改变了这一动态。高级语言自动构建一个涵盖实现选择的广阔多维搜索空间。例如，单个 `hl.tile`

调用会隐式指示自动调优器探索不同的块大小和循环顺序，并考虑是否将迭代空间展平为一维。因此，一个Helion内核定义可以映射到数千种Triton配置，使自动调优器能够创建更大、更丰富的搜索空间，从而发现更优的配置。
...
**ROCm（ROCm（Radeon 开放计算平台）） 7.0**: 按照[ROCm（ROCm（Radeon 开放计算平台）） install guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html)安装并验证ROCm（ROCm（Radeon 开放计算平台））。安装后，使用以下命令确认您的设置：此命令列出您的AMD GPU及其相关信息。

**注意**：对于 ROCm（ROCm（Radeon 开放计算平台）） 6.4 及更早版本，请使用 `rocm-smi`

而是使用命令。AMD还提供了预构建的ROCm（ROCm（Radeon 开放计算平台））Docker镜像，包括一个

[ROCm（ROCm（Radeon开放计算平台）） PyTorch镜像](https://hub.docker.com/r/rocm/pytorch)、[ROCm（ROCm（Radeon开放计算平台）） Ubuntu 22.04镜像](https://hub.docker.com/r/rocm/dev-ubuntu-22.04) 和 [ROCm（ROCm（Radeon开放计算平台）） Ubuntu 24.04镜像](https://hub.docker.com/r/rocm/dev-ubuntu-24.04)。您可以使用这些预构建的Docker镜像来减少搭建ROCm（ROCm（Radeon开放计算平台））环境所需的工作量。

**Docker**：确保已正确安装和配置Docker。请遵循适用于您操作系统的Docker安装指南。

**注意**：确保已正确配置Docker权限。若要为非root用户配置访问权限，请运行以下命令：
...
## 1. 使用Docker和ROCm进行环境设置（ROCm（Radeon开放计算平台））[#](#environment-setup-with-docker-and-rocm)
...
在容器中的目录。确保在运行 Docker 命令之前将笔记本文件复制到此目录，或者在 Jupyter Notebook 环境启动后上传到该环境。保存终端输出中提供的令牌或 URL，以便从 Web 浏览器访问笔记本。您可以从 [AI Developer Hub GitHub repository](https://github.com/ROCm（ROCm（Radeon 开放计算平台））/gpuaidev) 下载此笔记本。
...
在运行上述命令之前，请确保该端口未在您的系统上被占用。如果已被占用，您可以通过替换 `--port=8888` 来指定其他端口。
...
强烈建议您在项目中使用最新版本的Helion。AMD及其他供应商会定期更新[Helion](https://github.com/pytorch/helion)中的优化步骤和算法，这有助于提升您的Helion内核性能。
...
使用以下命令安装 Helion、Triton 以及其他依赖项。
...
该示例演示了如何使用Helion实现逐元素的指数（exp）函数。它利用Helion的分块系统实现了前向和反向传播，用于并行计算。该实现无缝集成了PyTorch的自动求导系统，支持高性能、可自动求导的操作。示例展示了如何针对完整的梯度支持，验证实现与原版PyTorch指数函数的一致性。
...
softmax函数常用于分类CNN模型，甚至基于Transformer的LLM模型。它通过归一化每个值的指数（即除以所有指数之和），将原始输出分数（也称为logits）转换为概率。这一过程确保输出值在(0,1)范围内且总和为1，从而可解释为概率。PyTorch已将softmax函数实现为[标准API](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html)。
...
为了达到数值稳定性，在计算每个输入元素的指数之前，减去行向量的最大值。因此定义变为：

其中\(x,y \in \mathbb{R}^V\)。这就是所谓的Safe Softmax算法。

根据softmax算法定义，Triton内核实现了朴素版本（公式2）。该内核需要两个for循环来获取最大值及所有指数对应的总和，还需要一个额外的for循环来计算最终的softmax结果。因此，总共使用了三个循环。安全Softmax算法在《Online normalizer calculation for softmax》（https://arxiv.org/pdf/1805.02867）中有更详细的描述。
...
此示例展示了 softmax 函数的多种 Helion 内核实现，包括对 PyTorch softmax 实现的简单包装器，以及一个数值优化的两遍版本。它还包含一个检查函数，用于将这些内核与内置的 PyTorch softmax 函数进行比较，以验证正确性。
...
本节比较了 Helion 与 Triton、PyTorch 和 Aiter 的性能表现。

### 对照组：Triton fused-softmax 和 Aiter softmax[#](#control-group-triton-fused-softmax-and-aiter-softmax)

本示例演示了如何使用 Triton 实现一个融合的 softmax 内核，并针对基于 CDNA（计算 DNA 架构）的后端具有架构感知能力，适用于 AMD ROCm（Radeon 开放计算平台）。

#### Triton融合softmax的实现[#](#implementation-of-triton-fused-softmax)

Triton 提供了一个名为 `fused-softmax` 的参考 softmax 示例。

基于在线softmax，它简化了最大数据计算，从而减少了一个for循环。它还通过增加warp数量，要求编译器每行使用更多线程。这通常经过调优以达到更佳性能。最后，它根据GPU硬件属性确定内核启动方案，从而提高了GPU内核占用率和性能。
...
# 块大小是大于n_cols的下一个2的幂，以便每行可放入一个块中
...
# 注意：Triton中的指数运算快速但近似（即，类似CUDA中的__expf（CUDA（统一计算设备架构）））
...
# 另一个技巧是通过增加分配每行的线程束数量（`num_warps`），让编译器为每行使用更多线程
num_warps = 8
# 软件流水线阶段数
num_stages = 4 if SIZE_SMEM > 200000 else 2
print(f"BLOCK_SIZE: {BLOCK_SIZE}, num_warps: {num_warps}, num_stages: {num_stages}")
# 内核的占用量受寄存器使用限制。为最大化占用量，预热内核以获取寄存器使用情况，并计算合适的程序数量。
# 预编译内核以获取寄存器使用并计算线程占用量
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg='provider', # 对应图中不同线条的自变量名称
...
祝贺！通过运行本Helion GPU内核开发教程，您学习了如何在AMD GPU上开发和优化Helion内核。

根据最终的性能基准测试结果，Helion不仅简化了高性能GPU kernel的开发，还能提供接近最优的性能，甚至优于基于Triton的GPU kernel。

理想情况下，本教程鼓励您在 ROCm（ROCm（Radeon 开放计算平台））和 AMD GPU 上编写、调优、测试并为 Helion 内核做出贡献，帮助塑造 AI 加速的未来。