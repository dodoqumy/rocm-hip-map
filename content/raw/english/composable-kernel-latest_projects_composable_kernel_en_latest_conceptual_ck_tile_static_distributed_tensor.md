---
title: "Static Distributed Tensor &#8212; Composable Kernel 1.2.0 Documentation"
source_url: "https://rocm.docs.amd.com/projects/composable_kernel/en/latest/conceptual/ck_tile/static_distributed_tensor.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-02T21:10:20.101452+00:00
content_hash: "a715073d1087224b"
---
...
Each thread in a workgroup owns a portion of the overall tensor data, stored in its registers or local memory. The distribution pattern follows the [tile distribution](tile_distribution.html#ck-tile-tile-distribution) rules, ensuring that collective operations across all threads reconstruct the complete logical tensor while individual threads operate only on their local portions.
...
## Thread-Local Storage Model[#](#thread-local-storage-model)
...
**Contiguous Storage**: Each thread’s data is stored in a contiguous array, optimizing register allocation and enabling vectorized operations.**Deterministic Mapping**: The Y-coordinate to buffer offset mapping is computed at compile time, eliminating runtime overhead.**Alignment Guarantees**: The storage layout respects hardware alignment requirements for efficient memory operations.

## Memory Layout and Access Patterns[#](#memory-layout-and-access-patterns)
...
## Element Access and Indexing[#](#element-access-and-indexing)
...
**Compile-Time Resolution**: When indices are known at compile time, the compiler can optimize away all indexing calculations.**Vectorized Access**: Accessing multiple elements as vectors enables efficient register-to-register transfers.**Boundary Checking**: Debug builds include automatic boundary checking to catch indexing errors early.

## Thread Coordination and Synchronization[#](#thread-coordination-and-synchronization)
...
**Accumulation**: Each thread maintains partial results that combine to form the final answer.**Scatter/Gather**: Threads can efficiently reorganize data through coordinated read/write patterns.**Reduction**: Tree-based reduction algorithms naturally map to the distributed storage model.
...
Optimizing static distributed tensor usage requires understanding several [performance factors](hardware/gpu_basics.html#ck-tile-gpu-basics):
...
**Memory Coalescing**: When loading/storing distributed tensors, ensure access patterns promote coalescing. See [Intro to AMD CDNA Architecture](hardware/gpu_basics.html#ck-tile-gpu-basics) for more information about coalescing.
...
## Integration with CK Tile Ecosystem[#](#integration-with-ck-tile-ecosystem)
...
**Automatic Distribution**: The[tile distribution](tile_distribution.html#ck-tile-tile-distribution)system handles all thread-to-data mapping**Register Efficiency**: Thread-local storage maps directly to registers when possible**Zero-Overhead Abstraction**: All distribution logic resolves at compile time**Seamless Integration**: Works naturally with[tile windows](tile_window.html#ck-tile-tile-window),[descriptors](descriptors.html#ck-tile-descriptors), and other CK Tile components**Performance Transparency**: The storage model makes performance characteristics clear and predictable
