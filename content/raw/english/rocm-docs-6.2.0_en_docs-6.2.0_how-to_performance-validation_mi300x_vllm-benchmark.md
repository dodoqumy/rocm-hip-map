...
source_url: "https://rocm.docs.amd.com/en/docs-6.2.0/how-to/performance-validation/mi300x/vllm-benchmark.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-04T16:22:56.383316+00:00
content_hash: "328fef288ae77e95"
---
...
[vLLM inference](../../llm-fine-tuning-optimization/llm-inference-frameworks.html#fine-tuning-llms-vllm) and [vLLM performance optimization](../../tuning-guides/mi300x/workload.html#mi300x-vllm-optimization) for more
information.
...
To optimize performance, disable automatic NUMA balancing. Otherwise, the GPU might hang until the periodic balancing is finalized. For more information, see

[AMD Instinct MI300X system optimization](../../system-optimization/mi300x.html#mi300x-disable-numa).# disable automatic NUMA balancing sh -c 'echo 0 > /proc/sys/kernel/numa_balancing' # check if NUMA balancing is disabled (returns 0 if disabled) cat /proc/sys/kernel/numa_balancing 0

Download the

[ROCm vLLM Docker image](#vllm-benchmark-unified-docker).Use the following command to pull the Docker image from Docker Hub.
...
Refer to the [Standalone benchmarking](#vllm-benchmark-standalone) section.
...
[Docker container](#vllm-benchmark-get-started) as shown in the following
snippet.
...
See [Options](#vllm-benchmark-standalone-options) for the list of
...
See the [examples](#vllm-benchmark-run-benchmark) for more information.
...
### Running the benchmark on the MI300X accelerator[#](#running-the-benchmark-on-the-mi300x-accelerator)
...
[AMD Instinct MI300X workload optimization](../../tuning-guides/mi300x/workload.html).To learn more about the options for latency and throughput benchmark scripts, see

[ROCm/vllm](https://github.com/ROCm/vllm/tree/main/benchmarks).To learn more about system settings and management practices to configure your system for MI300X accelerators, see

[AMD Instinct MI300X system optimization](../../system-optimization/mi300x.html).To learn how to run LLM models from Hugging Face or your own model, see

[Using ROCm for AI](../../rocm-for-ai/index.html).To learn how to optimize inference on LLMs, see

[Fine-tuning LLMs and inference optimization](../../llm-fine-tuning-optimization/index.html).For a list of other ready-made Docker images for ROCm, see the

[Docker image support matrix](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-6.2.0/reference/docker-image-support-matrix.html).
