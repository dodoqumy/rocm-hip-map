---
title: "Fine-tuning and inference using multiple accelerators"
source_url: "https://rocm.docs.amd.com/en/docs-7.0.0/how-to/rocm-for-ai/fine-tuning/multi-gpu-fine-tuning-and-inference.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-01T12:05:51.294701+00:00
content_hash: "cdc5abc7a2f3d071"
---

# Fine-tuning and inference using multiple accelerators[#](#fine-tuning-and-inference-using-multiple-accelerators)
...
[Single-accelerator fine-tuning](single-gpu-fine-tuning-and-inference.html) for a single accelerator or GPU setup.
...
[PyTorch installation guide](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-7.0.0/install/3rd-party/pytorch-install.html). For consistent installation, it’s recommended to use official ROCm prebuilt Docker images with the framework pre-installed.In the Docker container, check the availability of ROCM-capable accelerators using the following command.
...
During training and inference, you can check the memory usage by running the `rocm-smi`
...
integrated with [Transformers](https://huggingface.co/docs/transformers/en/index) allowing you to scale your PyTorch
...
Here, let’s reuse the code in [Single-accelerator fine-tuning](single-gpu-fine-tuning-and-inference.html#fine-tuning-llms-single-gpu-download-model-dataset)
...
[torchtune](https://pytorch.org/torchtune/main/) is a PyTorch-native library for easy single and multi-accelerator or
...
tune [-h] {download,ls,cp,run,validate} ... Welcome to the TorchTune CLI! options: -h, --help show this help message and exit subcommands: {download,ls,cp,run,validate}
...
download meta-llama/Llama-2-7b-hf --output-dir /tmp/Llama-2-7b-hf --hf-token
...
on four devices, run the following command:run --nnodes 1 --nproc_per_node 4 lora_finetune_distributed --config llama2/7B_lora

If successful, you should something like the following output:

is enabled. Instantiating Model on CPU for Rank 0 ... INFO:torchtune.utils.logging:Model instantiation took 7.32 secs INFO:torchtune.utils.logging:Memory Stats after model init: {'peak_memory_active': 9.478172672, 'peak_memory_alloc': 8.953868288, 'peak_memory_reserved': 11.112808448} INFO:torchtune.utils.logging:Optimizer and loss are initialized. INFO:torchtune.utils.logging:Dataset and Sampler are initialized. INFO:torchtune.utils.logging:Learning rate scheduler is initialized. 1|111|Loss: 1.5790324211120605: 7%|█ | 114/1618


Read more about inference frameworks in [LLM inference frameworks](../inference/llm-inference-frameworks.html).
