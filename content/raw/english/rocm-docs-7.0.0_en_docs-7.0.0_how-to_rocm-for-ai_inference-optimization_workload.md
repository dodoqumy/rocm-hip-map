...
source_url: "https://rocm.docs.amd.com/en/docs-7.0.0/how-to/rocm-for-ai/inference-optimization/workload.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-02T18:03:32.496770+00:00
content_hash: "f40e7c92ea54660b"
---
...
[ROCm Compute Profiler](#mi300x-rocprof-compute) offer detailed insights
...
inference on AMD Instinct MI300X accelerators. See [vLLM inference](../inference/llm-inference-frameworks.html#fine-tuning-llms-vllm)
...
[tensor-parallel-size parameter](#mi300x-vllm-multiple-gpus)to distribute tensor computations across multiple GPUs. Adjust parameters such as`batch-size`
...
For more information, see [vLLM inference performance testing](../inference/benchmark-docker/vllm.html).
...
[ROCProfiler](https://rocm.docs.amd.com/projects/rocprofiler/en/docs-7.0.0/index.html)documentation.ROCm Compute Profiler builds upon ROCProfiler but provides more guided analysis. For more information, see

[ROCm Compute Profiler documentation](https://rocm.docs.amd.com/projects/rocprofiler-compute/en/docs-7.0.0/index.html).

Refer to [Profiling and debugging](profiling-and-debugging.html)
...
[ROCProfiler](https://rocm.docs.amd.com/projects/rocprofiler/en/docs-7.0.0/index.html) is primarily a low-level API for accessing and extracting GPU hardware performance
...
utility also depends on the [ROCTracer and ROC-TX libraries](https://rocm.docs.amd.com/projects/roctracer/en/docs-7.0.0/index.html), giving it the
...
[ROCm Compute Profiler](https://rocm.docs.amd.com/projects/rocprofiler-compute/en/docs-7.0.0/index.html) is a system performance profiler for high-performance computing (HPC) and
machine learning (ML) workloads using Instinct accelerators. Under the hood, ROCm Compute Profiler uses
[ROCProfiler](#mi300x-rocprof) to collect hardware performance counters. The ROCm Compute Profiler tool performs
...
[ROCm Systems Profiler](https://rocm.docs.amd.com/projects/rocprofiler-systems/en/docs-7.0.0/index.html) is a comprehensive profiling and tracing tool for parallel applications,
...
its performance and ease of use. See [vLLM inference](../inference/llm-inference-frameworks.html#fine-tuning-llms-vllm) for a primer on vLLM with ROCm.

### Performance environment variables[#](#performance-environment-variables)
...
[Environment variables (MI300X)](https://instinct.docs.amd.com/projects/amdgpu-docs/en/latest/system-optimization/mi300x.html#environment-variables), the environment variable`HIP_FORCE_DEV_KERNARG`
...
to increase the number of channels on MI300X to potentially improve performance.Set the environment variable

`TORCH_BLAS_PREFER_HIPBLASLT=1`

to use hipBLASLt to improve performance.

### Auto-tuning using PyTorch TunableOp[#](#auto-tuning-using-pytorch-tunableop)
...
Learn more about TunableOp in the [PyTorch TunableOp](#mi300x-tunableop) section.

### Performance tuning based on vLLM engine configurations[#](#performance-tuning-based-on-vllm-engine-configurations)
...
for additional performance tips. [vLLM inference](../inference/llm-inference-frameworks.html#fine-tuning-llms-vllm) describes vLLM
usage with ROCm.
...
[vLLM inference performance testing](../inference/benchmark-docker/vllm.html).

### Evaluating performance by throughput measurement[#](#evaluating-performance-by-throughput-measurement)
...
Refer to the benchmarking script located at `benchmarks/benchmark_throughput.py`

in the [vLLM repository](https://github.com/ROCm/vllm/blob/main/benchmarks/benchmark_throughput.py).
...
. This dataset includes real-world conversational data, making it a good representation of typical use cases for language models. Download it using the following command:`https://huggingface.co/datasets/anon8231489123/ShareGPT_Vicuna_unfiltered/resolve/main/ShareGPT_V3_unfiltered_cleaned_split.json`
...
### Maximizing vLLM instances on a single node[#](#maximizing-vllm-instances-on-a-single-node)
...
The total throughput achieved by running `N`

instances of vLLM is generally much higher than running a
single vLLM instance across `N`

GPUs simultaneously (that is, configuring `tensor_parallel_size`

as N or
using the `-tp`

N option, where `1 < N ≤ 8`
...
### Configure the gpu_memory_utilization parameter[#](#configure-the-gpu-memory-utilization-parameter)

There are two ways to increase throughput by configuring `gpu-memory-utilization`
...
is 0.9. You can set it to`>0.9`

and`<1`

.For example, below benchmarking command set the

`gpu-memory-utilization`

as 0.98, or 98%.--gpu-memory-utilization 0.98 --input-len 1024 --output-len 128 --model /path/to/model

Decrease

`gpu-memory-utilization`

to maximize the number of vLLM instances on the same GPU.Specify GPU memory utilization to run as many instances of vLLM as possible on a single GPU. However, too many instances can result in no memory for KV-cache. For small models, run multiple instances of vLLM on the same GPU by specifying a smaller

`gpu-memory-utilization`

– as long as it would not cause HIP Out Of Memory.For example, run two instances of the Llama3-8b model at the same time on a single GPU by specifying

`--gpu-memory-utilization`

to 0.4 (40%) as follows (on GPU`0`

):CUDA_VISIBLE_DEVICES=0 python3 /vllm-workspace/benchmarks/benchmark_throughput.py --gpu-memory-utilization 0.4 --dataset "/path/to/dataset/ShareGPT_V3_unfiltered_cleaned_split.json" --model /path/to/model & CUDA_VISIBLE_DEVICES=0 python3 /vllm-workspace/benchmarks/benchmark_throughput.py --gpu-memory-utilization 0.4 --dataset "/path/to/dataset/ShareGPT_V3_unfiltered_cleaned_split.json" --model /path/to/model &


See [vLLM engine arguments](#vllm-engine-args) for other performance suggestions.
...
option to
specify multiple GPUs. Optionally, use the `CUDA_VISIBLE_DEVICES`
...
different GPUs by specifying different ports for each server and use `CUDA_VISIBLE_DEVICES`
...
### Choose an attention backend[#](#choose-an-attention-backend)
...
Refer to [Model acceleration libraries](model-acceleration-libraries.html#acceleration-flash-attention)
...
[vLLM’s engine arguments documentation](https://docs.vllm.ai/en/latest/serving/engine_args.html)
for a full list of configurable engine arguments.

#### Configure the max-num-seqs parameter[#](#configure-the-max-num-seqs-parameter)

Increase the `max-num-seqs`

parameter from the default `256`
...
#### Use the float16 dtype[#](#use-the-float16-dtype)

The default data type (`dtype`

) is specified in the model’s configuration file. For instance, some models use `torch.bfloat16`

as their default `dtype`
...
Setting `num-scheduler-steps`

for multi-step scheduling can increase performance. Set it between 10 to 15 (`--num-scheduler-steps 10`

).

#### Distributed executor backend[#](#distributed-executor-backend)

The vLLM supports two modes of distributed executor backend: `ray`

and `mp`

. When using the [ROCm/vllm](https://github.com/ROCm/vllm) fork, using the `mp`

backend (`--distributed_executor_backend mp`

) is recommended.

#### Graph mode max-seq-len-to-capture[#](#graph-mode-max-seq-len-to-capture)

Maximum sequence length covered by CUDA graphs. In the default mode (where `enforce_eager`
...
When working with models that support long context lengths, set the parameter `--max-seq-len-to-capture`

to 16384.
See this [vLLM blog](https://blog.vllm.ai/2024/10/23/vllm-serving-amd.html) for details.

An example of long context length model is Qwen2-7b.

#### Whether to enable chunked prefill[#](#whether-to-enable-chunked-prefill)
...
quantization are supported for ROCm.

#### FP8 quantization[#](#fp8-quantization)

[ROCm/vllm](https://github.com/ROCm/vllm) supports FP8 (8-bit floating point) weight and activation quantization using hardware acceleration on the Instinct MI300X.
Quantization of models with FP8 allows for a 2x reduction in model memory requirements and up to a 1.6x improvement in throughput with minimal impact on accuracy.

AMD publishes Quark Quantized OCP FP8 models on Hugging Face. For example:

To enable vLLM benchmarking to run on fp8 quantized models, use the `--quantization`

parameter with value `fp8`
...
To enable vLLM to run on `awq`

quantized models, using `--quantization`
...
#### fp8 kv-cached-dtype[#](#fp8-kv-cached-dtype)
...
parameter. If the parameter is not specified,
...
Cache](https://github.com/vllm-project/llm-compressor/blob/main/examples/quantization_kv_cache/README.md)
...
[Dockerfile.rocm](https://github.com/vllm-project/vllm/blob/main/docker/Dockerfile.rocm)
for `llama2-70b`
...
## PyTorch inductor max-autotune tuning knobs[#](#pytorch-inductor-max-autotune-tuning-knobs)
...
Experimental: TunableOp (see the [PyTorch TunableOp](#mi300x-tunableop) section) can also be used in combination
with `TorchInductor`

`max-autotune`

mode to boost ATen GEMM performance but will further increase tuning time.
The environment variable `TORCHINDUCTOR_AUTOTUNE_MULTI_DEVICE=1`

can be useful in single GPU workloads to distribute Triton GEMM tuning.

### Triton backend[#](#triton-backend)

The goal is to leverage Triton to achieve better performance. To tune Triton kernels with `gemm`
...
### Composable Kernel backend[#](#composable-kernel-backend)
...
`torch._inductor.max_autotune_gemm_backends`
...
```
install git+https://github.com/rocm/composable_kernel@develop
```
...
##### A quick view of the config YAML[#](#a-quick-view-of-the-config-yaml)
...
[hipBLASLt](https://rocm.docs.amd.com/projects/hipBLASLt/en/docs-7.0.0/index.html) has a backend assembly generator in
...
### TensileLite tuning flow[#](#tensilelite-tuning-flow)
...
final step ([step 7](#tensilelite-tuning-step-7)), these candidates are benchmarked against a predefined set
of problem sizes.

#### Step 1: Initial solution parameters[#](#step-1-initial-solution-parameters)

Before Tensile is able to benchmark a kernel parameter in Step 2 of the [preceding figure](#tensilelite-tuning-flow-fig),
...
#### Step 2: Benchmark common parameters[#](#step-2-benchmark-common-parameters)
...
#### Step 3: Fork parameters[#](#step-3-fork-parameters)
...
which is fastest. The [preceding figure](#tensilelite-tuning-flow-fig) shows 7 kernels being forked in Step 3.

#### Step 4: Benchmark fork parameters[#](#step-4-benchmark-fork-parameters)

Next, tuning continues its refinement by determining fastest parameters for each forked permutation, same as in Step 2.

#### Step 5: Join parameters[#](#step-5-join-parameters)
...
#### Step 6: Benchmark join parameters[#](#step-6-benchmark-join-parameters)
...
#### Step 7: Benchmark final parameters[#](#step-7-benchmark-final-parameters)
...
#### Tensile optimization and performance tuning tips[#](#tensile-optimization-and-performance-tuning-tips)
...
See [Optimizing with Composable Kernel](optimizing-with-composable-kernel.html)
...
What does [PerfDb](https://rocm.docs.amd.com/projects/MIOpen/en/docs-7.0.0/conceptual/perfdb.html) look like?
...
See [Using the performance database](https://rocm.docs.amd.com/projects/MIOpen/en/docs-7.0.0/conceptual/perfdb.html) for more information.
...
What does [FindDb](https://rocm.docs.amd.com/projects/MIOpen/en/docs-7.0.0/conceptual/finddb.html) look like?
...
See [Using the find APIs and immediate mode](https://rocm.docs.amd.com/projects/MIOpen/en/docs-7.0.0/how-to/find-and-immediate.html) for more information.
...
[RCCL](https://rocm.docs.amd.com/projects/rccl/en/docs-7.0.0/index.html) is a stand-alone library of standard collective
...
### Auto-tunable kernel configurations[#](#auto-tunable-kernel-configurations)
...
[Performance guidelines](https://rocm.docs.amd.com/projects/HIP/en/docs-7.0.0/how-to/performance_guidelines.html) section of the
HIP documentation.
...
See [HIP’s performance guidelines](https://rocm.docs.amd.com/projects/HIP/en/docs-7.0.0/how-to/performance_guidelines.html) for
greater detail.
...
[ROCr Debug Agent documentation](https://rocm.docs.amd.com/projects/rocr_debug_agent/en/docs-7.0.0/index.html).
