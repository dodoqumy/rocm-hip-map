...
credibility: 5
lifecycle: latest
fetched_at: 2026-05-03T12:01:30.930123+00:00
content_hash: "241b8578fe25376a"
---
...
pull rocm/megatron-lm:v25.6_py312

pull rocm/megatron-lm:v25.6_py310

Launch the Docker container.

run -it \ --device /dev/dri \ --device /dev/kfd \ --device /dev/infiniband \ --network host --ipc host \ --group-add video \ --cap-add SYS_PTRACE \ --security-opt seccomp=unconfined \ --privileged \ -v $HOME:$HOME \ -v $HOME/.ssh:/root/.ssh \ --shm-size 128G \ --name megatron_training_env \ rocm/megatron-lm:v25.6_py312

run -it \ --device /dev/dri \ --device /dev/kfd \ --device /dev/infiniband \ --network host --ipc host \ --group-add video \ --cap-add SYS_PTRACE \ --security-opt seccomp=unconfined \ --privileged \ -v $HOME:$HOME \ -v $HOME/.ssh:/root/.ssh \ --shm-size 128G \ --name megatron_training_env \ rocm/megatron-lm:v25.6_py310
...
[ROCm/Megatron-LM](https://github.com/ROCm/Megatron-LM/tree/rocm_dev), including necessary
training scripts.
...
Options can also be passed as command line arguments as described in [Run training](#amd-megatron-lm-run-training-v256).
...
otherwise when needed – see the [Tokenizer](#amd-megatron-lm-tokenizer-v256) section.
...
[key options](#amd-megatron-lm-benchmark-test-vars-v256), and run training on
