---
title: "ROCprof Trace Decoder and thread trace APIs &#8212; ROCprofiler-SDK 1.1.0 Documentation"
source_url: "https://rocm.docs.amd.com/projects/rocprofiler-sdk/en/latest/api-reference/thread_trace.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-01T09:24:42.411202+00:00
content_hash: "fd19ea16a017d7bd"
---

# ROCprof Trace Decoder and thread trace APIs[#](#rocprof-trace-decoder-and-thread-trace-apis)
...
## Thread trace service API[#](#thread-trace-service-api)

This section describes how to use the ROCprofiler-SDK thread trace API to configure and use the thread trace service. For fully functional examples, see [Samples](https://github.com/ROCm/rocm-systems/tree/develop/projects/rocprofiler-sdk/samples/thread_trace).
...
Configure callback tracing for code objects to get disassembly information:
...
The thread trace service is tied to a GPU agent. To extract the list of available agents, use the
...
Optionally, specify the configuration parameters:
...
ROCPROFILER_THREAD_TRACE_PARAMETER_TARGET_CU: Configures the target Compute Unit (CU) or WGP. Instruction tracing can only operate on a single CU or WGP at a time. The same target is used for all SEs in

`ROCPROFILER_THREAD_TRACE_PARAMETER_SHADER_ENGINE_MASK`

.ROCPROFILER_THREAD_TRACE_PARAMETER_SIMD_SELECT: Configures SIMD selection. For gfx9, this is a bitmask where each bit corresponds to a SIMD lane. For example, 0xF selects all SIMD lanes in the

`target_cu`

. For gfx10, gfx11, and gfx12, this selects a single SIMD ID to trace. Results are taken mod4 for compatibility with gfx9 so 0xF selects SIMD3 of the target WGP.ROCPROFILER_THREAD_TRACE_PARAMETER_BUFFER_SIZE: Configures the buffer size. This buffer is shared among all SEs specified in ROCPROFILER_THREAD_TRACE_PARAMETER_SHADER_ENGINE_MASK. There is a minimal side effect to specifying a larger buffer size, except for increased VRAM usage.
...
### Device thread trace[#](#device-thread-trace)
...
### Dispatch thread trace[#](#dispatch-thread-trace)
...
## ROCprof Trace Decoder API[#](#rocprof-trace-decoder-api)
...
### Trace Decoder setup[#](#trace-decoder-setup)
...
### Code object tracking[#](#code-object-tracking)
...
## Processing thread trace data[#](#processing-thread-trace-data)
...
The thread trace service asynchronously delivers raw trace data via a dedicated callback `shader_data_callback`

. This data must be processed using the Trace Decoder to generate useful information:
...
### Trace Decoder info events[#](#trace-decoder-info-events)

The Trace Decoder provides important information about the quality and comprehensiveness of the trace data through `ROCPROFILER_THREAD_TRACE_DECODER_RECORD_INFO`

events. It is important to handle these events to understand potential issues with your trace data:
...
**Possible causes:**The trace buffer size was too small for the workload

Memory bandwidth was exceeded


**Recommended actions:**Increase buffer sizes if possible
...
or increase`ROCPROFILER_THREAD_TRACE_PARAMETER_PERFCOUNTERS_CTRL`
...
**Possible causes:**The trace was started in the middle of a kernel execution:
...
event was triggered. If parts of the trace were missing, important information might not have been available to the decoder.There is a possible bug in the Trace Decoder. If you suspect this, report it to the ROCprofiler team.
