...
source_url: "https://rocm.docs.amd.com/en/docs-7.2.0/how-to/rocm-for-ai/inference-optimization/model-acceleration-libraries.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-02T18:02:34.932210+00:00
content_hash: "3930574c9772ff00"
---

# Model acceleration libraries[#](#model-acceleration-libraries)
...
[ROCm PyTorch Docker image](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-7.2.0/install/3rd-party/pytorch-install.html#using-docker-with-pytorch-pre-installed), which already includes the required libraries.
...
```
clone https://github.com/Dao-AILab/flash-attention.git
cd flash-attention/
pip install ninja
...
### Benchmarking Flash Attention 2[#](#benchmarking-flash-attention-2)
...
```
Today is the 10th anniversary of the 9/11 attacks. I remember that day like it was yesterday.
ckFAv2
...
### Installing CK xFormers[#](#installing-ck-xformers)
...
## PyTorch built-in acceleration[#](#pytorch-built-in-acceleration)
...
kernels from [rocBLAS](https://rocm.docs.amd.com/projects/rocBLAS/en/docs-7.2.0/index.html) and [hipBLASLt](https://rocm.docs.amd.com/projects/hipBLASLt/en/docs-7.2.0/index.html) libraries during runtime.
...
[Optimizing Triton kernels](workload.html#mi300x-tunableop).

## FBGEMM and FBGEMM_GPU[#](#fbgemm-and-fbgemm-gpu)
...
### Installing FBGEMM_GPU[#](#installing-fbgemm-gpu)
...
[package manager](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-7.2.0/install/install-methods/package-manager-index.html)Install the nightly
...
#### Set up the Miniconda environment[#](#set-up-the-miniconda-environment)
...
install -n ${env_name} -y \ click \ cmake \ hypothesis \ jinja2 \ make \ ncurses \ ninja \ numpy \ scikit-build \ wheel


#### Install the ROCm components[#](#install-the-rocm-components)
...
For more information, see [the ROCm installation guide](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-7.2.0/install/detailed-install.html).
The ROCm package also requires the [MIOpen](https://rocm.docs.amd.com/projects/MIOpen/en/docs-7.2.0/index.html) component as a dependency.
To install MIOpen, use the `apt install`

command.

```
install hipify-clang miopen-hip miopen-hip-dev
```
...
#### Perform the prebuild and build[#](#perform-the-prebuild-and-build)
