---
title: "rocSOLVER multi-level logging &#8212; rocSOLVER 3.32.0 Documentation"
source_url: "https://rocm.docs.amd.com/projects/rocSOLVER/en/latest/howto/logging.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-01T09:11:09.349337+00:00
content_hash: "8c93b921f8c1ec3c"
---

# rocSOLVER multi-level logging[#](#rocsolver-multi-level-logging)
...
similar to [rocBLAS logging](https://rocm.docs.amd.com/projects/rocBLAS/en/latest/reference/logging.html).
...
executable to call the function with the same size arguments.

### Profile logging[#](#profile-logging)

Profile logging is triggered whenever you call `rocsolver_log_write_profile`

or `rocsolver_log_flush_profile`

,
or terminate the logging session using `rocsolver_log_end`

. It outputs statistics on each
...
and `rocsolver_log_set_max_levels`
...
If these variables are not set, the layer mode defaults to `rocblas_layer_mode_none`

and the
maximum level depth defaults to `1`

. These defaults can be restored by calling the function
...
is not set, then there is no logging.If

`(ROCSOLVER_LAYER & 1) != 0`

, then there is trace logging.If

`(ROCSOLVER_LAYER & 2) != 0`

, then there is bench logging.If
...
sets the full path name for trace logging.`ROCSOLVER_LOG_BENCH_PATH`

sets the full path name for bench logging.`ROCSOLVER_LOG_PROFILE_PATH`
...
for the corresponding log file is set to `ROCSOLVER_LOG_PATH`

. If `ROCSOLVER_LOG_PATH`
...
. When logging facilities are no longer required, for example, at
...
For more details on these logging functions, see the [rocSOLVER Logging functions](../reference/helpers.html#api-logging)
reference section.
...
logging environment. With this approach, however, you cannot call `rocsolver_log_set_layer_mode`
...
, then kernel calls are added to the trace logIf

`(ROCSOLVER_LAYER & 20) != 0`

, then kernel calls are added to the profile log

## Multiple host threads[#](#multiple-host-threads)
...
functions are not thread safe. Calling a log function while any rocSOLVER
