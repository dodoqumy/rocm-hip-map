...
- [Deep learning training](#deep-learning-training){.reference .internal .nav-link}
- [Training phases](#training-phases){.reference .internal .nav-link}
- [Case studies](#case-studies){.reference .internal .nav-link}
  - [Inception V3 with PyTorch](#inception-v3-with-pytorch){.reference .internal .nav-link}
    - [Evaluating a pre-trained model](#evaluating-a-pre-trained-model){.reference .internal .nav-link}
    - [Training Inception V3](#training-inception-v3){.reference .internal .nav-link}
  - [Custom model with CIFAR-10 on PyTorch](#custom-model-with-cifar-10-on-pytorch){.reference .internal .nav-link}
  - [Case study: TensorFlow with Fashion-MNIST](#case-study-tensorflow-with-fashion-mnist){.reference .internal .nav-link}
  - [Case study: TensorFlow with text classification](#case-study-tensorflow-with-text-classification){.reference .internal .nav-link}
::::
:::::
::::::
...
::::::::::::::::::::::::: {#deep-learning-inception-v3-with-pytorch .section .tex2jax_ignore .mathjax_ignore}
# Deep learning: Inception V3 with PyTorch[\#](#deep-learning-inception-v3-with-pytorch "Link to this heading"){.headerlink}
...
::: {#deep-learning-training .section}
## Deep learning training[\#](#deep-learning-training "Link to this heading"){.headerlink}
...
## Training phases[\#](#training-phases "Link to this heading"){.headerlink}
...
::: pst-scrollable-table-container
  ...
  : [Types of Training Phases]{.caption-text}[\#](#id1 "Link to this table"){.headerlink} {#id1 .table}
:::
...
::: pst-scrollable-table-container
  ...
  : [Training vs. Inference]{.caption-text}[\#](#training-inference "Link to this table"){.headerlink} {#training-inference .table}
:::
...
## Case studies[\#](#case-studies "Link to this heading"){.headerlink}

The following sections contain case studies for the Inception V3 model.

::::::: {#inception-v3-with-pytorch .section}
### Inception V3 with PyTorch[\#](#inception-v3-with-pytorch "Link to this heading"){.headerlink}
...
The implementation uses PyTorch as a framework. This case study utilizes [TorchVision](https://pytorch.org/vision/stable/index.html){.reference .external}, a repository of popular datasets and model architectures, for obtaining the model. TorchVision also provides pre-trained weights as a starting point to develop new models or fine-tune the model for a new task.

::: {#evaluating-a-pre-trained-model .section}
#### Evaluating a pre-trained model[\#](#evaluating-a-pre-trained-model "Link to this heading"){.headerlink}
...
This example is adapted from the PyTorch research hub page on [Inception V3](https://pytorch.org/vision/master/models/inception.html){.reference .external}.

Follow these steps:

1.  Run the PyTorch ROCm-based Docker image or refer to the section [[Installing PyTorch]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/3rd-party/pytorch-install.html "(in ROCm installation on Linux v7.2.2)"){.reference .external} for setting up a PyTorch environment on ROCm.

    :::: {.highlight-dockerfile .notranslate}
    ::: highlight
...
::::: {#training-inception-v3 .section}
#### Training Inception V3[\#](#training-inception-v3 "Link to this heading"){.headerlink}
...
1.  Run the PyTorch ROCm Docker image or refer to the section [[Installing PyTorch]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/3rd-party/pytorch-install.html "(in ROCm installation on Linux v7.2.2)"){.reference .external} for setting up a PyTorch environment on ROCm.
...
2.  Download an ImageNet database. For this example, the [`tiny-imagenet-200`{.docutils .literal .notranslate}]{.pre}, a smaller ImageNet variant with 200 image classes and a training dataset with 100,000 images, was downsized to 64x64 color images.
...
3.  Process the database to set the validation directory to the format expected by PyTorch's [`DataLoader`{.docutils .literal .notranslate}]{.pre}.
...
6.  Import dependencies, including Torch, OS, and [TorchVision](https://github.com/pytorch/vision){.reference .external}.
...
7.  Set parameters to guide the training process.
    ...
    The device is set to [`"cuda"`{.docutils .literal .notranslate}]{.pre}. In PyTorch, [`"cuda"`{.docutils .literal .notranslate}]{.pre} is a generic keyword to denote a GPU.
    :::
...
8.  Set the data_path to the location of the training and validation data. In this case, the [`tiny-imagenet-200`{.docutils .literal .notranslate}]{.pre} is present as a subdirectory to the current directory.
...
9.  To smooth the image, use bilinear interpolation, a resampling method that uses the distance weighted average of the four nearest pixel values to estimate a new pixel value.
    ...
    The [`torch.optim`{.docutils .literal .notranslate}]{.pre} package provides methods to adjust the learning rate as the training progresses. This example uses the [`StepLR`{.docutils .literal .notranslate}]{.pre} scheduler, which decays the learning rate by [`lr_gamma`{.docutils .literal .notranslate}]{.pre} at every [`lr_step_size`{.docutils .literal .notranslate}]{.pre} number of epochs.
...
11. Adapt Inception V3 for the current dataset. [`tiny-imagenet-200`{.docutils .literal .notranslate}]{.pre} contains only 200 classes, whereas Inception V3 is designed for 1,000-class output. The last layer of Inception V3 is replaced to match the output features required.
...
![Inception V3 train and loss graph](../_images/inception-v3.png)
:::::
:::::::

::: {#custom-model-with-cifar-10-on-pytorch .section}
### Custom model with CIFAR-10 on PyTorch[\#](#custom-model-with-cifar-10-on-pytorch "Link to this heading"){.headerlink}
...
1.  Import dependencies, including Torch, OS, and [TorchVision](https://github.com/pytorch/vision){.reference .external}.
...
2.  The output of torchvision datasets is [`PILImage`{.docutils .literal .notranslate}]{.pre} images of range \[0, 1\]. Transform them to Tensors of normalized range \[-1, 1\].
...
7.  Import the [`torch.nn`{.docutils .literal .notranslate}]{.pre} for constructing neural networks and [`torch.nn.functional`{.docutils .literal .notranslate}]{.pre} to use the convolution functions.
...
::: {#case-study-tensorflow-with-fashion-mnist .section}
### Case study: TensorFlow with Fashion-MNIST[\#](#case-study-tensorflow-with-fashion-mnist "Link to this heading"){.headerlink}
...
11. To ensure the data is in the correct format and ready to build and train the network, display the first 25 images from the training set and the class name below each image.
    ...
    The basic building block of a neural network is the layer. Layers extract representations from the data fed into them. Deep learning consists of chaining together simple layers. Most layers, such as [`tf.keras.layers.Dense`{.docutils .literal .notranslate}]{.pre}, have parameters that are learned during training.
...
12. You must add the Loss function, Metrics, and Optimizer at the time of model compilation.
    ...
    11. [`tf.keras`{.docutils .literal .notranslate}]{.pre} models are optimized to make predictions on a batch, or collection, of examples at once. Accordingly, even though you are using a single image, you must add it to a list.
    ...
    13. [`tf.keras.Model.predict`{.docutils .literal .notranslate}]{.pre} returns a list of lists---one for each image in the batch of data. Grab the predictions for our (only) image in the batch.
...
::: {#case-study-tensorflow-with-text-classification .section}
### Case study: TensorFlow with text classification[\#](#case-study-tensorflow-with-text-classification "Link to this heading"){.headerlink}
...
8.  As you will see in a moment, you can train a model by passing a dataset directly to [`model.fit`{.docutils .literal .notranslate}]{.pre}. If you are new to [`tf.data`{.docutils .literal .notranslate}]{.pre}, you can also iterate over the dataset and print a few examples as follows:
...
1.  Standardize, tokenize, and vectorize the data using the helpful [`tf.keras.layers.TextVectorization`{.docutils .literal .notranslate}]{.pre} layer.
...
6.  You are nearly ready to train your model. As a final preprocessing step, apply the [`TextVectorization`{.docutils .literal .notranslate}]{.pre} layer we created earlier to train, validate, and test the dataset.
    ...
    The [`cache()`{.docutils .literal .notranslate}]{.pre} function keeps data in memory after it is loaded off disk. This ensures the dataset does not become a bottleneck while training your model. If your dataset is too large to fit into memory, you can also use this method to create a performant on-disk cache, which is more efficient to read than many small files.

    The [`prefetch()`{.docutils .literal .notranslate}]{.pre} function overlaps data preprocessing and model execution while training.
...
8.  A model needs a loss function and an optimizer for training. Since this is a binary classification problem and the model outputs a probability (a single-unit layer with a sigmoid activation), use [[`losses.BinaryCrossentropy`{.docutils .literal .notranslate}]{.pre}](https://www.tensorflow.org/api_docs/python/tf/keras/losses/BinaryCrossentropy){.reference .external} loss function.
...
10. See how the model performs. Two values are returned: loss (a number representing our error; lower values are better) and accuracy.
    ...
    [`model.fit()`{.docutils .literal .notranslate}]{.pre} returns a History object that contains a dictionary with everything that happened during training.
    :::
...
11. Four entries are for each monitored metric during training and validation. Use these to plot the training and validation loss for comparison, as well as the training and validation accuracy:
    ...
    ![Training and validation loss](../_images/TextClassification-6.png)

    ![Training and validation accuracy](../_images/TextClassification-7.png)
...
::::: prev-next-area
[](cmake-packages.html "previous page"){.left-prev}
...
[](../reference/api-libraries.html "next page"){.right-next}
