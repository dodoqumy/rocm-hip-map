...
The MaxText for ROCm training Docker ([`rocm/jax-training:maxtext-v25.4`{.docutils .literal .notranslate}]{.pre}) image provides a prebuilt environment for training on AMD Instinct MI300X and MI325X GPUs, including essential components like JAX, XLA, ROCm libraries, and MaxText utilities. It includes the following software components:
...
[]{#amd-maxtext-multi-node-setup-v254}

### Multi-node setup[\#](#multi-node-setup "Link to this heading"){.headerlink}

For multi-node environments, ensure you have all the necessary packages for your network device, such as, RDMA. If you're not using a multi-node setup with RDMA, skip ahead to [[Download the Docker image]{.std .std-ref}](#amd-maxtext-download-docker-v254){.reference .internal}.
...
2.  Set the following environment variables.
    ...
    5.  RDMA interface
        ...
        Ensure the [[required packages]{.std .std-ref}](#amd-maxtext-multi-node-setup-v254){.reference .internal} are installed on all nodes. Then, set the RDMA interfaces to use for communication.
...
[]{#amd-maxtext-download-docker-v254}
