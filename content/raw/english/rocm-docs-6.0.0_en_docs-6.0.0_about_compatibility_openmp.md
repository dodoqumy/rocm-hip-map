---
title: "OpenMP support in ROCm"
source_url: "https://rocm.docs.amd.com/en/docs-6.0.0/about/compatibility/openmp.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-04T16:10:36.528822+00:00
content_hash: "c3c92956ca78cfba"
---
...
this ROCm release. See the list of supported GPUs for [Linux](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-6.0.0/reference/system-requirements.html) and
[Windows](https://rocm.docs.amd.com/projects/install-on-windows/en/docs-6.0.0/reference/system-requirements.html).
...
```
-fopenmp-targets=amdgcn-amd-amdhsa -Xopenmp-target=amdgcn-amd-amdhsa -march=<gpu-arch>
```
...
with OpenMP[#](#using-rocprof-with-openmp)
...
% rocprof --stats <application> <args>
...
, refer to the [ROCProfilerV1 User Manual](https://rocm.docs.amd.com/projects/rocprofiler/en/docs-6.0.0/rocprofv1.html).

### Using tracing options[#](#using-tracing-options)
...
: This option is used to get a JSON output file with the HSA API execution traces and a flat profile in a CSV file.`--hsa-trace`

: This allows programmers to trace both HIP and HSA calls. Since this option results in loading`--sys-trace`
...
For more details on tracing, refer to the [ROCProfilerV1 User Manual](https://rocm.docs.amd.com/projects/rocprofiler/en/docs-6.0.0/rocprofv1.html).
...
|
To set the threshold size up to which data transfers are initiated asynchronously. The default threshold size is 1 |
|
...
### Asynchronous behavior in OpenMP target regions[#](#asynchronous-behavior-in-openmp-target-regions)
...
. See the [Environment Variables](#environment-variables) table for details.
...
### OMPT target support[#](#ompt-target-support)
...
### Floating point atomic operations[#](#floating-point-atomic-operations)
...
### AddressSanitizer tool[#](#addresssanitizer-tool)
...
### Clang compiler option for kernel optimization[#](#clang-compiler-option-for-kernel-optimization)
...
clause in an OpenMP target construct acts as an override and prevents the generation of the no-loop kernel. If the specification of`num_teams`
