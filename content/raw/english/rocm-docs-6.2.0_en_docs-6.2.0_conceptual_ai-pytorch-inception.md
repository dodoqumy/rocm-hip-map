...
source_url: "https://rocm.docs.amd.com/en/docs-6.2.0/conceptual/ai-pytorch-inception.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-04T16:23:25.754130+00:00
content_hash: "7323e4f6a573a2bf"
---
...
[Installing PyTorch](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-6.2.0/install/3rd-party/pytorch-install.html)for setting up a PyTorch environment on ROCm.run -it -v $HOME:/data --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --device=/dev/kfd --device=/dev/dri --group-add video --ipc=host --shm-size 8G rocm/pytorch:latest
...
#### Training Inception V3[#](#training-inception-v3)
...
[Installing PyTorch](https://rocm.docs.amd.com/projects/install-on-linux/en/docs-6.2.0/install/3rd-party/pytorch-install.html)for setting up a PyTorch environment on ROCm.pull rocm/pytorch:latest docker run -it --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --device=/dev/kfd --device=/dev/dri --group-add video --ipc=host --shm-size 8G rocm/pytorch:latest

Download an ImageNet database. For this example, the

`tiny-imagenet-200`

[[^Stanford_deep_learning]](#id8), a smaller ImageNet variant with 200 image classes and a training dataset with 100,000 images, was downsized to 64x64 color images.`http://cs231n.stanford.edu/tiny-imagenet-200.zip`
...
Set the loss criteria. For this example, Cross Entropy Loss

[[^cross_entropy]](#id9)is used.criterion = torch.nn.CrossEntropyLoss()
...
[[^cross_entropy]](#id10)is used.criterion = nn.CrossEntropyLoss() optimizer = optim.SGD(net.parameters(), lr=0.001, momentum=0.9)
...
### Case study: TensorFlow with text classification[#](#case-study-tensorflow-with-text-classification)
...
data from https://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz 84131840/84125825 [==============================] – 1s 0us/step 84149932/84125825 [==============================] – 1s 0us/step
