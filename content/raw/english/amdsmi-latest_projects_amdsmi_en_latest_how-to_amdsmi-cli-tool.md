---
title: "AMD SMI CLI tool usage &#8212; AMD SMI 26.2.2 documentation"
source_url: "https://rocm.docs.amd.com/projects/amdsmi/en/latest/how-to/amdsmi-cli-tool.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-03T06:25:35.553291+00:00
content_hash: "a1f6c3e8ce30f744"
---

# AMD SMI CLI tool usage[#](#amd-smi-cli-tool-usage)
...
telemetry tools. The [Python](amdsmi-py-lib.html) or [C++
library](amdsmi-cpp-lib.html) is recommended as a robust data source.

## Install the CLI Tool and Python library[#](#install-the-cli-tool-and-python-library)

Refer to the [installation instructions](../install/install.html).

## Get started[#](#get-started)

The `amd-smi`

command provides system management and monitoring capabilities for
...
~$ amd-smi
usage: amd-smi [-h] ...
AMD System Management Interface | Version: 26.2.2 | ROCm version: 7.2.1 | Platform: Linux Baremetal
options:
...
You can set one or more variables in front of any `amd-smi`
...
AMDSMI_GPU_METRICS_CACHE_MS - Controls the internal GPU metrics cache duration (ms). Default 100, set to 0 to disable. AMDSMI_ASIC_INFO_CACHE_MS - Controls the internal GPU asic info cache duration (ms). Default 10000, set to 0 to disable.
...
### amd-smi firmware[#](#amd-smi-firmware)
...
### amd-smi bad-pages[#](#amd-smi-bad-pages)
...
### amd-smi topology[#](#amd-smi-topology)
...
### amd-smi monitor[#](#amd-smi-monitor)
...
### amd-smi partition[#](#amd-smi-partition)
...
When you run an `amd-smi`

command, the tool presents detailed information
across various categories, each containing specific fields and their current
values.

### About N/A values[#](#about-n-a-values)

`N/A`

typically indicates that the data for a specific field is either unavailable or irrelevant in the current context for your device and its software environment. The exact reason may vary depending on the field and the GPU. In general, you can interpret `N/A`
...
**Not Available**: The information cannot be retrieved by the `amd-smi`

tool at this time. This could be due to one of the following reasons:
...
driver version does not support querying this particular piece of information through`amd-smi-lib`

.

### Example output from amd-smi static[#](#example-output-from-amd-smi-static)

The following block is example output from the `amd-smi static`

command without additional modifiers.
...
### Listing CPER entries using amd-smi[#](#listing-cper-entries-using-amd-smi)
