...
credibility: 5
lifecycle: latest
fetched_at: 2026-05-04T03:01:27.211488+00:00
content_hash: "8c58bce59e3e9674"
---
...
The `rocm/jax-training:maxtext-v25.9`
...
[versioned documentation](jax-maxtext-v25.9.html).
...
```
pull rocm/jax-training:maxtext-v25.11
```
...
See [Supported models](#amd-maxtext-model-support-v25-11) to switch to another available model.
...
[Supported models](#amd-maxtext-model-support-v25-11) to switch to another
...
If you need to collect a trace and the JAX profiler isn’t working, use `rocprofv3`

provided by the [ROCprofiler-SDK](https://rocm.docs.amd.com/projects/rocprofiler-sdk/en/latest/index.html) as a workaround. For example:
...
saved using `-d <TRACE_DIRECTORY>`

. The resulting traces can be
opened in Perfetto: [https://ui.perfetto.dev/](https://ui.perfetto.dev/).
...
For multi-node training examples, choose a model from [Supported models](#amd-maxtext-model-support-v25-11)
...
You might see minor loss spikes, or loss curve may have slightly higher convergence end values compared to the previous

`jax-training`

image.For FP8 training on MI355, many models will display a warning message like:
...
The compile step may take longer than usual, but training will run. This will be fixed in a future release.The built-in JAX profiler isn’t working. If you need to collect a trace and the JAX profiler isn’t working, use

`rocprofv3`

provided by the[ROCprofiler-SDK](https://rocm.docs.amd.com/projects/rocprofiler-sdk/en/latest/index.html)as a workaround. For example:\ --hip-trace \ --kernel-trace \ --memory-copy-trace \ --rccl-trace \ --output-format pftrace \ -d ./v3_traces \ # output directory -- ./jax-maxtext_benchmark_report.sh -m {{ model.model_repo }} # or desired command

You can set the directory where you want the .json traces to be saved using
