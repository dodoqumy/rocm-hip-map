...
source_url: "https://rocm.docs.amd.com/en/docs-6.4.0/how-to/rocm-for-ai/training/benchmark-docker/jax-maxtext.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-04T15:21:39.771302+00:00
content_hash: "baddd839f519bc6e"
---
...
The MaxText for ROCm training Docker (`rocm/jax-training:maxtext-v25.5`
...
with RDMA, skip ahead to [Pull the Docker image](#amd-maxtext-download-docker).
...
[required packages](#amd-maxtext-multi-node-setup)are installed on all nodes. Then, set the RDMA interfaces to use for communication.# If using Broadcom NIC export NCCL_IB_HCA=rdma0,rdma1,rdma2,rdma3,rdma4,rdma5,rdma6,rdma7 # If using Mellanox NIC export NCCL_IB_HCA=mlx5_0,mlx5_1,mlx5_2,mlx5_3,mlx5_4,mlx5_5,mlx5_8,mlx5_9
...
pull rocm/jax-training:maxtext-v25.5

Use the following command to launch the Docker container. Note that the benchmarking scripts used in the

[following section](#amd-maxtext-get-started)automatically launch the Docker container and execute the benchmark.run -it --device /dev/dri --device /dev/kfd --network host --ipc host --group-add video --cap-add SYS_PTRACE --security-opt seccomp=unconfined --privileged -v $HOME/.ssh:/root/.ssh --shm-size 128G --name maxtext_training rocm/jax-training:maxtext-v25.5
...
[ROCm/maxtext](https://github.com/ROCm/maxtext/blob/main/benchmarks/gpu-rocm/).
...
README at [ROCm/maxtext](https://github.com/ROCm/maxtext/blob/main/benchmarks/gpu-rocm/)
...
`https://raw.githubusercontent.com/ROCm/maxtext/refs/heads/main/benchmarks/gpu-rocm/llama3.3_70b.sh`
