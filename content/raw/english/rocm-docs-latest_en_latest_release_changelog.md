...
- CPER component output was not redirected when using the [`--follow`{.docutils .literal .notranslate}]{.pre} option.
...
- Corrected the return value of [`hipEventQuery`{.docutils .literal .notranslate}]{.pre} and [`hipEventSynchronize`{.docutils .literal .notranslate}]{.pre}. The HIP runtime now properly handles and restricts stream capture within these APIs.
...
- GPU and baseboard temperature options to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre} CLI.
  ...
  - [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--gpu-board-temps`{.docutils .literal .notranslate}]{.pre} for GPU board temperature sensors.

  - [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--base-board-temps`{.docutils .literal .notranslate}]{.pre} for base board temperature sensors.
...
- New Node Power Management (NPM) APIs and CLI options for node monitoring.
  ...
  - C++ API functions:
    ...
    - [`amdsmi_get_node_handle()`{.docutils .literal .notranslate}]{.pre} gets the handle for a node device.

    - [`amdsmi_get_npm_info()`{.docutils .literal .notranslate}]{.pre} retrieves Node Power Management information.

  - C++ types:

    - [`amdsmi_npm_status_t`{.docutils .literal .notranslate}]{.pre} indicates whether NPM is enabled or disabled.

    - [`amdsmi_npm_info_t`{.docutils .literal .notranslate}]{.pre} contains the status and node-level power limit in watts.

  - Added Python API wrappers for new node device functions.

  - Added [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`node`{.docutils .literal .notranslate}]{.pre} subcommand for NPM operations via CLI.

  - Currently supported for [`OAM_ID`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`0`{.docutils .literal .notranslate}]{.pre} only.

- The following C APIs are added to [`amdsmi_interface.py`{.docutils .literal .notranslate}]{.pre}:

  - [`amdsmi_get_cpu_handle()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_esmi_err_msg()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_gpu_event_notification()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_processor_count_from_handles()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_processor_handles_by_type()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_gpu_validate_ras_eeprom()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_init_gpu_event_notification()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_set_gpu_event_notification_mask()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_stop_gpu_event_notification()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_gpu_busy_percent()`{.docutils .literal .notranslate}]{.pre}

- Additional return value to [`amdsmi_get_xgmi_plpd()`{.docutils .literal .notranslate}]{.pre} API:

  - The entry [`policies`{.docutils .literal .notranslate}]{.pre} is added to the end of the dictionary to match API definition.

  - The entry [`plpds`{.docutils .literal .notranslate}]{.pre} is marked for deprecation as it has the same information as [`policies`{.docutils .literal .notranslate}]{.pre}.

- PCIe levels to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--bus`{.docutils .literal .notranslate}]{.pre} command.

  - The static [`--bus`{.docutils .literal .notranslate}]{.pre} option has been updated to include the range of PCIe levels that you can set for a device.

  - Levels are a 2-tuple composed of the PCIe speed and bandwidth.

- [`evicted_time`{.docutils .literal .notranslate}]{.pre} metric for KFD processes.

  - Time that queues are evicted on a GPU in milliseconds.

  - Added to CLI in [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-q`{.docutils .literal .notranslate}]{.pre} and [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`process`{.docutils .literal .notranslate}]{.pre}.

  - Added to C APIs and Python APIs: [`amdsmi_get_gpu_process_list()`{.docutils .literal .notranslate}]{.pre}, [`amdsmi_get_gpu_compute_process_info()`{.docutils .literal .notranslate}]{.pre} , and [`amdsmi_get_gpu_compute_process_info_by_pid()`{.docutils .literal .notranslate}]{.pre}.

- New VRAM types to [`amdsmi_vram_type_t`{.docutils .literal .notranslate}]{.pre}.

  - [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--vram`{.docutils .literal .notranslate}]{.pre} and [`amdsmi_get_gpu_vram_info()`{.docutils .literal .notranslate}]{.pre} now support the following types: [`DDR5`{.docutils .literal .notranslate}]{.pre}, [`LPDDR4`{.docutils .literal .notranslate}]{.pre}, [`LPDDR5`{.docutils .literal .notranslate}]{.pre}, and [`HBM3E`{.docutils .literal .notranslate}]{.pre}.
...
- Support for PPT1 power limit information.
  ...
  - New API added:
    ...
    - [`amdsmi_get_supported_power_cap()`{.docutils .literal .notranslate}]{.pre}: Returns power cap types supported on the device (PPT0, PPT1). This will allow you to know which power cap types you can get/set.
...
- The [`amd-smi`{.docutils .literal .notranslate}]{.pre} command now shows [`hsmp`{.docutils .literal .notranslate}]{.pre} rather than [`amd_hsmp`{.docutils .literal .notranslate}]{.pre}.
  ...
  - The [`hsmp`{.docutils .literal .notranslate}]{.pre} driver version can be shown without the [`amdgpu`{.docutils .literal .notranslate}]{.pre} version using [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`version`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-c`{.docutils .literal .notranslate}]{.pre}.

- The [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`set`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--power-cap`{.docutils .literal .notranslate}]{.pre} command now requires specification of the power cap type.

  - Command now takes the form: [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`set`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--power-cap`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`<power-cap-type>`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`<new-cap>`{.docutils .literal .notranslate}]{.pre}.
...
- Support for B Tensor type [`pk_int4_t`{.docutils .literal .notranslate}]{.pre} in the CK Tile weight preshuffle GEMM.
...
- Support for [`grouped_gemm`{.docutils .literal .notranslate}]{.pre} kernels to perform [`multi_d`{.docutils .literal .notranslate}]{.pre} elementwise operation.
...
- Support for [`f32`{.docutils .literal .notranslate}]{.pre} to FMHA (fwd/bwd).
...
- New HIP APIs
  ...
  - [`hipLibraryEnumerateKernels`{.docutils .literal .notranslate}]{.pre} returns kernel handles within a library.

  - [`hipKernelGetLibrary`{.docutils .literal .notranslate}]{.pre} returns library handle for a hipKernel_t handle.

  - [`hipKernelGetName`{.docutils .literal .notranslate}]{.pre} returns function name for a hipKernel_t handle.

  - [`hipLibraryLoadData`{.docutils .literal .notranslate}]{.pre} creates library object from code.

  - [`hipLibraryLoadFromFile`{.docutils .literal .notranslate}]{.pre} creates library object from file.

  - [`hipLibraryUnload`{.docutils .literal .notranslate}]{.pre} unloads library.

  - [`hipLibraryGetKernel`{.docutils .literal .notranslate}]{.pre} gets a kernel from the library.

  - [`hipLibraryGetKernelCount`{.docutils .literal .notranslate}]{.pre} gets kernel count in library.

  - [`hipStreamCopyAttributes`{.docutils .literal .notranslate}]{.pre} copies attributes from source stream to destination stream.

  - [`hipOccupancyAvailableDynamicSMemPerBlock`{.docutils .literal .notranslate}]{.pre} returns dynamic shared memory available per block when launching numBlocks blocks on CU.

- New HIP flags

  - [`hipMemLocationTypeHost`{.docutils .literal .notranslate}]{.pre} enables handling virtual memory management in host memory location, in addition to device memory.

  - Support for flags in [`hipGetProcAddress`{.docutils .literal .notranslate}]{.pre} enables searching for the per-thread version symbols:

    - [`HIP_GET_PROC_ADDRESS_DEFAULT`{.docutils .literal .notranslate}]{.pre}
...
- Full [`hipFFTw`{.docutils .literal .notranslate}]{.pre} support.

- [#2062](https://github.com/ROCm/HIPIFY/issues/2062){.reference .external} Partial hipification support for a particular CUDA API.

- [#2073](https://github.com/ROCm/HIPIFY/issues/2073){.reference .external} Detect CUDA version before hipification.
...
- [`hiptensorHandleWritePlanCacheToFile`{.docutils .literal .notranslate}]{.pre} to write the plan cache of a hipTensor handle to a file.

- [`hiptensorHandleReadPlanCacheFromFile`{.docutils .literal .notranslate}]{.pre} to read a plan cache from a file into a hipTensor handle.

- [`simple_contraction_plan_cache`{.docutils .literal .notranslate}]{.pre} to demonstrate plan cache usages.

- [`plan_cache_test`{.docutils .literal .notranslate}]{.pre} to test the plan cache across various tensor ranks.
...
- Dynamic shape support for [`DepthToSpace`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Op`{.docutils .literal .notranslate}]{.pre}.

- [`bias`{.docutils .literal .notranslate}]{.pre} and [`key_mask_padding`{.docutils .literal .notranslate}]{.pre} inputs for the [`MultiHeadAttention`{.docutils .literal .notranslate}]{.pre} operator.

- GEMM+GEMM fusions.

- [`dim_params`{.docutils .literal .notranslate}]{.pre} input parameter to the [`parse_onnx`{.docutils .literal .notranslate}]{.pre} Python call.

- Created an API to query supported ONNX Operators [`get_onnx_operators()`{.docutils .literal .notranslate}]{.pre}.
...
- Switched to [`--input-dim`{.docutils .literal .notranslate}]{.pre} instead of [`--batch`{.docutils .literal .notranslate}]{.pre} to set any dynamic dimensions when using [`migraphx-driver`{.docutils .literal .notranslate}]{.pre}.

- Different stride sizes are now supported in ONNX [`if`{.docutils .literal .notranslate}]{.pre} branches.

- ONNX version change to 1.18.0 to support PyTorch 2.9.1.

- Refactored [`GroupQueryAttention`{.docutils .literal .notranslate}]{.pre}.

- Enabled [`PipelineRepoRef`{.docutils .literal .notranslate}]{.pre} parameter in CI.
...
- Streamlined the [`find_matches`{.docutils .literal .notranslate}]{.pre} function.
...
- Fixed [`pointwise:`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Wrong`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`number`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`of`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`arguments`{.docutils .literal .notranslate}]{.pre} error when quantizing certain models to [`int8`{.docutils .literal .notranslate}]{.pre}.
...
- Fixed "reduce_sum: axes: value out of range" error in [`simplify_reshapes`{.docutils .literal .notranslate}]{.pre}.

- Updated README [`rbuild`{.docutils .literal .notranslate}]{.pre} installation instructions to use Python venv to avoid warning.
...
- [`Argument`{.docutils .literal .notranslate}]{.pre} class which stores the value and type of each argument in the Node.
...
- CXX Compiler: Use AMDClang++ compiler core location [`${ROCM_PATH}/lib/llvm/bin`{.docutils .literal .notranslate}]{.pre}.
...
- [`config_delta/gfx950_diff.yaml`{.docutils .literal .notranslate}]{.pre} to analysis config YAMLs to track the revision between the gfx9xx GPUs against the latest supported gfx950 GPUs.
...
- Analysis configs YAMLs are now managed with the new config management workflow in [`tools/config_management/`{.docutils .literal .notranslate}]{.pre}.
...
- A new cmake option, [`BENCHMARK_USE_AMDSMI`{.docutils .literal .notranslate}]{.pre}. It is set to [`OFF`{.docutils .literal .notranslate}]{.pre} by default. When this option is set to [`ON`{.docutils .literal .notranslate}]{.pre}, it lets benchmarks use AMD SMI to output more GPU statistics.

- The first tested example program for [`device_search`{.docutils .literal .notranslate}]{.pre}.

- [`apply_config_improvements.py`{.docutils .literal .notranslate}]{.pre}file , which generates improved configs by taking the best specializations from old and new configs.

  - Run the script with [`--help`{.docutils .literal .notranslate}]{.pre} for usage instructions, and see [rocPRIM Performance Tuning](https://rocm.docs.amd.com/projects/rocPRIM/en/latest/conceptual/rocPRIM-performance-tuning.html#rocprim-performance-tuning){.reference .external} for more information.
...
- Fixed [`float_bit_mask`{.docutils .literal .notranslate}]{.pre} for [`rocprim::half`{.docutils .literal .notranslate}]{.pre}.
...
- The following APIs now have an implementation for the GDA conduit:
  ...
  - [`rocshmem_p`{.docutils .literal .notranslate}]{.pre}

  - fetching atomics [`rocshmem_<TYPE>_fetch_<op>`{.docutils .literal .notranslate}]{.pre}
...
- The [`rocsparse_sptrsv`{.docutils .literal .notranslate}]{.pre} and [`rocsparse_sptrsm`{.docutils .literal .notranslate}]{.pre} routines for triangular solve.
...
- rocWMMA has been moved into the new rocm-libraries "monorepo" repository []{.fab .fa-github} [rocm-libraries](https://github.com/ROCm/rocm-libraries){.reference .external}. This repository consolidates a number of separate ROCm libraries and shared components.
...
- Caching for repeated ASIC information calls.
  ...
  - The cache added to [`amdsmi_get_gpu_asic_info`{.docutils .literal .notranslate}]{.pre} improves performance by avoiding redundant hardware queries.

  - The cache stores ASIC info for each GPU device with a configurable duration, defaulting to 10 seconds. Use the [`AMDSMI_ASIC_INFO_CACHE_MS`{.docutils .literal .notranslate}]{.pre} environment variable for cache duration configuration for [`amdsmi_get_gpu_asic_info`{.docutils .literal .notranslate}]{.pre} API calls.

- Support for GPU partition metrics.

  - Provides support for [`xcp_metrics`{.docutils .literal .notranslate}]{.pre} v1.0 and extends support for v1.1 (dynamic metrics).

  - Added [`amdsmi_get_gpu_partition_metrics_info`{.docutils .literal .notranslate}]{.pre}, which provides per XCP (partition) metrics.
...
- Fixed [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`event`{.docutils .literal .notranslate}]{.pre} not respecting the Linux timeout command.

- Fixed an issue where [`amdsmi_get_power_info`{.docutils .literal .notranslate}]{.pre} returned [`AMDSMI_STATUS_API_FAILED`{.docutils .literal .notranslate}]{.pre}.

  - VMs were incorrectly reporting [`AMDSMI_STATUS_API_FAILED`{.docutils .literal .notranslate}]{.pre} when unable to get the power cap within the [`amdsmi_get_power_info`{.docutils .literal .notranslate}]{.pre}.

  - The API now returns [`N/A`{.docutils .literal .notranslate}]{.pre} or [`UINT_MAX`{.docutils .literal .notranslate}]{.pre} for values that can't be retrieved, instead of failing.
...
- A segmentation fault occurred when the dynamic queue management mechanism was enabled. HIP runtime now ensures GPU queues aren't [`NULL`{.docutils .literal .notranslate}]{.pre} during marker submission, preventing crashes and improving robustness.
...
- [`rocm-bandwidth-test`{.docutils .literal .notranslate}]{.pre} folder fails to be removed after driver uninstallation:
  ...
  - After running [`amdgpu-uninstall`{.docutils .literal .notranslate}]{.pre}, the [`rocm-bandwidth-test`{.docutils .literal .notranslate}]{.pre} folder and package are still present.
...
- The following APIs have been deprecated:
  ...
  - [`rocshmem_wg_init`{.docutils .literal .notranslate}]{.pre}

  - [`rocshmem_wg_finalize`{.docutils .literal .notranslate}]{.pre}

  - [`rocshmem_wg_init_thread`{.docutils .literal .notranslate}]{.pre}

- [`rocshmem_ptr`{.docutils .literal .notranslate}]{.pre} can now return non-null pointer to a shared memory region when the IPC transport is available to reach that region. Previously, it would return a null pointer.
...
- Fixed issues caused by HIP changes:
  ...
  - Removed the [`.data`{.docutils .literal .notranslate}]{.pre} member from [`HIP_vector_type`{.docutils .literal .notranslate}]{.pre}.

  - Broadcast constructor now only writes to the first vector element.

- Fixed a bug related to [`int32_t`{.docutils .literal .notranslate}]{.pre} usage in [`hipRTC_gemm`{.docutils .literal .notranslate}]{.pre} for gfx942, caused by breaking changes in HIP.

- Replaced [`#pragma`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`unroll`{.docutils .literal .notranslate}]{.pre} with [`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`for`{.docutils .literal .notranslate}]{.pre} to fix a bug caused by the upgraded compiler which no longer supports using [`#pragma`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`unroll`{.docutils .literal .notranslate}]{.pre} with template parameter indices.
...
- [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`reset`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--profile`{.docutils .literal .notranslate}]{.pre} behavior so that it won't also reset the performance level.

  - The performance level can still be reset using [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`reset`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--perf-determinism`{.docutils .literal .notranslate}]{.pre}.

- Setting power cap is now available in Linux Guest. You can now use [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`set`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--power-cap`{.docutils .literal .notranslate}]{.pre} as usual in Linux Guest systems too.

- Changed [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--vbios`{.docutils .literal .notranslate}]{.pre} to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--ifwi`{.docutils .literal .notranslate}]{.pre}.
...
- Fixed an issue where using [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ras`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--folder`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`<folder_name>`{.docutils .literal .notranslate}]{.pre} was forcing the created folder's name to be lowercase. This fix also makes all string input options case-insensitive.
...
- New HIP APIs
  ...
  - [`hipModuleGetFunctionCount`{.docutils .literal .notranslate}]{.pre} returns the number of functions within a module

  - [`hipMemsetD2D8`{.docutils .literal .notranslate}]{.pre} sets 2D memory range with specified 8-bit values

  - [`hipMemsetD2D8Async`{.docutils .literal .notranslate}]{.pre} asynchronously sets 2D memory range with specified 8-bit values

  - [`hipMemsetD2D16`{.docutils .literal .notranslate}]{.pre} sets 2D memory range with specified 16-bit values

  - [`hipMemsetD2D16Async`{.docutils .literal .notranslate}]{.pre} asynchronously sets 2D memory range with specified 16-bit values

  - [`hipMemsetD2D32`{.docutils .literal .notranslate}]{.pre} sets 2D memory range with specified 32-bit values

  - [`hipMemsetD2D32Async`{.docutils .literal .notranslate}]{.pre} asynchronously sets 2D memory range with specified 32-bit values

  - [`hipStreamSetAttribute`{.docutils .literal .notranslate}]{.pre} sets attributes such as synchronization policy for a given stream

  - [`hipStreamGetAttribute`{.docutils .literal .notranslate}]{.pre} returns attributes such as priority for a given stream

  - [`hipModuleLoadFatBinary`{.docutils .literal .notranslate}]{.pre} loads fatbin binary to a module

  - [`hipMemcpyBatchAsync`{.docutils .literal .notranslate}]{.pre} asynchronously performs a batch copy of 1D or 2D memory

  - [`hipMemcpy3DBatchAsync`{.docutils .literal .notranslate}]{.pre} asynchronously performs a batch copy of 3D memory

  - [`hipMemcpy3DPeer`{.docutils .literal .notranslate}]{.pre} copies memory between devices

  - [`hipMemcpy3DPeerAsync`{.docutils .literal .notranslate}]{.pre} asynchronously copies memory between devices

  - [`hipMemsetD2D32Async`{.docutils .literal .notranslate}]{.pre} asynchronously sets 2D memory range with specified 32-bit values

  - [`hipMemPrefetchAsync_v2`{.docutils .literal .notranslate}]{.pre} prefetches memory to the specified location

  - [`hipMemAdvise_v2`{.docutils .literal .notranslate}]{.pre} advises about the usage of a given memory range

  - [`hipGetDriverEntryPoint`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate} gets function pointer of a HIP API.

  - [`hipSetValidDevices`{.docutils .literal .notranslate}]{.pre} sets a default list of devices that can be used by HIP
...
- Support for ReLU/Clamp activation functions with auxiliary output for the [`FP16`{.docutils .literal .notranslate}]{.pre} and [`BF16`{.docutils .literal .notranslate}]{.pre} data types for gfx942 to capture intermediate results. This feature is enabled for [`HIPBLASLT_EPILOGUE_RELU_AUX`{.docutils .literal .notranslate}]{.pre}, [`HIPBLASLT_EPILOGUE_RELU_AUX_BIAS`{.docutils .literal .notranslate}]{.pre}, [`HIPBLASLT_EPILOGUE_CLAMP_AUX_EXT`{.docutils .literal .notranslate}]{.pre}, and [`HIPBLASLT_EPILOGUE_CLAMP_AUX_BIAS_EXT`{.docutils .literal .notranslate}]{.pre}.

- Support for [`HIPBLAS_COMPUTE_32F_FAST_16BF`{.docutils .literal .notranslate}]{.pre} for FP32 data type for gfx950 only.

- CPP extension APIs [`setMaxWorkspaceBytes`{.docutils .literal .notranslate}]{.pre} and [`getMaxWorkspaceBytes`{.docutils .literal .notranslate}]{.pre}.

- Feature to print logs (using [`HIPBLASLT_LOG_MASK=32`{.docutils .literal .notranslate}]{.pre}) for Grouped GEMM.
...
- [`::hipcub::extents`{.docutils .literal .notranslate}]{.pre}, with limited parity to C++23's [`std::extents`{.docutils .literal .notranslate}]{.pre}. Only [`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`extents`{.docutils .literal .notranslate}]{.pre} is supported; [`dynamic`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`extents`{.docutils .literal .notranslate}]{.pre} is not. Helper structs have been created to perform computations on [`::hipcub::extents`{.docutils .literal .notranslate}]{.pre} only when the backend is rocPRIM. For the CUDA backend, similar functionality exists.

- [`projects/hipcub/hipcub/include/hipcub/backend/rocprim/util_mdspan.hpp`{.docutils .literal .notranslate}]{.pre} to support [`::hipcub::extents`{.docutils .literal .notranslate}]{.pre}.

- [`::hipcub::ForEachInExtents`{.docutils .literal .notranslate}]{.pre} API.

- [`hipcub::DeviceTransform::Transform`{.docutils .literal .notranslate}]{.pre} and [`hipcub::DeviceTransform::TransformStableArgumentAddresses`{.docutils .literal .notranslate}]{.pre}.

- hipCUB and its dependency rocPRIM have been moved into the new [`rocm-libraries`{.docutils .literal .notranslate}]{.pre} [monorepo repository](https://github.com/ROCm/rocm-libraries){.reference .external}. This repository contains a number of ROCm libraries that are frequently used together.

  - The repository migration requires a few changes to the way that hipCUB fetches library dependencies.

  - CMake build option [`ROCPRIM_FETCH_METHOD`{.docutils .literal .notranslate}]{.pre} may be set to one of the following:

    - [`PACKAGE`{.docutils .literal .notranslate}]{.pre} - (default) searches for a preinstalled packaged version of the dependency. If it is not found, the build will fall back using option [`DOWNLOAD`{.docutils .literal .notranslate}]{.pre}, below.
...
- Changed [`CUDA_STANDARD`{.docutils .literal .notranslate}]{.pre} for tests in [`test/hipcub`{.docutils .literal .notranslate}]{.pre}, due to C++17 APIs such as [`std::exclusive_scan`{.docutils .literal .notranslate}]{.pre} is used in some tests. Still use [`CUDA_STANDARD`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`14`{.docutils .literal .notranslate}]{.pre} for [`test/extra`{.docutils .literal .notranslate}]{.pre}.

- Changed [`CCCL_MINIMUM_VERSION`{.docutils .literal .notranslate}]{.pre} to [`2.8.2`{.docutils .literal .notranslate}]{.pre} to align with CUB.

- Changed [`cmake_minimum_required`{.docutils .literal .notranslate}]{.pre} from [`3.16`{.docutils .literal .notranslate}]{.pre} to [`3.18`{.docutils .literal .notranslate}]{.pre}, in order to support [`CUDA_STANDARD`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`17`{.docutils .literal .notranslate}]{.pre} as a valid value.

- Add support for large num_items [`DeviceScan`{.docutils .literal .notranslate}]{.pre}, [`DevicePartition`{.docutils .literal .notranslate}]{.pre} and [`Reduce::{ArgMin,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ArgMax}`{.docutils .literal .notranslate}]{.pre}.
...
- Deprecated [`hipcub::ConstantInputIterator`{.docutils .literal .notranslate}]{.pre}, use [`rocprim::constant_iterator`{.docutils .literal .notranslate}]{.pre} or [`rocthrust::constant_iterator`{.docutils .literal .notranslate}]{.pre} instead.

- Deprecated [`hipcub::CountingInputIterator`{.docutils .literal .notranslate}]{.pre}, use [`rocprim::counting_iterator`{.docutils .literal .notranslate}]{.pre} or [`rocthrust::counting_iterator`{.docutils .literal .notranslate}]{.pre} instead.

- Deprecated [`hipcub::DiscardOutputIterator`{.docutils .literal .notranslate}]{.pre}, use [`rocprim::discard_iterator`{.docutils .literal .notranslate}]{.pre} or [`rocthrust::discard_iterator`{.docutils .literal .notranslate}]{.pre} instead.

- Deprecated [`hipcub::TransformInputIterator`{.docutils .literal .notranslate}]{.pre}, use [`rocprim::transform_iterator`{.docutils .literal .notranslate}]{.pre} or [`rocthrust::transform_iterator`{.docutils .literal .notranslate}]{.pre} instead.

- Deprecated [`hipcub::AliasTemporaries`{.docutils .literal .notranslate}]{.pre}, which is considered to be an internal API. Moved to the detail namespace.
...
- Brain half float mixed precision for the following routines:
  ...
  - [`hipsparseAxpby`{.docutils .literal .notranslate}]{.pre} where X and Y use bfloat16 and result and the compute type use float.

  - [`hipsparseSpVV`{.docutils .literal .notranslate}]{.pre} where X and Y use bfloat16 and result and the compute type use float.

  - [`hipsparseSpMV`{.docutils .literal .notranslate}]{.pre} where A and X use bfloat16 and Y and the compute type use float.

  - [`hipsparseSpMM`{.docutils .literal .notranslate}]{.pre} where A and B use bfloat16 and C and the compute type use float.

  - [`hipsparseSDDMM`{.docutils .literal .notranslate}]{.pre} where A and B use bfloat16 and C and the compute type use float.

  - [`hipsparseSDDMM`{.docutils .literal .notranslate}]{.pre} where A and B and C use bfloat16 and the compute type use float.

- Half float mixed precision to [`hipsparseSDDMM`{.docutils .literal .notranslate}]{.pre} where A and B and C use float16 and the compute type use float.
...
- [`fixed_pad`{.docutils .literal .notranslate}]{.pre} operator for padding dynamic shapes to the maximum static shape.

- Matcher to upcast base [`Softmax`{.docutils .literal .notranslate}]{.pre} operations.

- Support for the [`convolution_backwards`{.docutils .literal .notranslate}]{.pre} operator through rocMLIR.

- [`LSE`{.docutils .literal .notranslate}]{.pre} output to attention fusion.

- Flags to [`EnableControlFlowGuard`{.docutils .literal .notranslate}]{.pre} due to BinSkim errors.

- New environment variable documentation and reorganized structure.

- [`stash_type`{.docutils .literal .notranslate}]{.pre} attribute for [`LayerNorm`{.docutils .literal .notranslate}]{.pre} and expanded test coverage.
...
- Updated [`src/CMakeLists.txt`{.docutils .literal .notranslate}]{.pre} to support [`msgpack`{.docutils .literal .notranslate}]{.pre} 6.x ([`msgpack-cxx`{.docutils .literal .notranslate}]{.pre}).

- Updated model zoo test generator to fix test issues and add summary logging.

- Updated [`rocMLIR`{.docutils .literal .notranslate}]{.pre} and [`ONNXRuntime`{.docutils .literal .notranslate}]{.pre} mainline references across commits.

- Updated module sorting algorithm for improved reliability.

- Restricted FP8 quantization to [`dot`{.docutils .literal .notranslate}]{.pre} and [`convolution`{.docutils .literal .notranslate}]{.pre} operators.

- Moved ONNX Runtime launcher script into MIGraphX and updated build scripts.

- Simplified ONNX [`Resize`{.docutils .literal .notranslate}]{.pre} operator parser for correctness and maintainability.
...
- Improved [`Graphviz`{.docutils .literal .notranslate}]{.pre} output (node color and layout updates).
...
- rocMLIR [`rewrite_reduce`{.docutils .literal .notranslate}]{.pre} issue (#4218).

- Bug with [`invert_permutation`{.docutils .literal .notranslate}]{.pre} on GPU (#4194).

- Compile error when [`MIOPEN`{.docutils .literal .notranslate}]{.pre} is disabled (missing [`std`{.docutils .literal .notranslate}]{.pre} includes) (#4281).

- ONNX [`Resize`{.docutils .literal .notranslate}]{.pre} parsing when input and output shapes are identical (#4133, #4161).
...
- [`reshape`{.docutils .literal .notranslate}]{.pre}, [`transpose`{.docutils .literal .notranslate}]{.pre}, and [`broadcast`{.docutils .literal .notranslate}]{.pre} rewrites between pointwise and reduce operators (#3978).
...
- [`rocpd`{.docutils .literal .notranslate}]{.pre} choice for [`--format-rocprof-output`{.docutils .literal .notranslate}]{.pre} option in profile mode.

- [`--retain-rocpd-output`{.docutils .literal .notranslate}]{.pre} option in profile mode to save large raw rocpd databases in workload directory.

- Feature to show description of metrics during analysis.

  - Use [`--include-cols`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Description`{.docutils .literal .notranslate}]{.pre} to show the Description column, which is excluded by default from the ROCm Compute Profiler CLI output.

- [`--set`{.docutils .literal .notranslate}]{.pre} filtering option in profile mode to enable single-pass counter collection for predefined subsets of metrics.

- [`--list-sets`{.docutils .literal .notranslate}]{.pre} filtering option in profile mode to list the sets available for single pass counter collection.

- Missing counters based on register specification which enables missing metrics.

  - Enabled [`SQC_DCACHE_INFLIGHT_LEVEL`{.docutils .literal .notranslate}]{.pre} counter and associated metrics.

  - Enabled [`TCP_TCP_LATENCY`{.docutils .literal .notranslate}]{.pre} counter and associated counter for all GPUs except MI300.

- Interactive metric descriptions in TUI analyze mode.

  - You can now left click on any metric cell to view detailed descriptions in the dedicated [`METRIC`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`DESCRIPTION`{.docutils .literal .notranslate}]{.pre} tab.

- Support for analysis report output as a SQLite database using [`--output-format`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`db`{.docutils .literal .notranslate}]{.pre} analysis mode option.

- [`Compute`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Throughput`{.docutils .literal .notranslate}]{.pre} panel to TUI's [`High`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Level`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Analysis`{.docutils .literal .notranslate}]{.pre} category with the following metrics: VALU FLOPs, VALU IOPs, MFMA FLOPs (F8), MFMA FLOPs (BF16), MFMA FLOPs (F16), MFMA FLOPs (F32), MFMA FLOPs (F64), MFMA FLOPs (F6F4) (in gfx950), MFMA IOPs (Int8), SALU Utilization, VALU Utilization, MFMA Utilization, VMEM Utilization, Branch Utilization, IPC

- [`Memory`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Throughput`{.docutils .literal .notranslate}]{.pre} panel to TUI's [`High`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Level`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Analysis`{.docutils .literal .notranslate}]{.pre} category with the following metrics: vL1D Cache BW, vL1D Cache Utilization, Theoretical LDS Bandwidth, LDS Utilization, L2 Cache BW, L2 Cache Utilization, L2-Fabric Read BW, L2-Fabric Write BW, sL1D Cache BW, L1I BW, Address Processing Unit Busy, Data-Return Busy, L1I-L2 Bandwidth, sL1D-L2 BW
...
- When [`--format-rocprof-output`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocpd`{.docutils .literal .notranslate}]{.pre} is used, only [`pmc_perf.csv`{.docutils .literal .notranslate}]{.pre} will be written to workload directory instead of multiple CSV files.
...
- Updated [`Unit`{.docutils .literal .notranslate}]{.pre} of the following [`Bandwidth`{.docutils .literal .notranslate}]{.pre} related metrics to [`Gbps`{.docutils .literal .notranslate}]{.pre} instead of [`Bytes`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`per`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Normalization`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Unit`{.docutils .literal .notranslate}]{.pre}:
...
- Updated the metric name for the following [`Bandwidth`{.docutils .literal .notranslate}]{.pre} related metrics whose [`Unit`{.docutils .literal .notranslate}]{.pre} is [`Percent`{.docutils .literal .notranslate}]{.pre} by adding [`Utilization`{.docutils .literal .notranslate}]{.pre}:
...
- Updated [`System`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Speed-of-Light`{.docutils .literal .notranslate}]{.pre} panel to [`GPU`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`Speed-of-Light`{.docutils .literal .notranslate}]{.pre} in TUI for the following metrics:
...
- Analysis output:
  ...
  - Replaced [`-o`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`/`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--output`{.docutils .literal .notranslate}]{.pre} analyze mode option with [`--output-format`{.docutils .literal .notranslate}]{.pre} and [`--output-name`{.docutils .literal .notranslate}]{.pre}.

    - Use [`--output-format`{.docutils .literal .notranslate}]{.pre} analysis mode option to select the output format of the analysis report.

    - Use [`--output-name`{.docutils .literal .notranslate}]{.pre} analysis mode option to override the default file/folder name.

  - Replaced [`--save-dfs`{.docutils .literal .notranslate}]{.pre} analyze mode option with [`--output-format`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`csv`{.docutils .literal .notranslate}]{.pre}.

- Command-line options:

  - [`--list-metrics`{.docutils .literal .notranslate}]{.pre} and [`--config-dir`{.docutils .literal .notranslate}]{.pre} options moved to general command-line options.

  - [`--list-metrics`{.docutils .literal .notranslate}]{.pre} option cannot be used without GPU architecture argument.

  - [`--list-metrics`{.docutils .literal .notranslate}]{.pre} option do not show number of L2 channels.

  - [`--list-available-metrics`{.docutils .literal .notranslate}]{.pre} profile mode option to display the metrics available for profiling in current GPU.

  - [`--list-available-metrics`{.docutils .literal .notranslate}]{.pre} analyze mode option to display the metrics available for analysis.

  - [`--block`{.docutils .literal .notranslate}]{.pre} option cannot be used with [`--list-metrics`{.docutils .literal .notranslate}]{.pre} and [`--list-available-metrics`{.docutils .literal .notranslate}]{.pre}options.

- Default [`rocprof`{.docutils .literal .notranslate}]{.pre} interface changed from [`rocprofv3`{.docutils .literal .notranslate}]{.pre} to [`rocprofiler-sdk`{.docutils .literal .notranslate}]{.pre}
...
- Support for [`rocpd`{.docutils .literal .notranslate}]{.pre} database output with the [`ROCPROFSYS_USE_ROCPD`{.docutils .literal .notranslate}]{.pre} configuration setting.

- Support for profiling PyTorch workloads using the [`rocpd`{.docutils .literal .notranslate}]{.pre} output database.
...
- [`rocprim::transform_output_iterator`{.docutils .literal .notranslate}]{.pre} and [`rocprim::make_transform_output_iterator`{.docutils .literal .notranslate}]{.pre}.

- Experimental support for SPIR-V, to use the correct tuned config for part of the appliable algorithms.

- A new cmake option, [`BUILD_OFFLOAD_COMPRESS`{.docutils .literal .notranslate}]{.pre}. When rocPRIM is build with this option enabled, the [`--offload-compress`{.docutils .literal .notranslate}]{.pre} switch is passed to the compiler. This causes the compiler to compress the binary that it generates. Compression can be useful in cases where you are compiling for a large number of targets, since this often results in a large binary. Without compression, in some cases, the generated binary may become so large symbols are placed out of range, resulting in linking errors. The new [`BUILD_OFFLOAD_COMPRESS`{.docutils .literal .notranslate}]{.pre} option is set to [`ON`{.docutils .literal .notranslate}]{.pre} by default.
...
- Scratch-memory trace information to the Perfetto output in [`rocprofv3`{.docutils .literal .notranslate}]{.pre}.

- New capabilities to the thread trace support in [`rocprofv3`{.docutils .literal .notranslate}]{.pre}:
  ...
  - [`MultiKernelDispatch`{.docutils .literal .notranslate}]{.pre} thread trace support is now available across all ASICs.
...
- Brain half float mixed precision for the following routines:
  ...
  - [`rocsparse_axpby`{.docutils .literal .notranslate}]{.pre} where X and Y use bfloat16 and result and the compute type use float.

  - [`rocsparse_spvv`{.docutils .literal .notranslate}]{.pre} where X and Y use bfloat16 and result and the compute type use float.

  - [`rocsparse_spmv`{.docutils .literal .notranslate}]{.pre} where A and X use bfloat16 and Y and the compute type use float.

  - [`rocsparse_spmm`{.docutils .literal .notranslate}]{.pre} where A and B use bfloat16 and C and the compute type use float.

  - [`rocsparse_sddmm`{.docutils .literal .notranslate}]{.pre} where A and B use bfloat16 and C and the compute type use float.

  - [`rocsparse_sddmm`{.docutils .literal .notranslate}]{.pre} where A and B and C use bfloat16 and the compute type use float.
...
- rocThrust, as well as its dependencies rocPRIM and rocRAND have been moved into the new [`rocm-libraries`{.docutils .literal .notranslate}]{.pre} monorepo repository (https://github.com/ROCm/rocm-libraries). This repository contains several ROCm libraries that are frequently used together.
  ...
  - cmake build options [`ROCPRIM_FETCH_METHOD`{.docutils .literal .notranslate}]{.pre} and [`ROCRAND_FETCH_METHOD`{.docutils .literal .notranslate}]{.pre} may be set to one of the following:

    - [`PACKAGE`{.docutils .literal .notranslate}]{.pre} - (default) searches for a preinstalled packaged version of the dependency. If it's not found, the build will fall back using option [`DOWNLOAD`{.docutils .literal .notranslate}]{.pre}, described below.
...
- Removed gpuboard and baseboard temperatures enums in amdsmi Python Library.
  ...
  - [`AmdSmiTemperatureType`{.docutils .literal .notranslate}]{.pre} had issues with referencing the correct attribute. As such, the following duplicate enums have been removed:

    - [`AmdSmiTemperatureType.GPUBOARD_NODE_FIRST`{.docutils .literal .notranslate}]{.pre}
...
- Fixed certain output in [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre} when GPUs are partitioned.

  - It fixes the amd-smi monitor such as: [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-Vqt`{.docutils .literal .notranslate}]{.pre}, [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-g`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`0`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-Vqt`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-w`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`1`{.docutils .literal .notranslate}]{.pre}, [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-Vqt`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--file`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`/tmp/test1`{.docutils .literal .notranslate}]{.pre}, etc. These commands will now be able to display as normal in partitioned GPU scenarios.

- Fixed an issue where using [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ras`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--folder`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`<folder_name>`{.docutils .literal .notranslate}]{.pre} was forcing the created folder's name to be lowercase. This fix also allows all string input options to be case insensitive.
...
- A permission-related error occurred during the execution of [`hipLaunchHostFunc`{.docutils .literal .notranslate}]{.pre}. This API is now supported and permitted to run during stream capture, aligning its behavior with CUDA.

- A numerical error during graph capture of kernels that rely on a remainder in [`globalWorkSize`{.docutils .literal .notranslate}]{.pre}, in frameworks like MIOpen and PyTorch, where the grid size is not a multiple of the block size. To ensure correct replay behavior, HIP runtime now stores this remainder in [`hip::GraphKernelNode`{.docutils .literal .notranslate}]{.pre} during [`hipExtModuleLaunchKernel`{.docutils .literal .notranslate}]{.pre} capture, enabling accurate execution and preventing corruption.
...
- Ability to restart the AMD GPU driver from the CLI and API.
  ...
  - [`amdsmi_gpu_driver_reload()`{.docutils .literal .notranslate}]{.pre} API and [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`reset`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--reload-driver`{.docutils .literal .notranslate}]{.pre} or [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`reset`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-r`{.docutils .literal .notranslate}]{.pre} CLI options.
...
- Default command:
  ...
  A default view has been added. The default view provides a snapshot of commonly requested information such as bdf, current partition mode, version information, and more. Users can access that information by simply typing [`amd-smi`{.docutils .literal .notranslate}]{.pre} with no additional commands or arguments. Users may also obtain this information through alternate output formats such as json or csv by using the default command with the respective output format: [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`default`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--json`{.docutils .literal .notranslate}]{.pre} or [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`default`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--csv`{.docutils .literal .notranslate}]{.pre}.

- Support for GPU metrics 1.8:

  - Added new fields for [`amdsmi_gpu_xcp_metrics_t`{.docutils .literal .notranslate}]{.pre} including:

    - Metrics to allow new calculations for violation status:

      - Per XCP metrics [`gfx_below_host_limit_ppt_acc[XCP][MAX_XCC]`{.docutils .literal .notranslate}]{.pre} - GFX Clock Host limit Package Power Tracking violation counts

      - Per XCP metrics [`gfx_below_host_limit_thm_acc[XCP][MAX_XCC]`{.docutils .literal .notranslate}]{.pre} - GFX Clock Host limit Thermal (TVIOL) violation counts

      - Per XCP metrics [`gfx_low_utilization_acc[XCP][MAX_XCC]`{.docutils .literal .notranslate}]{.pre} - violation counts for how did low utilization caused the GPU to be below application clocks.

      - Per XCP metrics [`gfx_below_host_limit_total_acc[XCP][MAX_XCC]`{.docutils .literal .notranslate}]{.pre}- violation counts for how long GPU was held below application clocks any limiter (see above new violation metrics).

  - Increased available JPEG engines to 40. Current ASICs might not support all 40. These are indicated as [`UINT16_MAX`{.docutils .literal .notranslate}]{.pre} or [`N/A`{.docutils .literal .notranslate}]{.pre} in CLI.

- Bad page threshold count.

  - Added [`amdsmi_get_gpu_bad_page_threshold`{.docutils .literal .notranslate}]{.pre} to Python API and CLI; root/sudo permissions are required to display the count.

- CPU model name for RDC.

  - Added new C and Python API [`amdsmi_get_cpu_model_name`{.docutils .literal .notranslate}]{.pre}.

  - Not sourced from esmi library.

- New API [`amdsmi_get_cpu_affinity_with_scope()`{.docutils .literal .notranslate}]{.pre}.

- [`socket`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`power`{.docutils .literal .notranslate}]{.pre} to [`amdsmi_get_power_info`{.docutils .literal .notranslate}]{.pre}

  - Previously, the C API had the value in the [`amdsmi_power_info`{.docutils .literal .notranslate}]{.pre} structure, but was unused.

  - The value is representative of the socket's power agnostic of the the GPU version.

- New event notification types to [`amdsmi_evt_notification_type_t`{.docutils .literal .notranslate}]{.pre}. The following values were added to the [`amdsmi_evt_notification_type_t`{.docutils .literal .notranslate}]{.pre} enum:

  - [`AMDSMI_EVT_NOTIF_EVENT_MIGRATE_START`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_EVENT_MIGRATE_END`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_EVENT_PAGE_FAULT_START`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_EVENT_PAGE_FAULT_END`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_EVENT_QUEUE_EVICTION`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_EVENT_QUEUE_RESTORE`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_EVENT_UNMAP_FROM_GPU`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_PROCESS_START`{.docutils .literal .notranslate}]{.pre}

  - [`AMDSMI_EVT_NOTIF_PROCESS_END`{.docutils .literal .notranslate}]{.pre}
...
- Disabled [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--violation`{.docutils .literal .notranslate}]{.pre} on guests. Modified [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-T/--throttle`{.docutils .literal .notranslate}]{.pre} to alias to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-v/--violation`{.docutils .literal .notranslate}]{.pre}.

- Updated [`amdsmi_get_clock_info`{.docutils .literal .notranslate}]{.pre} in [`amdsmi_interface.py`{.docutils .literal .notranslate}]{.pre}.

  - The [`clk_deep_sleep`{.docutils .literal .notranslate}]{.pre} field now returns the sleep integer value.

- The [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`topology`{.docutils .literal .notranslate}]{.pre} command has been enabled for guest environments.
...
- Expanded violation status tracking for GPU metrics 1.8.
  ...
  - The driver will no longer be supporting existing single-value GFX clock below host limit fields ([`acc_gfx_clk_below_host_limit`{.docutils .literal .notranslate}]{.pre}, [`per_gfx_clk_below_host_limit`{.docutils .literal .notranslate}]{.pre}, [`active_gfx_clk_below_host_limit`{.docutils .literal .notranslate}]{.pre}), they are now changed in favor of new per-XCP/XCC arrays.

  - Added new fields to [`amdsmi_violation_status_t`{.docutils .literal .notranslate}]{.pre} and related interfaces for enhanced violation breakdown:
    ...
    - Added 2D arrays to track per-XCP/XCC accumulators, percentage, and active status:
      ...
      - [`acc_gfx_clk_below_host_limit_pwr`{.docutils .literal .notranslate}]{.pre}, [`acc_gfx_clk_below_host_limit_thm`{.docutils .literal .notranslate}]{.pre}, [`acc_gfx_clk_below_host_limit_total`{.docutils .literal .notranslate}]{.pre}

      - [`per_gfx_clk_below_host_limit_pwr`{.docutils .literal .notranslate}]{.pre}, [`per_gfx_clk_below_host_limit_thm`{.docutils .literal .notranslate}]{.pre}, [`per_gfx_clk_below_host_limit_total`{.docutils .literal .notranslate}]{.pre}

      - [`active_gfx_clk_below_host_limit_pwr`{.docutils .literal .notranslate}]{.pre}, [`active_gfx_clk_below_host_limit_thm`{.docutils .literal .notranslate}]{.pre}, [`active_gfx_clk_below_host_limit_total`{.docutils .literal .notranslate}]{.pre}

      - [`acc_low_utilization`{.docutils .literal .notranslate}]{.pre}, [`per_low_utilization`{.docutils .literal .notranslate}]{.pre}, [`active_low_utilization`{.docutils .literal .notranslate}]{.pre}

  - Python API and CLI now report these expanded fields.

- The char arrays in the following structures have been changed.

  - [`amdsmi_vbios_info_t`{.docutils .literal .notranslate}]{.pre} member [`build_date`{.docutils .literal .notranslate}]{.pre} changed from [`AMDSMI_MAX_DATE_LENGTH`{.docutils .literal .notranslate}]{.pre} to [`AMDSMI_MAX_STRING_LENGTH`{.docutils .literal .notranslate}]{.pre}.

  - [`amdsmi_dpm_policy_entry_t`{.docutils .literal .notranslate}]{.pre} member [`policy_description`{.docutils .literal .notranslate}]{.pre} changed from [`AMDSMI_MAX_NAME`{.docutils .literal .notranslate}]{.pre} to [`AMDSMI_MAX_STRING_LENGTH`{.docutils .literal .notranslate}]{.pre}.

  - [`amdsmi_name_value_t`{.docutils .literal .notranslate}]{.pre} member [`name`{.docutils .literal .notranslate}]{.pre} changed from [`AMDSMI_MAX_NAME`{.docutils .literal .notranslate}]{.pre} to [`AMDSMI_MAX_STRING_LENGTH`{.docutils .literal .notranslate}]{.pre}.

- For backwards compatibility, updated [`amdsmi_bdf_t`{.docutils .literal .notranslate}]{.pre} union to have an identical unnamed struct.

- Updated [`amdsmi_get_temp_metric`{.docutils .literal .notranslate}]{.pre} and [`amdsmi_temperature_type_t`{.docutils .literal .notranslate}]{.pre} with new values.
...
- Unused definitions:
  ...
  - [`AMDSMI_MAX_NAME`{.docutils .literal .notranslate}]{.pre}, [`AMDSMI_256_LENGTH`{.docutils .literal .notranslate}]{.pre}, [`AMDSMI_MAX_DATE_LENGTH`{.docutils .literal .notranslate}]{.pre}, [`MAX_AMDSMI_NAME_LENGTH`{.docutils .literal .notranslate}]{.pre}, [`AMDSMI_LIB_VERSION_YEAR`{.docutils .literal .notranslate}]{.pre}, [`AMDSMI_DEFAULT_VARIANT`{.docutils .literal .notranslate}]{.pre}, [`AMDSMI_MAX_NUM_POWER_PROFILES`{.docutils .literal .notranslate}]{.pre}, [`AMDSMI_MAX_DRIVER_VERSION_LENGTH`{.docutils .literal .notranslate}]{.pre}.

- Unused member [`year`{.docutils .literal .notranslate}]{.pre} in struct [`amdsmi_version_t`{.docutils .literal .notranslate}]{.pre}.

- [`amdsmi_io_link_type_t`{.docutils .literal .notranslate}]{.pre} has been replaced with [`amdsmi_link_type_t`{.docutils .literal .notranslate}]{.pre}.

  - [`amdsmi_io_link_type_t`{.docutils .literal .notranslate}]{.pre} is no longer needed as [`amdsmi_link_type_t`{.docutils .literal .notranslate}]{.pre} is sufficient.

  - [`amdsmi_link_type_t`{.docutils .literal .notranslate}]{.pre} enum has changed; primarily, the ordering of the PCI and XGMI types.

  - This change will also affect [`amdsmi_link_metrics_t`{.docutils .literal .notranslate}]{.pre}, where the link_type field changes from [`amdsmi_io_link_type_t`{.docutils .literal .notranslate}]{.pre} to [`amdsmi_link_type_t`{.docutils .literal .notranslate}]{.pre}.

- [`amdsmi_get_power_info_v2()`{.docutils .literal .notranslate}]{.pre}.

  - The [`amdsmi_get_power_info()`{.docutils .literal .notranslate}]{.pre} has been unified and the v2 function is no longer needed or used.

- [`AMDSMI_EVT_NOTIF_RING_HANG`{.docutils .literal .notranslate}]{.pre} event notification type in [`amdsmi_evt_notification_type_t`{.docutils .literal .notranslate}]{.pre}.

- The [`amdsmi_get_gpu_vram_info`{.docutils .literal .notranslate}]{.pre} now provides vendor names as a string.

  - [`amdsmi_vram_vendor_type_t`{.docutils .literal .notranslate}]{.pre} enum structure is removed.

  - [`amdsmi_vram_info_t`{.docutils .literal .notranslate}]{.pre} member named [`amdsmi_vram_vendor_type_t`{.docutils .literal .notranslate}]{.pre} is changed to a character string.

  - [`amdsmi_get_gpu_vram_info`{.docutils .literal .notranslate}]{.pre} now no longer requires decoding the vendor name as an enum.

- Backwards compatibility for [`amdsmi_get_gpu_metrics_info()`{.docutils .literal .notranslate}]{.pre}'s,[`jpeg_activity`{.docutils .literal .notranslate}]{.pre}and [`vcn_activity`{.docutils .literal .notranslate}]{.pre} fields. Alternatively use [`xcp_stats.jpeg_busy`{.docutils .literal .notranslate}]{.pre} or [`xcp_stats.vcn_busy`{.docutils .literal .notranslate}]{.pre}.

  - Backwards compatibility is removed for [`jpeg_activity`{.docutils .literal .notranslate}]{.pre} and [`vcn_activity`{.docutils .literal .notranslate}]{.pre} fields, if the [`jpeg_busy`{.docutils .literal .notranslate}]{.pre} or [`vcn_busy`{.docutils .literal .notranslate}]{.pre} field is available.
...
- Removed partition information from the default [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre} CLI command.

  - Users can still retrieve the same data by calling [`amd-smi`{.docutils .literal .notranslate}]{.pre}, [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-p`{.docutils .literal .notranslate}]{.pre}, or [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`partition`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-c`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-m`{.docutils .literal .notranslate}]{.pre}/[`sudo`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`partition`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-a`{.docutils .literal .notranslate}]{.pre}.

  - Reading [`current_compute_partition`{.docutils .literal .notranslate}]{.pre} may momentarily wake the GPU up. This is due to reading XCD registers, which is expected behavior. Changing partitions is not a trivial operation, [`current_compute_partition`{.docutils .literal .notranslate}]{.pre} SYSFS controls this action.
...
- Support for Stream-K version of mixed [`FP8`{.docutils .literal .notranslate}]{.pre} / [`BF16`{.docutils .literal .notranslate}]{.pre} GEMM.

- Support for Multiple D GEMM.

- GEMM pipeline for microscaling (MX) [`FP8`{.docutils .literal .notranslate}]{.pre} / [`FP6`{.docutils .literal .notranslate}]{.pre} / [`FP4`{.docutils .literal .notranslate}]{.pre} data types.

- Support for [`FP16`{.docutils .literal .notranslate}]{.pre} 2:4 structured sparsity to universal GEMM.
...
- [`int8`{.docutils .literal .notranslate}]{.pre} support for CK_TILE GEMM.
...
- New HIP APIs
  ...
  - [`hipLaunchKernelEx`{.docutils .literal .notranslate}]{.pre} dispatches the provided kernel with the given launch configuration and forwards the kernel arguments.

  - [`hipLaunchKernelExC`{.docutils .literal .notranslate}]{.pre} launches a HIP kernel using a generic function pointer and the specified configuration.

  - [`hipDrvLaunchKernelEx`{.docutils .literal .notranslate}]{.pre} dispatches the device kernel represented by a HIP function object.

  - [`hipMemGetHandleForAddressRange`{.docutils .literal .notranslate}]{.pre} gets a handle for the address range requested.

  - [`__reduce_add_sync`{.docutils .literal .notranslate}]{.pre}, [`__reduce_min_sync`{.docutils .literal .notranslate}]{.pre}, and [`__reduce_max_sync`{.docutils .literal .notranslate}]{.pre} functions added for aritimetic reduction across lanes of a warp, and [`__reduce_and_sync`{.docutils .literal .notranslate}]{.pre}, [`__reduce_or_sync`{.docutils .literal .notranslate}]{.pre}, and [`__reduce_xor_sync`{.docutils .literal .notranslate}]{.pre} functions added for logical reduction. For details, see [Warp cross-lane functions](https://rocm.docs.amd.com/projects/HIP/en/latest/how-to/hip_cpp_language_extensions.html#warp-cross-lane-functions){.reference .external}.

- New support for Open Compute Project (OCP) floating-point [`FP4`{.docutils .literal .notranslate}]{.pre}/[`FP6`{.docutils .literal .notranslate}]{.pre}/[`FP8`{.docutils .literal .notranslate}]{.pre} as follows. For details, see [Low precision floating point document](https://rocm.docs.amd.com/projects/HIP/en/latest/reference/low_fp_types.html){.reference .external}.

  - Data types for [`FP4`{.docutils .literal .notranslate}]{.pre}/[`FP6`{.docutils .literal .notranslate}]{.pre}/[`FP8`{.docutils .literal .notranslate}]{.pre}.

  - HIP APIs for [`FP4`{.docutils .literal .notranslate}]{.pre}/[`FP6`{.docutils .literal .notranslate}]{.pre}/[`FP8`{.docutils .literal .notranslate}]{.pre}, which are compatible with corresponding CUDA APIs.

  - HIP Extensions APIs for microscaling formats, which are supported on AMD GPUs.

- New [`wptr`{.docutils .literal .notranslate}]{.pre} and [`rptr`{.docutils .literal .notranslate}]{.pre} values in [`ClPrint`{.docutils .literal .notranslate}]{.pre}, for better logging in dispatch barrier methods.

- The [`_sync()`{.docutils .literal .notranslate}]{.pre} version of crosslane builtins such as [`shfl_sync()`{.docutils .literal .notranslate}]{.pre} are enabled by default. These can be disabled by setting the preprocessor macro [`HIP_DISABLE_WARP_SYNC_BUILTINS`{.docutils .literal .notranslate}]{.pre}.

- Added [`constexpr`{.docutils .literal .notranslate}]{.pre} operators for [`fp16`{.docutils .literal .notranslate}]{.pre}/[`bf16`{.docutils .literal .notranslate}]{.pre}.

- Added warp level primitives: [`__syncwarp`{.docutils .literal .notranslate}]{.pre} and reduce intrinsics (e.g. [`__reduce_add_sync()`{.docutils .literal .notranslate}]{.pre}).

- Support for the flags in APIs as following, now allows uncached memory allocation.

  - [`hipExtHostRegisterUncached`{.docutils .literal .notranslate}]{.pre}, used in [`hipHostRegister`{.docutils .literal .notranslate}]{.pre}.

  - [`hipHostMallocUncached`{.docutils .literal .notranslate}]{.pre} and [`hipHostAllocUncached`{.docutils .literal .notranslate}]{.pre}, used in [`hipHostMalloc`{.docutils .literal .notranslate}]{.pre} and [`hipHostAlloc`{.docutils .literal .notranslate}]{.pre}.

- [`num_threads`{.docutils .literal .notranslate}]{.pre} total number of threads in the group. The legacy API size is alias.
...
- [`warpSize`{.docutils .literal .notranslate}]{.pre} has changed. In order to match the CUDA specification, the [`warpSize`{.docutils .literal .notranslate}]{.pre} variable is no longer [`constexpr`{.docutils .literal .notranslate}]{.pre}. In general, this should be a transparent change; however, if an application was using [`warpSize`{.docutils .literal .notranslate}]{.pre} as a compile-time constant, it will have to be updated to handle the new definition. For more information, see the discussion of [`warpSize`{.docutils .literal .notranslate}]{.pre} within the [HIP C++ language extensions](https://rocm.docs.amd.com/projects/HIP/en/latest/how-to/hip_cpp_language_extensions.html#warpsize){.reference .external}.

- Behavior changes

  - [`hipGetLastError`{.docutils .literal .notranslate}]{.pre} now returns the error code which is the last actual error caught in the current thread during the application execution.

  - Cooperative groups in [`hipLaunchCooperativeKernelMultiDevice`{.docutils .literal .notranslate}]{.pre} and [`hipLaunchCooperativeKernel`{.docutils .literal .notranslate}]{.pre} functions, additional input parameter validation checks are added.

  - [`hipPointerGetAttributes`{.docutils .literal .notranslate}]{.pre} returns [`hipSuccess`{.docutils .literal .notranslate}]{.pre} instead of an error with invalid value [`hipErrorInvalidValue`{.docutils .literal .notranslate}]{.pre}, in case [`NULL`{.docutils .literal .notranslate}]{.pre} host or attribute pointer is passed as input parameter. It now matches the functionality of [`cudaPointerGetAttributes`{.docutils .literal .notranslate}]{.pre} which changed with CUDA 11 and above releases.

  - [`hipFree`{.docutils .literal .notranslate}]{.pre} previously there was an implicit wait which was applicable for all memory allocations, for synchronization purpose. This wait is now disabled for allocations made with [`hipMallocAsync`{.docutils .literal .notranslate}]{.pre} and [`hipMallocFromPoolAsync`{.docutils .literal .notranslate}]{.pre}, to match the behavior of CUDA API [`cudaFree`{.docutils .literal .notranslate}]{.pre}.

  - [`hipFreeAsync`{.docutils .literal .notranslate}]{.pre} now returns [`hipSuccess`{.docutils .literal .notranslate}]{.pre} when the input pointer is NULL, instead of ` `{.docutils .literal .notranslate}[`hipErrorInvalidValue`{.docutils .literal .notranslate}]{.pre} , to be consistent with [`hipFree`{.docutils .literal .notranslate}]{.pre}.
...
- Changes in hipRTC.
  ...
  - Removal of [`hipRTC`{.docutils .literal .notranslate}]{.pre} symbols from HIP Runtime Library. Any application using [`hipRTC`{.docutils .literal .notranslate}]{.pre} APIs should link explicitly with the [`hipRTC`{.docutils .literal .notranslate}]{.pre} library. This makes the usage of [`hipRTC`{.docutils .literal .notranslate}]{.pre} library on Linux the same as on Windows and matches the behavior of CUDA [`nvRTC`{.docutils .literal .notranslate}]{.pre}.

  - [`hipRTC`{.docutils .literal .notranslate}]{.pre} compilation The device code compilation now uses namespace [`__hip_internal`{.docutils .literal .notranslate}]{.pre}, instead of the standard headers [`std`{.docutils .literal .notranslate}]{.pre}, to avoid namespace collision.

  - Changes of datatypes from [`hipRTC`{.docutils .literal .notranslate}]{.pre}. Datatype definitions such as [`int64_t`{.docutils .literal .notranslate}]{.pre}, [`uint64_t`{.docutils .literal .notranslate}]{.pre}, [`int32_t`{.docutils .literal .notranslate}]{.pre}, and [`uint32_t`{.docutils .literal .notranslate}]{.pre}, etc. are removed to avoid any potential conflicts in some applications. HIP now uses internal datatypes instead, prefixed with [`__hip`{.docutils .literal .notranslate}]{.pre}, for example, [`__hip_int64_t`{.docutils .literal .notranslate}]{.pre}.

- HIP header clean up

  - Usage of STD headers, HIP header files only include necessary STL headers.

  - Deprecated structure [`HIP_MEMSET_NODE_PARAMS`{.docutils .literal .notranslate}]{.pre} is removed. Developers can use the definition [`hipMemsetParams`{.docutils .literal .notranslate}]{.pre} instead.
...
- API signature/struct changes
  ...
  - API signatures are adjusted in some APIs to match corresponding CUDA APIs. Impacted APIs are as folloing:
    ...
    - [`hiprtcCreateProgram`{.docutils .literal .notranslate}]{.pre}

    - [`hiprtcCompileProgram`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpyHtoD`{.docutils .literal .notranslate}]{.pre}

    - [`hipCtxGetApiVersion`{.docutils .literal .notranslate}]{.pre}

  - HIP struct change in [`hipMemsetParams`{.docutils .literal .notranslate}]{.pre}, it is updated and compatible with CUDA.

  - HIP vector constructor change in [`hipComplex`{.docutils .literal .notranslate}]{.pre} initialization now generates correct values. The affected constructors will be small vector types such as [`float2`{.docutils .literal .notranslate}]{.pre}, [`int4`{.docutils .literal .notranslate}]{.pre}, etc.

- Stream Capture updates

  - Restricted stream capture mode, it is made in HIP APIs via adding the macro [`CHECK_STREAM_CAPTURE_SUPPORTED`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`()`{.docutils .literal .notranslate}]{.pre}. In the previous HIP enumeration [`hipStreamCaptureMode`{.docutils .literal .notranslate}]{.pre}, three capture modes were defined. With checking in the macro, the only supported stream capture mode is now [`hipStreamCaptureModeRelaxed`{.docutils .literal .notranslate}]{.pre}. The rest are not supported, and the macro will return [`hipErrorStreamCaptureUnsupported`{.docutils .literal .notranslate}]{.pre}. This update involves the following APIs, which is allowed only in relaxed stream capture mode:

    - [`hipMallocManaged`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemAdvise`{.docutils .literal .notranslate}]{.pre}
  ...
  - Checks stream capture mode, the following APIs check the stream capture mode and return error codes to match the behavior of CUDA.
    ...
    - [`hipLaunchCooperativeKernelMultiDevice`{.docutils .literal .notranslate}]{.pre}

    - [`hipEventQuery`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamAddCallback`{.docutils .literal .notranslate}]{.pre}

  - Returns error during stream capture. The following HIP APIs now returns specific error [`hipErrorStreamCaptureUnsupported`{.docutils .literal .notranslate}]{.pre} on the AMD platform, but not always [`hipSuccess`{.docutils .literal .notranslate}]{.pre}, to match behavior with CUDA:

    - [`hipDeviceSetMemPool`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemPoolCreate`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemPoolDestroy`{.docutils .literal .notranslate}]{.pre}

    - [`hipDeviceSetSharedMemConfig`{.docutils .literal .notranslate}]{.pre}

    - [`hipDeviceSetCacheConfig`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpyWithStream`{.docutils .literal .notranslate}]{.pre}
...
- Error code update Returned error/value codes are updated in the following HIP APIs to match the corresponding CUDA APIs.
  ...
  - Module Management Related APIs:
    ...
    - [`hipModuleLaunchKernel`{.docutils .literal .notranslate}]{.pre}

    - [`hipExtModuleLaunchKernel`{.docutils .literal .notranslate}]{.pre}

    - [`hipExtLaunchKernel`{.docutils .literal .notranslate}]{.pre}

    - [`hipDrvLaunchKernelEx`{.docutils .literal .notranslate}]{.pre}

    - [`hipLaunchKernel`{.docutils .literal .notranslate}]{.pre}

    - [`hipLaunchKernelExC`{.docutils .literal .notranslate}]{.pre}

    - [`hipModuleLaunchCooperativeKernel`{.docutils .literal .notranslate}]{.pre}

    - [`hipModuleLoad`{.docutils .literal .notranslate}]{.pre}

  - Texture Management Related APIs: The following APIs update the return codes to match the behavior with CUDA:

    - [`hipTexObjectCreate`{.docutils .literal .notranslate}]{.pre}, supports zero width and height for 2D image. If either is zero, will not return [`false`{.docutils .literal .notranslate}]{.pre}.

    - [`hipBindTexture2D`{.docutils .literal .notranslate}]{.pre}, adds extra check, if pointer for texture reference or device is NULL, returns [`hipErrorNotFound`{.docutils .literal .notranslate}]{.pre}.

    - [`hipBindTextureToArray`{.docutils .literal .notranslate}]{.pre}, if any NULL pointer is input for texture object, resource descriptor, or texture descriptor, returns error [`hipErrorInvalidChannelDescriptor`{.docutils .literal .notranslate}]{.pre}, instead of [`hipErrorInvalidValue`{.docutils .literal .notranslate}]{.pre}.

    - [`hipGetTextureAlignmentOffset`{.docutils .literal .notranslate}]{.pre}, adds a return code [`hipErrorInvalidTexture`{.docutils .literal .notranslate}]{.pre} when the texture reference pointer is NULL.

  - Cooperative Group Related APIs, more calidations are added in the following API implementation:

    - [`hipLaunchCooperativeKernelMultiDevice`{.docutils .literal .notranslate}]{.pre}

    - [`hipLaunchCooperativeKernel`{.docutils .literal .notranslate}]{.pre}

- Invalid stream input parameter handling In order to match the CUDA runtime behavior more closely, HIP APIs with streams passed as input parameters no longer check the stream validity. Previously, the HIP runtime returned an error code [`hipErrorContextIsDestroyed`{.docutils .literal .notranslate}]{.pre} if the stream was invalid. In CUDA version 12 and later, the equivalent behavior is to raise a segmentation fault. HIP runtime now matches the CUDA by causing a segmentation fault. The list of APIs impacted by this change are as follows:

  - Stream Management Related APIs

    - [`hipStreamGetCaptureInfo`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamGetPriority`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamGetFlags`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamDestroy`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamAddCallback`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamQuery`{.docutils .literal .notranslate}]{.pre}

    - [`hipLaunchHostFunc`{.docutils .literal .notranslate}]{.pre}

  - Graph Management Related APIs

    - [`hipGraphUpload`{.docutils .literal .notranslate}]{.pre}

    - [`hipGraphLaunch`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamBeginCaptureToGraph`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamBeginCapture`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamIsCapturing`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamGetCaptureInfo`{.docutils .literal .notranslate}]{.pre}

    - [`hipGraphInstantiateWithParams`{.docutils .literal .notranslate}]{.pre}

  - Memory Management Related APIs

    - [`hipMemcpyPeerAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpy2DValidateParams`{.docutils .literal .notranslate}]{.pre}

    - [`hipMallocFromPoolAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipFreeAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipMallocAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpyAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpyToSymbolAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipStreamAttachMemAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemPrefetchAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipDrvMemcpy3D`{.docutils .literal .notranslate}]{.pre}

    - [`hipDrvMemcpy3DAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipDrvMemcpy2DUnaligned`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpyParam2D`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpyParam2DAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpy2DArrayToArray`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpy2D`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpy2DAsync`{.docutils .literal .notranslate}]{.pre}

    - [`hipDrvMemcpy2DUnaligned`{.docutils .literal .notranslate}]{.pre}

    - [`hipMemcpy3D`{.docutils .literal .notranslate}]{.pre}
...
- Reduced usage of the lock scope in events and kernel handling.
  ...
  - Switches to [`shared_mutex`{.docutils .literal .notranslate}]{.pre} for event validation, uses [`std::unique_lock`{.docutils .literal .notranslate}]{.pre} in HIP runtime to create/destroy event, instead of [`scopedLock`{.docutils .literal .notranslate}]{.pre}.

  - Reduces the [`scopedLock`{.docutils .literal .notranslate}]{.pre} in handling of kernel execution. HIP runtime now calls [`scopedLock`{.docutils .literal .notranslate}]{.pre} during kernel binary creation/initialization, doesn't call it again during kernel vector iteration before launch.
...
- Advanced support for SPIRV, now kernel compilation caching is enabled by default. This feature is controlled by the environment variable [`AMD_COMGR_CACHE`{.docutils .literal .notranslate}]{.pre}, for details, see [hip_rtc document](https://rocm.docs.amd.com/projects/HIP/en/latest/how-to/hip_rtc.html){.reference .external}.

- Programmatic support for scratch limits on the AMD Instinct MI300 and MI350 Series up GPU devices. More enumeration values were added in [`hipLimit_t`{.docutils .literal .notranslate}]{.pre} as following:

  - [`hipExtLimitScratchMin`{.docutils .literal .notranslate}]{.pre}, minimum allowed value in bytes for scratch limit on the device.

  - [`hipExtLimitScratchMax`{.docutils .literal .notranslate}]{.pre}, maximum allowed value in bytes for scratch limit on the device.

  - [`hipExtLimitScratchCurrent`{.docutils .literal .notranslate}]{.pre}, current scratch limit threshold in bytes on the device. Must be between the value [`hipExtLimitScratchMin`{.docutils .literal .notranslate}]{.pre} and [`hipExtLimitScratchMax`{.docutils .literal .notranslate}]{.pre}. Developers can now use the environment variable [`HSA_SCRATCH_SINGLE_LIMIT_ASYNC`{.docutils .literal .notranslate}]{.pre} to change the default allocation size with expected scratch limit in ROCR runtime. On top of it, this value can also be overwritten programmatically in the application using the HIP API [`hipDeviceSetLimit(hipExtLimitScratchCurrent,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`value)`{.docutils .literal .notranslate}]{.pre} to reset the scratch limit value.
...
- The issue of incorrect return error [`hipErrorNoDevice`{.docutils .literal .notranslate}]{.pre}, when a crash occurred on GPU device due to illegal operation or memory violation. HIP runtime now handles the failure on the GPU side properly and reports the precise error code based on the last error seen on the GPU.
...
- A crash in TensorFlow related application. HIP runtime now combines multiple definitions of [`callbackQueue`{.docutils .literal .notranslate}]{.pre} into a single function, in case of an exception, passes its handler to the application and provides corresponding error code.

- Fixed issue of handling the kernel parameters for the graph launch.

- Failures in roc-obj tools. HIP runtime now makes [`DEPRECATED`{.docutils .literal .notranslate}]{.pre} message in roc-obj tools as [`STDERR`{.docutils .literal .notranslate}]{.pre}.
...
- Compilation failure, HIP runtime refactored the vector type alignment with [`__hip_vec_align_v`{.docutils .literal .notranslate}]{.pre}.
...
- Fused Swish/SiLU GEMM (enabled by [`HIPBLASLT_EPILOGUE_SWISH_EXT`{.docutils .literal .notranslate}]{.pre} and [`HIPBLASLT_EPILOGUE_SWISH_BIAS_EXT`{.docutils .literal .notranslate}]{.pre}).

- Support for [`HIPBLASLT_EPILOGUE_GELU_AUX_BIAS`{.docutils .literal .notranslate}]{.pre} for gfx942.

- [`HIPBLASLT_TUNING_USER_MAX_WORKSPACE`{.docutils .literal .notranslate}]{.pre} to constrain the maximum workspace size for user offline tuning.

- [`HIPBLASLT_ORDER_COL16_4R16`{.docutils .literal .notranslate}]{.pre} and [`HIPBLASLT_ORDER_COL16_4R8`{.docutils .literal .notranslate}]{.pre} to [`hipblasLtOrder_t`{.docutils .literal .notranslate}]{.pre} to support [`FP16`{.docutils .literal .notranslate}]{.pre}/[`BF16`{.docutils .literal .notranslate}]{.pre} swizzle GEMM and [`FP8`{.docutils .literal .notranslate}]{.pre} / [`BF8`{.docutils .literal .notranslate}]{.pre} swizzle GEMM respectively.
...
- Single pass operators in [`agent/single_pass_scan_operators.hpp`{.docutils .literal .notranslate}]{.pre} which contains the following API:

  - [`BlockScanRunningPrefixOp`{.docutils .literal .notranslate}]{.pre}

  - [`ScanTileStatus`{.docutils .literal .notranslate}]{.pre}

  - [`ScanTileState`{.docutils .literal .notranslate}]{.pre}

  - [`ReduceByKeyScanTileState`{.docutils .literal .notranslate}]{.pre}

  - [`TilePrefixCallbackOp`{.docutils .literal .notranslate}]{.pre}

- Support for gfx950.

- An overload of [`BlockScan::InclusiveScan`{.docutils .literal .notranslate}]{.pre} that accepts an initial value to seed the scan.

- An overload of [`WarpScan::InclusiveScan`{.docutils .literal .notranslate}]{.pre} that accepts an initial value to seed the scan.

- [`UnrolledThreadLoad`{.docutils .literal .notranslate}]{.pre}, [`UnrolledCopy`{.docutils .literal .notranslate}]{.pre}, and [`ThreadLoadVolatilePointer`{.docutils .literal .notranslate}]{.pre} were added to align hipCUB with CUB.
...
- Updated [`thread_load`{.docutils .literal .notranslate}]{.pre} and [`thread_store`{.docutils .literal .notranslate}]{.pre} to align hipCUB with CUB.

- All kernels now have hidden symbol visibility. All symbols now have inline namespaces that include the library version, (for example, [`hipcub::HIPCUB_300400_NS::symbol`{.docutils .literal .notranslate}]{.pre} instead of [`hipcub::symbol`{.docutils .literal .notranslate}]{.pre}), letting the user link multiple libraries built with different versions of hipCUB.
...
- Deprecated [`hipcub::AsmThreadLoad`{.docutils .literal .notranslate}]{.pre} is removed, use [`hipcub::ThreadLoad`{.docutils .literal .notranslate}]{.pre} instead.

- Deprecated [`hipcub::AsmThreadStore`{.docutils .literal .notranslate}]{.pre} is removed, use [`hipcub::ThreadStore`{.docutils .literal .notranslate}]{.pre} instead.

- Deprecated [`BlockAdjacentDifference::FlagHeads`{.docutils .literal .notranslate}]{.pre}, [`BlockAdjacentDifference::FlagTails`{.docutils .literal .notranslate}]{.pre} and [`BlockAdjacentDifference::FlagHeadsAndTails`{.docutils .literal .notranslate}]{.pre} have been removed.
...
- [#1859](https://github.com/ROCm/HIPIFY/issues/1859){.reference .external}\[hipify-perl\] Fix warnings on unsupported Driver or Runtime APIs which were erroneously not reported.

- [#1930](https://github.com/ROCm/HIPIFY/issues/1930){.reference .external} Revise [`JIT`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`API`{.docutils .literal .notranslate}]{.pre}.
...
- Half float mixed precision to [`hipsparseAxpby`{.docutils .literal .notranslate}]{.pre} where X and Y use [`float16`{.docutils .literal .notranslate}]{.pre} and the result and compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`hipsparseSpVV`{.docutils .literal .notranslate}]{.pre} where X and Y use [`float16`{.docutils .literal .notranslate}]{.pre} and the result and compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`hipsparseSpMM`{.docutils .literal .notranslate}]{.pre} where A and B use [`float16`{.docutils .literal .notranslate}]{.pre} and C and the compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`hipsparseSDDMM`{.docutils .literal .notranslate}]{.pre} where A and B use [`float16`{.docutils .literal .notranslate}]{.pre} and C and the compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float uniform precision to the [`hipsparseScatter`{.docutils .literal .notranslate}]{.pre} and [`hipsparseGather`{.docutils .literal .notranslate}]{.pre} routines.

- Half float uniform precision to the [`hipsparseSDDMM`{.docutils .literal .notranslate}]{.pre} routine.
...
- Support for the following data type combinations for the LLVM target gfx950:
  ...
  - [`FP8`{.docutils .literal .notranslate}]{.pre}(E4M3) inputs, [`F32`{.docutils .literal .notranslate}]{.pre} output, and [`F32`{.docutils .literal .notranslate}]{.pre} Matrix Core accumulation.

  - [`BF8`{.docutils .literal .notranslate}]{.pre}(E5M2) inputs, [`F32`{.docutils .literal .notranslate}]{.pre} output, and [`F32`{.docutils .literal .notranslate}]{.pre} Matrix Core accumulation.
...
- CMake check for [`f8`{.docutils .literal .notranslate}]{.pre} datatype availability.

- [`hiptensorDestroyOperationDescriptor`{.docutils .literal .notranslate}]{.pre} to free all resources related to the provided descriptor.

- [`hiptensorOperationDescriptorSetAttribute`{.docutils .literal .notranslate}]{.pre} to set attribute of a [`hiptensorOperationDescriptor_t`{.docutils .literal .notranslate}]{.pre} object.

- [`hiptensorOperationDescriptorGetAttribute`{.docutils .literal .notranslate}]{.pre} to retrieve an attribute of the provided [`hiptensorOperationDescriptor_t`{.docutils .literal .notranslate}]{.pre} object.

- [`hiptensorCreatePlanPreference`{.docutils .literal .notranslate}]{.pre} to allocate the [`hiptensorPlanPreference_t`{.docutils .literal .notranslate}]{.pre} and enabled users to limit the applicable kernels for a given plan or operation.

- [`hiptensorDestroyPlanPreference`{.docutils .literal .notranslate}]{.pre} to free all resources related to the provided preference.

- [`hiptensorPlanPreferenceSetAttribute`{.docutils .literal .notranslate}]{.pre} to set attribute of a [`hiptensorPlanPreference_t`{.docutils .literal .notranslate}]{.pre} object.

- [`hiptensorPlanGetAttribute`{.docutils .literal .notranslate}]{.pre} to retrieve information about an already-created plan.

- [`hiptensorEstimateWorkspaceSize`{.docutils .literal .notranslate}]{.pre} to determine the required workspace size for the given operation.
...
- Replaced [`hipDataType`{.docutils .literal .notranslate}]{.pre} with [`hiptensorDataType_t`{.docutils .literal .notranslate}]{.pre} for all supported types, for example, [`HIP_R_32F`{.docutils .literal .notranslate}]{.pre} to [`HIPTENSOR_R_32F`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorComputeType_t`{.docutils .literal .notranslate}]{.pre} with [`hiptensorComputeDescriptor_t`{.docutils .literal .notranslate}]{.pre} for all supported types.

- Replaced [`hiptensorInitTensorDescriptor`{.docutils .literal .notranslate}]{.pre} with [`hiptensorCreateTensorDescriptor`{.docutils .literal .notranslate}]{.pre}.

- Changed handle type and API usage from [`*handle`{.docutils .literal .notranslate}]{.pre} to [`handle`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorContractionDescriptor_t`{.docutils .literal .notranslate}]{.pre} with [`hipTensorOperationDescriptor_t`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorInitContractionDescriptor`{.docutils .literal .notranslate}]{.pre} with [`hiptensorCreateContraction`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorContractionFind_t`{.docutils .literal .notranslate}]{.pre} with [`hiptensorPlanPreference_t`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorInitContractionFind`{.docutils .literal .notranslate}]{.pre} with [`hiptensorCreatePlanPreference`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorContractionGetWorkspaceSize`{.docutils .literal .notranslate}]{.pre} with [`hiptensorEstimateWorkspaceSize`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`HIPTENSOR_WORKSPACE_RECOMMENDED`{.docutils .literal .notranslate}]{.pre} with [`HIPTENSOR_WORKSPACE_DEFAULT`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorContractionPlan_t`{.docutils .literal .notranslate}]{.pre} with [`hiptensorPlan_t`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorInitContractionPlan`{.docutils .literal .notranslate}]{.pre} with [`hiptensorCreatePlan`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorContraction`{.docutils .literal .notranslate}]{.pre} with [`hiptensorContract`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorPermutation`{.docutils .literal .notranslate}]{.pre} with [`hiptensorPermute`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorReduction`{.docutils .literal .notranslate}]{.pre} with [`hiptensorReduce`{.docutils .literal .notranslate}]{.pre}.

- Replaced [`hiptensorElementwiseBinary`{.docutils .literal .notranslate}]{.pre} with [`hiptensorElementwiseBinaryExecute`{.docutils .literal .notranslate}]{.pre}.
...
- [`llvm-flang`{.docutils .literal .notranslate}]{.pre}, AMD's next-generation Fortran compiler. It's a re-implementation of the Fortran frontend that can be found at [`llvm/llvm-project/flang`{.docutils .literal .notranslate}]{.pre} on GitHub.
...
- [`BF16::max`{.docutils .literal .notranslate}]{.pre} is now represented by its encoding rather than its expected value.

- Direct warnings now go to [`cout`{.docutils .literal .notranslate}]{.pre} rather [`cerr`{.docutils .literal .notranslate}]{.pre}.

- [`FP8`{.docutils .literal .notranslate}]{.pre} uses hipBLASLt rather than rocBLAS.
...
- Node names are printed to track parsing within the ONNX graph when using the [`MIGRAPHX_TRACE_ONNX_PARSER`{.docutils .literal .notranslate}]{.pre} flag.
...
- Reduced the number of literals used in [`GridSample`{.docutils .literal .notranslate}]{.pre} linear sampler.
...
- Fix bug with [`reshape_lazy`{.docutils .literal .notranslate}]{.pre} in MLIR.

- Quantizelinear fixed for Nearbyint operation.

- Check for empty strings in ONNX node inputs for operations like Resize.

- Parse Resize fix: only check [`keep_aspect_ratio_policy`{.docutils .literal .notranslate}]{.pre} attribute for sizes input.
...
- \[Conv\] Enabled CK wrw solver on gfx950 for the [`BF16`{.docutils .literal .notranslate}]{.pre} data type.
...
- Support for [`unroll=1`{.docutils .literal .notranslate}]{.pre} in device-code generation to improve performance.

- Set a default of 112 channels for a single node with [`8`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`*`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`gfx950`{.docutils .literal .notranslate}]{.pre}.

- Enabled LL128 protocol on the gfx950.

- The ability to choose the unroll factor at runtime using [`RCCL_UNROLL_FACTOR`{.docutils .literal .notranslate}]{.pre}. This can be set at runtime to 1, 2, or 4. This change currently increases compilation and linking time because it triples the number of kernels generated.

- Added MSCCL support for AllGather multinode on the gfx942 and gfx950 (for instance, 16 and 32 GPUs). To enable this feature, set the environment variable [`RCCL_MSCCL_FORCE_ENABLE=1`{.docutils .literal .notranslate}]{.pre}. The maximum message size for MSCCL AllGather usage is [`12292`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`*`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`sizeof(datatype)`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`*`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`nGPUs`{.docutils .literal .notranslate}]{.pre}.

- Thread thresholds for LL/LL128 are selected in Tuning Models for the AMD Instinct MI300X. This impacts the number of channels used for AllGather and ReduceScatter. The channel tuning model is bypassed if [`NCCL_THREAD_THRESHOLDS`{.docutils .literal .notranslate}]{.pre}, [`NCCL_MIN_NCHANNELS`{.docutils .literal .notranslate}]{.pre}, or [`NCCL_MAX_NCHANNELS`{.docutils .literal .notranslate}]{.pre} are set.
...
- Fixed unit test failures in tests ending with the [`ManagedMem`{.docutils .literal .notranslate}]{.pre} and [`ManagedMemGraph`{.docutils .literal .notranslate}]{.pre} suffixes.
...
- Internal API logging for [`gemm`{.docutils .literal .notranslate}]{.pre} debugging using [`ROCBLAS_LAYER`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`8`{.docutils .literal .notranslate}]{.pre}.
...
- [`rocblas_hgemm_kernel_name`{.docutils .literal .notranslate}]{.pre}, [`rocblas_sgemm_kernel_name`{.docutils .literal .notranslate}]{.pre}, and [`rocblas_dgemm_kernel_name`{.docutils .literal .notranslate}]{.pre} API functions.

- The use of [`warpSize`{.docutils .literal .notranslate}]{.pre} as a constexpr.

- The use of deprecated behavior of [`hipPeekLastError`{.docutils .literal .notranslate}]{.pre}.
...
- Optimized [`gemv`{.docutils .literal .notranslate}]{.pre} for small [`m`{.docutils .literal .notranslate}]{.pre} and [`n`{.docutils .literal .notranslate}]{.pre} with a large batch count on gfx942.

- Improved the performance of Level 1 [`dot`{.docutils .literal .notranslate}]{.pre} for all precisions and variants when [`N`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`>`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`100000000`{.docutils .literal .notranslate}]{.pre} on gfx942.

- Improved the performance of Level 1 [`asum`{.docutils .literal .notranslate}]{.pre} and [`nrm2`{.docutils .literal .notranslate}]{.pre} for all precisions and variants on gfx942.
...
- Support numerics when [`trsm`{.docutils .literal .notranslate}]{.pre} is running with [`rocblas_status_perf_degraded`{.docutils .literal .notranslate}]{.pre}.

- Fixed the build dependency installation of [`joblib`{.docutils .literal .notranslate}]{.pre} on some operating systems.

- Return [`rocblas_status_internal_error`{.docutils .literal .notranslate}]{.pre} when [`rocblas_[set,get]_`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`[matrix,vector]`{.docutils .literal .notranslate}]{.pre} is called with a host pointer in place of a device pointer.
...
- Plugin architecture:
  ...
  - [`rocm_bandwidth_test`{.docutils .literal .notranslate}]{.pre} is now the [`framework`{.docutils .literal .notranslate}]{.pre} for individual [`plugins`{.docutils .literal .notranslate}]{.pre} and features. The [`framework`{.docutils .literal .notranslate}]{.pre} is available at: [`/opt/rocm/bin/`{.docutils .literal .notranslate}]{.pre}
...
- Analysis report based filtering
  ...
  - [`-b`{.docutils .literal .notranslate}]{.pre} option in profile mode now also accepts metric id(s) for analysis report based filtering.

  - [`-b`{.docutils .literal .notranslate}]{.pre} option in profile mode also accepts hardware IP block for filtering; however, this filter support will be deprecated soon.

  - [`--list-metrics`{.docutils .literal .notranslate}]{.pre} option added in profile mode to list possible metric id(s), similar to analyze mode.
...
- Changed [`normal_unit`{.docutils .literal .notranslate}]{.pre} default to [`per_kernel`{.docutils .literal .notranslate}]{.pre}.

- Decreased profiling time by not collecting unused counters in post-analysis.

- Updated Dash to \>=3.0.0 (for web UI).

- Changed the condition when Roofline PDFs are generated during general profiling and [`--roof-only`{.docutils .literal .notranslate}]{.pre} profiling (skip only when [`--no-roof`{.docutils .literal .notranslate}]{.pre} option is present).
...
- Fixed an issue of TCC channel counters collection in [`rocprofv3`{.docutils .literal .notranslate}]{.pre}.

- Fixed peak FLOPS of [`F8`{.docutils .literal .notranslate}]{.pre}, [`I8`{.docutils .literal .notranslate}]{.pre}, [`F16`{.docutils .literal .notranslate}]{.pre}, and [`BF16`{.docutils .literal .notranslate}]{.pre} on AMD Instinct MI300.

- Fixed not detecting memory clock issue when using [`amd-smi`{.docutils .literal .notranslate}]{.pre}.
...
- On AMD Instinct MI100, accumulation counters are not collected, resulting in the following metrics failing to show up in the analysis: Instruction Fetch Latency, Wavefront Occupancy, LDS Latency.
  ...
  - As a workaround, use the environment variable [`ROCPROF=rocprof`{.docutils .literal .notranslate}]{.pre}, to use [`rocprof`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`v1`{.docutils .literal .notranslate}]{.pre} for profiling on AMD Instinct MI100.

- GPU id filtering is not supported when using [`rocprofv3`{.docutils .literal .notranslate}]{.pre}.

- Analysis of previously collected workload data will not work due to sysinfo.csv schema change.

  - As a workaround, re-run the profiling operation for the workload and interrupt the process after 10 seconds. Followed by copying the [`sysinfo.csv`{.docutils .literal .notranslate}]{.pre} file from the new data folder to the old one. This assumes your system specification hasn't changed since the creation of the previous workload data.

- Analysis of new workloads might require providing shader/memory clock speed using [`--specs-correction`{.docutils .literal .notranslate}]{.pre} operation if amd-smi or rocminfo does not provide clock speeds.
...
- [`rocprof`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`v1/v2/v3`{.docutils .literal .notranslate}]{.pre} interfaces will be removed in favor of the ROCprofiler-SDK interface, which directly accesses [`rocprofv3`{.docutils .literal .notranslate}]{.pre} C++ tool. Using [`rocprof`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`v1/v2/v3`{.docutils .literal .notranslate}]{.pre} interfaces will trigger a deprecation warning.
  ...
  - To use ROCprofiler-SDK interface, set environment variable [`ROCPROF=rocprofiler-sdk`{.docutils .literal .notranslate}]{.pre} and optionally provide profile mode option [`--rocprofiler-sdk-library-path`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`/path/to/librocprofiler-sdk.so`{.docutils .literal .notranslate}]{.pre}. Add [`--rocprofiler-sdk-library-path`{.docutils .literal .notranslate}]{.pre} runtime option to choose the path to ROCprofiler-SDK library to be used.

- Hardware IP block based filtering using [`-b`{.docutils .literal .notranslate}]{.pre} option in profile mode will be removed in favor of analysis report block based filtering using [`-b`{.docutils .literal .notranslate}]{.pre} option in profile mode.
...
- Support for GPU metrics 1.8.
  ...
  - Added new fields for [`rsmi_gpu_metrics_t`{.docutils .literal .notranslate}]{.pre} including:
...
- Removed backwards compatibility for [`rsmi_dev_gpu_metrics_info_get()`{.docutils .literal .notranslate}]{.pre}'s [`jpeg_activity`{.docutils .literal .notranslate}]{.pre} and [`vcn_activity`{.docutils .literal .notranslate}]{.pre} fields. Alternatively use [`xcp_stats.jpeg_busy`{.docutils .literal .notranslate}]{.pre} and [`xcp_stats.vcn_busy`{.docutils .literal .notranslate}]{.pre}.
  ...
  - Backwards compatibility is removed for [`jpeg_activity`{.docutils .literal .notranslate}]{.pre} and [`vcn_activity`{.docutils .literal .notranslate}]{.pre} fields, if the [`jpeg_busy`{.docutils .literal .notranslate}]{.pre} or [`vcn_busy`{.docutils .literal .notranslate}]{.pre} field is available.
    ...
    - Providing both [`vcn_activity`{.docutils .literal .notranslate}]{.pre}/[`jpeg_activity`{.docutils .literal .notranslate}]{.pre} and XCP (partition) stats [`vcn_busy`{.docutils .literal .notranslate}]{.pre}/[`jpeg_busy`{.docutils .literal .notranslate}]{.pre} caused confusion for users about which field to use. By removing backward compatibility, it is easier to identify the relevant field.
...
- ROCprofiler-SDK is now used to trace RCCL API and collect communication counters.
  ...
  - Use the setting [`ROCPROFSYS_USE_RCCLP`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ON`{.docutils .literal .notranslate}]{.pre} to enable profiling and tracing of RCCL application data.
...
- [`rocprim::accumulator_t`{.docutils .literal .notranslate}]{.pre} to ensure parity with CCCL.

- Test for [`rocprim::accumulator_t`{.docutils .literal .notranslate}]{.pre}.

- [`rocprim::invoke_result_r`{.docutils .literal .notranslate}]{.pre} to ensure parity with CCCL.

- Function [`is_build_in`{.docutils .literal .notranslate}]{.pre} into [`rocprim::traits::get`{.docutils .literal .notranslate}]{.pre}.

- Virtual shared memory as a fallback option in [`rocprim::device_merge`{.docutils .literal .notranslate}]{.pre} when it exceeds shared memory capacity, similar to [`rocprim::device_select`{.docutils .literal .notranslate}]{.pre}, [`rocprim::device_partition`{.docutils .literal .notranslate}]{.pre}, and [`rocprim::device_merge_sort`{.docutils .literal .notranslate}]{.pre}, which already include this feature.

- Initial value support to device level inclusive scans.

- New optimization to the backend for [`device_transform`{.docutils .literal .notranslate}]{.pre} when the input and output are pointers.

- [`LoadType`{.docutils .literal .notranslate}]{.pre} to [`transform_config`{.docutils .literal .notranslate}]{.pre}, which is used for the [`device_transform`{.docutils .literal .notranslate}]{.pre} when the input and output are pointers.

- [`rocprim:device_transform`{.docutils .literal .notranslate}]{.pre} for n-ary transform operations API with as input [`n`{.docutils .literal .notranslate}]{.pre} number of iterators inside a [`rocprim::tuple`{.docutils .literal .notranslate}]{.pre}.

- [`rocprim::key_value_pair::operator==`{.docutils .literal .notranslate}]{.pre}.

- The [`rocprim::unrolled_copy`{.docutils .literal .notranslate}]{.pre} thread function to copy multiple items inside a thread.

- The [`rocprim::unrolled_thread_load`{.docutils .literal .notranslate}]{.pre} function to load multiple items inside a thread using [`rocprim::thread_load`{.docutils .literal .notranslate}]{.pre}.

- [`rocprim::int128_t`{.docutils .literal .notranslate}]{.pre} and [`rocprim::uint128_t`{.docutils .literal .notranslate}]{.pre} to benchmarks for improved performance evaluation on 128-bit integers.

- [`rocprim::int128_t`{.docutils .literal .notranslate}]{.pre} to the supported autotuning types to improve performance for 128-bit integers.

- The [`rocprim::merge_inplace`{.docutils .literal .notranslate}]{.pre} function for merging in-place.
...
- Marked the initialisation constructor of [`rocprim::reverse_iterator<Iter>`{.docutils .literal .notranslate}]{.pre} [`explicit`{.docutils .literal .notranslate}]{.pre}, use [`rocprim::make_reverse_iterator`{.docutils .literal .notranslate}]{.pre}.

- Merged [`radix_key_codec`{.docutils .literal .notranslate}]{.pre} into type_traits system.

- Renamed [`type_traits_interface.hpp`{.docutils .literal .notranslate}]{.pre} to [`type_traits.hpp`{.docutils .literal .notranslate}]{.pre}, rename the original [`type_traits.hpp`{.docutils .literal .notranslate}]{.pre} to [`type_traits_functions.hpp`{.docutils .literal .notranslate}]{.pre}.
...
- The default scan accumulator types for device-level scan algorithms have changed. This is a breaking change. The previous default accumulator types could lead to situations in which unexpected overflow occurred, such as when the input or initial type was smaller than the output type. This is a complete list of affected functions and how their default accumulator types are changing:
  ...
  - [`rocprim::inclusive_scan`{.docutils .literal .notranslate}]{.pre}

    - Previous default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`typename`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`std::iterator_traits<InputIterator>::value_type>`{.docutils .literal .notranslate}]{.pre}

    - Current default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::accumulator_t<BinaryFunction,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`typename`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`std::iterator_traits<InputIterator>::value_type>`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::deterministic_inclusive_scan`{.docutils .literal .notranslate}]{.pre}

    - Previous default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`typename`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`std::iterator_traits<InputIterator>::value_type>`{.docutils .literal .notranslate}]{.pre}

    - Current default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::accumulator_t<BinaryFunction,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`typename`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`std::iterator_traits<InputIterator>::value_type>`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::exclusive_scan`{.docutils .literal .notranslate}]{.pre}

    - Previous default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`detail::input_type_t<InitValueType>>`{.docutils .literal .notranslate}]{.pre}

    - Current default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::accumulator_t<BinaryFunction,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::detail::input_type_t<InitValueType>>`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::deterministic_exclusive_scan`{.docutils .literal .notranslate}]{.pre}

    - Previous default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`detail::input_type_t<InitValueType>>`{.docutils .literal .notranslate}]{.pre}

    - Current default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::accumulator_t<BinaryFunction,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::detail::input_type_t<InitValueType>>`{.docutils .literal .notranslate}]{.pre}

- Undeprecated internal [`detail::raw_storage`{.docutils .literal .notranslate}]{.pre}.

- A new version of [`rocprim::thread_load`{.docutils .literal .notranslate}]{.pre} and [`rocprim::thread_store`{.docutils .literal .notranslate}]{.pre} replaces the deprecated [`rocprim::thread_load`{.docutils .literal .notranslate}]{.pre} and [`rocprim::thread_store`{.docutils .literal .notranslate}]{.pre} functions. The versions avoid inline assembly where possible, and don't hinder the optimizer as much as a result.
...
- [`rocprim::traits::is_fundamental`{.docutils .literal .notranslate}]{.pre}, use [`rocprim::traits::get<T>::is_fundamental()`{.docutils .literal .notranslate}]{.pre} directly.

- The deprecated parameters [`short_radix_bits`{.docutils .literal .notranslate}]{.pre} and [`ShortRadixBits`{.docutils .literal .notranslate}]{.pre} from the [`segmented_radix_sort`{.docutils .literal .notranslate}]{.pre} config. They were unused, it is only an API change.

- The deprecated [`operator<<`{.docutils .literal .notranslate}]{.pre} from the iterators.

- The deprecated [`TwiddleIn`{.docutils .literal .notranslate}]{.pre} and [`TwiddleOut`{.docutils .literal .notranslate}]{.pre}. Use [`radix_key_codec`{.docutils .literal .notranslate}]{.pre} instead.

- The deprecated flags API of [`block_adjacent_difference`{.docutils .literal .notranslate}]{.pre}. Use [`subtract_left()`{.docutils .literal .notranslate}]{.pre} or [`block_discontinuity::flag_heads()`{.docutils .literal .notranslate}]{.pre} instead.

- The deprecated [`to_exclusive`{.docutils .literal .notranslate}]{.pre} functions in the warp scans.

- The [`rocprim::load_cs`{.docutils .literal .notranslate}]{.pre} from the [`cache_load_modifier`{.docutils .literal .notranslate}]{.pre} enum. Use [`rocprim::load_nontemporal`{.docutils .literal .notranslate}]{.pre} instead.

- The [`rocprim::store_cs`{.docutils .literal .notranslate}]{.pre} from the [`cache_store_modifier`{.docutils .literal .notranslate}]{.pre} enum. Use [`rocprim::store_nontemporal`{.docutils .literal .notranslate}]{.pre} instead.

- The deprecated header file [`rocprim/detail/match_result_type.hpp`{.docutils .literal .notranslate}]{.pre}. Include [`rocprim/type_traits.hpp`{.docutils .literal .notranslate}]{.pre} instead. This header included:

  - [`rocprim::detail::invoke_result`{.docutils .literal .notranslate}]{.pre}. Use [`rocprim::invoke_result`{.docutils .literal .notranslate}]{.pre} instead.

  - [`rocprim::detail::invoke_result_binary_op`{.docutils .literal .notranslate}]{.pre}. Use [`rocprim::invoke_result_binary_op`{.docutils .literal .notranslate}]{.pre} instead.

  - [`rocprim::detail::match_result_type`{.docutils .literal .notranslate}]{.pre}. Use [`rocprim::invoke_result_binary_op_t`{.docutils .literal .notranslate}]{.pre} instead.

- The deprecated [`rocprim::detail::radix_key_codec`{.docutils .literal .notranslate}]{.pre} function. Use [`rocprim::radix_key_codec`{.docutils .literal .notranslate}]{.pre} instead.

- Removed [`rocprim/detail/radix_sort.hpp`{.docutils .literal .notranslate}]{.pre}, functionality can now be found in [`rocprim/thread/radix_key_codec.hpp`{.docutils .literal .notranslate}]{.pre}.

- Removed C++14 support. Only C++17 is supported.

- Due to the removal of [`__AMDGCN_WAVEFRONT_SIZE`{.docutils .literal .notranslate}]{.pre} in the compiler, the following deprecated warp size-related symbols have been removed:

  - [`rocprim::device_warp_size()`{.docutils .literal .notranslate}]{.pre}

    - For compile-time constants, this is replaced with [`rocprim::arch::wavefront::min_size()`{.docutils .literal .notranslate}]{.pre} and [`rocprim::arch::wavefront::max_size()`{.docutils .literal .notranslate}]{.pre}. Use this when allocating global or shared memory.

    - For run-time constants, this is replaced with [`rocprim::arch::wavefront::size().`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::warp_size()`{.docutils .literal .notranslate}]{.pre}

    - Use [`rocprim::host_warp_size()`{.docutils .literal .notranslate}]{.pre}, [`rocprim::arch::wavefront::min_size()`{.docutils .literal .notranslate}]{.pre} or [`rocprim::arch::wavefront::max_size()`{.docutils .literal .notranslate}]{.pre} instead.

  - [`ROCPRIM_WAVEFRONT_SIZE`{.docutils .literal .notranslate}]{.pre}

    - Use [`rocprim::arch::wavefront::min_size()`{.docutils .literal .notranslate}]{.pre} or [`rocprim::arch::wavefront::max_size()`{.docutils .literal .notranslate}]{.pre} instead.

  - [`__AMDGCN_WAVEFRONT_SIZE`{.docutils .literal .notranslate}]{.pre}
...
- Fixed an issue where [`device_segmented_reduce`{.docutils .literal .notranslate}]{.pre} reported autotuning throughput being 5x lower than it was in reality.

- Fixed device radix sort not returning the correct required temporary storage when a double buffer contains [`nullptr`{.docutils .literal .notranslate}]{.pre}.

- Fixed constness of equality operators ([`==`{.docutils .literal .notranslate}]{.pre} and [`!=`{.docutils .literal .notranslate}]{.pre}) in [`rocprim::key_value_pair`{.docutils .literal .notranslate}]{.pre}.
...
- [`rocprofiler_create_counter`{.docutils .literal .notranslate}]{.pre} to facilitate adding custom derived counters at runtime.

- Support in [`rocprofv3`{.docutils .literal .notranslate}]{.pre} for iteration based counter multiplexing.

- Perfetto support for counter collection.

- Support for negating [`rocprofv3`{.docutils .literal .notranslate}]{.pre} tracing options when using aggregate options such as [`--sys-trace`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--hsa-trace=no`{.docutils .literal .notranslate}]{.pre}.

- [`--agent-index`{.docutils .literal .notranslate}]{.pre} option in [`rocprofv3`{.docutils .literal .notranslate}]{.pre} to specify the agent naming convention in the output:
...
- MI300 and MI350 stochastic (hardware-based) PC sampling support in ROCProfiler-SDK and [`rocprofv3`{.docutils .literal .notranslate}]{.pre}.

- Python bindings for [`rocprofiler-sdk-roctx`{.docutils .literal .notranslate}]{.pre}.

- SQLite3 output support for [`rocprofv3`{.docutils .literal .notranslate}]{.pre} using [`--output-format`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocpd`{.docutils .literal .notranslate}]{.pre}.

- [`rocprofiler-sdk-rocpd`{.docutils .literal .notranslate}]{.pre} package:

  - Public API in [`include/rocprofiler-sdk-rocpd/rocpd.h`{.docutils .literal .notranslate}]{.pre}.

  - Library implementation in [`librocprofiler-sdk-rocpd.so`{.docutils .literal .notranslate}]{.pre}.

  - Support for [`find_package(rocprofiler-sdk-rocpd)`{.docutils .literal .notranslate}]{.pre}.

  - [`rocprofiler-sdk-rocpd`{.docutils .literal .notranslate}]{.pre} DEB and RPM packages.

- [`--version`{.docutils .literal .notranslate}]{.pre} option in [`rocprofv3`{.docutils .literal .notranslate}]{.pre}.

- [`rocpd`{.docutils .literal .notranslate}]{.pre} Python package.

- Thread trace as experimental API.

- ROCprof Trace Decoder as experimental API:

  - Requires [ROCprof Trace Decoder plugin](https://github.com/rocm/rocprof-trace-decoder){.reference .external}.

- Thread trace option in the [`rocprofv3`{.docutils .literal .notranslate}]{.pre} tool under the [`--att`{.docutils .literal .notranslate}]{.pre} parameters:

  - See [using thread trace with rocprofv3](https://rocm.docs.amd.com/projects/rocprofiler-sdk/en/docs-7.0.0/how-to/using-thread-trace.html){.reference .external}

  - Requires [ROCprof Trace Decoder plugin](https://github.com/rocm/rocprof-trace-decoder){.reference .external}.

- [`rocpd`{.docutils .literal .notranslate}]{.pre} output format documentation:

  - Requires [ROCprof Trace Decoder plugin](https://github.com/rocm/rocprof-trace-decoder){.reference .external}.

- Perfetto support for scratch memory.

- Support in the [`rocprofv3`{.docutils .literal .notranslate}]{.pre} avail tool for command-line arguments.
...
- [`vaddr-to-file-offset`{.docutils .literal .notranslate}]{.pre} mapping in [`disassembly.hpp`{.docutils .literal .notranslate}]{.pre} to use the dedicated comgr API.

- [`rocprofiler_uuid_t`{.docutils .literal .notranslate}]{.pre} ABI to hold 128 bit value.

- [`rocprofv3`{.docutils .literal .notranslate}]{.pre} shorthand argument for [`--collection-period`{.docutils .literal .notranslate}]{.pre} to [`-P`{.docutils .literal .notranslate}]{.pre} (upper-case) while [`-p`{.docutils .literal .notranslate}]{.pre} (lower-case) is reserved for later use.

- Default output format for [`rocprofv3`{.docutils .literal .notranslate}]{.pre} to [`rocpd`{.docutils .literal .notranslate}]{.pre} (SQLite3 database).

- [`rocprofv3`{.docutils .literal .notranslate}]{.pre} avail tool to be renamed from [`rocprofv3_avail`{.docutils .literal .notranslate}]{.pre} to [`rocprofv3-avail`{.docutils .literal .notranslate}]{.pre} tool.
...
- Fixed queue preemption error and [`HSA_STATUS_ERROR_INVALID_PACKET_FORMAT`{.docutils .literal .notranslate}]{.pre} error for stochastic PC-sampling in MI300X, leading to stabler runs.

- Fixed the system hang issue for host-trap PC-sampling on AMD Instinct MI300X.

- Fixed [`rocpd`{.docutils .literal .notranslate}]{.pre} counter collection issue when counter collection alone is enabled. [`rocpd_kernel_dispatch`{.docutils .literal .notranslate}]{.pre} table is updated to be populated by counters data instead of kernel_dispatch data.

- Fixed [`rocprofiler_*_id_t`{.docutils .literal .notranslate}]{.pre} structs for inconsistency related to a "null" handle:
...
- Changed the unnecessarily large 64-bit data type for constants used for skipping in [`MRG32K3A`{.docutils .literal .notranslate}]{.pre} to the 32-bit data type.
...
- Removed instances of the deprecated clang definition [`__AMDGCN_WAVEFRONT_SIZE`{.docutils .literal .notranslate}]{.pre}.
...
- Directly accessing the (scrambled) sobol32 and sobol64 constants and direction vectors is no longer supported. For:
  ...
  - [`h_scrambled_sobol32_constants`{.docutils .literal .notranslate}]{.pre}, use [`rocrand_get_scramble_constants32`{.docutils .literal .notranslate}]{.pre} instead.

  - [`h_scrambled_sobol64_constants`{.docutils .literal .notranslate}]{.pre}, use [`rocrand_get_scramble_constants64`{.docutils .literal .notranslate}]{.pre} instead.

  - [`rocrand_h_sobol32_direction_vectors`{.docutils .literal .notranslate}]{.pre}, use [`rocrand_get_direction_vectors32`{.docutils .literal .notranslate}]{.pre} instead.

  - [`rocrand_h_sobol64_direction_vectors`{.docutils .literal .notranslate}]{.pre}, use [`rocrand_get_direction_vectors64`{.docutils .literal .notranslate}]{.pre} instead.
...
- New API [`hsa_amd_portable_export_dmabuf_v2`{.docutils .literal .notranslate}]{.pre} extension of existing [`hsa_amd_portable_export_dmabuf`{.docutils .literal .notranslate}]{.pre} API to support new flags parameter. This allows specifying the new [`HSA_AMD_DMABUF_MAPPING_TYPE_PCIE`{.docutils .literal .notranslate}]{.pre} flag when exporting dma-bufs.
...
- New APIs: [`rocshmem_ctx_barrier`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_ctx_barrier_wave`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_ctx_barrier_wg`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_barrier_all`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_barrier_all_wave`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_barrier_all_wg`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_ctx_sync`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_ctx_sync_wave`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_ctx_sync_wg`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_sync_all`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_sync_all_wave`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_sync_all_wg`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_init_attr`{.docutils .literal .notranslate}]{.pre}, [`rocshmem_get_uniqueid`{.docutils .literal .notranslate}]{.pre}, and [`rocshmem_set_attr_uniqueid_args`{.docutils .literal .notranslate}]{.pre}.

- [`dlmalloc`{.docutils .literal .notranslate}]{.pre} based allocator.
...
- The [`v2_SpMV`{.docutils .literal .notranslate}]{.pre} generic routine for computing sparse matrix vector multiplication. As opposed to the deprecated [`rocsparse_spmv`{.docutils .literal .notranslate}]{.pre} routine, this routine does not use a fallback algorithm if a non-implemented configuration is encountered and will return an error in such a case. For the deprecated [`rocsparse_spmv`{.docutils .literal .notranslate}]{.pre} routine, the user can enable warning messages in situations where a fallback algorithm is used by either calling the [`rocsparse_enable_debug`{.docutils .literal .notranslate}]{.pre} routine upfront or exporting the variable [`ROCSPARSE_DEBUG`{.docutils .literal .notranslate}]{.pre} (with the shell command [`export`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ROCSPARSE_DEBUG=1`{.docutils .literal .notranslate}]{.pre}).

- Half float mixed precision to [`rocsparse_axpby`{.docutils .literal .notranslate}]{.pre} where X and Y use [`float16`{.docutils .literal .notranslate}]{.pre} and the result and compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`rocsparse_spvv`{.docutils .literal .notranslate}]{.pre} where X and Y use [`float16`{.docutils .literal .notranslate}]{.pre} and the result and compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`rocsparse_spmv`{.docutils .literal .notranslate}]{.pre} where A and X use [`float16`{.docutils .literal .notranslate}]{.pre} and Y and the compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`rocsparse_spmm`{.docutils .literal .notranslate}]{.pre} where A and B use [`float16`{.docutils .literal .notranslate}]{.pre} and C and the compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float mixed precision to [`rocsparse_sddmm`{.docutils .literal .notranslate}]{.pre} where A and B use [`float16`{.docutils .literal .notranslate}]{.pre} and C and the compute type use [`float`{.docutils .literal .notranslate}]{.pre}.

- Half float uniform precision to the [`rocsparse_scatter`{.docutils .literal .notranslate}]{.pre} and [`rocsparse_gather`{.docutils .literal .notranslate}]{.pre} routines.

- Half float uniform precision to the [`rocsparse_sddmm`{.docutils .literal .notranslate}]{.pre} routine.

- The [`rocsparse_spmv_alg_csr_rowsplit`{.docutils .literal .notranslate}]{.pre} algorithm.
...
- Fixed a memory access fault in the [`rocsparse_Xbsrilu0`{.docutils .literal .notranslate}]{.pre} routines.

- Fixed failures that could occur in [`rocsparse_Xbsrsm_solve`{.docutils .literal .notranslate}]{.pre} or [`rocsparse_spsm`{.docutils .literal .notranslate}]{.pre} with BSR format when using host pointer mode.
...
- [`test_param_fixtures.hpp`{.docutils .literal .notranslate}]{.pre} to store all the parameters for typed test suites.

- [`test_real_assertions.hpp`{.docutils .literal .notranslate}]{.pre} to handle unit test assertions for real numbers.

- [`test_imag_assertions.hpp`{.docutils .literal .notranslate}]{.pre} to handle unit test assertions for imaginary numbers.

- [`clang++`{.docutils .literal .notranslate}]{.pre} is now used to compile google benchmarks on Windows.
...
- Updated linkage of [`rocwmma::synchronize_workgroup`{.docutils .literal .notranslate}]{.pre} to inline.

- rocWMMA [`mma_sync`{.docutils .literal .notranslate}]{.pre} API now supports [`wave`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`tile`{.docutils .literal .notranslate}]{.pre} fragment sizes.
...
- The [`logical_operations`{.docutils .literal .notranslate}]{.pre} function category has been renamed to [`bitwise_operations`{.docutils .literal .notranslate}]{.pre}.

- TurboJPEG package installation enabled for RPP Test Suite with [`sudo`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`apt-get`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`install`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`libturbojpeg0-dev`{.docutils .literal .notranslate}]{.pre}. Instructions have been updated in utilities/test_suite/README.md.

- The [`swap_channels`{.docutils .literal .notranslate}]{.pre} augmentation has been changed to [`channel_permute`{.docutils .literal .notranslate}]{.pre}. [`channel_permute`{.docutils .literal .notranslate}]{.pre} now also accepts a new argument, [`permutationTensor`{.docutils .literal .notranslate}]{.pre} (pointer to an unsigned int tensor), that provides the permutation order to swap the RGB channels of each input image in the batch in any order:

  [`RppStatus`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rppt_swap_channels_host(RppPtr_t`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`srcPtr,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`RpptDescPtr`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`srcDescPtr,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`RppPtr_t`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`dstPtr,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`RpptDescPtr`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`dstDescPtr,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rppHandle_t`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rppHandle);`{.docutils .literal .notranslate}]{.pre}
...
- Building issue for [`COMGR`{.docutils .literal .notranslate}]{.pre} dynamic load on Fedora and other Distros. HIP runtime now doesn't link against [`libamd_comgr.so`{.docutils .literal .notranslate}]{.pre}.

- Failure in the API [`hipStreamDestroy`{.docutils .literal .notranslate}]{.pre}, when stream type is [`hipStreamLegacy`{.docutils .literal .notranslate}]{.pre}. The API now returns error code [`hipErrorInvalidResourceHandle`{.docutils .literal .notranslate}]{.pre} on this condition.

- Kernel launch errors, such as [`shared`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`object`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`initialization`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`failed`{.docutils .literal .notranslate}]{.pre}, [`invalid`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`device`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`function`{.docutils .literal .notranslate}]{.pre} or [`kernel`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`execution`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`failure`{.docutils .literal .notranslate}]{.pre}. HIP runtime now loads [`COMGR`{.docutils .literal .notranslate}]{.pre} properly considering the file with its name and mapped image.
...
- Due to an upcoming compiler change, the following symbols related to warp size have been marked as deprecated and will be removed in an upcoming major release:
  ...
  - [`rocprim::device_warp_size()`{.docutils .literal .notranslate}]{.pre}. This has been replaced by [`rocprim::arch::wavefront::min_size()`{.docutils .literal .notranslate}]{.pre} and [`rocprim::arch::wavefront::max_size()`{.docutils .literal .notranslate}]{.pre} for compile-time constants. Use these when allocating global or shared memory. For run-time constants, use [`rocprim::arch::wavefront::size()`{.docutils .literal .notranslate}]{.pre}.

  - [`rocprim::warp_size()`{.docutils .literal .notranslate}]{.pre}

  - [`ROCPRIM_WAVEFRONT_SIZE`{.docutils .literal .notranslate}]{.pre}

- The default scan accumulator types for device-level scan algorithms will be changed in an upcoming release, resulting in a breaking change. Previously, the default accumulator type was set to the input type for the inclusive scans and to the initial value type for the exclusive scans. This could lead to unexpected overflow if the input or initial type was smaller than the output type when the accumulator type wasn't explicitly set using the [`AccType`{.docutils .literal .notranslate}]{.pre} template parameter. The new default accumulator types will be set to the type that results when the input or initial value type is applied to the scan operator.

  The following is the complete list of affected functions and how their default accumulator types are changing:

  - [`rocprim::inclusive_scan`{.docutils .literal .notranslate}]{.pre}

    - current default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`typename`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`std::iterator_traits<InputIterator>::value_type>`{.docutils .literal .notranslate}]{.pre}

    - future default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rocprim::invoke_result_binary_op_t<typename`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`std::iterator_traits<InputIterator>::value_type,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`BinaryFunction>`{.docutils .literal .notranslate}]{.pre}
  ...
  - [`rocprim::exclusive_scan`{.docutils .literal .notranslate}]{.pre}
    ...
    - current default: [`class`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`AccType`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`detail::input_type_t<InitValueType>>`{.docutils .literal .notranslate}]{.pre}
...
- Dumping CPER entries from RAS tool [`amdsmi_get_gpu_cper_entries()`{.docutils .literal .notranslate}]{.pre} to Python and C APIs.
  ...
  - Dumping CPER entries consist of [`amdsmi_cper_hdr_t`{.docutils .literal .notranslate}]{.pre}.
...
- Added dynamic virtualization mode detection.
  ...
  - Added new C and Python API [`amdsmi_get_gpu_virtualization_mode_info`{.docutils .literal .notranslate}]{.pre}.

  - Added new C and Python enum [`amdsmi_virtualization_mode_t`{.docutils .literal .notranslate}]{.pre}.

- Added TVIOL_ACTIVE to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}.

- Added support for GPU metrics 1.7 to [`amdsmi_get_gpu_metrics_info()`{.docutils .literal .notranslate}]{.pre}.

- Added new API [`amdsmi_get_gpu_xgmi_link_status()`{.docutils .literal .notranslate}]{.pre} and CLI [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`xgmi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--link-status`{.docutils .literal .notranslate}]{.pre}.

- Added fclk and socclk info to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-c/--clock`{.docutils .literal .notranslate}]{.pre}.
...
- Added new Python dependencies: [`python3-setuptools`{.docutils .literal .notranslate}]{.pre} and [`python3-wheel`{.docutils .literal .notranslate}]{.pre}.

- Removed initialization requirements for [`amdsmi_get_lib_version()`{.docutils .literal .notranslate}]{.pre} and added [`amdsmi_get_rocm_version()`{.docutils .literal .notranslate}]{.pre} to the Python API & CLI.

- Added an additional argument [`sensor_ind`{.docutils .literal .notranslate}]{.pre} to [`amdsmi_get_power_info()`{.docutils .literal .notranslate}]{.pre}.

  - This change breaks previous C API calls and will require a change.

  - Python API now accepts [`sensor_ind`{.docutils .literal .notranslate}]{.pre} as an optional argument. This does not impact previous usage.

- Deprecated enum [`AMDSMI_NORMAL_STRING_LENGTH`{.docutils .literal .notranslate}]{.pre} in favor of [`AMDSMI_MAX_STRING_LENGTH`{.docutils .literal .notranslate}]{.pre}.

- Changed to use thread local mutex by default.

  - Most [`sysfs`{.docutils .literal .notranslate}]{.pre} reads do not require cross-process level mutex and writes to [`sysfs`{.docutils .literal .notranslate}]{.pre} should be protected by the kernel already.

  - Users can still switch to the old behavior by setting the environment variable [`AMDSMI_MUTEX_CROSS_PROCESS=1`{.docutils .literal .notranslate}]{.pre}.

- Changed [`amdsmi_vram_vendor_type_t`{.docutils .literal .notranslate}]{.pre} enum names impacting the [`amdsmi_vram_info_t`{.docutils .literal .notranslate}]{.pre} structure. This change also impacts the usage of the [`vram_vendor`{.docutils .literal .notranslate}]{.pre} output of [`amdsmi_get_gpu_vram_info()`{.docutils .literal .notranslate}]{.pre}.

- Changed the [`amdsmi_nps_caps_t`{.docutils .literal .notranslate}]{.pre} struct impacting [`amdsmi_memory_partition_config_t`{.docutils .literal .notranslate}]{.pre}, [`amdsmi_accelerator_partition_t`{.docutils .literal .notranslate}]{.pre}, [`amdsmi_accelerator_partition_profile_config_t`{.docutils .literal .notranslate}]{.pre}. Affected functions are:

  - [`amdsmi_get_gpu_memory_partition_config()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_gpu_accelerator_partition_profile()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_get_gpu_accelerator_partition_profile_config()`{.docutils .literal .notranslate}]{.pre}

- Corrected CLI CPU argument name. [`--cpu-pwr-svi-telemtry-rails`{.docutils .literal .notranslate}]{.pre} is now [`--cpu-pwr-svi-telemetry-rails`{.docutils .literal .notranslate}]{.pre}.

- Added amdgpu driver version and amd_hsmp driver version to the [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`version`{.docutils .literal .notranslate}]{.pre} command.

- All [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`set`{.docutils .literal .notranslate}]{.pre} and [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`reset`{.docutils .literal .notranslate}]{.pre} options are now mutually exclusive. You can now only use one [`set`{.docutils .literal .notranslate}]{.pre} option as a time.

- Changed the name of the [`power`{.docutils .literal .notranslate}]{.pre} field to [`energy_accumulator`{.docutils .literal .notranslate}]{.pre} in the Python API for [`amdsmi_get_energy_count()`{.docutils .literal .notranslate}]{.pre}.

- Added violation status output for Graphics Clock Below Host Limit to [`amd-smi`{.docutils .literal .notranslate}]{.pre} CLI: [`amdsmi_get_violation_status()`{.docutils .literal .notranslate}]{.pre}, [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}`  `{.docutils .literal .notranslate}[`--throttle`{.docutils .literal .notranslate}]{.pre}, and [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--violation`{.docutils .literal .notranslate}]{.pre}. Users can retrieve violation status through either our Python or C++ APIs. Only available for MI300 Series+ ASICs.
...
This is a known limitation of the Linux kernel; not the driver. Other drivers, such as those using PCIe space (for example, [`ast`{.docutils .literal .notranslate}]{.pre}), might occupy the necessary DRM nodes. You can check the number of DRM nodes using [`ls`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`/sys/class/drm`{.docutils .literal .notranslate}]{.pre}.
...
- Remove other devices occupying DRM nodes.
  ...
  1.  Unload amdgpu - [`sudo`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rmmod`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`amdgpu`{.docutils .literal .notranslate}]{.pre}.

  2.  Remove unnecessary driver(s) - ex. [`sudo`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rmmod`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ast`{.docutils .literal .notranslate}]{.pre}.

  3.  Reload amgpu - [`sudo`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`modprobe`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`amdgpu`{.docutils .literal .notranslate}]{.pre}.

  4.  Confirm [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`list`{.docutils .literal .notranslate}]{.pre} reports all nodes (this can vary per MI ASIC).
...
- ONNX Operator Support for [`com.microsoft.MultiHeadAttention`{.docutils .literal .notranslate}]{.pre}, [`com.microsoft.NhwcConv`{.docutils .literal .notranslate}]{.pre}, and [`com.microsoft.MatMulIntgerFloat`{.docutils .literal .notranslate}]{.pre}

- The [`migraphx-driver`{.docutils .literal .notranslate}]{.pre} can now produce output for Netron.

- The [`migraphx-driver`{.docutils .literal .notranslate}]{.pre} now includes a [`time`{.docutils .literal .notranslate}]{.pre} parameter (similar to [`perf`{.docutils .literal .notranslate}]{.pre}) that is more accurate for very fast kernels.

- An end-to-end Stable Diffusion 3 example with an option to disable T5 encoder on VRAM-limited GPUs has been added.

- Support to track broadcast axes in [`shape_transform_descriptor`{.docutils .literal .notranslate}]{.pre}.

- Support for unsigned types with [`rocMLIR`{.docutils .literal .notranslate}]{.pre}.
...
- Included the min/max/median of the [`perf`{.docutils .literal .notranslate}]{.pre} run as part of the summary report.

- Enabled non-packed inputs for [`rocMLIR`{.docutils .literal .notranslate}]{.pre}.
...
- [`__AMDGCN_WAVEFRONT_SIZE`{.docutils .literal .notranslate}]{.pre} has been deprecated.
...
- Allowed the [`reduce`{.docutils .literal .notranslate}]{.pre} operator to be split across an axis to improve fusion performance. The [`MIGRAPHX_SPLIT_REDUCE_SIZE`{.docutils .literal .notranslate}]{.pre} environment variable has been added to allow the minimum size of the reduction to be adjusted for a possible model-specific performance improvement.

- Added [`MIGRAPHX_DISABLE_PASSES`{.docutils .literal .notranslate}]{.pre} environment variable for debugging.
...
- FMHA fwd splitKV optimization for decode ([`seqlen_q=1`{.docutils .literal .notranslate}]{.pre}).
...
- New HIP APIs
  ...
  - [`hipDeviceGetTexture1DLinearMaxWidth`{.docutils .literal .notranslate}]{.pre} returns the maximum width of elements in a 1D linear texture, which can be allocated on the specified device.

  - [`hipStreamBatchMemOp`{.docutils .literal .notranslate}]{.pre} enqueues an array of batch memory operations in the stream, for stream synchronization.

  - [`hipGraphAddBatchMemOpNode`{.docutils .literal .notranslate}]{.pre} creates a batch memory operation node and adds it to a graph.

  - [`hipGraphBatchMemOpNodeGetParams`{.docutils .literal .notranslate}]{.pre} returns the pointer of parameters from the batch memory operation node.

  - [`hipGraphBatchMemOpNodeSetParams`{.docutils .literal .notranslate}]{.pre} sets parameters for the batch memory operation node.

  - [`hipGraphExecBatchMemOpNodeSetParams`{.docutils .literal .notranslate}]{.pre} sets the parameters for a batch memory operation node in the given executable graph.

  - [`hipLinkAddData`{.docutils .literal .notranslate}]{.pre} adds SPIR-V code object data to linker instance with options.

  - [`hipLinkAddFile`{.docutils .literal .notranslate}]{.pre} adds SPIR-V code object file to linker instance with options.

  - [`hipLinkCreate`{.docutils .literal .notranslate}]{.pre} creates linker instance at runtime with options.
...
- Runtime now uses unified copy approach:
  ...
  - Unpinned [`H2D`{.docutils .literal .notranslate}]{.pre} copies are no longer blocking until the size of 1 MB.

  - Kernel copy path is enabled for unpinned [`H2D`{.docutils .literal .notranslate}]{.pre}/[`D2H`{.docutils .literal .notranslate}]{.pre} methods.
...
- Behavior of [`hipPointerGetAttributes`{.docutils .literal .notranslate}]{.pre} is changed to match corresponding CUDA API in version 11 and later releases.
...
- Return error/value code updates in the following hip APIs to match the corresponding CUDA APIs,
  ...
  - [`hipGetTextureAlignmentOffset`{.docutils .literal .notranslate}]{.pre}

  - [`hipTexObjectCreate`{.docutils .literal .notranslate}]{.pre}

  - [`hipBindTexture2D`{.docutils .literal .notranslate}]{.pre}

  - [`hipBindTextureToArray`{.docutils .literal .notranslate}]{.pre}

  - [`hipModuleLoad`{.docutils .literal .notranslate}]{.pre}

  - [`hipLaunchCooperativeKernelMultiDevice`{.docutils .literal .notranslate}]{.pre}

  - [`hipExtLaunchCooperativeKernelMultiDevice`{.docutils .literal .notranslate}]{.pre}

- HIPRTC implementation, the compilation of [`hiprtc`{.docutils .literal .notranslate}]{.pre} now uses namespace ` `{.docutils .literal .notranslate}[`__hip_internal`{.docutils .literal .notranslate}]{.pre}, instead of the standard headers [`std`{.docutils .literal .notranslate}]{.pre}.
...
- Stream capture mode updates in the following HIP APIs. Streams can only be captured in relax mode, to match the behavior of the corresponding CUDA APIs,
  ...
  - [`hipMemPoolDestory`{.docutils .literal .notranslate}]{.pre}

  - [`hipDeviceSetMemPool`{.docutils .literal .notranslate}]{.pre}

  - [`hipEventQuery`{.docutils .literal .notranslate}]{.pre}

- The implementation of [`hipStreamAddCallback`{.docutils .literal .notranslate}]{.pre} is updated, to match the behavior of CUDA.

- Removal of [`hiprtc`{.docutils .literal .notranslate}]{.pre} symbols from hip library.

  - [`hiprtc`{.docutils .literal .notranslate}]{.pre} will be a independent library, and all symbols supported in HIP library are removed.

  - Any application using [`hiprtc`{.docutils .literal .notranslate}]{.pre} APIs should link explicitly with [`hiprtc`{.docutils .literal .notranslate}]{.pre} library.
...
- Output the profile logging if [`HIPBLASLT_LOG_MASK=64`{.docutils .literal .notranslate}]{.pre} is set.

- Support for the [`FP16`{.docutils .literal .notranslate}]{.pre} compute type.
...
- Added extended tests to [`rtest.py`{.docutils .literal .notranslate}]{.pre}. These tests are extra tests that did not fit the criteria of smoke and regression tests. These tests will take much longer than smoke and regression tests. Use [`python`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rtest.py`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`[--emulation|-e|--test|-t]=extended`{.docutils .literal .notranslate}]{.pre} to run these tests.

- Added [`ForEach`{.docutils .literal .notranslate}]{.pre}, [`ForEachN`{.docutils .literal .notranslate}]{.pre}, [`ForEachCopy`{.docutils .literal .notranslate}]{.pre}, [`ForEachCopyN`{.docutils .literal .notranslate}]{.pre} and [`Bulk`{.docutils .literal .notranslate}]{.pre} functions to have parity with CUB.

- Added the [`hipcub::CubVector`{.docutils .literal .notranslate}]{.pre} type for CUB parity.

- Added [`--emulation`{.docutils .literal .notranslate}]{.pre} option for [`rtest.py`{.docutils .literal .notranslate}]{.pre}

- Unit tests can be run with [`[--emulation|-e|--test|-t]=<test_name>;`{.docutils .literal .notranslate}]{.pre}.

- Added [`DeviceSelect::FlaggedIf`{.docutils .literal .notranslate}]{.pre} and its inplace overload.
...
- Changed the subset of tests that are run for smoke tests such that the smoke test will complete with faster run time and never exceed 2 GB of VRAM usage. Use [`python`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rtest.py`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`[--emulation|-e|--test|-t]=smoke`{.docutils .literal .notranslate}]{.pre} to run these tests.

- The [`rtest.py`{.docutils .literal .notranslate}]{.pre} options have changed. [`rtest.py`{.docutils .literal .notranslate}]{.pre} is now run with at least either [`--test|-t`{.docutils .literal .notranslate}]{.pre} or [`--emulation|-e`{.docutils .literal .notranslate}]{.pre}, but not both options.
...
- Use [`const_cast<const`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`char**>`{.docutils .literal .notranslate}]{.pre} for the last arguments in the [`hiprtcCreateProgram`{.docutils .literal .notranslate}]{.pre} and [`hiprtcCompileProgram`{.docutils .literal .notranslate}]{.pre} function calls, as in CUDA, they are of the [`const`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`char*`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`const*`{.docutils .literal .notranslate}]{.pre} type
...
- On gfx12, all functions now support full [`rocblas_int`{.docutils .literal .notranslate}]{.pre} dynamic range for [`batch_count`{.docutils .literal .notranslate}]{.pre}.
...
- Reduced the workspace memory requirements for Level 1 ILP64: [`dot`{.docutils .literal .notranslate}]{.pre}, [`asum`{.docutils .literal .notranslate}]{.pre}, and [`nrm2`{.docutils .literal .notranslate}]{.pre}.
...
- Added a [`gfortran`{.docutils .literal .notranslate}]{.pre} package dependency for Azure Linux OS.
...
- Support for generic code object targets:
  ...
  - [`gfx9-generic`{.docutils .literal .notranslate}]{.pre}

  - [`gfx9-4-generic`{.docutils .literal .notranslate}]{.pre}

  - [`gfx10-1-generic`{.docutils .literal .notranslate}]{.pre}

  - [`gfx10-3-generic`{.docutils .literal .notranslate}]{.pre}
...
- Plan descriptions can now be reused with different strides for different plans. For more information, see the [rocFFT GitHub issue](https://github.com/ROCm/rocFFT/issues/504){.reference .external}.
...
- RVS [IET](https://github.com/ROCm/ROCmValidationSuite/tree/a6177fc5e3f2679f98bbbc80dc536d535a43fb69/iet.so){.reference .external}, [PEBB](https://github.com/ROCm/ROCmValidationSuite/tree/a6177fc5e3f2679f98bbbc80dc536d535a43fb69/pebb.so){.reference .external}, and [memory bandwidth tests](https://github.com/ROCm/ROCmValidationSuite/tree/a6177fc5e3f2679f98bbbc80dc536d535a43fb69/babel.so){.reference .external}
...
- The parallel device-level function, [`rocprim::adjacent_find`{.docutils .literal .notranslate}]{.pre}, which is similar to the C++ Standard Library [`std::adjacent_find`{.docutils .literal .notranslate}]{.pre} algorithm, has been added.

- Configuration autotuning has been added to device adjacent find ([`rocprim::adjacent_find`{.docutils .literal .notranslate}]{.pre}) for improved performance on selected architectures.

- [`rocprim::numeric_limits`{.docutils .literal .notranslate}]{.pre} has been added. This is an extension of [`std::numeric_limits`{.docutils .literal .notranslate}]{.pre} that supports 128-bit integers.

- [`rocprim::int128_t`{.docutils .literal .notranslate}]{.pre} and [`rocprim::uint128_t`{.docutils .literal .notranslate}]{.pre} have been added.

- The parallel [`search`{.docutils .literal .notranslate}]{.pre} and [`find_end`{.docutils .literal .notranslate}]{.pre} device functions have been added. These are similar to [`std::search`{.docutils .literal .notranslate}]{.pre} and [`std::find_end`{.docutils .literal .notranslate}]{.pre}. These functions search for the first and last occurrence of the sequence, respectively.

- A parallel device-level function, [`rocprim::search_n`{.docutils .literal .notranslate}]{.pre}, has been added. [`rocprim::search_n`{.docutils .literal .notranslate}]{.pre} is similar to the C++ Standard Library [`std::search_n`{.docutils .literal .notranslate}]{.pre} algorithm.

- New constructors, a [`base`{.docutils .literal .notranslate}]{.pre} function, and a [`constexpr`{.docutils .literal .notranslate}]{.pre} specifier have been added to all functions in [`rocprim::reverse_iterator`{.docutils .literal .notranslate}]{.pre} to improve parity with the C++17 [`std::reverse_iterator`{.docutils .literal .notranslate}]{.pre}.

- hipGraph support has been added to the device run-length-encode for non-trivial runs ([`rocprim::run_length_encode_non_trivial_runs`{.docutils .literal .notranslate}]{.pre}).

- Configuration autotuning has been added to the device run-length-encode for non-trivial runs ([`rocprim::run_length_encode_non_trivial_runs`{.docutils .literal .notranslate}]{.pre}) for improved performance on selected architectures.

- Configuration autotuning has been added to the device run-length-encode for trivial runs ([`rocprim::run_length_encode`{.docutils .literal .notranslate}]{.pre}) for improved performance on selected architectures.

- The [`--emulation`{.docutils .literal .notranslate}]{.pre} option has been added to [`rtest.py`{.docutils .literal .notranslate}]{.pre}. Unit tests can be run with [`python`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`rtest.py`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`[--emulation|-e|--test|-t]=<test_name>`{.docutils .literal .notranslate}]{.pre}.
...
- Fixed an issue where [`rocprim::partial_sort_copy`{.docutils .literal .notranslate}]{.pre} would yield a compile error if the input iterator was a const.

- Fixed incorrect 128-bit signed and unsigned integer type traits.

- Fixed a compilation issue when [`rocprim::radix_key_codec<...>`{.docutils .literal .notranslate}]{.pre} is specialized with a 128-bit integer.
...
- [`reduce()`{.docutils .literal .notranslate}]{.pre} operation for counter expression with respect to dimension.

- [`--collection-period`{.docutils .literal .notranslate}]{.pre} feature in [`rocprofv3`{.docutils .literal .notranslate}]{.pre} to enable filtering using time.

- [`--collection-period-unit`{.docutils .literal .notranslate}]{.pre} feature in [`rocprofv3`{.docutils .literal .notranslate}]{.pre} to control time units used in the collection period option.
...
- SDK: [`rocprofiler_agent_v0_t`{.docutils .literal .notranslate}]{.pre} support for agent UUIDs.

- SDK: [`rocprofiler_agent_v0_t`{.docutils .literal .notranslate}]{.pre} support for agent visibility based on gpu isolation environment variables such as [`ROCR_VISIBLE_DEVICES`{.docutils .literal .notranslate}]{.pre} and so on.

- Accumulation VGPR support for [`rocprofv3`{.docutils .literal .notranslate}]{.pre}.
...
- Added test filters [`smoke`{.docutils .literal .notranslate}]{.pre}, [`regression`{.docutils .literal .notranslate}]{.pre}, and [`extended`{.docutils .literal .notranslate}]{.pre} for emulation tests.

- Added [`rocsparse_[s|d|c|z]csritilu0_compute_ex`{.docutils .literal .notranslate}]{.pre} routines for iterative ILU.

- Added [`rocsparse_[s|d|c|z]csritsv_solve_ex`{.docutils .literal .notranslate}]{.pre} routines for iterative triangular solve.
...
- Improved the adaptive CSR sparse matrix-vector multiplication algorithm when the sparse matrix has many empty rows at the beginning or at the end of the matrix. This improves the routines [`rocsparse_spmv`{.docutils .literal .notranslate}]{.pre} and [`rocsparse_spmv_ex`{.docutils .literal .notranslate}]{.pre} when the adaptive algorithm [`rocsparse_spmv_alg_csr_adaptive`{.docutils .literal .notranslate}]{.pre} is used.

- Improved stream CSR sparse matrix-vector multiplication algorithm when the sparse matrix size (number of rows) decreases. This improves the routines [`rocsparse_spmv`{.docutils .literal .notranslate}]{.pre} and [`rocsparse_spmv_ex`{.docutils .literal .notranslate}]{.pre} when the stream algorithm [`rocsparse_spmv_alg_csr_stream`{.docutils .literal .notranslate}]{.pre} is used.

- Compared to [`rocsparse_[s|d|c|z]csritilu0_compute`{.docutils .literal .notranslate}]{.pre}, the routines [`rocsparse_[s|d|c|z]csritilu0_compute_ex`{.docutils .literal .notranslate}]{.pre} introduce several free iterations. A free iteration is an iteration that does not compute the evaluation of the stopping criteria, if enabled. This allows the user to tune the algorithm for performance improvements.
...
- Made TBB an optional dependency with the new [`BUILD_HIPSTDPAR_TEST_WITH_TBB`{.docutils .literal .notranslate}]{.pre} flag. When the flag is [`OFF`{.docutils .literal .notranslate}]{.pre} and TBB is not already on the machine, it will compile without TBB. Otherwise, it will compile with TBB.
...
- Deprecation warning fixed for the [`sprintf`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`is`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`deprecated`{.docutils .literal .notranslate}]{.pre} warning.
...
- Virtual environment (venv) for [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre} invocation on Linux.

- Flag to keep [`build_tmp`{.docutils .literal .notranslate}]{.pre} when running Tensile.
...
- Replaced Perl script with [`hipcc.bat`{.docutils .literal .notranslate}]{.pre} as a compiler on Microsoft Windows.
...
- Refactored [`writeKernels`{.docutils .literal .notranslate}]{.pre} in [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre}.
...
- Fixed [`accvgpr`{.docutils .literal .notranslate}]{.pre} overflow.
...
- A new path to avoid [`WaitAny`{.docutils .literal .notranslate}]{.pre} calls in [`AsyncEventsLoop`{.docutils .literal .notranslate}]{.pre}. The new path is selected by default.
...
- Runtime optimization which combines all logic of [`WaitAny`{.docutils .literal .notranslate}]{.pre} in a single processing loop and avoids extra memory allocations or reference counting. The runtime won't spin on the CPU if all events are busy.
...
- Segmentation fault with [`hipStreamLegacy`{.docutils .literal .notranslate}]{.pre} while using the API [`hipStreamWaitEvent`{.docutils .literal .notranslate}]{.pre}.
...
- Fixed an issue with merging multiple [`.proto`{.docutils .literal .notranslate}]{.pre} files.
...
- Support for GPU metrics 1.6 to [`amdsmi_get_gpu_metrics_info()`{.docutils .literal .notranslate}]{.pre}

- New violation status outputs and APIs: [`amdsmi_status_t`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`amdsmi_get_violation_status()`{.docutils .literal .notranslate}]{.pre}, [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}`  `{.docutils .literal .notranslate}[`--throttle`{.docutils .literal .notranslate}]{.pre}, and [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--violation`{.docutils .literal .notranslate}]{.pre}. This feature is only available on MI300+ ASICs

- Ability to view XCP (Graphics Compute Partition) activity within [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--usage`{.docutils .literal .notranslate}]{.pre}. Partition-specific features are only available on MI300+ ASICs

- Added [`LC_PERF_OTHER_END_RECOVERY`{.docutils .literal .notranslate}]{.pre} CLI output to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--pcie`{.docutils .literal .notranslate}]{.pre} and updated [`amdsmi_get_pcie_info()`{.docutils .literal .notranslate}]{.pre} to include this value. This feature is only available on MI300+ ASICs

- Ability to retrieve a set of GPUs that are nearest to a given device at a specific link type level

  - Added [`amdsmi_get_link_topology_nearest()`{.docutils .literal .notranslate}]{.pre} function to amd-smi C and Python Libraries.

- More supported utilization count types to [`amdsmi_get_utilization_count()`{.docutils .literal .notranslate}]{.pre}

- [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`set`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-L/--clk-limit`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`...`{.docutils .literal .notranslate}]{.pre} command. This is equivalent to rocm-smi's [`--extremum`{.docutils .literal .notranslate}]{.pre} command which sets sclk's or mclk's soft minimum or soft maximum clock frequency.

- Unittest functionality to test [`amdsmi`{.docutils .literal .notranslate}]{.pre} API calls in Python

- GPU memory overdrive percentage to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-o`{.docutils .literal .notranslate}]{.pre}

  - Added [`amdsmi_get_gpu_mem_overdrive_level()`{.docutils .literal .notranslate}]{.pre} function to AMD SMI C and Python Libraries.

- Ability to retrieve connection type and P2P capabilities between two GPUs

  - Added [`amdsmi_topo_get_p2p_status()`{.docutils .literal .notranslate}]{.pre} function to amd-smi C and Python Libraries.

  - Added retrieving P2P link capabilities to CLI [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`topology`{.docutils .literal .notranslate}]{.pre}.

- New [`amdsmi_kfd_info_t`{.docutils .literal .notranslate}]{.pre} type and added information under [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`list`{.docutils .literal .notranslate}]{.pre}
...
- Moved Python tests directory path install location.
  ...
  - [`/opt/<rocm-path>/share/amd_smi/pytest/..`{.docutils .literal .notranslate}]{.pre} to [`/opt/<rocm-path>/share/amd_smi/tests/python_unittest/..`{.docutils .literal .notranslate}]{.pre}

  - Removed PyTest dependency. Python testing now depends on the unittest framework only.

- Changed the [`power`{.docutils .literal .notranslate}]{.pre} parameter in [`amdsmi_get_energy_count()`{.docutils .literal .notranslate}]{.pre} to [`energy_accumulator`{.docutils .literal .notranslate}]{.pre}.

  - Changes propagate forwards into the Python interface as well. Backwards compatibility is maintained.

- Updated Partition APIs and struct information and added [`partition_id`{.docutils .literal .notranslate}]{.pre} to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--partition`{.docutils .literal .notranslate}]{.pre}.

  - As part of an overhaul to partition information, some partition information will be made available in the [`amdsmi_accelerator_partition_profile_t`{.docutils .literal .notranslate}]{.pre}.
...
- [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`reset`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--compute-partition`{.docutils .literal .notranslate}]{.pre} and [`...`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--memory-partition`{.docutils .literal .notranslate}]{.pre} and associated APIs
  ...
  - Associated APIs include [`amdsmi_reset_gpu_compute_partition()`{.docutils .literal .notranslate}]{.pre} and [`amdsmi_reset_gpu_memory_partition()`{.docutils .literal .notranslate}]{.pre}
...
- Added a preliminary [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`partition`{.docutils .literal .notranslate}]{.pre} command.
...
- New HIP APIs:
  ...
  - [`hipGraphExecGetFlags`{.docutils .literal .notranslate}]{.pre} returns the flags on executable graph.

  - [`hipGraphNodeSetParams`{.docutils .literal .notranslate}]{.pre} updates the parameters of a created node.

  - [`hipGraphExecNodeSetParams`{.docutils .literal .notranslate}]{.pre} updates the parameters of a created node on an executable graph.

  - [`hipDrvGraphMemcpyNodeGetParams`{.docutils .literal .notranslate}]{.pre} gets a memcpy node's parameters.

  - [`hipDrvGraphMemcpyNodeSetParams`{.docutils .literal .notranslate}]{.pre} sets a memcpy node's parameters.

  - [`hipDrvGraphAddMemFreeNode`{.docutils .literal .notranslate}]{.pre} creates a memory free node and adds it to a graph.
...
- Memory leak in the API [`hipGraphInstantiate`{.docutils .literal .notranslate}]{.pre} when kernel is launched using [`hipExtLaunchKernelGGL`{.docutils .literal .notranslate}]{.pre} with event.
...
- Support for data type [`INT8`{.docutils .literal .notranslate}]{.pre} in with [`INT8`{.docutils .literal .notranslate}]{.pre} out

- Support for data type [`FP32`{.docutils .literal .notranslate}]{.pre}/[`FP64`{.docutils .literal .notranslate}]{.pre} for gfx110x

- Extension API [`hipblaslt_ext::matmulIsTuned`{.docutils .literal .notranslate}]{.pre}

- Output [`atol`{.docutils .literal .notranslate}]{.pre} and [`rtol`{.docutils .literal .notranslate}]{.pre} for [`hipblaslt-bench`{.docutils .literal .notranslate}]{.pre} validation
...
- Initial support for direct hipification of cuDNN into MIOpen under the [`--roc`{.docutils .literal .notranslate}]{.pre} option
...
- Auxiliary functions:
  ...
  - [`hipsolverSetDeterministicMode`{.docutils .literal .notranslate}]{.pre}, [`hipsolverGetDeterministicMode`{.docutils .literal .notranslate}]{.pre}

- Compatibility-only functions:

  - [`potrf`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnXpotrf_bufferSize`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnXpotrf`{.docutils .literal .notranslate}]{.pre}

  - [`potrs`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnXpotrs`{.docutils .literal .notranslate}]{.pre}

  - [`geqrf`{.docutils .literal .notranslate}]{.pre}
...
- ASAN builds use [`-mcmodel=large`{.docutils .literal .notranslate}]{.pre} to accommodate library sizes greater than 2GB.
...
- Support for [`FP8`{.docutils .literal .notranslate}]{.pre} and [`INT4`{.docutils .literal .notranslate}]{.pre}

- Support for the Log2 internal operator

- Support for the GCC 14 compiler

- The [`BitwiseAnd`{.docutils .literal .notranslate}]{.pre}, [`Scan`{.docutils .literal .notranslate}]{.pre}, [`SoftmaxCrossEntropyLoss`{.docutils .literal .notranslate}]{.pre}, [`GridSample`{.docutils .literal .notranslate}]{.pre}, and [`NegativeLogLikelihoodLoss`{.docutils .literal .notranslate}]{.pre} ONNX operators

- The [`MatMulNBits`{.docutils .literal .notranslate}]{.pre}, [`QuantizeLinear`{.docutils .literal .notranslate}]{.pre}/[`DequantizeLinear`{.docutils .literal .notranslate}]{.pre}, [`GroupQueryAttention`{.docutils .literal .notranslate}]{.pre}, [`SkipSimplifiedLayerNormalization`{.docutils .literal .notranslate}]{.pre}, and [`SimpliedLayerNormalizationMicrosoft`{.docutils .literal .notranslate}]{.pre} Contrib operators

- Dynamic batch parameter support to [`OneHot`{.docutils .literal .notranslate}]{.pre} operator
...
- [`--mlir`{.docutils .literal .notranslate}]{.pre} flag the migraphx-driver program to offload entire module to MLIR
...
- [`MIGRAPHX_MLIR_DUMP`{.docutils .literal .notranslate}]{.pre} environment variable to dump MLIR modules to MXRs

- The [`3`{.docutils .literal .notranslate}]{.pre} option to [`MIGRAPHX_TRACE_BENCHMARKING`{.docutils .literal .notranslate}]{.pre} to print the MLIR program for improved debug output

- [`MIGRAPHX_ENABLE_HIPBLASLT_GEMM`{.docutils .literal .notranslate}]{.pre} environment variable to call hipBLASLt libraries

- [`MIGRAPHX_VERIFY_DUMP_DIFF`{.docutils .literal .notranslate}]{.pre} to improve the debugging of accuracy issues
...
- Improved:
  ...
  - Performance for the [`FP8`{.docutils .literal .notranslate}]{.pre} datatype
  ...
  - Summary section of the [`migraphx-driver`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`perf`{.docutils .literal .notranslate}]{.pre} command
...
- Fixed a bug with [`dot_broadcast`{.docutils .literal .notranslate}]{.pre} and [`inner_broadcast`{.docutils .literal .notranslate}]{.pre} that caused compile failures.
...
- Performance collection to [`rccl_replayer`{.docutils .literal .notranslate}]{.pre}
...
- Enabled tracing with [`rocprof`{.docutils .literal .notranslate}]{.pre}
...
- Implemented experimental APIs to allow computing FFTs on data distributed across multiple MPI ranks. These APIs can be enabled with the [`ROCFFT_MPI_ENABLE`{.docutils .literal .notranslate}]{.pre} CMake option. This option defaults to [`OFF`{.docutils .literal .notranslate}]{.pre}.

  When [`ROCFFT_MPI_ENABLE`{.docutils .literal .notranslate}]{.pre} is [`ON`{.docutils .literal .notranslate}]{.pre}:
...
- Renamed to ROCm Compute Profiler from Omniperf.
  ...
  - New package name: [`rocprofiler-compute`{.docutils .literal .notranslate}]{.pre}
...
- **Added [`rsmi_dev_memory_partition_capabilities_get`{.docutils .literal .notranslate}]{.pre} which returns driver memory partition capablities.**\
  Driver now has the ability to report what the user can set memory partition modes to. User can now see available memory partition modes upon an invalid argument return from memory partition mode set ([`rsmi_dev_memory_partition_set`{.docutils .literal .notranslate}]{.pre}).
...
- Renamed to ROCm Systems Profiler from Omnitrace.
  ...
  - New package name: [`rocprofiler-systems`{.docutils .literal .notranslate}]{.pre}

  - New repository: [ROCm/rocprofiler-systems](https://github.com/ROCm/rocprofiler-systems){.github .reference .external}
...
- The [`--seed`{.docutils .literal .notranslate}]{.pre} option has been added to [`run_benchmarks.py`{.docutils .literal .notranslate}]{.pre}. The [`--seed`{.docutils .literal .notranslate}]{.pre} option specifies a seed for the generation of random inputs. When the option is omitted, the default behavior is to use a random seed for each benchmark measurement.

- Added configuration autotuning to device partition ([`rocprim::partition`{.docutils .literal .notranslate}]{.pre}, [`rocprim::partition_two_way`{.docutils .literal .notranslate}]{.pre}, and [`rocprim::partition_three_way`{.docutils .literal .notranslate}]{.pre}), to device select ([`rocprim::select`{.docutils .literal .notranslate}]{.pre}, [`rocprim::unique`{.docutils .literal .notranslate}]{.pre}, and [`rocprim::unique_by_key`{.docutils .literal .notranslate}]{.pre}), and to device reduce by key ([`rocprim::reduce_by_key`{.docutils .literal .notranslate}]{.pre}) to improve performance on selected architectures.

- Added [`rocprim::uninitialized_array`{.docutils .literal .notranslate}]{.pre} to provide uninitialized storage in local memory for user-defined types.

- Added large segment support for [`rocprim:segmented_reduce`{.docutils .literal .notranslate}]{.pre}.

- Added a parallel [`nth_element`{.docutils .literal .notranslate}]{.pre} device function similar to [`std::nth_element`{.docutils .literal .notranslate}]{.pre}. [`nth_element`{.docutils .literal .notranslate}]{.pre} places elements that are smaller than the nth element before the nth element, and elements that are bigger than the nth element after the nth element.
...
- Resolved an issue in [`rtest.py`{.docutils .literal .notranslate}]{.pre} where it crashed if the [`build`{.docutils .literal .notranslate}]{.pre} folder was created without [`release`{.docutils .literal .notranslate}]{.pre} or [`debug`{.docutils .literal .notranslate}]{.pre} subdirectories.

- Resolved an issue with [`rtest.py`{.docutils .literal .notranslate}]{.pre} on Windows where passing an absolute path to [`--install_dir`{.docutils .literal .notranslate}]{.pre} caused a [`FileNotFound`{.docutils .literal .notranslate}]{.pre} error.

- rocPRIM functions are no longer forcefully inlined on Windows. This significantly reduces the build time of debug builds.

- [`block_load`{.docutils .literal .notranslate}]{.pre}, [`block_store`{.docutils .literal .notranslate}]{.pre}, [`block_shuffle`{.docutils .literal .notranslate}]{.pre}, [`block_exchange`{.docutils .literal .notranslate}]{.pre}, and [`warp_exchange`{.docutils .literal .notranslate}]{.pre} now use placement [`new`{.docutils .literal .notranslate}]{.pre} instead of copy assignment ([`operator=`{.docutils .literal .notranslate}]{.pre}) when writing to local memory. This fixes the behavior of custom types with non-trivial copy assignments.

- Fixed a bug in the generation of input data for benchmarks, which caused incorrect performance to be reported in specific cases. It may affect the reported performance for one-byte types ([`uint8_t`{.docutils .literal .notranslate}]{.pre} and [`int8_t`{.docutils .literal .notranslate}]{.pre}) and instantiations of [`custom_type`{.docutils .literal .notranslate}]{.pre}. Specifically, device binary search, device histogram, device merge and warp sort are affected.

- Fixed a bug for [`rocprim::merge_path_search`{.docutils .literal .notranslate}]{.pre} where using [`unsigned`{.docutils .literal .notranslate}]{.pre} offsets would produce incorrect results.

- Fixed a bug for [`rocprim::thread_load`{.docutils .literal .notranslate}]{.pre} and [`rocprim::thread_store`{.docutils .literal .notranslate}]{.pre} where [`float`{.docutils .literal .notranslate}]{.pre} and [`double`{.docutils .literal .notranslate}]{.pre} were not cast to the correct type, resulting in incorrect results.

- Resolved an issue where tests were failing when they were compiled with [`-D_GLIBCXX_ASSERTIONS=ON`{.docutils .literal .notranslate}]{.pre}.
...
- [`--no-serialization`{.docutils .literal .notranslate}]{.pre} flag to disable kernel serialization when [`rocprofv2`{.docutils .literal .notranslate}]{.pre} is in counter collection mode. This allows [`rocprofv2`{.docutils .literal .notranslate}]{.pre} to avoid deadlock when profiling certain programs in counter collection mode.

- [`FP64_ACTIVE`{.docutils .literal .notranslate}]{.pre} and [`ENGINE_ACTIVE`{.docutils .literal .notranslate}]{.pre} metrics to AMD Instinct MI300 GPU
...
- Setting the [`trace-start`{.docutils .literal .notranslate}]{.pre} option for [`rocprof`{.docutils .literal .notranslate}]{.pre} to [`off`{.docutils .literal .notranslate}]{.pre} now disables kernel tracing
...
- Perfetto plugin issue of [`roctx`{.docutils .literal .notranslate}]{.pre} trace not getting displayed

- [`--help`{.docutils .literal .notranslate}]{.pre} for counter collection

- Signal management issues in [`queue.cpp`{.docutils .literal .notranslate}]{.pre}

- Perfetto tracks for multi-GPU

- Perfetto plugin usage with [`rocsys`{.docutils .literal .notranslate}]{.pre}
...
- The system hang caused while running [`--kernel-trace`{.docutils .literal .notranslate}]{.pre} with Perfetto for certain applications

- Missing profiler records issue caused while running [`--trace-period`{.docutils .literal .notranslate}]{.pre}
...
- Replaced deprecated [`hipHostMalloc`{.docutils .literal .notranslate}]{.pre} and [`hipHostFree`{.docutils .literal .notranslate}]{.pre} functions with [`hipExtHostAlloc`{.docutils .literal .notranslate}]{.pre} and [`hipFreeHost`{.docutils .literal .notranslate}]{.pre} for ROCm versions starting 6.3

- Updated [`rocprofv3`{.docutils .literal .notranslate}]{.pre} [`--help`{.docutils .literal .notranslate}]{.pre} options
...
- Fixed [`MeanOccupancy`{.docutils .literal .notranslate}]{.pre} metrics

- Fixed aborted-application validation test to properly check for [`hipExtHostAlloc`{.docutils .literal .notranslate}]{.pre} command
...
- 64-bit APIs for existing functions:
  ...
  - [`LACGV_64`{.docutils .literal .notranslate}]{.pre}

  - [`LARF_64`{.docutils .literal .notranslate}]{.pre}

  - [`LARFG_64`{.docutils .literal .notranslate}]{.pre}

  - [`GEQR2_64`{.docutils .literal .notranslate}]{.pre} (with batched and strided_batched versions)

  - [`GEQRF_64`{.docutils .literal .notranslate}]{.pre} (with batched and strided_batched versions)

  - [`POTF2_64`{.docutils .literal .notranslate}]{.pre} (with batched and strided_batched versions)
...
- Fixed a race condition in [`bsrgemm`{.docutils .literal .notranslate}]{.pre} that could on rare occasions cause incorrect results.

- Fixed an issue in [`hyb2csr`{.docutils .literal .notranslate}]{.pre} where the CSR row pointer array was not being properly filled when [`n=0`{.docutils .literal .notranslate}]{.pre}, [`coo_nnz=0`{.docutils .literal .notranslate}]{.pre}, or [`ell_nnz=0`{.docutils .literal .notranslate}]{.pre}.

- Fixed scaling in [`rocsparse_Xhybmv`{.docutils .literal .notranslate}]{.pre} when only performing [`y=beta*y`{.docutils .literal .notranslate}]{.pre}, for example, where [`alpha==0`{.docutils .literal .notranslate}]{.pre} in [`y=alpha*Ax+beta*y`{.docutils .literal .notranslate}]{.pre}.
...
- [`ROCWMMA_*`{.docutils .literal .notranslate}]{.pre} preprocessor configurations are now all assigned values
...
- [`tPrint`{.docutils .literal .notranslate}]{.pre} and reconcile printing options

- Python unit test coverage report

- Factor embed library logic into function and test

- [`clang++`{.docutils .literal .notranslate}]{.pre} as [`cxx`{.docutils .literal .notranslate}]{.pre} compiler option for Windows

- Logic to cope with different compilers -[`toFile`{.docutils .literal .notranslate}]{.pre} function to include [`generateManifest`{.docutils .literal .notranslate}]{.pre} and moved to utilities

- Profiling CI job

- Support for [`amdclang`{.docutils .literal .notranslate}]{.pre} and use defaults

- Architecture management functions in [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre}

- [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre} CLI reference docs
...
- Reordered extensions for Windows in [`which`{.docutils .literal .notranslate}]{.pre} function

- updated [`amdclang++`{.docutils .literal .notranslate}]{.pre} and [`asm`{.docutils .literal .notranslate}]{.pre} directories
...
- Bumped rocm-docs-core from 1.2.0 to 1.5.0 in [`/docs/sphinx`{.docutils .literal .notranslate}]{.pre}

- Refactored kernel duplicate matching

- Refactored [`generateLogicDataAndSolutions`{.docutils .literal .notranslate}]{.pre}

- Restricted XCC mapping to gfx942

- Refactored argument parsing in [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre}

- Disabled failing rhel9 tests

- Changed line length to 100 characters for formatting

- Changed YAML operations to use C [`libyaml`{.docutils .literal .notranslate}]{.pre} backend

- Improved warning text

- Updated clang support for Windows

- Updated [`supportedCompiler`{.docutils .literal .notranslate}]{.pre} function

- Clang support on Windows to require use of conditional choices and defaults

- Refactored sanity check in [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre}

- Moved client config logic from [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre} main into [`createClientConfig`{.docutils .literal .notranslate}]{.pre}

- Updated [`verifyManifest`{.docutils .literal .notranslate}]{.pre} in [`TensileCreateLibrary`{.docutils .literal .notranslate}]{.pre}
...
- Globals from [`prepAsm`{.docutils .literal .notranslate}]{.pre}
...
- Fixed tuning imports for [`pyyaml`{.docutils .literal .notranslate}]{.pre}
...
- Fixed fully qualified [`std::get`{.docutils .literal .notranslate}]{.pre} in contraction solutions

- Fixed [`add`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-v`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`flag`{.docutils .literal .notranslate}]{.pre} and change system invocation
...
- Added the following functionality:
  ...
  - [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`dmon`{.docutils .literal .notranslate}]{.pre} is now available as an alias to [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}.

  - An optional process table under [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`monitor`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`-q`{.docutils .literal .notranslate}]{.pre}.
...
- Multiple structure updates in [`amdsmi.h`{.docutils .literal .notranslate}]{.pre} and [`amdsmi_interface.py`{.docutils .literal .notranslate}]{.pre} to align with host/guest.

  - Added [`amdsmi.h`{.docutils .literal .notranslate}]{.pre} and [`amdsmi_interface.py`{.docutils .literal .notranslate}]{.pre}.

  - [`amdsmi_clk_info_t`{.docutils .literal .notranslate}]{.pre} struct

  - Added [`AMDSMI`{.docutils .literal .notranslate}]{.pre} prefix to multiple structures.

- Updated [`dpm_policy`{.docutils .literal .notranslate}]{.pre} references to [`soc_pstate`{.docutils .literal .notranslate}]{.pre}.

- Updated [`amdsmi_get_gpu_board_info()`{.docutils .literal .notranslate}]{.pre} product_name to fallback to [`pciids`{.docutils .literal .notranslate}]{.pre} file.
...
- Added support for:
  ...
  - K or C equal to 1 for [`fp16`{.docutils .literal .notranslate}]{.pre} in grouped convolution backward weight (#1280).
...
- Added the [`_sync()`{.docutils .literal .notranslate}]{.pre} version of crosslane builtins such as [`shfl_sync()`{.docutils .literal .notranslate}]{.pre}, [`__all_sync()`{.docutils .literal .notranslate}]{.pre} and [`__any_sync()`{.docutils .literal .notranslate}]{.pre}. These take a 64-bit integer as an explicit mask argument.
  ...
  - In HIP 6.2, these are hidden behind the preprocessor macro [`HIP_ENABLE_WARP_SYNC_BUILTINS`{.docutils .literal .notranslate}]{.pre}, and will be enabled unconditionally in a future HIP release.

- Added new HIP APIs:

  - [`hipGetProcAddress`{.docutils .literal .notranslate}]{.pre} returns the pointer to driver function, corresponding to the defined driver function symbol.

  - [`hipGetFuncBySymbol`{.docutils .literal .notranslate}]{.pre} returns the pointer to device entry function that matches entry function [`symbolPtr`{.docutils .literal .notranslate}]{.pre}.

  - [`hipStreamBeginCaptureToGraph`{.docutils .literal .notranslate}]{.pre} begins graph capture on a stream to an existing graph.

  - [`hipGraphInstantiateWithParams`{.docutils .literal .notranslate}]{.pre} creates an executable graph from a graph.

- Added a new flag [`integrated`{.docutils .literal .notranslate}]{.pre} -- supported in device property.

  - The integrated flag is added in the struct [`hipDeviceProp_t`{.docutils .literal .notranslate}]{.pre}. On the integrated APU system, the runtime driver detects and sets this flag to [`1`{.docutils .literal .notranslate}]{.pre}, in which case the API [`hipDeviceGetAttribute`{.docutils .literal .notranslate}]{.pre} returns enum [`hipDeviceAttribute_t`{.docutils .literal .notranslate}]{.pre} for [`hipDeviceAttributeIntegrated`{.docutils .literal .notranslate}]{.pre} as value 1, for integrated GPU device.
...
- Added a new ILP64 API for level 1 [`_ex`{.docutils .literal .notranslate}]{.pre} functions.

- The [`install.sh`{.docutils .literal .notranslate}]{.pre} script now invokes the [`rmake.py`{.docutils .literal .notranslate}]{.pre} script. Made other various improvements to the build scripts.

- Changed library dependencies in the [`install.sh`{.docutils .literal .notranslate}]{.pre} script from [`rocblas`{.docutils .literal .notranslate}]{.pre} and [`rocsolver`{.docutils .literal .notranslate}]{.pre} to the development packages [`rocblas-dev`{.docutils .literal .notranslate}]{.pre} and [`rocsolver-dev`{.docutils .literal .notranslate}]{.pre}.
...
- Added extension APIs: \*[`hipblasltExtAMaxWithScale`{.docutils .literal .notranslate}]{.pre}.
  ...
  - [`GemmTuning`{.docutils .literal .notranslate}]{.pre} extension parameter to set [`wgm`{.docutils .literal .notranslate}]{.pre} by user.

- Added support for:

  - [`HIPBLASLT_MATMUL_DESC_AMAX_D_POINTER`{.docutils .literal .notranslate}]{.pre} for [`FP8`{.docutils .literal .notranslate}]{.pre}/[`BF8`{.docutils .literal .notranslate}]{.pre} datatype.

  - [`FP8`{.docutils .literal .notranslate}]{.pre}/[`BF8`{.docutils .literal .notranslate}]{.pre} input, [`FP32/FP16/BF16/F8/BF8`{.docutils .literal .notranslate}]{.pre} output (gfx94x platform only).
...
- Enabled [`hipcc`{.docutils .literal .notranslate}]{.pre} and [`hipconfig`{.docutils .literal .notranslate}]{.pre} binaries ([`hipcc.bin`{.docutils .literal .notranslate}]{.pre}, [`hipconfig.bin`{.docutils .literal .notranslate}]{.pre}) by default, instead of their Perl counterparts.
...
- Added [`enum`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`WarpExchangeAlgorithm`{.docutils .literal .notranslate}]{.pre} to the rocPRIM backend, which is used as the new optional template argument for [`WarpExchange`{.docutils .literal .notranslate}]{.pre}.

  - The potential values for the enum are [`WARP_EXCHANGE_SMEM`{.docutils .literal .notranslate}]{.pre} and [`WARP_EXCHANGE_SHUFFLE`{.docutils .literal .notranslate}]{.pre}.

  - [`WARP_EXCHANGE_SMEM`{.docutils .literal .notranslate}]{.pre} stands for the previous algorithm, while [`WARP_EXCHANGE_SHUFFLE`{.docutils .literal .notranslate}]{.pre} performs the exchange via shuffle operations.

  - [`WARP_EXCHANGE_SHUFFLE`{.docutils .literal .notranslate}]{.pre} does not require any pre-allocated shared memory, but the [`ItemsPerThread`{.docutils .literal .notranslate}]{.pre} must be a divisor of [`WarpSize`{.docutils .literal .notranslate}]{.pre}.

- Added [`tuple.hpp`{.docutils .literal .notranslate}]{.pre} which defines templates [`hipcub::tuple`{.docutils .literal .notranslate}]{.pre}, [`hipcub::tuple_element`{.docutils .literal .notranslate}]{.pre}, [`hipcub::tuple_element_t`{.docutils .literal .notranslate}]{.pre} and [`hipcub::tuple_size`{.docutils .literal .notranslate}]{.pre}.

- Added new overloaded member functions to [`BlockRadixSort`{.docutils .literal .notranslate}]{.pre} and [`DeviceRadixSort`{.docutils .literal .notranslate}]{.pre} that expose a [`decomposer`{.docutils .literal .notranslate}]{.pre} argument. Keys of a custom type ([`key_type`{.docutils .literal .notranslate}]{.pre}) can be sorted via these overloads, if an appropriate decomposer is passed. The decomposer has to implement [`operator(const`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`key_type&)`{.docutils .literal .notranslate}]{.pre} which returns a [`hipcub::tuple`{.docutils .literal .notranslate}]{.pre} of references pointing to members of [`key_type`{.docutils .literal .notranslate}]{.pre}.

- On AMD GPUs (using the HIP backend), you can now issue hipCUB API calls inside of HIP graphs, with several exceptions:

  - [`CachingDeviceAllocator`{.docutils .literal .notranslate}]{.pre}

  - [`GridBarrierLifetime`{.docutils .literal .notranslate}]{.pre}
...
- [`hiprandCreateGeneratorHost`{.docutils .literal .notranslate}]{.pre} dispatches to the host generator in the rocRAND backend instead of returning with [`uHIPRAND_STATUS_NOT_IMPLEMENTED`{.docutils .literal .notranslate}]{.pre}.
...
- Previously, for internal testing with HMM the environment variable [`ROCRAND_USE_HMM`{.docutils .literal .notranslate}]{.pre} was used in previous versions. The environment variable is now named [`HIPRAND_USE_HMM`{.docutils .literal .notranslate}]{.pre}.
...
- Added compatibility-only functions:
  ...
  - [`auxiliary`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnCreateParams`{.docutils .literal .notranslate}]{.pre}, [`hipsolverDnDestroyParams`{.docutils .literal .notranslate}]{.pre}, [`hipsolverDnSetAdvOptions`{.docutils .literal .notranslate}]{.pre}

  - [`getrf`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnXgetrf_bufferSize`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnXgetrf`{.docutils .literal .notranslate}]{.pre}

  - [`getrs`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverDnXgetrs`{.docutils .literal .notranslate}]{.pre}

- Added support for building on Ubuntu 24.04 and CBL-Mariner.

- Added [`hip::host`{.docutils .literal .notranslate}]{.pre} to [`roc::hipsolver`{.docutils .literal .notranslate}]{.pre} usage requirements.

- Added functions

  - [`syevdx`{.docutils .literal .notranslate}]{.pre}/[`heevdx`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverSsyevdx_bufferSize`{.docutils .literal .notranslate}]{.pre}, [`hipsolverDsyevdx_bufferSize`{.docutils .literal .notranslate}]{.pre}, [`hipsolverCheevdx_bufferSize`{.docutils .literal .notranslate}]{.pre}, [`hipsolverZheevdx_bufferSize`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverSsyevdx`{.docutils .literal .notranslate}]{.pre}, [`hipsolverDsyevdx`{.docutils .literal .notranslate}]{.pre}, [`hipsolverCheevdx`{.docutils .literal .notranslate}]{.pre}, [`hipsolverZheevdx`{.docutils .literal .notranslate}]{.pre}

  - [`sygvdx`{.docutils .literal .notranslate}]{.pre}/[`hegvdx`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverSsygvdx_bufferSize`{.docutils .literal .notranslate}]{.pre}, [`hipsolverDsygvdx_bufferSize`{.docutils .literal .notranslate}]{.pre}, [`hipsolverChegvdx_bufferSize`{.docutils .literal .notranslate}]{.pre}, [`hipsolverZhegvdx_bufferSize`{.docutils .literal .notranslate}]{.pre}

    - [`hipsolverSsygvdx`{.docutils .literal .notranslate}]{.pre}, [`hipsolverDsygvdx`{.docutils .literal .notranslate}]{.pre}, [`hipsolverChegvdx`{.docutils .literal .notranslate}]{.pre}, [`hipsolverZhegvdx`{.docutils .literal .notranslate}]{.pre}
...
- Prefer [`amd-llvm-devel`{.docutils .literal .notranslate}]{.pre} package before system LLVM library.

- Preferred compilers changed to [`CC=amdclang`{.docutils .literal .notranslate}]{.pre} [`CXX=amdclang++`{.docutils .literal .notranslate}]{.pre}.
...
- LLVM IR
  ...
  - The [`llvm.stacksave`{.docutils .literal .notranslate}]{.pre} and [`llvm.stackrestore`{.docutils .literal .notranslate}]{.pre} intrinsics now use an overloaded pointer type to support non-0 address spaces.

  - Added [`llvm.exp10`{.docutils .literal .notranslate}]{.pre} intrinsic.

- LLVM infrastructure

  - The minimum Clang version to build LLVM in C++20 configuration is now [`clang-17.0.6`{.docutils .literal .notranslate}]{.pre}.

- TableGen

  - Added constructs for debugging TableGen files:

    - [`dump`{.docutils .literal .notranslate}]{.pre} keyword to dump messages to standard error. See [#68793](https://github.com/llvm/llvm-project/pull/68793){.reference .external}.

    - [`!repr`{.docutils .literal .notranslate}]{.pre} bang operator to inspect the content of values. See [#68716](https://github.com/llvm/llvm-project/pull/68716){.reference .external}.
...
- AMDGPU backend
  ...
  - [`llvm.sqrt.f32`{.docutils .literal .notranslate}]{.pre} is now lowered correctly. Use [`llvm.amdgcn.sqrt.f32`{.docutils .literal .notranslate}]{.pre} for raw instruction access.

  - Implemented [`llvm.stacksave`{.docutils .literal .notranslate}]{.pre} and [`llvm.stackrestore`{.docutils .literal .notranslate}]{.pre} intrinsics.

  - Implemented [`llvm.get.rounding`{.docutils .literal .notranslate}]{.pre}.
...
- RISC-V backend
  ...
  - The [`Zfa`{.docutils .literal .notranslate}]{.pre} extension version was upgraded to 1.0 and is no longer experimental.

  - [`Zihintntl`{.docutils .literal .notranslate}]{.pre} extension version was upgraded to 1.0 and is no longer experimental.

  - Intrinsics were added for [`Zk*`{.docutils .literal .notranslate}]{.pre}, [`Zbb`{.docutils .literal .notranslate}]{.pre}, and [`Zbc`{.docutils .literal .notranslate}]{.pre}. See [Scalar Bit Manipulation Extension Intrinsics](https://github.com/riscv-non-isa/riscv-c-api-doc/blob/main/src/c-api.adoc#scalar-bit-manipulation-extension-intrinsics){.reference .external} in the RISC-V C API specification.

  - Default ABI with F but without D was changed to ilp32f for RV32 and to lp64f for RV64.

  - The [`Zvbb`{.docutils .literal .notranslate}]{.pre}, [`Zvbc`{.docutils .literal .notranslate}]{.pre}, [`Zvkb`{.docutils .literal .notranslate}]{.pre}, [`Zvkg`{.docutils .literal .notranslate}]{.pre}, [`Zvkn`{.docutils .literal .notranslate}]{.pre}, [`Zvknc`{.docutils .literal .notranslate}]{.pre}, [`Zvkned`{.docutils .literal .notranslate}]{.pre}, [`Zvkng`{.docutils .literal .notranslate}]{.pre}, [`Zvknha`{.docutils .literal .notranslate}]{.pre}, [`Zvknhb`{.docutils .literal .notranslate}]{.pre}, [`Zvks`{.docutils .literal .notranslate}]{.pre}, [`Zvksc`{.docutils .literal .notranslate}]{.pre}, [`Zvksed`{.docutils .literal .notranslate}]{.pre}, [`Zvksg`{.docutils .literal .notranslate}]{.pre}, [`Zvksh`{.docutils .literal .notranslate}]{.pre}, and [`Zvkt`{.docutils .literal .notranslate}]{.pre} extension version was upgraded to 1.0 and is no longer experimental. However, the C intrinsics for these extensions are still experimental. To use the C intrinsics for these extensions, [`-menable-experimental-extensions`{.docutils .literal .notranslate}]{.pre} needs to be passed to Clang.

  - [`-mcpu=sifive-p450`{.docutils .literal .notranslate}]{.pre} was added.

  - CodeGen of [`RV32E`{.docutils .literal .notranslate}]{.pre} and [`RV64E`{.docutils .literal .notranslate}]{.pre} is supported experimentally.

  - CodeGen of [`ilp32e`{.docutils .literal .notranslate}]{.pre} and [`lp64e`{.docutils .literal .notranslate}]{.pre} is supported experimentally.
...
- X86 backend
  ...
  - Support ISA of AMX-FP16 which contains [`tdpfp16ps`{.docutils .literal .notranslate}]{.pre} instruction.
  ...
  - [`-mcpu=raptorlake`{.docutils .literal .notranslate}]{.pre}, [`-mcpu=meteorlake`{.docutils .literal .notranslate}]{.pre} and [`-mcpu=emeraldrapids`{.docutils .literal .notranslate}]{.pre} are now supported.

  - [`-mcpu=sierraforest`{.docutils .literal .notranslate}]{.pre}, [`-mcpu=graniterapids`{.docutils .literal .notranslate}]{.pre} and [`-mcpu=grandridge`{.docutils .literal .notranslate}]{.pre} are now supported.

  - [`__builtin_unpredictable`{.docutils .literal .notranslate}]{.pre} (unpredictable metadata in LLVM IR), is handled by X86 Backend. X86CmovConversion pass now respects this builtin and does not convert CMOVs to branches.
  ...
  - [`-mcpu=graniterapids-d`{.docutils .literal .notranslate}]{.pre} is now supported.

  - The [`i128`{.docutils .literal .notranslate}]{.pre} type now matches GCC and clang's [`__int128`{.docutils .literal .notranslate}]{.pre} type. This mainly benefits external projects such as Rust which aim to be binary compatible with C, but also fixes code generation where LLVM already assumed that the type matched and called into [`libgcc`{.docutils .literal .notranslate}]{.pre} helper functions.

  - Support ISA of USER_MSR.

  - Support ISA of AVX10.1-256 and AVX10.1-512.

  - [`-mcpu=pantherlake`{.docutils .literal .notranslate}]{.pre} and [`-mcpu=clearwaterforest`{.docutils .literal .notranslate}]{.pre} are now supported.

  - [`-mapxf`{.docutils .literal .notranslate}]{.pre} is supported.

  - Marking global variables with [`code_model`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`=`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`"small"/"large"`{.docutils .literal .notranslate}]{.pre} in the IR now overrides the global code model to allow 32-bit relocations or require 64-bit relocations to the global variable.

  - The medium code model's code generation was audited to be more similar to the small code model where possible.

- C API

  - Added [`LLVMGetTailCallKind`{.docutils .literal .notranslate}]{.pre} and [`LLVMSetTailCallKind`{.docutils .literal .notranslate}]{.pre} to allow getting and setting [`tail`{.docutils .literal .notranslate}]{.pre}, [`musttail`{.docutils .literal .notranslate}]{.pre}, and [`notail`{.docutils .literal .notranslate}]{.pre} attributes on call instructions.

  - Added [`LLVMCreateTargetMachineWithOptions`{.docutils .literal .notranslate}]{.pre}, along with helper functions for an opaque option structure, as an alternative to [`LLVMCreateTargetMachine`{.docutils .literal .notranslate}]{.pre}. The option structure exposes an additional setting (that is, the target ABI) and provides default values for unspecified settings.

  - Added [`LLVMGetNNeg`{.docutils .literal .notranslate}]{.pre} and [`LLVMSetNNeg`{.docutils .literal .notranslate}]{.pre} for getting and setting the new [`nneg`{.docutils .literal .notranslate}]{.pre} flag on zext instructions, and [`LLVMGetIsDisjoint`{.docutils .literal .notranslate}]{.pre} and [`LLVMSetIsDisjoint`{.docutils .literal .notranslate}]{.pre} for getting and setting the new disjoint flag on or instructions.
  ...
  - Added the following functions for manipulating operand bundles, as well as building call and invoke instructions that use operand bundles:
    ...
    - [`LLVMBuildCallWithOperandBundles`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMBuildInvokeWithOperandBundles`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMCreateOperandBundle`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMDisposeOperandBundle`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMGetNumOperandBundles`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMGetOperandBundleAtIndex`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMGetNumOperandBundleArgs`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMGetOperandBundleArgAtIndex`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMGetOperandBundleTag`{.docutils .literal .notranslate}]{.pre}

  - Added [`LLVMGetFastMathFlags`{.docutils .literal .notranslate}]{.pre} and [`LLVMSetFastMathFlags`{.docutils .literal .notranslate}]{.pre} for getting and setting the fast-math flags of an instruction, as well as [`LLVMCanValueUseFastMathFlags`{.docutils .literal .notranslate}]{.pre} for checking if an instruction can use such flag.

- CodeGen infrastructure

  - A new debug type [`isel-dump`{.docutils .literal .notranslate}]{.pre} is added to show only the SelectionDAG dumps after each ISel phase (i.e. [`-debug-only=isel-dump`{.docutils .literal .notranslate}]{.pre}). This new debug type can be filtered by function names using [`-filter-print-funcs=<function`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`names>`{.docutils .literal .notranslate}]{.pre}, the same flag used to filter IR dumps after each Pass. Note that the existing [`-debug-only=isel`{.docutils .literal .notranslate}]{.pre} will take precedence over the new behavior and print SelectionDAG dumps of every single function regardless of [`-filter-print-funcs`{.docutils .literal .notranslate}]{.pre}'s values.

- Metadata info

  - Added a new loop metadata [`!{!”llvm.loop.align”,`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`i32`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`64}`{.docutils .literal .notranslate}]{.pre}.

- LLVM tools

  - [`llvm-symbolizer`{.docutils .literal .notranslate}]{.pre} now treats invalid input as an address for which source information is not found.

  - [`llvm-readelf`{.docutils .literal .notranslate}]{.pre} now supports [`--extra-sym-info`{.docutils .literal .notranslate}]{.pre} (-X) to display extra information (section name) when showing symbols.

  - [`llvm-readobj`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--elf-output-style=JSON`{.docutils .literal .notranslate}]{.pre} no longer prefixes each JSON object with the file name. Previously, each object file's output looked like [`"main.o":{"FileSummary":{"File":"main.o"},...}`{.docutils .literal .notranslate}]{.pre} but is now [`{"FileSummary":{"File":"main.o"},...}`{.docutils .literal .notranslate}]{.pre}. This allows each JSON object to be parsed in the same way, since each object no longer has a unique key. Tools that consume [`llvm-readobj`{.docutils .literal .notranslate}]{.pre}'s JSON output should update their parsers accordingly.

  - [`llvm-objdump`{.docutils .literal .notranslate}]{.pre} now uses [`--print-imm-hex`{.docutils .literal .notranslate}]{.pre} by default, which brings its default behavior closer in line with [`objdump`{.docutils .literal .notranslate}]{.pre}.

  - [`llvm-nm`{.docutils .literal .notranslate}]{.pre} now supports the [`--line-numbers`{.docutils .literal .notranslate}]{.pre} ([`-l`{.docutils .literal .notranslate}]{.pre}) option to use debugging information to print symbols' filenames and line numbers.

  - [`llvm-symbolizer`{.docutils .literal .notranslate}]{.pre} and [`llvm-addr2line`{.docutils .literal .notranslate}]{.pre} now support addresses specified as symbol names.

  - [`llvm-objcopy`{.docutils .literal .notranslate}]{.pre} now supports [`--gap-fill`{.docutils .literal .notranslate}]{.pre} and [`--pad-to`{.docutils .literal .notranslate}]{.pre} options, for ELF input and binary output files only.

- LLDB

  - [`SBType::FindDirectNestedType`{.docutils .literal .notranslate}]{.pre} function is added. It's useful for formatters to quickly find directly nested type when it's known where to search for it, avoiding more expensive global search via [`SBTarget::FindFirstType`{.docutils .literal .notranslate}]{.pre}.

  - Renamed [`lldb-vscode`{.docutils .literal .notranslate}]{.pre} to [`lldb-dap`{.docutils .literal .notranslate}]{.pre} and updated its installation instructions to reflect this. The underlying functionality remains unchanged.

  - The [`mte_ctrl`{.docutils .literal .notranslate}]{.pre} register can now be read from AArch64 Linux core files.
  ...
  - LLDB now supports symbol and binary acquisition automatically using the DEBUFINFOD protocol. The standard mechanism of specifying DEBUFINOD servers in the DEBUGINFOD_URLS environment variable is used by default. In addition, users can specify servers to request symbols from using the LLDB setting [`plugin.symbol-locator.debuginfod.server_urls`{.docutils .literal .notranslate}]{.pre}, override or adding to the environment variable.

  - When running on AArch64 Linux, [`lldb-server`{.docutils .literal .notranslate}]{.pre} now provides register field information for the following registers: [`cpsr`{.docutils .literal .notranslate}]{.pre}, [`fpcr`{.docutils .literal .notranslate}]{.pre}, [`fpsr`{.docutils .literal .notranslate}]{.pre}, [`svcr`{.docutils .literal .notranslate}]{.pre} and [`mte_ctrl`{.docutils .literal .notranslate}]{.pre}.
...
- LLVM IR
  ...
  - The constant expression variants of the following instructions have been removed:
    ...
    - [`and`{.docutils .literal .notranslate}]{.pre}

    - [`or`{.docutils .literal .notranslate}]{.pre}

    - [`lshr`{.docutils .literal .notranslate}]{.pre}

    - [`ashr`{.docutils .literal .notranslate}]{.pre}

    - [`zext`{.docutils .literal .notranslate}]{.pre}

    - [`sext`{.docutils .literal .notranslate}]{.pre}

    - [`fptrunc`{.docutils .literal .notranslate}]{.pre}

    - [`fpext`{.docutils .literal .notranslate}]{.pre}

    - [`fptoui`{.docutils .literal .notranslate}]{.pre}

    - [`fptosi`{.docutils .literal .notranslate}]{.pre}

    - [`uitofp`{.docutils .literal .notranslate}]{.pre}

    - [`sitofp`{.docutils .literal .notranslate}]{.pre}
...
- C API
  ...
  - The following functions for creating constant expressions have been removed, because the underlying constant expressions are no longer supported. Instead, an instruction should be created using the [`LLVMBuildXYZ`{.docutils .literal .notranslate}]{.pre} APIs, which will constant fold the operands if possible and create an instruction otherwise:

    - [`LLVMConstAnd`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstOr`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstLShr`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstAShr`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstZExt`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstSExt`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstZExtOrBitCast`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstSExtOrBitCast`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstIntCast`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstFPTrunc`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstFPExt`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstFPToUI`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstFPToSI`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstUIToFP`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstSIToFP`{.docutils .literal .notranslate}]{.pre}

    - [`LLVMConstFPCast`{.docutils .literal .notranslate}]{.pre}

- CodeGen infrastructure

  - [`PrologEpilogInserter`{.docutils .literal .notranslate}]{.pre} no longer supports register scavenging during forwards frame index elimination. Targets should use backwards frame index elimination instead.

  - [`RegScavenger`{.docutils .literal .notranslate}]{.pre} no longer supports forwards register scavenging. Clients should use backwards register scavenging instead, which is preferred because it does not depend on accurate kill flags.
...
- Added [`FP8`{.docutils .literal .notranslate}]{.pre} Python API.

- Added examples for SD 2.1 and SDXL.

- Added support for BERT to Dynamic Batch.

- Added a [`--test`{.docutils .literal .notranslate}]{.pre} flag in [`migraphx-driver`{.docutils .literal .notranslate}]{.pre} to validate the installation.

- Added support for ONNX Operator: Einsum.

- Added [`uint8`{.docutils .literal .notranslate}]{.pre} support in ONNX Operators.
...
- Reorganized [`memcpys`{.docutils .literal .notranslate}]{.pre} with ONNX Runtime to improve performance.
...
- Fixed an issue causing the mod operator with [`int8`{.docutils .literal .notranslate}]{.pre} and [`int32`{.docutils .literal .notranslate}]{.pre} inputs.

- Fixed an issue by preventing the spawning too many threads for constant propagation when parallel STL is not enabled.

- Fixed a bug when running [`migraphx-driver`{.docutils .literal .notranslate}]{.pre} with the [`--run`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`1`{.docutils .literal .notranslate}]{.pre} option.

- Fixed Layernorm accuracy: calculations in [`FP32`{.docutils .literal .notranslate}]{.pre}.

- Fixed update Docker generator script to ROCm 6.1 to point at Jammy.

- Fixed a floating point exception for [`dim`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`(-1)`{.docutils .literal .notranslate}]{.pre} in the reshape operator.

- Fixed issue with [`int8`{.docutils .literal .notranslate}]{.pre} accuracy and models which were failing due to requiring a fourth bias input.

- Fixed missing inputs not previously handled for quantized bias for the weights, and data values of the input matrix.

- Fixed order of operations for [`int8`{.docutils .literal .notranslate}]{.pre} quantization which were causing inaccuracies and slowdowns.
...
- Omniperf might not work with AMD Instinct MI300 GPUs out of the box, resulting in the following error: "*ERROR gfx942 is not enabled rocprofv1. Available profilers include: \['rocprofv2'\]*". As a workaround, add the environment variable [`export`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ROCPROF=rocprofv2`{.docutils .literal .notranslate}]{.pre}.
...
- Added [`fp16`{.docutils .literal .notranslate}]{.pre} and [`fp8`{.docutils .literal .notranslate}]{.pre} cases to unit tests.

- Added a new unit test for main kernel stack size.

- Added the new [`-n`{.docutils .literal .notranslate}]{.pre} option for [`topo_expl`{.docutils .literal .notranslate}]{.pre} to override the number of nodes.
...
- Replaced [`rccl_bfloat16`{.docutils .literal .notranslate}]{.pre} with hip_bfloat16.
...
- Deprecated the following:
  ...
  - [`LocalMatrix::ReadFileCSR`{.docutils .literal .notranslate}]{.pre}

  - [`LocalMatrix::WriteFileCSR`{.docutils .literal .notranslate}]{.pre}
...
- Added cache flush timing for [`gemm_batched_ex`{.docutils .literal .notranslate}]{.pre}, [`gemm_strided_batched_ex`{.docutils .literal .notranslate}]{.pre}, and [`axpy`{.docutils .literal .notranslate}]{.pre}.

- Added Benchmark class for common timing code.

- Added an environment variable [`ROCBLAS_DEFAULT_ATOMICS_MODE`{.docutils .literal .notranslate}]{.pre}; to set default atomics mode during creation of [`rocblas_handle`{.docutils .literal .notranslate}]{.pre}.

- Added support for single-precision ([`fp32_r`{.docutils .literal .notranslate}]{.pre}) input and double-precision ([`fp64_r`{.docutils .literal .notranslate}]{.pre}) output and compute types by extending [`dot_ex`{.docutils .literal .notranslate}]{.pre}.
...
- The internal accumulator type of [`inclusive_scan(_by_key)`{.docutils .literal .notranslate}]{.pre} and [`exclusive_scan(_by_key)`{.docutils .literal .notranslate}]{.pre} is now exposed as an optional type parameter.
...
- Added a new overload for [`device_adjacent_difference_inplace`{.docutils .literal .notranslate}]{.pre} that allows separate input and output iterators, but allows them to point to the same element.

- Added new public APIs for deriving resulting type on device-only functions:

  - [`rocprim::invoke_result`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::invoke_result_t`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::invoke_result_binary_op`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::invoke_result_binary_op_t`{.docutils .literal .notranslate}]{.pre}

- Added the new [`rocprim::batch_copy`{.docutils .literal .notranslate}]{.pre} function. Similar to [`rocprim::batch_memcpy`{.docutils .literal .notranslate}]{.pre}, but copies by element, not with memcpy.

- Added more test cases, to better cover supported data types.

- Added an optional [`decomposer`{.docutils .literal .notranslate}]{.pre} argument for all member functions of [`rocprim::block_radix_sort`{.docutils .literal .notranslate}]{.pre} and all functions of [`device_radix_sort`{.docutils .literal .notranslate}]{.pre}. To sort keys of an user-defined type, a decomposer functor should be passed. The decomposer should produce a [`rocprim::tuple`{.docutils .literal .notranslate}]{.pre} of references to arithmetic types from the key.

- Added [`rocprim::predicate_iterator`{.docutils .literal .notranslate}]{.pre} which acts as a proxy for an underlying iterator based on a predicate. It iterates over proxies that holds the references to the underlying values, but only allow reading and writing if the predicate is [`true`{.docutils .literal .notranslate}]{.pre}. It can be instantiated with:

  - [`rocprim::make_predicate_iterator`{.docutils .literal .notranslate}]{.pre}

  - [`rocprim::make_mask_iterator`{.docutils .literal .notranslate}]{.pre}

- Added custom radix sizes as the last parameter for [`block_radix_sort`{.docutils .literal .notranslate}]{.pre}. The default value is 4, it can be a number between 0 and 32.
...
- Fixed incorrect results returned when calling device [`unique_by_key`{.docutils .literal .notranslate}]{.pre} with overlapping [`values_input`{.docutils .literal .notranslate}]{.pre} and [`values_output`{.docutils .literal .notranslate}]{.pre}.

- Fixed incorrect output type used in [`device_adjacent_difference`{.docutils .literal .notranslate}]{.pre}.

- Fixed an issue causing incorrect results on the GFX10 (RDNA1, RDNA2) ISA and GFX11 ISA on device scan algorithms [`rocprim::inclusive_scan(_by_key)`{.docutils .literal .notranslate}]{.pre} and [`rocprim::exclusive_scan(_by_key)`{.docutils .literal .notranslate}]{.pre} with large input types.

- Fixed an issue with [`device_adjacent_difference`{.docutils .literal .notranslate}]{.pre}. It now considers both the input and the output type for selecting the appropriate kernel launch config. Previously only the input type was considered, which could result in compilation errors due to excessive shared memory usage.
...
- Added [`rocrand_create_generator_host`{.docutils .literal .notranslate}]{.pre}.
  ...
  - The following generators are supported:
    ...
    - [`ROCRAND_RNG_PSEUDO_MRG31K3P`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_MRG32K3A`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_PHILOX4_32_10`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_THREEFRY2_32_20`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_THREEFRY2_64_20`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_THREEFRY4_32_20`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_THREEFRY4_64_20`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_PSEUDO_XORWOW`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_QUASI_SCRAMBLED_SOBOL32`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_QUASI_SCRAMBLED_SOBOL64`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_QUASI_SOBOL32`{.docutils .literal .notranslate}]{.pre}

    - [`ROCRAND_RNG_QUASI_SOBOL64`{.docutils .literal .notranslate}]{.pre}

  - The host-side generators support multi-core processing. On Linux, this requires the TBB (Thread Building Blocks) development package to be installed on the system when building rocRAND ([`libtbb-dev`{.docutils .literal .notranslate}]{.pre} on Ubuntu and derivatives).
...
- C++ wrapper:
  ...
  - Added support for [`lfsr113_engine`{.docutils .literal .notranslate}]{.pre} being constructed with a seed of type [`unsigned`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`long`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`long`{.docutils .literal .notranslate}]{.pre}, not only [`uint4`{.docutils .literal .notranslate}]{.pre}.

  - Added optional order parameter to the constructor of [`mt19937_engine`{.docutils .literal .notranslate}]{.pre}.

- Added the following functions for the [`ROCRAND_RNG_PSEUDO_MTGP32`{.docutils .literal .notranslate}]{.pre} generator:

  - [`rocrand_normal2`{.docutils .literal .notranslate}]{.pre}

  - [`rocrand_normal_double2`{.docutils .literal .notranslate}]{.pre}

  - [`rocrand_log_normal2`{.docutils .literal .notranslate}]{.pre}

  - [`rocrand_log_normal_double2`{.docutils .literal .notranslate}]{.pre}

- Added [`rocrand_create_generator_host_blocking`{.docutils .literal .notranslate}]{.pre} which dispatches without stream semantics.

- Added host-side generator for [`ROCRAND_RNG_PSEUDO_MTGP32`{.docutils .literal .notranslate}]{.pre}.

- Added offset and skipahead functionality to LFSR113 generator.

- Added dynamic ordering for architecture [`gfx1102`{.docutils .literal .notranslate}]{.pre}.

- For device-side generators, you can now wrap calls to [`rocrand_generate_*`{.docutils .literal .notranslate}]{.pre} inside of a hipGraph. There are a few things to be aware of:

  - Generator creation ([`rocrand_create_generator`{.docutils .literal .notranslate}]{.pre}), initialization ([`rocrand_initialize_generator`{.docutils .literal .notranslate}]{.pre}), and destruction ([`rocrand_destroy_generator`{.docutils .literal .notranslate}]{.pre}) must still happen outside the hipGraph.

  - After the generator is created, you may call API functions to set its seed, offset, and order.

  - After the generator is initialized (but before stream capture or manual graph creation begins), use [`rocrand_set_stream`{.docutils .literal .notranslate}]{.pre} to set the stream the generator will use within the graph.
  ...
  - API calls for the poisson distribution (for example, [`rocrand_generate_poisson`{.docutils .literal .notranslate}]{.pre}) are not yet supported inside of hipGraphs.

  - For sample usage, see the unit tests in [`test/test_rocrand_hipgraphs.cpp`{.docutils .literal .notranslate}]{.pre}
...
- C++ wrapper: the [`min()`{.docutils .literal .notranslate}]{.pre} and [`max()`{.docutils .literal .notranslate}]{.pre} member functions of the generators and distributions are now [`static`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`constexpr`{.docutils .literal .notranslate}]{.pre}.
...
- Deprecated the following typedefs. Please use the unified [`state_type`{.docutils .literal .notranslate}]{.pre} alias instead.
  ...
  - [`rocrand_device::threefry2x32_20_engine::threefry2x32_20_state`{.docutils .literal .notranslate}]{.pre}

  - [`rocrand_device::threefry2x64_20_engine::threefry2x64_20_state`{.docutils .literal .notranslate}]{.pre}

  - [`rocrand_device::threefry4x32_20_engine::threefry4x32_20_state`{.docutils .literal .notranslate}]{.pre}

  - [`rocrand_device::threefry4x64_20_engine::threefry4x64_20_state`{.docutils .literal .notranslate}]{.pre}

- Deprecated the following internal headers:

  - [`src/rng/distribution/distributions.hpp`{.docutils .literal .notranslate}]{.pre}.

  - [`src/rng/device_engines.hpp`{.docutils .literal .notranslate}]{.pre}.
...
- Added 64-bit APIs for existing functions:
  ...
  - GETF2_64 (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)

  - GETRF_64 (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)

  - GETRS_64 (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)
...
- Added partial eigenvalue decomposition routines for symmetric/hermitian matrices using Divide & Conquer and Bisection:
  ...
  - SYEVDX (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)

  - HEEVDX (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)

- Added partial generalized symmetric/hermitian-definite eigenvalue decomposition using Divide & Conquer and Bisection:

  - SYGVDX (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)

  - HEGVDX (with [`batched`{.docutils .literal .notranslate}]{.pre} and [`strided_batched`{.docutils .literal .notranslate}]{.pre} versions)
...
- Added [`rocsparse_set_identity_permutation`{.docutils .literal .notranslate}]{.pre}.
...
- Added changes from upstream CCCL/thrust 2.2.0.
  ...
  - Updated the contents of [`system/hip`{.docutils .literal .notranslate}]{.pre} and [`test`{.docutils .literal .notranslate}]{.pre} with the upstream changes.

- Updated internal calls to [`rocprim::detail::invoke_result`{.docutils .literal .notranslate}]{.pre} to use the public API [`rocprim::invoke_result`{.docutils .literal .notranslate}]{.pre}.

- Updated to use [`rocprim::device_adjacent_difference`{.docutils .literal .notranslate}]{.pre} for [`adjacent_difference`{.docutils .literal .notranslate}]{.pre} API call.
...
- Improved loading layouts by increasing vector width for fragments with [`blockDim`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`>`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`32`{.docutils .literal .notranslate}]{.pre}.

- API [`applyDataLayout`{.docutils .literal .notranslate}]{.pre} transform now accepts WaveCount template argument for cooperative fragments.

- API [`applyDataLayout`{.docutils .literal .notranslate}]{.pre} transform now physically applies aos\<-\>soa transform as necessary.
...
- Use [`CHECK_RETURN_STATUS`{.docutils .literal .notranslate}]{.pre} everywhere to adhere to C++17 for HIP.
...
- Fixed [`MasterSolutionLibrary`{.docutils .literal .notranslate}]{.pre} indexing for multiple architecture build.
...
- Fixed mismatch issue with [`GlobalReadCoalesceGroup`{.docutils .literal .notranslate}]{.pre}.
...
- Added process isolation and clean shader APIs and CLI commands.
  ...
  - [`amdsmi_get_gpu_process_isolation()`{.docutils .literal .notranslate}]{.pre}

  - [`amdsmi_set_gpu_process_isolation()`{.docutils .literal .notranslate}]{.pre}
...
- [`amd-smi`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`metric`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`--power`{.docutils .literal .notranslate}]{.pre} now correctly details power output for RDNA3, RDNA2, and MI1x devices.

- Fixed the [`amdsmitstReadWrite.TestPowerCapReadWrite`{.docutils .literal .notranslate}]{.pre} test for RDNA3, RDNA2, and MI100 devices.
...
- Added [`hipblasltExtLayerNorm`{.docutils .literal .notranslate}]{.pre} extension API.

- Added [`hipblasltExtAMax`{.docutils .literal .notranslate}]{.pre} extension API.
...
- Added the [`Partition`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`ID`{.docutils .literal .notranslate}]{.pre} field to the [`rocm-smi`{.docutils .literal .notranslate}]{.pre} CLI.
...
- Fixed memory leaks caused by not closing directories and creating maps nodes instead of using [`.at()`{.docutils .literal .notranslate}]{.pre}.
...
- Split the [`rocm-llvm`{.docutils .literal .notranslate}]{.pre} package. This package has been split into a required and an optional package:
...
- Added new APIs supporting Windows only, under development on Linux
  ...
  - [`hipMallocMipmappedArray`{.docutils .literal .notranslate}]{.pre} for allocating a mipmapped array on the device

  - [`hipFreeMipmappedArray`{.docutils .literal .notranslate}]{.pre} for freeing a mipmapped array on the device

  - [`hipGetMipmappedArrayLevel`{.docutils .literal .notranslate}]{.pre} for getting a mipmap level of a HIP mipmapped array

  - [`hipMipmappedArrayCreate`{.docutils .literal .notranslate}]{.pre} for creating a mipmapped array
...
- [`DeviceReduce::Sum`{.docutils .literal .notranslate}]{.pre} does not compile on CUDA platform for mixed extended-floating-point/floating-point InputT and OutputT types.
...
- Removed erroneous implementation of device-level [`inclusive_scan`{.docutils .literal .notranslate}]{.pre} and [`exclusive_scan`{.docutils .literal .notranslate}]{.pre}. The prior default implementation using lookback-scan now is the only available implementation.
...
- New benchmark for the device API using Google Benchmark, [`benchmark_rocrand_device_api`{.docutils .literal .notranslate}]{.pre}, replacing [`benchmark_rocrand_kernel`{.docutils .literal .notranslate}]{.pre}. [`benchmark_rocrand_kernel`{.docutils .literal .notranslate}]{.pre} is deprecated and will be removed in a future version. Likewise, [`benchmark_curand_host_api`{.docutils .literal .notranslate}]{.pre} is added to replace [`benchmark_curand_generate`{.docutils .literal .notranslate}]{.pre} and [`benchmark_curand_device_api`{.docutils .literal .notranslate}]{.pre} is added to replace [`benchmark_curand_kernel`{.docutils .literal .notranslate}]{.pre}.
...
- ROCMInstallTargets now defaults [`CMAKE_INSTALL_LIBDIR`{.docutils .literal .notranslate}]{.pre} to [`lib`{.docutils .literal .notranslate}]{.pre} if not otherwise specified.
...
- New device level [`adjacent_difference`{.docutils .literal .notranslate}]{.pre} primitives.
...
- Header file installation location changed to match other libraries.
  ...
  - Using the [`rocrand.h`{.docutils .literal .notranslate}]{.pre} header file should now use [`#include`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`&lt;rocrand/rocrand.h&gt;`{.docutils .literal .notranslate}]{.pre}, rather than [`#include`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`&lt;rocrand/rocrand.h&gt;`{.docutils .literal .notranslate}]{.pre}
...
- Device_scan and device_segmented_scan: [`inclusive_scan`{.docutils .literal .notranslate}]{.pre} now uses the input-type as accumulator-type, [`exclusive_scan`{.docutils .literal .notranslate}]{.pre} uses initial-value-type.
...
- Added [`rocsolver_get_version_string_size`{.docutils .literal .notranslate}]{.pre} to help with version string queries
...
- Fixed [`addrVgpr`{.docutils .literal .notranslate}]{.pre} for atomic GSU
