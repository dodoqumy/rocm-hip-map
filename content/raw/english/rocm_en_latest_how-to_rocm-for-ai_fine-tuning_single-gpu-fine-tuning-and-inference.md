---
title: "Fine-tuning and inference using a single GPU"
source_url: "https://rocm.docs.amd.com/en/latest/how-to/rocm-for-ai/fine-tuning/single-gpu-fine-tuning-and-inference.html"
...
- [Environment setup](#environment-setup){.reference .internal .nav-link}
  ...
  - [Download the base model and fine-tuning dataset](#download-the-base-model-and-fine-tuning-dataset){.reference .internal .nav-link}
  - [Configure fine-tuning parameters](#configure-fine-tuning-parameters){.reference .internal .nav-link}
- [Fine-tuning](#fine-tuning){.reference .internal .nav-link}
  - [Saving adapters or fully fine-tuned models](#saving-adapters-or-fully-fine-tuned-models){.reference .internal .nav-link}
- [Basic model inference](#basic-model-inference){.reference .internal .nav-link}
::::
:::::
::::::
...
:::::::::::::::::::::::::::::::::::::::::::::::: {#fine-tuning-and-inference-using-a-single-gpu .section}
# Fine-tuning and inference using a single GPU[\#](#fine-tuning-and-inference-using-a-single-gpu "Link to this heading"){.headerlink}
...
This section explains model fine-tuning and inference techniques on a single-accelerator system. See [[Multi-accelerator fine-tuning]{.doc}](multi-gpu-fine-tuning-and-inference.html){.reference .internal} for a setup with multiple GPUs.
...
::: pst-scrollable-table-container
  ...
  Libraries    [`transformers`{.docutils .literal .notranslate}]{.pre} [`datasets`{.docutils .literal .notranslate}]{.pre} [`huggingface-hub`{.docutils .literal .notranslate}]{.pre} [`peft`{.docutils .literal .notranslate}]{.pre} [`trl`{.docutils .literal .notranslate}]{.pre} [`scipy`{.docutils .literal .notranslate}]{.pre}
...
[]{#fine-tuning-llms-single-gpu-env-setup}
...
1.  Install PyTorch for ROCm. Refer to the [[PyTorch installation guide]{.xref .std .std-doc}](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/3rd-party/pytorch-install.html "(in ROCm installation on Linux v7.2.2)"){.reference .external}. For a consistent installation, it's recommended to use official ROCm prebuilt Docker images with the framework pre-installed.
...
4.  Install the required dependencies.
    ...
    bitsandbytes is a library that facilitates quantization to improve the efficiency of deep learning models. Learn more about its use in [[Model quantization techniques]{.doc}](../inference-optimization/model-quantization.html){.reference .internal}.

    See the [[Optimizations for model fine-tuning]{.std .std-ref}](overview.html#fine-tuning-llms-concept-optimizations){.reference .internal} for a brief discussion on PEFT and TRL.
...
::: {#download-the-base-model-and-fine-tuning-dataset .section}
[]{#fine-tuning-llms-single-gpu-download-model-dataset}

### Download the base model and fine-tuning dataset[\#](#download-the-base-model-and-fine-tuning-dataset "Link to this heading"){.headerlink}

1.  Request to access to download the [Meta's official Llama model](https://huggingface.co/meta-llama){.reference .external} from Hugging Face. After permission is granted, log in with the following command using your personal access tokens:
    ...
    You can also use the [NousResearch Llama-2-7b-chat-hf](https://huggingface.co/NousResearch/Llama-2-7b-chat-hf){.reference .external} as a substitute. It has the same model weights as the original.
    :::
...
3.  Now, let's fine-tune the base model for a question-and-answer task using a small dataset called [mlabonne/guanaco-llama2-1k](https://huggingface.co/datasets/mlabonne/guanaco-llama2-1k){.reference .external}, which is a 1000 sample subset of the [timdettmers/openassistant-guanaco](https://huggingface.co/datasets/OpenAssistant/oasst1){.reference .external} dataset.
...
::::: {#configure-fine-tuning-parameters .section}
[]{#fine-tuning-llms-single-gpu-configure-params}

### Configure fine-tuning parameters[\#](#configure-fine-tuning-parameters "Link to this heading"){.headerlink}

To set up [`SFTTrainer`{.docutils .literal .notranslate}]{.pre} parameters, you can use the following code as reference.
...
::::::::::::::::::: {#fine-tuning .section}
[]{#fine-tuning-llms-single-gpu-start}

## Fine-tuning[\#](#fine-tuning "Link to this heading"){.headerlink}

In this section, you'll see two ways of training: with the LoRA technique and without. See [[Optimizations for model fine-tuning]{.std .std-ref}](overview.html#fine-tuning-llms-concept-optimizations){.reference .internal} for an introduction to LoRA. Training with LoRA uses the [`SFTTrainer`{.docutils .literal .notranslate}]{.pre} API with its PEFT integration. Training without LoRA forgoes these benefits.
...
2.  Initialize [`SFTTrainer`{.docutils .literal .notranslate}]{.pre} with a PEFT LoRA configuration and run the trainer.
...
::::::::::::::: {#saving-adapters-or-fully-fine-tuned-models .section}
[]{#fine-tuning-llms-single-gpu-saving}

### Saving adapters or fully fine-tuned models[\#](#saving-adapters-or-fully-fine-tuned-models "Link to this heading"){.headerlink}
...
PEFT adapters can't be loaded by [`AutoModelForCausalLM`{.docutils .literal .notranslate}]{.pre} from the Transformers library as they do not contain full model parameters and model configurations, for example, [`config.json`{.docutils .literal .notranslate}]{.pre}. To use it as a normal transformer model, you need to merge them into the base model.
:::
:::::::::::::::
:::::::::::::::::::

:::::::::::::: {#basic-model-inference .section}
## Basic model inference[\#](#basic-model-inference "Link to this heading"){.headerlink}
...
The PEFT library provides a [`merge_and_unload`{.docutils .literal .notranslate}]{.pre} method, which merges the adapter layers into the base model. This is needed if someone wants to save the adapted model into local storage and use it as a normal standalone model.
...
::::::: {.sd-tab-content .docutils}
If you have a fully fine-tuned model not using PEFT, you can load it like any other pre-trained language model in [Hugging Face Hub](https://huggingface.co/docs/hub/en/index){.reference .external} using the [Transformers](https://huggingface.co/docs/transformers/en/index){.reference .external} library.
...
If using multiple GPUs, see [[Multi-accelerator fine-tuning and inference]{.std .std-ref}](multi-gpu-fine-tuning-and-inference.html#fine-tuning-llms-multi-gpu-hugging-face-accelerate){.reference .internal} to explore popular libraries that simplify fine-tuning and inference in a multiple-GPU system.

Read more about inference frameworks like vLLM and Hugging Face TGI in [[LLM inference frameworks]{.doc}](../inference/llm-inference-frameworks.html){.reference .internal}.
::::::::::::::
::::::::::::::::::::::::::::::::::::::::::::::::

::::: prev-next-area
[](fine-tuning-and-inference.html "previous page"){.left-prev}
...
[](multi-gpu-fine-tuning-and-inference.html "next page"){.right-next}
