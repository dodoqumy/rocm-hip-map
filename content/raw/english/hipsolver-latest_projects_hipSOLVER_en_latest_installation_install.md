---
title: "Installing and building hipSOLVER &#8212; hipSOLVER 3.2.0 Documentation"
source_url: "https://rocm.docs.amd.com/projects/hipSOLVER/en/latest/installation/install.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-01T09:21:07.349438+00:00
content_hash: "2cfe1bc806fbf47c"
---

# Installing and building hipSOLVER[#](#installing-and-building-hipsolver)
...
Download the ROCm packages from the package servers following the [ROCm installation guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/index.html).
Run the following command to install hipSOLVER using the package manager.

```
apt update && sudo apt install hipsolver
```

## Build and install the library using a script (Ubuntu only)[#](#build-and-install-the-library-using-a-script-ubuntu-only)

The root directory of the [hipSOLVER project](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipsolver)
in the [rocm-libraries GitHub](https://github.com/ROCm/rocm-libraries) includes the `install.sh`
...
A more extensive set of options can be specified by invoking `cmake`
...
To build hipSOLVER for ROCm 7.0.2 and earlier, use the hipSOLVER repository at [ROCm/hipSOLVER](https://github.com/ROCm/hipSOLVER).
...
Run these commands from the `rocm-libraries/projects/hipsolver`

directory.

`./install.sh -id`

: Build the library and dependencies and install them (the`-d`

flag only needs to be passed once on a system).`./install.sh -ic`

: Build library and clients (tests, benchmarks, and samples) and install them.

To list more options, use the `-h`

(help) option of the install script.

```
-h
```

## Build and install the library manually[#](#build-and-install-the-library-manually)

For a standard library installation, follow these steps:

```
-p <HIPSOLVER_BUILD_DIR_PATH>/release
...
is required to install hipSOLVER to a system directory, such as `/opt/rocm`
...
to specify a build configuration, such as`Debug`
...
[rocSPARSE](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocsparse)(optional)[SuiteSparse](https://github.com/DrTimothyAldenDavis/SuiteSparse):`CHOLMOD`

and`SuiteSparse_config`
...
about building and installing rocSOLVER, see the [rocSOLVER installation guide](https://rocm.docs.amd.com/projects/rocSOLVER/en/latest/installation/installlinux.html).
...
functionality for the `hipsolverSp`

API. By default, both libraries are run-time dependencies.
They are dynamically loaded by `dlopen`

if they are
present on the system. If the `BUILD_WITH_SPARSE`
...
## Build the library, tests, benchmarks, and samples manually[#](#build-the-library-tests-benchmarks-and-samples-manually)

The repository contains source code for client programs that serve as tests, benchmarks, and samples.
The client source code can be found in the `clients`
...
[LAPACK](https://github.com/Reference-LAPACK/lapack-release)(Adds a dependency on a Fortran compiler)[hipBLAS](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipblas)(Optional)[hipSPARSE](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipsparse)(Optional, but required with the default settings)
...
of these dependencies, setting `CMAKE_PREFIX_PATH`
...
```
-p <HIPSOLVER_BUILD_DIR_PATH>/release/deps
...
ignored if the `BUILD_HIPSPARSE_TESTS`

option is set to `OFF`

. It’s used
to create objects required by tests for the `hipsolverSp`
...
For more details, see the [hipBLAS](https://rocm.docs.amd.com/projects/hipBLAS/en/latest/index.html)
and [hipSPARSE](https://rocm.docs.amd.com/projects/hipSPARSE/en/latest/index.html) documentation.

### Library and clients[#](#library-and-clients)
