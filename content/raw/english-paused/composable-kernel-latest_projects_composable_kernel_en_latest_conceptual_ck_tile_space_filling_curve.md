---
title: "Space-Filling Curves - Optimal Memory Traversal &#8212; Composable Kernel 1.2.0 Documentation"
source_url: "https://rocm.docs.amd.com/projects/composable_kernel/en/latest/conceptual/ck_tile/space_filling_curve.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-03T00:10:26.908314+00:00
content_hash: "79ba915e89b7bfc5"
---

# Space-Filling Curves - Optimal Memory Traversal[#](#space-filling-curves-optimal-memory-traversal)

## Overview[#](#overview)

The SpaceFillingCurve (SFC) class provides a systematic way to traverse multi-dimensional tensors, supporting both scalar and vectorized access patterns. This is particularly important for optimizing memory access patterns in [GPU kernels](hardware/gpu_basics.html#ck-tile-gpu-basics), where the order of memory accesses can dramatically impact performance through cache utilization, memory coalescing, and prefetching effectiveness.

A space-filling curve is a continuous curve that visits every point in a multi-dimensional space exactly once. In the context of CK Tile, it defines a mapping from a 1D access index to multi-dimensional [tensor coordinates](coordinate_systems.html#ck-tile-coordinate-systems), enabling efficient traversal patterns that maximize hardware utilization.

## Key Concepts[#](#key-concepts)

### Tensor Traversal[#](#tensor-traversal)
...
[GPUs](hardware/gpu_basics.html#ck-tile-gpu-basics) support vector load and store instructions that can access multiple consecutive elements in a single operation. SpaceFillingCurve supports this by allowing specification of how many elements to access per dimension (`scalars_per_access`

), enabling efficient utilization of these hardware features.
...
### Scalar Access Patterns[#](#scalar-access-patterns)
...
### Vectorized Access Patterns[#](#vectorized-access-patterns)
...
### Column-Major vs Row-Major[#](#column-major-vs-row-major)
...
### Snake Pattern for Cache Optimization[#](#snake-pattern-for-cache-optimization)
...
### GEMM Tile Access Pattern[#](#gemm-tile-access-pattern)

For [matrix multiplication](hardware/gemm_optimization.html#ck-tile-gemm-optimization), optimal access patterns are crucial:
...
### 3D Tensor Patterns[#](#d-tensor-patterns)
...
### Step Analysis for Memory Patterns[#](#step-analysis-for-memory-patterns)
...
### Non-Divisible Dimensions[#](#non-divisible-dimensions)
...
## Integration with CK Tile[#](#integration-with-ck-tile)

### LoadStoreTraits Integration[#](#loadstoretraits-integration)

[LoadStoreTraits](load_store_traits.html#ck-tile-load-store-traits) uses space-filling curves to optimize memory access:
...
### TileWindow Usage[#](#tilewindow-usage)

[TileWindow](tile_window.html#ck-tile-tile-window) leverages space-filling curves for systematic tile traversal:
...
**Choose Appropriate Dimension Order**// For row-major storage, use row-major traversal using RowMajorSFC = space_filling_curve<2, Shape, sequence<0, 1>, ...>; // For column-major storage, use column-major traversal using ColMajorSFC = space_filling_curve<2, Shape, sequence<1, 0>, ...>;

**Optimize Vector Size**// Match vector size to cache line for optimal bandwidth constexpr index_t optimal_vector = min( tensor_length_fast_dim, cache_line_size / sizeof(DataType) );

**Enable Snake Pattern for Large Tensors**// Snake pattern helps when jumping between rows/planes using CacheFriendlySFC = space_filling_curve< NDim, Lengths, Order, Scalars, true // Enable snake >;

**Consider Memory Layout**// Align access patterns with physical memory layout static_assert( SFC::scalars_per_access[fastest_dim] * sizeof(DataType) % cache_line_size == 0, "Vector access should align with cache lines" );
...
**Systematic traversal**: Convert N-D access to 1D iteration**Vectorization support**: Efficient use of vector load and store instructions**Cache optimization**: Snake patterns and dimension ordering for locality**Flexibility**: Adaptable to different[tensor shapes](descriptors.html#ck-tile-descriptors)and access patterns**Performance**: Compile-time optimization with zero runtime overhead

The advanced traversal patterns enabled by space-filling curves are fundamental to achieving high performance in GPU kernels, ensuring that memory access patterns align with [hardware capabilities](hardware/gpu_basics.html#ck-tile-gpu-basics).

## Next Steps[#](#next-steps)

[LoadStoreTraits - Memory Access Optimization Engine](load_store_traits.html#ck-tile-load-store-traits)- How curves optimize memory access[Sweep Tile](sweep_tile.html#ck-tile-sweep-tile)- Traversing distributed tensors[Static Distributed Tensor](static_distributed_tensor.html#ck-tile-static-distributed-tensor)- The data structures being traversed[Tile Window - Data Access Gateway](tile_window.html#ck-tile-tile-window)- Integration with data loading[A Block GEMM on MI300](hardware/gemm_optimization.html#ck-tile-gemm-optimization)- Real-world application example
