...
source_url: "https://rocm.docs.amd.com/en/docs-6.2.0/how-to/llm-fine-tuning-optimization/llm-inference-frameworks.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-04T16:17:08.591760+00:00
content_hash: "e998527d53fb1121"
---
...
[single-accelerator](single-gpu-fine-tuning-and-inference.html) and
[multi-accelerator](multi-gpu-fine-tuning-and-inference.html) systems.
...
.clone https://github.com/vllm-project/vllm.git cd vllm docker build -f Dockerfile.rocm -t vllm-rocm .
...
to isolate each instance to a different accelerator.For example, to run two API servers, one on port 8000 using GPU 0 and 1, one on port 8001 using GPU 2 and 3, use a a command like the following.
...
Refer to [vLLM performance optimization](../tuning-guides/mi300x/workload.html#mi300x-vllm-optimization) for performance optimization tips.
...
Inside the container, launch a model using TGI server on a single accelerator.
...
Inside the container, launch a model using TGI server on multiple accelerators (4 in this case).
