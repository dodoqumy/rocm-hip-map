---
title: "Memory Swizzling with Morton Ordering &#8212; Composable Kernel 1.2.0 Documentation"
source_url: "https://rocm.docs.amd.com/projects/composable_kernel/en/latest/conceptual/ck_tile/swizzling_example.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-02T21:10:16.291277+00:00
content_hash: "dd3abb2432349178"
---

# Memory Swizzling with Morton Ordering[#](#memory-swizzling-with-morton-ordering)

## Overview[#](#overview)

This chapter demonstrates a practical application of tensor descriptors for implementing memory swizzling patterns, specifically Morton ordering (Z-order curve) within tiles. Memory swizzling is used to optimize GPU memory access patterns and reduce [bank conflicts](hardware/lds_bank_conflicts.html#ck-tile-lds-bank-conflicts). Morton ordering provides a space-filling curve that maintains spatial locality while enabling efficient parallel access. See [Space-Filling Curves - Optimal Memory Traversal](space_filling_curve.html#ck-tile-space-filling-curve) for more information about parallel access.

Morton ordering is widely used in:

**GPU Texture Memory**: Optimizing cache efficiency for 2D texture access**Matrix Operations**: Reducing memory bank conflicts in shared memory**Image Processing**: Improving locality for block-based algorithms**Scientific Computing**: Enhancing data access patterns for stencil operations

## Understanding Morton Ordering[#](#understanding-morton-ordering)
...
### Morton Pattern Analysis[#](#morton-pattern-analysis)
...
## Stage 1: Tiling with UnmergeTransform[#](#stage-1-tiling-with-unmergetransform)

First, we split our texture into tiles using tensor descriptors (see [Tensor Descriptors - Complete Tensor Specifications](descriptors.html#ck-tile-descriptors) and [Individual Transform Operations](transforms.html#ck-tile-transforms)). This creates a hierarchical structure: (Y_blk, y_in, X_blk, x_in).
...
## Stage 2: Morton Ordering with MergeTransform[#](#stage-2-morton-ordering-with-mergetransform)
...
This leverages the coordinate transformation system described in [Coordinate Systems - The Mathematical Foundation](coordinate_systems.html#ck-tile-coordinate-systems).
...
### Complete Morton Implementation[#](#complete-morton-implementation)
...
## GPU Kernel Implementation[#](#gpu-kernel-implementation)
...
## Bank Conflict Reduction[#](#bank-conflict-reduction)

Morton ordering is particularly effective for reducing shared memory bank conflicts (complementing the XOR preshuffle technique described in [Load Data Share Index Swapping](lds_index_swapping.html#ck-tile-lds-index-swapping)):
...
For complete GEMM optimization techniques, see [A Block GEMM on MI300](hardware/gemm_optimization.html#ck-tile-gemm-optimization).
...
**MergeTransform**is essential for expressing Morton bit interleaving**Manual bit manipulation**provides reliable and efficient implementation**Tiling + Morton**combines hierarchical locality with local optimization**GPU-specific tuning**adapts patterns to hardware characteristics
...
For more examples of practical CK Tile usage, see [Convolution Implementation with CK Tile](convolution_example.html#ck-tile-convolution-example). For the underlying buffer and tensor abstractions, see [Buffer Views - Raw Memory Access](buffer_views.html#ck-tile-buffer-views) and [Tensor Views - Multi-Dimensional Structure](tensor_views.html#ck-tile-tensor-views).
