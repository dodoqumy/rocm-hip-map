---
title: "Logging HIP activity &#8212; HIP 7.2.53211 Documentation"
source_url: "https://rocm.docs.amd.com/projects/HIP/en/latest/how-to/logging.html"
source_type: official
source_org: amd
credibility: 5
lifecycle: latest
fetched_at: 2026-05-01T06:59:20.051774+00:00
content_hash: "5b20380830ed66de"
---
...
:::::::::::::::::::::::
{#logging-hip-activity .section}
# Logging HIP activity[\#](#logging-hip-activity "Link to this heading"){.headerlink}
...
## Logging level[\#](#logging-level "Link to this heading"){.headerlink}

HIP logging is disabled by default. You can enable it via the [`AMD_LOG_LEVEL`{.docutils .literal .notranslate}]{.pre} environment variable. The value of this variable controls your logging level. Levels are defined as follows:
...
You can call a logging function with different logging levels. All information under the value set for [`AMD_LOG_LEVEL`{.docutils .literal .notranslate}]{.pre} is printed.

::
{#logging-mask .section}
## Logging mask[\#](#logging-mask "Link to this heading"){.headerlink}

The logging mask is designed to print functionality types when you're running a HIP application. Once you set [`AMD_LOG_LEVEL`{.docutils .literal .notranslate}]{.pre}, the logging mask is set as the default value ([`0x7FFFFFFF`{.docutils .literal .notranslate}]{.pre}). You can change this to any of the valid values:
...
You can also define the logging mask via the [`AMD_LOG_MASK`{.docutils .literal .notranslate}]{.pre} environment variable.
::

::::
{#logging-command .section}
## Logging command[\#](#logging-command "Link to this heading"){.headerlink}
...
Using HIP code, call the [`ClPrint()`{.docutils .literal .notranslate}]{.pre} function with the desired input variables. For example:
...
::::
{#logging-examples .section}
## Logging examples[\#](#logging-examples "Link to this heading"){.headerlink}

On **Linux**, you can enable HIP logging and retrieve logging information when you run [`hipinfo`{.docutils .literal .notranslate}]{.pre}.
...
On **Windows**, you can set [`AMD_LOG_LEVEL`{.docutils .literal .notranslate}]{.pre} via environment variable from the advanced system settings or the command prompt (when run as administrator). The following example shows debug log information when calling the backend runtime.
...
::
{#logging-hipcc-commands .section}
## Logging hipcc commands[\#](#logging-hipcc-commands "Link to this heading"){.headerlink}

To see the detailed commands that hipcc issues, set the environment variable [`HIPCC_VERBOSE`{.docutils .literal .notranslate}]{.pre}. Doing so will print the HIP-clang (or NVCC) commands that hipcc generates to [`stderr`{.docutils .literal .notranslate}]{.pre}.
